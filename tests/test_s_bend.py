...
def test_s_bend_fn(tmp_path, epc=None):
    ...
    bend_b_centre_xz = (
        flat_dx_di * float(ni_tail - 2.0 * ni_half_mid),
        ...
        if k == nk // 2 + 1:
            ...
            for i in range(ni + 1):
                if i < ni_tail + 1:
                    ...
                    z = (
                        top_depth + float(k) * layer_thickness
                    )  # will introduce a thick layer after pinchout
                    ...
                    x = flat_dx_di * float(
                        (i - (ni - ni_tail)) + ni_tail - 2 * ni_half_mid
                    )
                    if i == ni - 1 or i == ni - 4:
    ...
    model = rq.Model(
        epc_file=epc, new_epc=True, create_basics=True, create_hdf5_ext=True
    )
    ...
    x = np.array(
        [
            0.0,
            flat_dx_di * float(ni_tail) + outer_radius,
            flat_dx_di * (float(ni_tail) - 0.5),
            0.0,
            -outer_radius,
        ]
    )
    ...
    trajectory = rqw.Trajectory(
        model, md_datum=datum, data_frame=df, length_uom="m", well_name="ANGLED_WELL"
    )
    ...
    traj_2 = rqw.Trajectory(
        model, md_datum=datum, data_frame=df, length_uom="m", well_name="HORST_WELL"
    )
    traj_2.write_hdf5()
    traj_2.create_xml()
    ...
    traj_3 = rqw.Trajectory(
        model, md_datum=datum, data_frame=df, length_uom="m", well_name="VERTICAL_WELL"
    )
    traj_3.write_hdf5()
    traj_3.create_xml()
    ...
    traj_4 = rqw.Trajectory(
        model, md_datum=datum, data_frame=df, length_uom="m", well_name="NESSIE_WELL"
    )
    traj_4.write_hdf5()
    traj_4.create_xml()
    ...
    # JK plane faults
    cp[:, :, ni_tail + ni_bend // 2 :, :, :, :, 0] -= (
        flat_dx_di * 0.57
    )  # horizontal break mid top bend
    cp[:, :, ni_tail + ni_bend + ni_half_mid :, :, :, :, 2] += (
        layer_thickness * 1.27
    )  # vertical break in mid section
    ...
    cp[
        :,
        :,
        ni - (ni_tail + ni_bend // 2 + 1) : ni - (ni_tail + ni_bend // 2 - 1),
        :,
        :,
        :,
        :,
    ] -= (horst_dz, 0.0, bend_horst_dz)
