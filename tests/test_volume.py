...
def test_one_cell_volume():
    ...
    assert maths.isclose(
        vol.tetra_cell_volume(cp, off_hand=False), 1.0
    ), "unit cube cell volume calculation fails"
    # stretch in x (I)
    cp[:, :, 1, 0] *= 2.5
    assert maths.isclose(
        vol.tetra_cell_volume(cp, off_hand=False), 2.5
    ), "square prism cell volume calculation fails"
    # stretch in y & z (J & K)
    cp[:, 1, :, 1] *= 2.0
    cp[1, :, :, 2] *= 3.0
    assert maths.isclose(
        vol.tetra_cell_volume(cp, off_hand=False), 15.0
    ), "regular cuboid cell volume calculation fails"
    # apply a shear in x & y (should not change volume)
    cp[1, :, :, :2] += 0.5
    assert maths.isclose(
        vol.tetra_cell_volume(cp, off_hand=False), 15.0
    ), "parallelepiped cell volume calculation fails"
    # translate into negative coordinate realm
    cp[:] -= 3.827
    assert maths.isclose(
        vol.tetra_cell_volume(cp, off_hand=False), 15.0
    ), "translated cell volume calculation fails "
    # invert one axis
    cp[:, :, :, 1] *= -1.0
    assert maths.isclose(
        vol.tetra_cell_volume(cp, off_hand=True), 15.0
    ), "off hand True cell volume calculation fails"
    # collapse to plane
    cp[:, :, :, 2] = 0.0
    assert maths.isclose(
        vol.tetra_cell_volume(cp, off_hand=False), 0.0
    ), "planar cell volume calculation fails"
    assert maths.isclose(
        vol.tetra_cell_volume(cp, off_hand=True), 0.0
    ), "off hand True planar cell volume calculation fails"
    # test wedge
    cp = unit_cube().copy()
    cp[1, 1, :, 2] = 0.0
    assert maths.isclose(
        vol.tetra_cell_volume(cp, off_hand=False), 0.5
    ), "triangular prism volume calculation fails"
...
def test_array_volume():
    ...
    translate = (random(cp.size // 8) * 123.2356).reshape(
        (cp.shape[0], cp.shape[1], cp.shape[2], 1, 1, 1, 3)
    )
    cp[:] += translate
    ...
    translate = (random(cp.size // 8) * 3.23478).reshape(
        (cp.shape[0], cp.shape[1], cp.shape[2], 1, 1, 1, 3)
    )
    cp[:] -= translate
    v2 = vol.tetra_volumes(cp, off_hand=False)
