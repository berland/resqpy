...
def test_dataframe(tmp_path):
    ...
    propertied = rqdf.TimeTable(
        model, df=df, realization=0, title="time table realisation", time_series=ts
    )
    propertied.write_hdf5_and_create_xml()

    united = rqdf.TimeTable(
        model,
        df=df,
        realization=0,
    ...
    v_root = model.root(
        obj_type="Grid2dRepresentation", extra={"dataframe": "true"}, title="vanilla"
    )
    ...
    tt_root = model.root(
        obj_type="Grid2dRepresentation", extra={"dataframe": "true"}, title="time table"
    )
