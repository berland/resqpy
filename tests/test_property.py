...
@pytest.mark.parametrize(
    "case_sensitive, input_uom, expected_uom",
    [
        (False, "gapi", "gAPI"),
        (True, "gapi", "Euc"),
        (False, "m", "m"),
...
def test_uom_from_string(case_sensitive, input_uom, expected_uom):
    validated_uom = rqp.validate_uom_from_string(
        input_uom, case_sensitive=case_sensitive
    )
...
def test_property_kinds():
    ...
    assert (
        prop_kinds["amplitude"] == "Amplitude of the acoustic signal recorded. "
