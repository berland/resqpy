    ...
    def __init__(
        ...
        if earlier_timestamp is not None and later_timestamp is not None:
            if earlier_timestamp.endswith("Z"):
                earlier_timestamp = earlier_timestamp[
                    :-1
                ]  # Trailing Z is not part of iso format
...
class TimeSeries:
    ...
    def __init__(
        ...
        self.uuid = None
        if extract_from_xml:
            self.time_series_root = self.model.resolve_time_series_root(
                time_series_root
            )
            if self.time_series_root is None:
                return  # no time series in model
            ...
            self.timestamps.append(
                first_timestamp
            )  # todo: check format of first_timestamp
    ...
    def is_equivalent(self, other_ts, tol_seconds=1):
        ...
        for t_index in range(self.number_of_timestamps()):
            diff = TimeDuration(
                earlier_timestamp=self.timestamps[t_index],
    ...
    def duration_between_timestamps(self, earlier_index, later_index):
        ...
        if (
            earlier_index < 0
            or later_index >= len(self.timestamps)
            or later_index < earlier_index
        ):
            return None
        return TimeDuration(
            earlier_timestamp=self.timestamps[earlier_index],
    ...
    def create_xml(
        self, add_as_part=True, root=None, title="time series", originator=None
    ):
...
def time_series_from_nexus_summary(summary_file, parent_model=None):
    ...
    try:
        ...
        summary_entries = (
            []
        )  # list of (time step no., cumulative time (days), date (dd/mm/yyyy))
        with open(summary_file, "r") as fp:
            while True:
                ...
                date_str = words[4]
                assert 9 <= len(date_str) <= 10, (
                    "invalid date string length in summary file: " + date_str
                )
                if us_date_format:
                    date = dt.date(
                        int(date_str[-4:]), int(date_str[:-8]), int(date_str[-7:-5])
                    )
                else:
                    date = dt.date(
                        int(date_str[-4:]), int(date_str[-7:-5]), int(date_str[:-8])
                    )
        ...
        for entry in summary_entries:
            if entry[0] <= last_timestep:
                log.warning(
                    "ignoring out of sequence time step in summary file: "
                    + str(entry[0])
                )
                continue
        ...
        log.exception(
            "failed to create TimeSeries object from summary file: " + summary_file
        )
    return None
