...
class MdDatum:
    ...
    def __init__(
        ...
        if md_datum_root is not None:  # load from xml
            ...
            self.location = (
                rqet.find_tag_float(location_node, "Coordinate1"),
                rqet.find_tag_float(location_node, "Coordinate2"),
                rqet.find_tag_float(location_node, "Coordinate3"),
            )
            self.md_reference = (
                rqet.node_text(rqet.find_tag(md_datum_root, "MdReference"))
                .strip()
                .lower()
            )
            ...
            location is not None or md_reference
        ):  # set attribs from arguments; does not create xml
    ...
    def create_xml(
        ...
        arguments:
           ...
           root (optional, usually None): if not None, the newly created md datum node is appended as a child to
              this node
        ...
        assert md_reference in valid_md_reference_list, (
            "invalid measured depth reference: " + md_reference
        )
        ...
        if self.uuid is None:
            self.uuid = bu.uuid_from_string(datum.attrib["uuid"])
        else:
            datum.attrib["uuid"] = str(self.uuid)

        self.model.create_citation(root=datum, title=title, originator=originator)
...
class DeviationSurvey:
    ...
    def __init__(
        ...
        # Referenced objects
        self.md_datum = (
            md_datum  # md datum is an object in its own right, with a related crs!
        )
        ...
        if deviation_survey_root is not None:
            warnings.warn(
                "Argument deviation_survey_root is deprecated, please use uuid"
            )
    ...
    def from_data_frame(
        ...
        assert (
            station_count >= 2
        )  # vertical well could be hamdled by allowing a single station in survey?
    ...
    def from_ascii_file(
        ...
        try:
            ...
            log.error(
                "failed to read ascii deviation survey file " + deviation_survey_file
            )
            raise
    ...
    def load_from_xml(self):
        ...
        # Load XML data
        self.md_uom = rqet.length_units_from_node(
            rqet.find_tag(node, "MdUom", must_exist=True)
        )
        ...
        self.first_station = extract_xyz(
            rqet.find_tag(node, "FirstStationLocation", must_exist=True)
        )
    ...
    def create_xml(
        ...
        self.model.create_citation(
            root=ds_node, title=self.title, originator=self.originator
        )
        ...
        self.model.create_solitary_point3d(
            "FirstStationLocation", ds_node, self.first_station
        )
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "Mds", root=mds_values_node
        )
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "Azimuths", root=azimuths_values_node
        )
        ...
        inclinations_values_node = rqet.SubElement(
            inclinations, ns["resqml2"] + "Values"
        )
        inclinations_values_node.set(ns["xsi"] + "type", ns["resqml2"] + "Hdf5Dataset")
        inclinations_values_node.text = rqet.null_xml_text

        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "Inclinations", root=inclinations_values_node
        )
        ...
        if add_as_part:
            ...
            if add_relationships:
                ...
                self.model.create_reciprocal_relationship(
                    ds_node, "destinationObject", md_datum_root, "sourceObject"
                )
                ...
                self.model.create_reciprocal_relationship(
                    ds_node, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )
    ...
    def write_hdf5(self, file_name=None, mode="a"):
        ...
        h5_reg.register_dataset(
            self.uuid, "Inclinations", self.inclinations, dtype=float
        )
    ...
    def _load_related_datum(self):
        ...
        md_datum_uuid = bu.uuid_from_string(
            rqet.find_tag(rqet.find_tag(self.root, "MdDatum"), "UUID")
        )
    ...
    def _load_related_wellbore_interp(self):
        ...
        interp_uuid = rqet.find_nested_tags_text(
            self.root, ["RepresentedInterpretation", "UUID"]
        )
        if interp_uuid is None:
            represented_interp = None
        else:
            wellbore_interp_part = self.model.part_for_uuid(interp_uuid)
            represented_interp = rqo.WellboreInterpretation(
...
class Trajectory:
    ...
    def __init__(
        ...
        arguments:
           ...
           trajectory_root (optional): the root node of an xml tree representing the trajectory;
           ...
           md_datum (MdDatum object): the datum that the depths for this trajectory are measured from;
              not used if trajectory_root is not None
           deviation_survey (DeviationSurvey object, optional): if present and trajectory_root is None
           ...
           data_frame (optional): a pandas dataframe with columns 'MD', 'X', 'Y' and 'Z', holding
              the measured depths, and corresponding node locations; ignored if trajectory_root is not None
           ...
           cell_kji0_list (numpy int array of shape (N, 3)): ordered list of cell indices to be visited by
              the trajectory; ignored if trajectory_root is not None
           ...
           deviation_survey_file (string): filename of an ascii file holding the trajectory
              in a tabular form; ignored if trajectory_root is not None
           ...
           md_domain (string, optional): if present, must be 'logger' or 'driller'; the source of the original
              deviation data; ignored if trajectory_root is not None
           represented_interp (wellbore interpretation object, optional): if present, is noted as the wellbore
              interpretation object which this trajectory relates to; ignored if trajectory_root is not None
        ...
        self.measured_depths = (
            None  # known as control point parameters in the parametric line geometry
        )
        ...
        if trajectory_root is not None:
            self.load_from_xml(trajectory_root, hdf5_source_model=hdf5_source_model)
            if (
                set_tangent_vectors
                and self.knot_count > 1
                and self.tangent_vectors is None
            ):
                self.set_tangents()
        elif self.deviation_survey is not None:
            self.compute_from_deviation_survey(
                method="minimum curvature", set_tangent_vectors=set_tangent_vectors
            )
        elif data_frame is not None:
            self.load_from_data_frame(
                data_frame,
                md_uom=length_uom,
                md_datum=md_datum,
            ...
            self.load_from_wellspec(
                grid, wellspec_file, well_name, spline_mode, length_uom
            )
        elif deviation_survey_file:
            self.load_from_ascii_file(
                deviation_survey_file,
        ...
        if self.md_datum is None and self.control_points is not None:
            self.md_datum = MdDatum(
                self.model, crs_root=self.crs_root, location=self.control_points[0]
            )
    ...
    def load_from_xml(self, node, hdf5_source_model=None):
        ...
        self.crs_root = self.model.referenced_node(
            rqet.find_tag(geometry_node, "LocalCrs")
        )
        self.knot_count = int(
            rqet.node_text(rqet.find_tag(geometry_node, "KnotCount")).strip()
        )
        self.line_kind_index = int(
            rqet.node_text(rqet.find_tag(geometry_node, "LineKindIndex")).strip()
        )
        ...
        assert (
            md_datum_uuid is not None
        ), "failed to fetch uuid of md datum for trajectory"
        md_datum_part = relatives_model.part_for_uuid(md_datum_uuid)
        assert md_datum_part, "md datum part not found in model"
        self.md_datum = MdDatum(
            self.model, md_datum_root=relatives_model.root_for_part(md_datum_part)
        )
        ...
        if (
            ds_uuid is not None
        ):  # this will probably not work when relatives model is different from self.model
        ...
        if interp_uuid is None:
            ...
            self.wellbore_interpretation = rqo.WellboreInterpretation(
                self.model,
                root_node=relatives_model.root_for_part(wellbore_interp_part),
            )

    def compute_from_deviation_survey(
        self,
        survey=None,
        ...
        assert method in [
            "minimum curvature"
        ]  # if adding other methods, set line_kind_index appropriately
        ...
        if not self.title:
            self.title = rqet.find_nested_tags_text(
                survey.root_node, ["Citation", "Title"]
            )
        ...
        for sp in range(1, self.knot_count):
            ...
            if i1 == i2 and az1 == az2:
                ...
                cos_theta = min(
                    max(
                        maths.cos(i2 - i1)
                        - sin_i1 * sin_i2 * (1.0 - maths.cos(az2 - az1)),
                        -1.0,
                    ),
                    1.0,
                )
                ...
                delta_y = (
                    delta_md
                    * half_rf
                    * ((sin_i1 * maths.cos(az1)) + (sin_i2 * maths.cos(az2)))
                )
                delta_x = (
                    delta_md
                    * half_rf
                    * ((sin_i1 * maths.sin(az1)) + (sin_i2 * maths.sin(az2)))
                )
    ...
    def load_from_cell_list(self, grid, cell_kji0_list, spline_mode="cube", md_uom="m"):
        ...
        assert (
            grid is not None
        ), "grid argument missing for trajectory initislisation from cell list"
        ...
        self.line_kind_index = (
            5  # 5 means minimum curvature spline; todo: set to cubic spline value?
        )
        self.md_uom = bwam.rq_length_unit(md_uom)
        self.start_md = 0.0
        ...
        if spline_mode == "none":
            ...
            self.control_points = rql.spline(
                points, tangent_weight=spline_mode, min_subdivisions=3
            )
            self.knot_count = len(self.control_points)
        self.set_measured_depths()

    def load_from_wellspec(
        self, grid, wellspec_file, well_name, spline_mode="cube", md_uom="m"
    ):

        col_list = ["IW", "JW", "L"]
        wellspec_dict = wsk.load_wellspecs(
            wellspec_file, well=well_name, column_list=col_list
        )

        assert len(wellspec_dict) == 1, (
            "no wellspec data found in file " + wellspec_file + " for well " + well_name
        )

        df = wellspec_dict[well_name]
        assert len(df) > 0, (
            "no rows of perforation data found in wellspec for well " + well_name
        )
    ...
    def load_from_ascii_file(
        ...
        try:
            ...
            log.error(
                "failed to read ascii deviation survey file " + str(trajectory_file)
            )
            raise
    ...
    def set_tangents(self, force=False, write_hdf5=False, weight="cube"):
        ...
        assert (
            self.control_points is not None
            and len(self.control_points) == self.knot_count
        )
    ...
    def xyz_for_md(self, md):
        ...
        def search(md, i1, i2):
            if i2 - i1 == 1:
                return interpolate(
                    self.control_points[i1],
                    self.control_points[i2],
                    (md - self.measured_depths[i1])
                    / (self.measured_depths[i2] - self.measured_depths[i1]),
                )
            im = i1 + (i2 - i1) // 2
    ...
    def set_measured_depths(self):
        ...
        for sk in range(1, self.knot_count):
            self.measured_depths[sk] = self.measured_depths[sk - 1] + vec.naive_length(
                self.control_points[sk] - self.control_points[sk - 1]
            )
        self.finish_md = self.measured_depths[-1]
    ...
    def create_feature_and_interpretation(self):
        ...
        if self.wellbore_interpretation is None:
            ...
            self.wellbore_feature = rqo.WellboreFeature(
                parent_model=self.model, feature_name=self.title, extract_from_xml=False
            )
            self.wellbore_interpretation = rqo.WellboreInterpretation(
    ...
    def create_xml(
        ...
        if self.feature_and_interpretation_to_be_written:
            ...
            if self.wellbore_feature is not None:
                self.wellbore_feature.create_xml(
                    add_as_part=add_as_part, originator=originator
                )
            self.wellbore_interpretation.create_xml(
        ...
        wbt_node = self.model.new_obj_node("WellboreTrajectoryRepresentation")

        if self.uuid is None:
            self.uuid = bu.uuid_from_string(wbt_node.attrib["uuid"])
        else:
            wbt_node.attrib["uuid"] = str(self.uuid)

        self.model.create_citation(root=wbt_node, title=title, originator=originator)
        ...
        if (
            self.line_kind_index != 0
        ):  # 0 means vertical well, which doesn't need a geometry
            ...
            self.model.create_hdf5_dataset_ref(
                ext_uuid, self.uuid, "controlPointParameters", root=cpp_values_node
            )
            ...
            self.model.create_hdf5_dataset_ref(
                ext_uuid, self.uuid, "controlPoints", root=cp_coords_node
            )
            ...
            if self.tangent_vectors is not None:
                ...
                self.model.create_hdf5_dataset_ref(
                    ext_uuid, self.uuid, "tangentVectors", root=tv_coords_node
                )
        ...
        if root is not None:
            root.append(wbt_node)
        if add_as_part:
            self.model.add_part(
                "obj_WellboreTrajectoryRepresentation", self.uuid, wbt_node
            )
            if add_relationships:
                crs_root = self.crs_root
                if crs_root is None:
                ...
                self.model.create_reciprocal_relationship(
                    wbt_node, "destinationObject", crs_root, "sourceObject"
                )
                self.model.create_reciprocal_relationship(
                    wbt_node,
                    "destinationObject",
                    self.md_datum.root_node,
                    "sourceObject",
                )
                if self.deviation_survey is not None:
                    self.model.create_reciprocal_relationship(
                        wbt_node,
                        "destinationObject",
                        self.deviation_survey.root_node,
                        "sourceObject",
                    )
                if interp_root is not None:
                    self.model.create_reciprocal_relationship(
                ...
                ext_node = self.model.root_for_part(ext_part)
                self.model.create_reciprocal_relationship(
                    wbt_node, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )

        self.root_node = wbt_node
    ...
    def write_hdf5(self, file_name=None, mode="a"):
        ...
        h5_reg = rwh5.H5Register(self.model)
        h5_reg.register_dataset(
            self.uuid, "controlPointParameters", self.measured_depths
        )
...
class WellboreFrame:
    ...
    def __init__(
        ...
        #: Citation title of the wellbore frame
        self.title = None
    ...
    def load_from_xml(self, node):
        ...
        assert (
            trajectory_uuid is not None
        ), "wellbore frame trajectory reference not found in xml"
        if self.trajectory is None:
            trajectory_part = (
                "obj_WellboreTrajectoryRepresentation_" + str(trajectory_uuid) + ".xml"
            )
            self.trajectory = Trajectory(
                self.model,
                trajectory_root=self.model.root_for_part(
                    trajectory_part, is_rels=False
                ),
            )
        else:
            assert bu.matching_uuids(
                self.trajectory.uuid, trajectory_uuid
            ), "wellbore frame trajectory uuid mismatch"

        self.node_count = rqet.find_tag_int(node, "NodeCount")
        assert (
            self.node_count is not None
        ), "node count not found in xml for wellbore frame"
        assert self.node_count > 1, "fewer than 2 nodes for wellbore frame"

        mds_node = rqet.find_tag(node, "NodeMd")
        assert (
            mds_node is not None
        ), "wellbore frame measured depths hdf5 reference not found in xml"
        load_hdf5_array(self, mds_node, "node_mds")

        assert (
            self.node_mds is not None
            and self.node_mds.ndim == 1
            and self.node_mds.size == self.node_count
        )
    ...
    def create_xml(
        ...
        wf_node = self.model.new_obj_node("WellboreFrameRepresentation")

        if self.uuid is None:
            self.uuid = bu.uuid_from_string(wf_node.attrib["uuid"])
        else:
            wf_node.attrib["uuid"] = str(self.uuid)

        self.model.create_citation(root=wf_node, title=title, originator=originator)
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "NodeMd", root=mds_values_node
        )
        ...
        if add_as_part:
            ...
            if add_relationships:
                ...
                ext_node = self.model.root_for_part(ext_part)
                self.model.create_reciprocal_relationship(
                    wf_node, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )
                if self.wellbore_interpretation is not None:
                    interp_root = self.wellbore_interpretation.root_node
                    self.model.create_reciprocal_relationship(
...
class BlockedWell:
    ...
    def __init__(
        ...
        arguments:
           ...
           blocked_wellbore_root (optional): the root node of an xml tree representing the blocked well;
        ...
        self.cell_indices = (
            None  # cell_count natural cell indices, paired with non-null grid_indices
        )
        ...
        self.face_index_map = np.array(
            [[0, 1], [2, 4], [5, 3]], dtype=int
        )  # order: top, base, J-, I+, J+, I-
        # and the inverse, maps from 0..5 to (axis, p01)
        #     self.face_index_inverse_map = np.array([[0, 0], [0, 1], [1, 1], [2, 1], [1, 0], [2, 0]], dtype = int)
        self.face_index_inverse_map = np.array(
            [[0, 0], [0, 1], [1, 0], [2, 1], [1, 1], [2, 0]], dtype=int
        )
        # note: the rework_face_pairs() method, below, overwrites the face indices based on I, J cell indices

        if blocked_well_root is not None:
            self.load_from_xml(blocked_well_root)
        else:
            self.wellbore_interpretation = represented_interp
            if grid is None:
            ...
            if self.trajectory is not None:
                ...
                okay = self.derive_from_wellspec(
                    wellspec_file,
                    well_name,
                    grid,
    ...
    def load_from_xml(self, node):
        ...
        if not self.well_name:
            self.well_name = rqet.citation_title_for_node(node)
        ...
        assert (
            trajectory_uuid is not None
        ), "blocked well trajectory reference not found in xml"
        if self.trajectory is None:
            trajectory_part = (
                "obj_WellboreTrajectoryRepresentation_" + str(trajectory_uuid) + ".xml"
            )
            ...
            assert bu.matching_uuids(
                self.trajectory.uuid, trajectory_uuid
            ), "blocked well trajectory uuid mismatch"

        self.node_count = rqet.find_tag_int(node, "NodeCount")
        assert (
            self.node_count is not None and self.node_count >= 2
        ), "problem with blocked well node count"

        mds_node = rqet.find_tag(node, "NodeMd")
        assert (
            mds_node is not None
        ), "blocked well node measured depths hdf5 reference not found in xml"
        load_hdf5_array(self, mds_node, "node_mds")
        ...
        assert (
            ci_node is not None
        ), "blocked well cell indices hdf5 reference not found in xml"
        load_hdf5_array(self, ci_node, "cell_indices", dtype=int)
        assert (
            self.cell_indices is not None
            and self.cell_indices.ndim == 1
            and self.cell_indices.size == self.cell_count
        ), "mismatch in number of cell indices for blocked well"
        ...
        assert (
            fi_node is not None
        ), "blocked well face indices hdf5 reference not found in xml"
        load_hdf5_array(self, fi_node, "raw_face_indices", dtype="int")
        assert (
            self.raw_face_indices is not None
        ), "failed to load face indices for blocked well"
        assert (
            self.raw_face_indices.size == 2 * self.cell_count
        ), "mismatch in number of cell faces for blocked well"
        if self.raw_face_indices.ndim > 1:
            self.raw_face_indices = self.raw_face_indices.reshape(
                (self.raw_face_indices.size,)
            )
        ...
        assert (
            gi_node is not None
        ), "blocked well grid indices hdf5 reference not found in xml"
        load_hdf5_array(self, gi_node, "grid_indices", dtype="int")
        assert (
            self.grid_indices is not None
            and self.grid_indices.ndim == 1
            and self.grid_indices.size == self.node_count - 1
        )
        unique_grid_indices = np.unique(
            self.grid_indices
        ...
        assert (
            len(grid_node_list) > 0
        ), "blocked well grid reference(s) not found in xml"
        assert unique_grid_indices[0] >= -1 and unique_grid_indices[-1] < len(
            grid_node_list
        ), "blocked well grid index out of range"
        assert (
            np.count_nonzero(self.grid_indices >= 0) == self.cell_count
        ), "mismatch in number of blocked well intervals"
        self.grid_list = []
        for grid_ref_node in grid_node_list:
            grid_node = self.model.referenced_node(grid_ref_node)
            assert (
                grid_node is not None
            ), "grid referenced in blocked well xml is not present in model"
    ...
    def single_grid(self):
        ...
        ), "blocked well is not referring to exactly one grid"
    ...
    def cell_indices_and_grid_list(self):
        ...
        for cell_number in range(self.cell_count):
            ...
            cell_indices[cell_number] = grid.denaturalized_cell_index(
                self.cell_indices[cell_number]
            )
    ...
    def cell_indices_for_grid_uuid(self, grid_uuid):
        ...
        for cell_number in range(len(ci_list)):
            mask[cell_number] = bu.matching_uuids(
                grid_list[cell_number].uuid, grid_uuid
            )
    ...
    def compute_from_trajectory(
        ...
        if np.any(np.isnan(grid.points_ref(masked=False))):
            log.warning(
                "grid does not have geometry defined everywhere: attempting fill"
            )
            import resqpy.derived_model as rqdm

            fill_grid = rqdm.copy_grid(grid)
            fill_grid.set_geometry_is_defined(
                nullify_partial_pillars=True, complete_all=True
            )
            ...
            grid = fill_grid
        assert (
            trajectory.control_points is not None
            and trajectory.crs_root is not None
            and grid.crs_root is not None
        )
        ...
        bw = rgs.populate_blocked_well_from_trajectory(
            self,
            grid,
        ...
        if bw is None:
            raise Exception(
                "failed to generate blocked well from trajectory with uuid: "
                + str(trajectory.uuid)
            )
    ...
    def set_for_column(self, well_name, grid, col_ji0, skip_inactive=True):
        ...
        pinch_col = grid.pinched_out(cache_cp_array=True, cache_pinchout_array=True)[
            :, col_ji0[0], col_ji0[1]
        ]
        ...
        return self.derive_from_dataframe(
            df, self.well_name, grid, use_face_centres=True
        )

    def derive_from_wellspec(
        self,
        wellspec_file,
        well_name,
        grid,
    ...
    def derive_from_dataframe(
        self, df, well_name, grid, grid_name_to_check=None, use_face_centres=False
    ):
        ...
        angles_present = (
            "ANGLV" in df.columns
            and "ANGLA" in df.columns
            and not pd.isnull(df.iloc[0]["ANGLV"])
            and not pd.isnull(df.iloc[0]["ANGLA"])
        )

        if not angles_present and not use_face_centres:
            log.warning(
                f"ANGLV and/or ANGLA data unavailable for well {well_name}: using face centres"
            )
        ...
        for i in range(len(df)):  # for each row in the dataframe for this well
            ...
            if pd.isna(row[0]) or pd.isna(row[1]) or pd.isna(row[2]):
                log.error(
                    "missing cell index in wellspec data for well "
                    + str(well_name)
                    + " row "
                    + str(i + 1)
                )
            ...
            if (
                grid_name_to_check
                and pd.notna(row["GRID"])
                and grid_name_to_check != str(row["GRID"]).upper()
            ):
                other_grid = str(row["GRID"])
                if skipped_warning_grid != other_grid:
                    log.warning(
                        "skipping perforation(s) in grid "
                        + other_grid
                        + " for well "
                        + str(well_name)
                    )
            ...
            assert not np.any(
                np.isnan(cp)
            ), "missing geometry for perforation cell for well " + str(well_name)
            use_k = pd.isna(row[3]) or pd.isna(
                row[4]
            )  # angla or anglv missing in wellspecs; could treat as vertical instead?
            ...
            if use_k:
                ...
                if angles_present:
                    ...
                    if inclination < 0.1:
                        ...
                        azimuth = (
                            vec.azimuth(i_vector) - angla
                        )  # see Nexus keyword reference doc
                    well_vector = (
                        vec.unit_vector_from_azimuth_and_inclination(
                            azimuth, inclination
                        )
                        * 10000.0
                    )
                ...
                if (
                    use_face_centres
                ):  # override the vector based xyz entry and exit points with face centres
                    if entry_axis == 0:
            ...
            if not vec.isclose(
                previous_xyz, entry_xyz, tolerance=0.05
            ):  # add an unblocked interval
                log.debug("adding unblocked interval")
                trajectory_points.append(entry_xyz)
                new_md = trajectory_mds[-1] + vec.naive_length(
                    entry_xyz - previous_xyz
                )  # assumes x, y & z units are same
        ...
        self.create_md_datum_and_trajectory(
            grid, trajectory_mds, trajectory_points, length_uom, well_name
        )
    ...
    def import_from_rms_cellio(
        self, cellio_file, well_name, grid, include_overburden_unblocked_interval=False
    ):
        ...
        log.debug(
            "grid z increasing downwards: "
            + str(grid_z_inc_down)
            + "(type: "
            + str(type(grid_z_inc_down))
            + ")"
        )
        ...
        try:
            ...
            with open(cellio_file, "r") as fp:
                ...
                while not kf.blank_line(fp):
                    ...
                    assert (
                        len(words) >= 9
                    ), "not enough items on data line in cell I/O file, minimum 9 expected"
                    ...
                    assert np.all(0 <= cell_kji0) and np.all(
                        cell_kji0 < grid.extent_kji
                    ), "cell I/O cell index not within grid extent"
                    entry_xyz = np.array(
                    ...
                    cp = grid.corner_points(
                        cell_kji0=cell_kji0, cache_resqml_array=False
                    )
                    assert not np.any(np.isnan(cp)), (
                        "missing geometry for perforation cell(kji0) "
                        + str(cell_kji0)
                        + " for well "
                        + str(well_name)
                    )
                    ...
                    (
                        ...
                        exit_polarity,
                        facial_exit_xyz,
                    ) = find_entry_and_exit(cp, entry_vector, exit_vector, well_name)
                    ...
                    log.debug(
                        "adding blocked interval for cell kji0: " + str(cell_kji0)
                    )
                    trajectory_points.append(exit_xyz)
                    new_md = trajectory_mds[-1] + vec.naive_length(
                        exit_xyz - previous_xyz
                ...
                self.cell_indices = grid.natural_cell_indices(
                    np.array(blocked_cells_kji0)
                )
            ...
            log.exception(
                "failed to import info for blocked well "
                + str(well_name)
                + " from cell I/O file "
                + str(cellio_file)
            )
    ...
    def dataframe(
        ...
        def get_ref_vector(grid, grid_crs, cell_kji0, mode):
            ...
            if mode == "z+":
                ...
                ref_vector = vec.unit_vector(
                    vec.cross_product(cell_axial_vectors[1], cell_axial_vectors[2])
                )
        ...
        if "KH" in column_list or min_kh is not None or "WBC" in column_list:
            assert (
                perm_i_uuid is not None
            ), "WBC or KH requested (or minimum specified) without I direction permeabilty being specified"
            doing_kh = True
        do_well_inflow = (
            "WI" in column_list or "WBC" in column_list or "RADB" in column_list
        )
        if do_well_inflow:
            assert (
                perm_i_uuid is not None
            ), "WI, RADB or WBC requested without I direction permeabilty being specified"
        if doing_kh or do_well_inflow:
            if perm_j_uuid is None and perm_k_uuid is None:
                ...
                isotropic_perm = bu.matching_uuids(
                    perm_i_uuid, perm_j_uuid
                ) and bu.matching_uuids(perm_i_uuid, perm_k_uuid)
        if max_satw is not None:
            assert (
                satw_uuid is not None
            ), "water saturation limit specified without saturation property array"
        if min_sato is not None:
            assert (
                sato_uuid is not None
            ), "oil saturation limit specified without saturation property array"
        if max_satg is not None:
            assert (
                satg_uuid is not None
            ), "gas saturation limit specified without saturation property array"
        if region_list is not None:
            assert (
                region_uuid is not None
            ), "region list specified without region property array"
        ...
        if (
            "LENGTH" in column_list
            and "PPERF" in column_list
            and "KH" not in column_list
            and perforation_list is not None
        ):
            log.warning(
                "both LENGTH and PPERF will include effects of partial perforation; only one should be used in WELLSPEC"
            )
        elif (
            perforation_list is not None
            and "LENGTH" not in column_list
            and "PPERF" not in column_list
            and "KH" not in column_list
            and "WBC" not in column_list
        ):
            log.warning(
                "perforation list supplied but no use of LENGTH, KH, PPERF nor WBC"
            )
        if min_k0 is None:
            min_k0 = 0
        else:
        ...
        if k0_list is not None and len(k0_list) == 0:
            log.warning(
                "no layers included for blocked well dataframe: no rows will be included"
            )
        if perforation_list is not None and len(perforation_list) == 0:
            log.warning(
                "empty perforation list specified for blocked well dataframe: no rows will be included"
            )
        doing_angles = (
            "ANGLA" in column_list
            or "ANGLV" in column_list
            or doing_kh
            or do_well_inflow
        )
        doing_xyz = "X" in column_list or "Y" in column_list or "DEPTH" in column_list
        doing_entry_exit = doing_angles or (
            "LENGTH" in column_list and length_mode == "straight"
        )
        grid_crs_list = []
        for grid in self.grid_list:
            ...
            if (
                grid_crs.z_units != grid_crs.xy_units
                and (
                    len(column_list) > 1
                    or (len(column_list) == 1 and column_list[0] != "GRID")
                )
                is not None
            ):
                log.error(
                    "grid "
                    + str(rqet.citation_title_for_node(grid.root_node))
        ...
        for interval in range(interval_count):
            ...
            if active_only and grid.inactive is not None and grid.inactive[tuple_kji0]:
                continue
            if (min_k0 is not None and cell_kji0[0] < min_k0) or (
                max_k0 is not None and cell_kji0[0] > max_k0
            ):
                continue
            if k0_list is not None and cell_kji0[0] not in k0_list:
                continue
            if (
                region_list is not None
                and prop_array(region_uuid, grid)[tuple_kji0] not in region_list
            ):
                continue
            if (
                max_satw is not None
                and prop_array(satw_uuid, grid)[tuple_kji0] > max_satw
            ):
                continue
            if (
                min_sato is not None
                and prop_array(sato_uuid, grid)[tuple_kji0] < min_sato
            ):
                continue
            if (
                max_satg is not None
                and prop_array(satg_uuid, grid)[tuple_kji0] > max_satg
            ):
                continue
            ...
            if perforation_list is not None:
                ...
                for perf_start, perf_end in perforation_list:
                    if (
                        perf_end <= self.node_mds[interval]
                        or perf_start >= self.node_mds[interval + 1]
                    ):
                        continue
                    if perf_start <= self.node_mds[interval]:
                        if perf_end >= self.node_mds[interval + 1]:
                            perf_length += (
                                self.node_mds[interval + 1] - self.node_mds[interval]
                            )
                            break
                        else:
            ...
            if doing_entry_exit:
                ...
                if use_face_centres:
                    entry_xyz = grid.face_centre(
                        cell_kji0,
                        self.face_pair_indices[interval, 0, 0],
                        self.face_pair_indices[interval, 0, 1],
                    )
                    if self.face_pair_indices[interval, 1, 0] >= 0:
                        exit_xyz = grid.face_centre(
                            cell_kji0,
                            self.face_pair_indices[interval, 1, 0],
            ...
            if length_mode == "MD":
                ...
                if (
                    length_uom is not None
                    and self.trajectory is not None
                    and length_uom != self.trajectory.md_uom
                ):
                    length = bwam.convert_lengths(
                        length, self.trajectory.md_uom, length_uom
                    )
            else:  # use straight line length between entry and exit
                length = vec.naive_length(
                    np.array(exit_xyz) - np.array(entry_xyz)
                )  # trajectory crs, unless use_face_centres!
                if length_uom is not None:
                    length = bwam.convert_lengths(length, ee_crs.z_units, length_uom)
                elif self.trajectory is not None:
                    length = bwam.convert_lengths(
                        length, ee_crs.z_units, self.trajectory.md_uom
                    )
            if perforation_list is not None:
            ...
            if doing_angles and not (
                set_k_face_intervals_vertical
                and (
                    np.all(self.face_pair_indices[ci] == k_face_check)
                    or np.all(self.face_pair_indices[ci] == k_face_check_end)
                )
            ):
                vector = vec.unit_vector(
                    np.array(exit_xyz) - np.array(entry_xyz)
                )  # nominal wellbore vector for interval
                if (
                    traj_z_inc_down is not None
                    and traj_z_inc_down != grid_crs.z_inc_down
                ):
                    vector[2] = -vector[2]
                ...
                if angla_plane_ref == anglv_ref:
                    ...
                    a_ref_vector = get_ref_vector(
                        grid, grid_crs, cell_kji0, angla_plane_ref
                    )
                #           log.debug('a ref vector: ' + str(a_ref_vector))
                cosine_anglv = min(
                    max(vec.dot_product(vector, v_ref_vector), -1.0), 1.0
                )
                ...
                if anglv != 0.0:
                    ...
                    if (
                        a_ref_vector is not None
                    ):  # project vector and i axis onto a plane
                ...
                if isotropic_perm and ntg_is_one:
                    ...
                    if np.isnan(k_i) or np.isnan(k_j):
                        ...
                        if k_e == 0.0:
                            ...
                            l_j = (
                                length * maths.sqrt(k_e / k_j) * sine_anglv * sine_angla
                            )
            ...
            if do_well_inflow:
                if isotropic_perm and ntg_is_one:
                    ...
                    r_wi = (
                        ...
                        * radw
                        * (maths.sqrt(k_ei / k_j) + maths.sqrt(k_ei / k_k))
                    )
                    r_wj = (
                        0.0
                        if k_ej == 0.0
                        ...
                        * radw
                        * (maths.sqrt(k_ej / k_i) + maths.sqrt(k_ej / k_k))
                    )
                    r_wk = (
                        0.0
                        if k_ek == 0.0
                        ...
                        * radw
                        * (maths.sqrt(k_ek / k_i) + maths.sqrt(k_ek / k_j))
                    )
                ...
                for axis in range(3):
                    d2[axis] = np.sum(
                        cell_axial_vectors[axis] * cell_axial_vectors[axis]
                    )
                r_bi = (
                    0.0
                    if k_ei == 0.0
                    else 0.14 * maths.sqrt(k_ei * (d2[1] / k_j + d2[0] / k_k))
                )
                r_bj = (
                    0.0
                    if k_ej == 0.0
                    else 0.14 * maths.sqrt(k_ej * (d2[2] / k_i + d2[0] / k_k))
                )
                r_bk = (
                    0.0
                    if k_ek == 0.0
                    else 0.14 * maths.sqrt(k_ek * (d2[2] / k_i + d2[1] / k_j))
                )
                ...
                wi = (
                    0.0
                    if radb <= 0.0
                    else 2.0 * maths.pi / (maths.log(radb / radw) + skin)
                )
                if "WBC" in column_list:
                    conversion_constant = (
                        8.5270171e-5 if length_uom == "m" else 0.006328286
                    )
                    wbc = (
                        conversion_constant * kh * wi
                    )  # note: pperf aleady accounted for in kh
                else:
                    wbc = None
            else:
                radb = wi = wbc = None
    ...
    def write_wellspec(
        ...
        with open(wellspec_file, mode=mode) as fp:
            ...
            if (
                ...
                and self.trajectory is not None
                and (
                    "LENGTH" in extra_columns_list
                    or "MD" in extra_columns_list
                    or "KH" in extra_columns_list
                )
            ):
                fp.write(
    ...
    def create_feature_and_interpretation(self, shared_interpretation=True):
        ...
        """
        if self.trajectory is not None:
            traj_interp_uuid = self.model.uuid(
                obj_type="WellboreInterpretation", related_uuid=self.trajectory.uuid
            )
            if traj_interp_uuid is not None:
                if shared_interpretation:
                    interp_root = self.model.root(uuid=traj_interp_uuid)
                    if interp_root is not None:
                        self.wellbore_interpretation = rqo.WellboreInterpretation(
                            parent_model=self.model, root_node=interp_root
                        )
                traj_feature_root = self.model.root(
                    obj_type="WellboreFeature", related_uuid=traj_interp_uuid
                )
                if traj_feature_root is not None:
                    self.wellbore_feature = rqo.WellboreFeature(
                        parent_model=self.model, root_node=traj_feature_root
                    )
        if self.wellbore_feature is None:
            self.wellbore_feature = rqo.WellboreFeature(
                parent_model=self.model,
                feature_name=self.trajectory.title,
                extract_from_xml=False,
            )
        ...
        if self.wellbore_interpretation is None:
            ...
            if (
                self.trajectory.wellbore_interpretation is None
                and shared_interpretation
            ):
                self.trajectory.wellbore_interpretation = self.wellbore_interpretation
            self.interpretation_to_be_written = True

    def create_md_datum_and_trajectory(
        self,
        grid,
        trajectory_mds,
        trajectory_points,
    ...
    def create_xml(
        ...
        if self.interpretation_to_be_written:
            ...
            self.wellbore_interpretation.create_xml(
                add_as_part=add_as_part,
                title_suffix="blocked well",
                add_relationships=add_relationships,
                originator=originator,
            )

        if (
            create_for_trajectory_if_needed
            and self.trajectory_to_be_written
            and self.trajectory.root_node is None
        ):
            md_datum_root = self.trajectory.md_datum.create_xml(
                add_as_part=add_as_part,
                add_relationships=add_relationships,
                title=title + " simulator md datum",
                originator=originator,
            )
            self.trajectory.root_node = self.trajectory.create_xml(
                ext_uuid,
        ...
        bw_node = self.model.new_obj_node("BlockedWellboreRepresentation")

        if self.uuid is None:
            self.uuid = bu.uuid_from_string(bw_node.attrib["uuid"])
        else:
            bw_node.attrib["uuid"] = str(self.uuid)

        self.model.create_citation(root=bw_node, title=title, originator=originator)
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "CellIndices", root=cis_values_node
        )
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "GridIndices", root=gis_values_node
        )

        fis_node = rqet.SubElement(
            bw_node, ns["resqml2"] + "LocalFacePairPerCellIndices"
        )
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "LocalFacePairPerCellIndices", root=fis_values_node
        )
        ...
        if add_as_part:
            ...
            if add_relationships:
                ...
                for grid in self.grid_list:
                    self.model.create_reciprocal_relationship(
                        bw_node, "destinationObject", grid.grid_root, "sourceObject"
                    )
                if interp_root is not None:
                    self.model.create_reciprocal_relationship(
                ...
                ext_node = self.model.root_for_part(ext_part)
                self.model.create_reciprocal_relationship(
                    bw_node, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )

        self.root_node = bw_node

        return bw_node

    def write_hdf5(
        self, file_name=None, mode="a", create_for_trajectory_if_needed=True
    ):
        ...
        h5_reg.register_dataset(self.uuid, "NodeMd", self.node_mds)
        h5_reg.register_dataset(
            self.uuid, "CellIndices", self.cell_indices
        )  # could use int32?
        h5_reg.register_dataset(
            self.uuid, "GridIndices", self.grid_indices
        )  # could use int32?
        # convert face index pairs from [axis, polarity] back to strange local face numbering
        mask = (self.face_pair_indices.flatten() == -1).reshape(
            (-1, 2)
        )  # 2nd axis is (axis, polarity)
        masked_face_indices = np.where(
            mask, 0, self.face_pair_indices.reshape((-1, 2))
        )  # 2nd axis is (axis, polarity)
        ...
        raw_face_indices = np.where(
            mask[:, 0],
            -1,
            self.face_index_map[
                masked_face_indices[:, 0], masked_face_indices[:, 1]
            ].flatten(),
        ).reshape(-1)

        h5_reg.register_dataset(
            self.uuid, "LocalFacePairPerCellIndices", raw_face_indices
        )  # could use uint8?

        h5_reg.write(file=file_name, mode=mode)


class WellboreMarkerFrame:
    ...
    def __init__(self, parent_model, wellbore_marker_frame_root=None, trajectory=None):
        """Creates a new wellbore marker object and optionally loads it from xml, or trajectory, or Nexus wellspec file.

        arguments:
           parent_model (model.Model object): the model which the new blocked well belongs to
           wellbore_marker_root (optional): the root node of an xml tree representing the wellbore marker;
           trajectory (optional, Trajectory object): the trajectory of the well, to be intersected with the grid;
        ...
        self.node_count = (
            None  # number of measured depth nodes, each being for a marker
        )
        ...
        if wellbore_marker_frame_root is not None:
            self.root_node = wellbore_marker_frame_root
            self.load_from_xml(wellbore_marker_frame_root)
        else:
            log.error("no root node was passed in")
    ...
    def get_trajectory_obj(self, trajectory_uuid):
        ...
        if trajectory_uuid is None:
            ...
            assert (
                trajectory_root_node is not None
            ), "referenced wellbore trajectory missing from model"
    ...
    def load_from_xml(self, wellbore_marker_frame_root):
        ...
        self.uuid = bu.uuid_from_string(
            wellbore_marker_frame_root.attrib["uuid"].strip()
        )

        if self.trajectory is None:
            self.trajectory = self.get_trajectory_obj(
                rqet.find_nested_tags_text(
                    wellbore_marker_frame_root, ["Trajectory", "UUID"]
                )
            )
        if wellbore_marker_frame_root is None:
            log.error("no wellbore marker root found")
            return None
        ...
        for tag in rqet.list_of_tag(wellbore_marker_frame_root, "WellboreMarker"):
            interp_tag = rqet.content_type(
                rqet.find_nested_tags_text(tag, ["Interpretation", "ContentType"])
            )
            if interp_tag is not None:
                interp_obj = self.get_interpretation_obj(
                    rqet.find_nested_tags_text(tag, ["Interpretation", "UUID"]),
                    interp_tag,
                )
            else:
                interp_obj = None
            self.wellbore_marker_list.append(
                (
                    str(rqet.uuid_for_part_root(tag)),
                    rqet.find_tag_text(tag, "GeologicBoundaryKind"),
                    rqet.find_nested_tags_text(tag, ["Citation", "Title"]),
                    interp_obj,
                )
            )
        ...
        for i in range(self.node_count):
            ...
            if interp:
                if boundary_kind == "horizon":
                    feature_name = rqo.GeneticBoundaryFeature(
                        self.model, root_node=interp.feature_root
                    ).feature_name
                elif boundary_kind == "fault":
                    feature_name = rqo.TectonicBoundaryFeature(
                        self.model, root_node=interp.feature_root
                    ).feature_name
            ...
            if self.trajectory.wellbore_interpretation is None:
                ...
                )  # todo: trace through wellbore interp to wellbore feature name
            else:
                well_name = (
                    '"' + self.trajectory.wellbore_interpretation.title + '"'
                )  # use wellbore_interpretation title instead, RMS exports have feature_name as "Wellbore feature"
    ...
    def create_xml(
        ...
        wbm_node = self.model.new_obj_node("WellboreMarkerFrameRepresentation")
        if self.uuid is not None:
            wbm_node.attrib["uuid"] = str(self.uuid)
        else:
            self.uuid = wbm_node.attrib["uuid"]

        self.model.create_citation(root=wbm_node, title=title, originator=originator)
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "mds", root=md_values_node
        )
        ...
        for marker in self.wellbore_marker_list:
            ...
            wbm_node_obj = self.model.new_obj_node(
                "WellboreMarker", is_top_lvl_obj=False
            )
            wbm_node_obj.set("uuid", marker[0])
            wbm_node.append(wbm_node_obj)
            wbm_gb_node = rqet.SubElement(
                wbm_node_obj, ns["resqml2"] + "GeologicBoundaryKind"
            )
            ...
            if interp is not None:
                ...
                if "HorizonInterpretation" in str(type(marker[3])):
                    self.model.create_ref_node(
                        "Interpretation",
                        rqet.find_tag(
                            rqet.find_tag(interp_root, "Citation"), "Title"
                        ).text,
                        bu.uuid_from_string(interp_root.attrib["uuid"]),
        ...
        self.root_node = wbm_node

        # add as part
        if add_as_part:
            self.model.add_part(
                "obj_WellboreMarkerFrameRepresentation", self.uuid, wbm_node
            )
            ...
            if add_relationships:
                ...
                ext_node = self.model.root_for_part(ext_part)
                self.model.create_reciprocal_relationship(
                    wbm_node, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )

                for marker in self.wellbore_marker_list:
                    self.model.create_reciprocal_relationship(
                        wbm_node, "destinationObject", marker[3].root, "sourceObject"
                    )
...
def add_las_to_trajectory(
    las: lasio.LASFile,
    trajectory,
    realization=None,
    check_well_name=False,
    case_sensitive_units=False,
):
    ...
    bwam.convert_lengths(
        depth_values, from_units=las_depth_uom, to_units=trajectory.md_uom
    )
    ...
    for curve in las.curves[1:]:
        ...
        uom = rqp.validate_uom_from_string(
            curve.unit, case_sensitive=case_sensitive_units
        )
...
def add_logs_from_cellio(blockedwell, cellio):
    ...
    with open(cellio, "r") as fp:
            ...
            if wellfound:
                if len(words) > 2 and not words[0].isdigit():
                    ...
                    if len(words) > 9:
                        assert len(cols) == len(
                            words
                        ), "Number of columns found should match header of file"
                        data.append(words)
            if len(words) == 3:
                if words[0].upper() == well_name.upper():
                    wellfound = True
        assert (
            len(data) > 0 and len(cols) > 3
        ), f"No data for well {well_name} found in file"
        ...
        df["cell_indices"] = grid.natural_cell_indices(
            np.array(
                (df["k_index"] - 1, df["j_index"] - 1, df["i_index"] - 1), dtype=int
            ).T
        )
        df = df.drop(
            [
    ...
    assert (
        df["cell_indices"] == blockedwell.cell_indices
    ...
    for log in df.columns:
        if log not in ["cell_indices"]:
            ...
            if data_type == "categorical":
                lookup_uuid = lookup_dict[
                    log
                ]  # For categorical data, find or generate a StringLookupTable
            else:
                lookup_uuid = None
            ...
            for i, index in enumerate(blockedwell.cell_grid_link):
                if index == -1:
                    ...
                    if (
                        blockedwell.cell_indices[index]
                        == list(df["cell_indices"])[index]
                    ):
            ...
            collection.write_hdf5_for_imported_list()
            collection.create_xml_for_imported_list_and_add_parts_to_model(
                string_lookup_uuid=lookup_uuid
            )
...
def lookup_from_cellio(line, model):
    ...
    for existing in model.parts_list_of_type("obj_StringTableLookup"):
        table = rqp.StringLookup(
            parent_model=model, root_node=model.root_for_part(existing)
        )
        if table.title == title:
            if table.str_dict == lookup_dict:
                return (
                    table.uuid
                )  # If the exact table exists, reuse it by returning the uuid

    # If no matching StringLookupTable exists, make a new one and return the uuid
    lookup = rqp.StringLookup(
        parent_model=model, int_to_str_dict=lookup_dict, title=title
    )
...
def add_wells_from_ascii_file(
    ...
    assert (
        crs_root is not None
    ), "coordinate reference system not found when trying to add wells"
    ...
    if len(unique_wells) == 0:
        log.warning(
            "no well data found in ascii trajectory file: " + str(trajectory_file)
        )
    ...
    for well_name in unique_wells:
        ...
        if first_row[md_col] == 0.0:
            ...
            md_datum = MdDatum(
                model,
                crs_root=crs_root,
                location=(first_row[x_col], first_row[y_col], 0.0),
            )  # sea level datum
        md_datum.create_xml(title=str(well_name))
        md_datum_list.append(md_datum)

        # create a well feature and add as part
        feature = rqo.WellboreFeature(
            model, extract_from_xml=False, feature_name=well_name
        )
        feature.create_xml()
        feature_list.append(feature)

        # create interpretation and add as part
        interpretation = rqo.WellboreInterpretation(
            model, extract_from_xml=False, is_drilled=drilled, wellbore_feature=feature
        )
...
def well_name(well_object, model=None):
    ...
    def best_root_for_object(well_object, model=None):
        ...
        if isinstance(well_object, str):
            ...
            assert (
                obj_uuid is not None
            ), "well_name string argument could not be interpreted as uuid"
        ...
        if isinstance(well_object, bu.uuid.UUID):
            ...
            ), "uuid not found in model when looking for well name"
            ...
            if well_object.trajectory is not None:
                traj_root = well_object.trajectory.root_node
        elif isinstance(
            well_object, WellboreMarkerFrame
        ):  # note: trajectory might be None
        ...
        if obj_type == "WellboreFeature":
            ...
            feat_roots = model.roots(
                obj_type="WellboreFeature", related_uuid=obj_uuid
            )  # should return one root
            traj_roots = model.roots(
                obj_type="WellboreTrajectoryRepresentation", related_uuid=obj_uuid
            )
            ...
            root_list = [
                best_root_for_object(
                    model.root(obj_type="MdDatum", related_uuid=obj_uuid), model=model
                )
            ]
...
def add_blocked_wells_from_wellspec(model, grid, wellspec_file):
    ...
    for well in well_list_dict:
        ...
        if (
            not bw.node_count
        ):  # failed to load from wellspec, eg. because of no perforations in grid
            log.warning("no wellspec data loaded for well: " + str(well))
            continue
        bw.write_hdf5(
            model.h5_file_name(), mode="a", create_for_trajectory_if_needed=True
        )
...
def extract_xyz(xyz_node):
    ...
    xyz = np.zeros(3)
    for axis in range(3):
        xyz[axis] = rqet.find_tag_float(
            xyz_node, "Coordinate" + str(axis + 1), must_exist=True
        )
    return tuple(xyz)
...
def load_hdf5_array(
    object, node, array_attribute, tag="Values", dtype="float", model=None
):
...
def find_entry_and_exit(cp, entry_vector, exit_vector, well_name):
    ...
    cell_centre = np.mean(cp, axis=(0, 1, 2))
    face_triangles = gf.triangles_for_cell_faces(cp).reshape(
        -1, 3, 3
    )  # flattened first index 4 values per face
    entry_points = intersect.line_triangles_intersects(
        cell_centre, entry_vector, face_triangles, line_segment=True
    )
    ...
    for t in range(24):
        if not np.any(np.isnan(entry_points[t])):
            ...
            break
    assert (
        entry_axis is not None
    ), "failed to find entry face for a perforation in well " + str(well_name)
    exit_points = intersect.line_triangles_intersects(
        cell_centre, exit_vector, face_triangles, line_segment=True
    )
    ...
    for t in range(24):
        if not np.any(np.isnan(exit_points[t])):
            ...
            break
    assert (
        exit_axis is not None
    ), "failed to find exit face for a perforation in well " + str(well_name)
