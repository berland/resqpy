...
class FineCoarse:
    ...
    def assert_valid(self):
        ...
        for axis in range(3):
            assert (
                self.constant_ratios[axis] is None
                or _is_int(self.constant_ratios[axis])
                and self.constant_ratios[axis] > 0
            )
            if self.constant_ratios[axis] is None:
                assert (
                    self.vector_ratios[axis] is not None
                    and isinstance(self.vector_ratios[axis], np.ndarray)
                    and self.vector_ratios[axis].ndim == 1
                    and str(self.vector_ratios[axis].dtype).startswith("int")
                )
                ...
                assert (
                    self.coarse_extent_kji[axis] * self.constant_ratios[axis]
                    == self.fine_extent_kji[axis]
                )
            ...
            if self.equal_proportions[axis]:
                ...
                assert (
                    self.vector_proportions[axis] is not None
                    and isinstance(self.vector_proportions[axis], list)
                    and len(self.vector_proportions[axis])  # could allow a tuple
                    == self.coarse_extent_kji[axis]
                )
                for c0 in range(self.coarse_extent_kji[axis]):
                    assert (
                        isinstance(self.vector_proportions[axis][c0], np.ndarray)
                        and self.vector_proportions[axis][c0].ndim == 1
                        and str(self.vector_proportions[axis][c0].dtype).startswith(
                            "float"
                        )
                        and len(self.vector_proportions[axis][c0])
                        == self.ratio(axis, c0)
                    )
    ...
    def coarse_for_fine_kji0(self, fine_kji0):
        ...
        if self.fine_to_coarse_mapping is None:
            self._set_fine_to_coarse_mapping()
        return (
            self.fine_to_coarse_mapping[0][fine_kji0[0]],
    ...
    def fine_base_for_coarse_axial(self, axis, c0):
        ...
        return np.sum(
            self.vector_ratios[axis][:c0]
        )  # todo: check this returns zero for c0 == 0
    ...
    def set_constant_ratio(self, axis):
        ...
        assert 0 <= axis < 3
        extent_ratio, remainder = divmod(
            self.fine_extent_kji[axis], self.coarse_extent_kji[axis]
        )
        assert remainder == 0, (
            "coarse extent in " + "KJI"[axis] + " is not a multiple of fine extent"
        )
    ...
    def set_ratio_vector(self, axis, vector):
        ...
        assert (
            len(vector) == self.coarse_extent_kji[axis]
        ), "length of vector of refinement ratios does not match coarse extent"
        assert (
            np.sum(vector) == self.fine_extent_kji[axis]
    ...
    def set_proportions_list_of_vectors(self, axis, list_of_vectors):
        ...
        for c0 in range(self.coarse_extent_kji[axis]):
            ...
            assert len(vector) == count, (
                "wrong number of proportions for axis: "
                + str(axis)
                + " slice(0): "
                + c0
            )
    ...
    def write_cartref(
        ...
        assert (
            self.within_coarse_box is not None
        ), "box within larger coarse grid required for cartref"
        ...
        if len(lgr_name) > 8 or (root_name and len(root_name) > 8):
            log.warning(
                "Nexus might only differentiate first 8 characters of grid names"
            )
        with open(filename, mode) as fp:
            ...
            fp.write(
                "   "
                + box.spaced_string_iijjkk1_for_box_kji0(self.within_coarse_box)
                + "\n"
            )
            for axis in range(2, -1, -1):
                ...
                if not self.equal_proportions[axis]:
                    log.warning(
                        "unequal propertions in axis "
                        + "KJI"[axis]
                        + ": define Lgr corp separately"
                    )
            fp.write("ENDREF\n")
            fp.write("ENDLGR\n")
    ...
    def _assert_extents_valid(self):
        ...
        for axis in range(3):
            assert _is_int(self.fine_extent_kji[axis]) and _is_int(
                self.coarse_extent_kji[axis]
            )
    ...
    def _set_fine_to_coarse_mapping(self):
        ...
        for axis in range(3):
            self.fine_to_coarse_mapping[axis] = np.empty(
                self.fine_extent_kji[axis], dtype=int
            )
            f0 = 0
            for c0 in range(self.coarse_extent_kji[axis]):
...
def tartan_refinement(
    ...
    for axis in range(3):
        ...
        if decay_mode == "linear":
            ...
            floating_ratio[:] = (
                fovea_ratios_kji[axis] * rates[:, axis]
            )  # exponential decay
        rounded_ratio = np.around(floating_ratio).astype(int)
        while np.any(rounded_ratio > 1) and (
            slice_a > 0 or slice_b < coarse_extent_kji[axis] - 1
        ):
            slice_a -= 1
            slice_b += 1
            if slice_a >= 0:
    ...
    fc = FineCoarse(
        fine_extent_kji, coarse_extent_kji, within_coarse_box=within_coarse_box
    )
...
def _assert_valid_box(extent_kji, box):
    assert (
        isinstance(box, np.ndarray)
        and box.shape == (2, 3)
        and str(box.dtype).startswith("int")
    )
    for axis in range(3):
