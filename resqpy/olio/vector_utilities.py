...
def points_direction_vector(a, axis):
    ...
    assert (
        a.ndim > 1 and 0 <= axis < a.ndim - 1 and a.shape[-1] > 1 and a.shape[axis] > 1
    )
...
def rotate_xyz_array_around_z_axis(a, target_xy_vector):
    ...
    rotation_matrix = rotation_matrix_3d_axial(
        2, rotation_angle
    )  # todo: check sign of rotation angle
...
def tilt_3d_matrix(azimuth, dip):
    ...
    matrix = rotation_matrix_3d_axial(
        2, -azimuth
    )  # will yield rotation around z axis so azimuth goes north
    matrix = np.dot(matrix, rotation_matrix_3d_axial(0, dip))  # adjust for dip
    matrix = np.dot(
        matrix, rotation_matrix_3d_axial(2, azimuth)
    )  # rotate azimuth back to original
    return matrix
...
def tilt_points(pivot_xyz, azimuth, dip, points):
    ...
    points[:] = (
        np.matmul(matrix, points.reshape((-1, 3)).transpose())
        .transpose()
...
def perspective_vector(xyz_box, view_axis, vanishing_distance, vector):
    ...
    factor = 1.0 - (vector[view_axis] - xyz_box[0, view_axis]) / (
        vanishing_distance * (xyz_ranges[view_axis])
    )
...
def in_triangle(a, b, c, d):
    ...
    return (
        clockwise(a, b, d) < 0.0
        and clockwise(b, c, d) < 0.0
        and clockwise(c, a, d) < 0.0
    )
...
def point_distance_to_line_2d(p, l1, l2):
    ...
    return abs(
        p[0] * (l1[1] - l2[1]) + l1[0] * (l2[1] - p[1]) + l2[0] * (p[1] - l1[1])
