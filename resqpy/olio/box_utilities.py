...
def extent_of_box(
    box,
):  # returns 3 element extent of box (box can be kji or ijk, 0 or 1 based)
...
def cell_in_box(cell, box):
    ...
    return (
        (box[0, 0] <= cell[0] <= box[1, 0])
        and (box[0, 1] <= cell[1] <= box[1, 1])
        and (box[0, 2] <= cell[2] <= box[1, 2])
    )
...
def local_box_cell_from_parent_cell(box, parent_cell, based_0_or_1=0):
    ...
    if np.all(box[0] <= parent_cell) and np.all(
        parent_cell <= box[1]
    ):  # numpy 1D array ops
...
def boxes_overlap(box_a, box_b):
    ...
    return not (
        (box_a[1, 0] < box_b[0, 0])
        or (box_a[0, 0] > box_b[1, 0])
        or (box_a[1, 1] < box_b[0, 1])
        or (box_a[0, 1] > box_b[1, 1])
        or (box_a[1, 2] < box_b[0, 2])
        or (box_a[0, 2] > box_b[1, 2])
    )
...
def overlapping_boxes(established_box, new_box, trim_box):
    ...
    assert (
        established_box.ndim == 2
        and established_box.shape == (2, 3)
        and established_box.dtype == "int"
    )
    ...
    new_box_area = np.zeros(
        3, dtype="int"
    )  # compute number of cells in a 2D slice of new_box, taken in k, j or i directions
    # note: '_area' is a cell count, not an area in xyz space
    new_box_area[0] = (new_box[1, 1] - new_box[0, 1] + 1) * (
        new_box[1, 2] - new_box[0, 2] + 1
    )  # k slice
    new_box_area[1] = (new_box[1, 0] - new_box[0, 0] + 1) * (
        new_box[1, 2] - new_box[0, 2] + 1
    )  # j slice
    new_box_area[2] = (new_box[1, 0] - new_box[0, 0] + 1) * (
        new_box[1, 1] - new_box[0, 1] + 1
    )  # i slice
    ...
    for dir in range(3):
        ...
        trim_cost[dir] = (overlap_box[1, dir] - overlap_box[0, dir] + 1) * new_box_area[
            dir
        ]
...
def trim_box_by_box_returning_new_mask(box_to_be_trimmed, trim_box, mask_kji0):
    ...
    if np.all(
        trim_box[0] == box_to_be_trimmed[0]
    ):  # shift box_to_be_trimmed [0] (minumum) up (in logical space)
        if trim_box[1, 0] != box_to_be_trimmed[1, 0]:  # trim in k direction
            assert (trim_box[1, 1] == box_to_be_trimmed[1, 1]) and (
                trim_box[1, 2] == box_to_be_trimmed[1, 2]
            )
        ...
        trim_box[1] == box_to_be_trimmed[1]
    ):  # shift box_to_be_trimmed [1] (maxumum) down (in logical space)
        if trim_box[0, 0] != box_to_be_trimmed[0, 0]:  # trim in k direction
            assert (trim_box[0, 1] == box_to_be_trimmed[0, 1]) and (
                trim_box[0, 2] == box_to_be_trimmed[0, 2]
            )
        ...
        log.critical(
            "Trim box code failure: box to be trimmed is %s",
            string_iijjkk1_for_box_kji0(box_to_be_trimmed),
        )
        log.critical(
            "Trim box code failure:          trim box is %s",
            string_iijjkk1_for_box_kji0(trim_box),
        )
        assert False
    assert np.all(box_to_be_trimmed[0] <= box_to_be_trimmed[1])
    return (
        mask_kji0[
            local_box[0, 0] : local_box[1, 0] + 1,
            local_box[0, 1] : local_box[1, 1] + 1,
            local_box[0, 2] : local_box[1, 2] + 1,
        ]
    ).copy()
...
def trim_box_to_mask_returning_new_mask(bounding_box_kji0, mask_kji0):
    ...
    assert (
        bounding_box_kji0.ndim == 2
        and bounding_box_kji0.shape == (2, 3)
        and bounding_box_kji0.dtype == "int"
    )
    assert (
        mask_kji0.ndim == 3
        and np.all(
            np.array([mask_kji0.shape], dtype="int") == extent_of_box(bounding_box_kji0)
        )
        and mask_kji0.dtype == "bool"
    )
