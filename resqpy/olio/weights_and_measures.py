...
def rq_uom(units):
    ...
    if ul in [
        ...
        return ul
    if ul in ["m", "metre", "metres", "meter"]:
        return "m"
    if ul in ["ft", "foot", "feet", "ft[us]"]:
        return "ft"  # NB. treating different foot sizes as identical
    if units == "d" or ul in ["days", "day"]:
        return "d"
    if units in ["kPa", "Pa", "mD"]:
        return units
    if ul in ["psi", "psia"]:
        return "psi"
    if ul in ["1000 bbl", "mstb", "mbbl"]:
        return "1000 bbl"
    if ul in ["bbl", "stb"]:
        return "bbl"
    if ul in ["1E6 bbl", "mmstb", "mmbbl"]:
        return "1E6 bbl"
    if ul in ["1E6 ft3", "mmscf"]:
        return "1E6 ft3"
    if ul in ["1000 ft3", "mscf"]:
        return "1000 ft3"
    if ul in ["ft3", "scf"]:
        return "ft3"
    if ul in ["bbl/d", "stb/d", "bbl/day", "stb/day"]:
        return "bbl/d"
    if ul in ["1000 bbl/d", "mstb/day", "mbbl/day", "mstb/d", "mbbl/d"]:
        return "1000 bbl/d"
    if ul in ["1E6 bbl/d", "mmstb/day", "mmbbl/day", "mmstb/d", "mmbbl/d"]:
        return "1E6 bbl/d"
    if ul in ["1000 ft3/d", "mscf/day", "mscf/d"]:
        return "1000 ft3/d"
    if ul in ["ft3/d", "scf/day", "scf/d"]:
        return "ft3/d"
    if ul in ["1E6 ft3/d", "mmscf/day", "mmscf/d"]:
        return "1E6 ft3/d"
    if ul in ["ft3/bbl", "scf/bbl", "ft3/stb", "scf/stb"]:
        return "ft3/bbl"
    if ul in ["1000 ft3/bbl", "mscf/bbl", "mscf/stb"]:
        return "1000 ft3/bbl"
    if ul in ["m3", "sm3"]:
        return "m3"
    if ul in ["m3/d", "m3/day", "sm3/d", "sm3/day"]:
        return "m3/d"
    if ul == "1000 m3":
        return "1000 m3"
    if ul in ["1000 m3/d", "1000 m3/day"]:
        return "1000 m3/d"
    if ul == "1E6 m3":
        return "1E6 m3"
    if ul in ["1E6 m3/d", "1E6 m3/day"]:
        return "1E6 m3/d"
    if units in ["mD.m", "mD.ft"]:
        return units
    if ul == "count":
        return "Euc"
    ...
    if units in uom_list:
        return units
    if ul in uom_list:
        return ul  # dangerous! for example, 'D' means D'Arcy and 'd' means day
    return "Euc"
...
def p_length_unit(units):
    ...
    if units.lower() in ["m", "metre", "metres"]:
        return "metres"
    if units.lower() in ["ft", "foot", "feet", "ft[us]"]:
        return "feet"
...
def p_time_unit(units):
    ...
    if units.lower() in ["s", "sec", "secs", "second", "seconds"]:
        return "seconds"
    if units.lower() in ["ms", "msec", "millisecs", "millisecond", "milliseconds"]:
        return "milliseconds"
    if units.lower() in ["min", "mins", "minute", "minutes"]:
        return "minutes"
    if units.lower() in ["h", "hr", "hour", "hours"]:
        return "hours"
    if units.lower() in ["wk", "week", "weeks"]:
        return "weeks"
    if units.lower() in ["a", "yr", "year", "years"]:
        return "years"
...
def convert_lengths(a, from_units, to_units):
    ...
    if from_units == to_units:
        return a
    if from_units == "ft" and to_units == "m":
        a *= feet_to_metres
...
def convert_pressures(a, from_units, to_units):
    ...
    assert from_units in ["kPa", "Pa", "bar", "psi"] and to_units in [
        "kPa",
        "Pa",
        "bar",
        "psi",
    ...
    if from_units == to_units:
        return a
    if from_units in ["kPa", "Pa", "bar"] and to_units == "psi":
        factor = kPa_to_psi
    elif from_units == "psi" and to_units in ["kPa", "Pa", "bar"]:
        factor = psi_to_kPa
    else:
        factor = 1.0
    if from_units == "Pa":
...
def convert_volumes(a, from_units, to_units):
    ...
    if from_units == "m3":
        ...
        if to_units == "m3":
            ...
            a *= ft3_to_m3 * m3_to_bbl
    else:  # from_units == 'bbl'
        if to_units == "m3":
            a *= bbl_to_m3
        else:
            a *= bbl_to_m3 * m3_to_ft3
    return a
