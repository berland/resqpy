...
def pip_array_cn(p_a, poly):
    ...
    for edge in range(vertex_count):
        ...
        crossings += np.where(
            np.logical_and(
                ...
                np.logical_or(
                    np.logical_and(v1[0] > p[:, 0], v2[0] > p[:, 0]),
                    (
                        p[:, 0]
                        < (
                            v1[0]
    ...
    return np.array(np.bitwise_and(crossings, 1), dtype=bool).reshape(
        list(p_a.shape)[:-1]
    )
...
def points_in_polygon(x, y, polygon_file, poly_unit_multiplier=None):
    ...
    assert (
        x.shape == y.shape
    ), "x and y arrays have differing shapes or are not numpy arrays"
    assert polygon_file and os.path.exists(polygon_file), "polygon file is missing"
    if poly_unit_multiplier is not None:
        assert (
            poly_unit_multiplier != 0.0
        ), "zero multiplier for polygon units not allowed"
