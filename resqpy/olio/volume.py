...
def tetra_volumes_slow(cp, centres=None, off_hand=False):
    ...
    for cell in range(cells):
        volumes[cell] = tetra_cell_volume(
            flat[cell], centre=centres[cell], off_hand=off_hand
        )
...
def tetra_volumes(cp, centres=None, off_hand=False):
    ...
    def tets(
        ra, ab, ac, ad, db
    ):  # returns 6 * volume of one quad pyramid (for each cell)
        return np.sum(ra * np.cross(db, ac), axis=-1) + 0.5 * np.sum(
            ac * np.cross(ad, ab), axis=-1
        )
    ...
    v += tets(
        r0,
        flat[:, 0, 0, 1] - flat[:, 0, 0, 0],
        flat[:, 0, 1, 1] - flat[:, 0, 0, 0],
        flat[:, 0, 1, 0] - flat[:, 0, 0, 0],
        flat[:, 0, 1, 0] - flat[:, 0, 0, 1],
    )
    v += tets(
        r0,
        flat[:, 1, 0, 0] - flat[:, 0, 0, 0],
        flat[:, 1, 0, 1] - flat[:, 0, 0, 0],
        flat[:, 0, 0, 1] - flat[:, 0, 0, 0],
        flat[:, 0, 0, 1] - flat[:, 1, 0, 0],
    )
    v += tets(
        r0,
        flat[:, 0, 1, 0] - flat[:, 0, 0, 0],
        flat[:, 1, 1, 0] - flat[:, 0, 0, 0],
        flat[:, 1, 0, 0] - flat[:, 0, 0, 0],
        flat[:, 1, 0, 0] - flat[:, 0, 1, 0],
    )
    v += tets(
        r1,
        flat[:, 1, 0, 1] - flat[:, 1, 1, 1],
        flat[:, 1, 0, 0] - flat[:, 1, 1, 1],
        flat[:, 1, 1, 0] - flat[:, 1, 1, 1],
        flat[:, 1, 1, 0] - flat[:, 1, 0, 1],
    )
    v += tets(
        r1,
        flat[:, 1, 1, 0] - flat[:, 1, 1, 1],
        flat[:, 0, 1, 0] - flat[:, 1, 1, 1],
        flat[:, 0, 1, 1] - flat[:, 1, 1, 1],
        flat[:, 0, 1, 1] - flat[:, 1, 1, 0],
    )
    v += tets(
        r1,
        flat[:, 0, 1, 1] - flat[:, 1, 1, 1],
        flat[:, 0, 0, 1] - flat[:, 1, 1, 1],
        flat[:, 1, 0, 1] - flat[:, 1, 1, 1],
        flat[:, 1, 0, 1] - flat[:, 0, 1, 1],
    )
