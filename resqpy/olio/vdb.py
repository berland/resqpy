...
class RawData:
    ...
    def __init__(self, fp, place, item_type, count, max_count):
        ...
        if dtype is None:
            if form_ch == "c":
                ...
                if item_type == "C":
                    ...
                    for i in range(count):
                        self.c.append(
                            chars[i * byte_size : (i + 1) * byte_size]
                            .decode()
                            .strip()
                            .upper()
                        )
            ...
            self.a.data = fp.read(
                count * byte_size
            )  # todo: check C ordering; endianess etc.
...
class Data:
    ...
    def __init__(self, fp, header):
        ...
        if header is None:
            ...
            if (
                raw is not None
                and raw.a is not None
                and raw.a.size == 1
                and raw.a.dtype == "int32"
                and header.next != null_uint32
            ):
                #            log.debug('   skipping integer value of ' + str(raw.a[0]))
                next_head = Header(fp, header.next)
                raw = RawData(
                    fp,
                    next_head.data_place,
                    next_head.item_type,
                    next_head.number_of_items,
                    next_head.max_items,
                )
...
class Fragment:
    ...
    def __init__(self, fp, place, header):
        ...
        if self.head.number_of_items == 0:
            ...
            raw = RawData(
                fp,
                self.head.data_place,
                header.item_type,
                self.head.number_of_items,
                header.max_items,
            )
            self.a = raw.a
            self.c = raw.c
...
class FragmentChain:
    ...
    def __init__(
        self, fp, place, header
    ):  # returns either 1D numpy array (numeric data) or list of 8
        ...
        assert (
            self.c is None or self.a is None
        ), "mixture of character and numeric data in fragment chain"
...
class KP:
    ...
    def __init__(self, fp, place=4):
        ...
        assert (
            self.p_head.number_of_items == self.k_head.number_of_items
        ), "number of items in Pointer header does not match number in Key header"
        self.keywords = Data(fp, self.k_head).c
        assert self.keywords is not None and isinstance(
            self.keywords, list
        ), "list of keywords not extracted from Key record"
...
class VDB:
    ...
    def __init__(self, path):
        ...
        if self.zipped:
            ...
            if not os.path.isdir(path):
                log.warning(
                    "vdb path is neither a directory nor a zip file: " + str(path)
                )
        self.case_list = None
        self.use_case = None
        ...
        try:
            ...
            if self.zipped:
                ...
                assert os.path.exists(xml_file), (
                    "could not find vdb main xml file: " + xml_file
                )
    ...
    def set_use_case(self, case):
        ...
        try:
            assert case in self.cases(), (
                "failed to find case " + case + " in list of vdb cases"
            )
            self.use_case = case
        except:
    ...
    def print_header_tree(self, relative_path):
        ...
        def print_tree(fp, place=4, level=0, dots=False, file_size=None):
            try:
                ...
                assert (
                    0 <= int(place) < file_size
                ), f"place {place} not within bounds of file of size {file_size}"
                assert (
                    place not in visited
                ), "circular pointers in vdb file header structure"
                ...
                if head.item_type == "C":
                    s = RawData(
                        fp, head.data_place, "C", head.number_of_items, head.max_items
                    )
                ...
                if head.item_type == "P":
                    pointers = RawData(
                        fp, head.data_place, "P", head.number_of_items, head.max_items
                    )
                    assert pointers is not None and pointers.a is not None
                    for p in pointers.a:
                        if p in visited:
                            log.warning(
                                "breaking at circular pointer to place " + str(p)
                            )
                            break
                        if int(p) == 0:
                            ...
                            log.warning(
                                "skipping pointer to place beyond end of file " + str(p)
                            )
                        else:
                            print_tree(
                                fp,
                ...
                if head.next != null_uint32:
                    if not 0 <= int(head.next) < file_size:
                        log.warning(
                            f"skipping next {head.next} outwith bounds of file of size {file_size}"
                        )
                    else:
                        print_tree(
                            fp, place=head.next, level=level, file_size=file_size
                        )
            except:
                print("?")
    ...
    def print_key_tree(self, relative_path):
        ...
        def print_tree(fp, place=4, level=0):
            try:
                ...
                if place in visited:
                    log.warning(
                        f"returning at circular pointer at place {place} in vdb file key structure"
                    )
                    return
                visited.append(place)
    ...
    def data_for_keyword(self, relative_path, keyword, search=True):
        ...
        try:
            ...
            if self.zipped:
                with zf.ZipFile(self.zip_file) as zfp:
                    with zfp.open(path) as fp:
                        assert fp.read(4) == b"NT32", (
                            "first 4 characters not NT32 in file " + path
                        )
                        kp = KP(fp)
    ...
    def data_for_keyword_chain(self, relative_path, keyword_chain):
        ...
        def dfkc_fp(fp, path, keyword_chain):
            ...
            while len(chain_list) > 1:
                ...
                assert head_place is not None, (
                    "failed to find chained keyword: " + chain_list[0]
                )
                chain_list.pop(0)
            kp = KP(fp, place=head_place)
        ...
        try:
            ...
            log.exception(
                "failed to read chained keyword data from vdb binary file: " + path
            )
        return None
    ...
    def fetch_corp_patch(self, relative_path):
        ...
        assert a is not None, (
            "failed to extract corp data from vdb relative path " + relative_path
        )
    ...
    def grid_corp(self, grid_name):
        ...
        try:
            ...
            glob_path = os.path.join(
                self.path, self.use_case, "INIT", str(grid_name) + "_corp"
            )
            glob_path += "_*.bin"
            if self.zipped:
            ...
            gf.resequence_nexus_corp(
                ap
            )  # move from Nexus corp ordering to Pagoda ordering
            if (self.use_case, grid_name) in self.grid_extents_kji:
                extent_kji = self.grid_extents_kji[self.use_case, grid_name]
                assert (
                    cells == extent_kji[0] * extent_kji[1] * extent_kji[2]
                ), "corp extent mismatch in vdb"
            else:
                extent_kji = gf.determine_corp_extent(ap)
                if extent_kji is None:
                    log.warning(
                        "failed to determine extent of root grid from corp data"
                    )
    ...
    def load_init_mapdata_array(
        self, file, keyword, dtype=None, unpack=False, grid_name="ROOT"
    ):
        ...
        try:
            ...
            assert a is not None, (
                "failed to extract data for keyword "
                + keyword
                + " from vdb relative path "
                + relative_path
            )
            if unpack:
                ...
                if un is not None:
                    null_start = np.zeros(
                        (a.size + 1,), dtype=a.dtype
                    )  # better to use Nan for null value?
                    null_start[1:] = a
                    a = null_start[un]
    ...
    def load_recurrent_mapdata_array(
        self, file, keyword, dtype=None, unpack=False, grid_name="ROOT"
    ):
        ...
        try:
            if not keyword or not keyword.isalnum():
                log.warning(
                    "ignoring attempt to load recurrent vdb mapdata for corrupt keyword"
                )
                return None
            if self.use_case is None:
                self.cases()
            ...
            log.exception(
                "failed to extract grid recurrent data from vdb for keyword: " + keyword
            )
        return None
    ...
    def grid_dad(self, grid_name):
        ...
        return self.load_init_mapdata_array(
            file="I" + grid_name + "DAD.bin",
            keyword="DAD",
            dtype="int32",
            unpack=False,
            grid_name=grid_name,
    ...
    def grid_unpack(self, grid_name):
        ...
        if (
            ...
            and (self.use_case, grid_name) in self.grid_packings.keys()
        ):
        ...
        un = self.load_init_mapdata_array(
            file="I" + grid_name + "UNPACK.bin",
            keyword="UNPACK",
            dtype="int32",
            unpack=False,
            grid_name=grid_name,
        ...
        if self.use_case is not None:
            self.grid_packings[
                self.use_case, grid_name
            ] = un  # cache unpack array for later packing & unpacking operations
        return un
    ...
    def grid_list_of_static_properties(self, grid_name):
        ...
        grid_name = grid_name.upper()
        glob_path = os.path.join(
            self.path, self.use_case, "INIT", "MAPDATA", "I" + grid_name + "*.bin"
        )
        if self.zipped:
    ...
    def grid_list_of_timesteps(self, grid_name):
        ...
        grid_name = grid_name.upper()
        glob_path = os.path.join(
            self.path, self.use_case, "RECUR", "MAPDATA", "R" + grid_name + "_*.bin"
        )
        if self.zipped:
            file_list = self.zip_glob(glob_path)
        else:
    ...
    def grid_list_of_recurrent_properties(self, grid_name, timestep):
        ...
        def keyword_list_from_fp(fp, path):
            ...
            if len(bad_key_indices):
                log.warning(
                    str(len(bad_key_indices))
                    + " non-ascii keywords ignored in recurrent file "
                    + path
                )
    ...
    def root_recurrent_property_for_timestep(
        self, keyword, timestep, dtype=None, unpack=True
    ):
        """Loads and returns a ROOT recurrent property array for one timestep."""

        return self.grid_recurrent_property_for_timestep(
            "ROOT", keyword, timestep, dtype=dtype, unpack=unpack
        )

    def grid_recurrent_property_for_timestep(
        self, grid_name, keyword, timestep, dtype=None, unpack=True
    ):
        ...
        grid_name = grid_name.upper()
        keyword = keyword.strip().upper()
        return self.load_recurrent_mapdata_array(
            file="R" + grid_name + "_" + str(timestep) + ".bin",
            keyword=keyword,
            dtype=dtype,
            unpack=unpack,
            grid_name=grid_name,
    ...
    def grid_shaped(self, grid_name, a):
        ...
        if (
            self.use_case is not None
            and (self.use_case, grid_name) in self.grid_extents_kji.keys()
        ):
