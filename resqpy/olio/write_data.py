...
def write_array_to_ascii_file(
    ...
    assert (
        target_simulator == "nexus"
    )  # no other simulator formats supported at the moment
        ...
        try:
            ...
            with open(file_name, file_mode) as new_file:
                ...
                if headers:  # write 3 comment lines at start of file
                    new_file.write(
                        "! Data written by write_array_to_ascii_file() python function\n"
                    )
                    new_file.write(
                        "! Extent of array is: ["
                        + str(extent_kji[2])
                    ...
                    new_file.write(
                        "! Maximum " + str(columns) + " data items per line\n"
                    )
                    ...
                    for j in range(extent_kji[1]):
                        for i in range(extent_kji[2]):
                            if i % columns == 0:
                                ...
                                new_file.write("\t")
                            if (
                                data_type == "real" or data_type == "float"
                            ):  # todo: speed up by using common write, different format
                                v = a[k, j, i]
