...
class Consolidation:
    def __init__(self, resident_model):
        ...
        log.debug(
            f"new consolidation for {self.model.epc_file} with {len(self.model.uuids())} uuids"
        )
        self.map = {}  # dictionary mapping immigrant uuid to primary uuid
        self.stale = True

    def equivalent_uuid_for_part(
        self, part, immigrant_model=None, ignore_identical_part=False
    ):
        ...
        if obj_type.endswith("Interpretation") or obj_type.endswith("Feature"):
            ...
            immigrant_obj = rqt.TimeSeries(
                immigrant_model, time_series_root=immigrant_root
            )
        ...
        for resident_uuid in resident_uuids:
            ...
            if obj_type.endswith("Interpretation") or obj_type.endswith("Feature"):
                ...
                resident_obj = rqt.TimeSeries(
                    self.model, time_series_root=resident_root
                )
            elif obj_type == "StringTableLookup":
            ...
            if (
                immigrant_obj == resident_obj
            ):  # note: == operator overloaded with equivalence method for these classes
...
def sort_parts_list(model, parts_list):
    ...
    pair_list = [
        (ordering(model.type_of_part(part, strip_obj=True)), part)
