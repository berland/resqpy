...
class H5Register:
    ...
    def __init__(self, model):
        ...
        self.dataset_dict = (
            {}
        )  # dictionary mapping from (object_uuid, group_tail) to (numpy_array, dtype)
        self.model = model
...
def copy_h5(
    file_in, file_out, uuid_inclusion_list=None, uuid_exclusion_list=None, mode="w"
):
    ...
    assert (
        file_out != file_in
    ), "identical input and output files specified for hdf5 copy"
    assert (
        uuid_inclusion_list is None or uuid_exclusion_list is None
    ...
    with h5py.File(file_out, mode) as fp_out:
        ...
        with h5py.File(file_in, "r") as fp_in:
            ...
            assert main_group_in is not None, (
                "failed to find RESQML group in hdf5 file: " + file_in
            )
            if mode == "w":
            ...
            for group in main_group_in:
                ...
                if uuid is None:
                    log.warning(
                        "RESQML group name in hdf5 file is not a uuid, skipping: "
                        + str(group)
                    )
                    continue
                ...
                if group in main_group_out:
                    log.warning(
                        "not copying hdf5 data due to pre-existence for: " + str(group)
                    )
                    continue
