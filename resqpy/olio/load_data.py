...
def file_exists(file_name, must_be_more_recent_than_file=None):
    ...
    if not existe:
        return False
    if (
        not must_be_more_recent_than_file
        or file_name == must_be_more_recent_than_file
        or not os.path.exists(must_be_more_recent_than_file)
    ):
        return True
...
def load_corp_array_from_file(
    ...
    if corp_bin:
        ...
        bin_cell_count, remainder = divmod(
            bin_file_size, 26 * 4
        )  # corp bin format has records of 24 + 2 (head, tail) 32 bit words
        if remainder:
            log.error(
                "corp binary file "
                + str(file_name)
        ...
        if swap_bytes:
            dt = dt.newbyteorder()
        cp_array = np.fromfile(file_name, dtype=dt, count=26 * bin_cell_count).reshape(
            (-1, 26)
        )[:, 1:25]
        if extent_kji is not None:
            if bin_cell_count != (extent_kji[0] * extent_kji[1] * extent_kji[2]):
                log.error(
                    ...
                    + " contains data for "
                    + str(bin_cell_count)
                    + " cells; extent requires "
                    + str(extent_kji[0] * extent_kji[1] * extent_kji[2])
                )

    else:
        cp_array = load_array_from_file(
            file_name,
            extent=extent,
            data_type="real",
            keyword="CORP",
    ...
    if remainder:
        log.error(
            ...
            + " contains "
            + str(cp_array.size)
            + " data, which is not a multiple of 24"
        )
        return None

    cp_array = cp_array.reshape(
        1, 1, cell_count, 2, 2, 2, 3
    )  # pagoda 7D, temporarily with all cells on I axis
    log.debug("resequencing corner point data")
    gf.resequence_nexus_corp(
        cp_array, eight_mode=eight_mode
    )  # switches IP points where JP = 1 (unless eight_mode)
...
def load_array_from_file(
    ...
    if not use_binary:
        return load_array_from_ascii_file(
            file_name,
            extent=extent,
            data_type=data_type,
            keyword=keyword,
    ...
    (extension, ab_type) = abt.binary_file_extension_and_np_type_for_data_type(
        data_type
    )
    ...
    if extent is None:
        ...
        log.debug(
            "Loading unknown number of array data elements from file " + file_name
        )
    else:
    ...
    result = load_array_from_ascii_file(
        file_name,
        extent=extent,
        data_type=data_type,
        keyword=keyword,
...
def load_array_from_ascii_file(
    file_name,
    extent=None,
    data_type="real",
    keyword=None,
    ...
    if extent is None:
        cell_count = -1  # np.fromfile interprets this as 'read everything'
        log.debug(
            "Loading unknown number of array data elements from ascii file " + file_name
        )
    else:
        cell_count = pu.cell_count_from_extent(extent)
        log.debug(
            "Loading %1d array data elements from ascii file %s", cell_count, file_name
        )
    ...
    if data_type in ["real", "float", float]:
        ...
        assert False, "Unknown data_type passed to load_array_from_ascii_file" + str(
            data_type
        )
    ...
    with open(read_file_name, "r") as data_file:
        ...
        if keyword:
            keyword_found = kf.find_keyword(
                data_file, keyword, max_lines=max_lines_for_keyword
            )
            if keyword_found:
                data_file.readline()  # skip keyword line

        kf.skip_blank_lines_and_comments(
            data_file, comment_char=comment_char, skip_c_space=skip_c_space
        )

        result = None

        if (
            data_free_of_comments
        ):  # use numpy fromfile function after passing header comments
        ...
        if result is None:
            ...
            assert (
                extent is not None
            )  # TODO: remove this restriction by dynamically extending a flat array
            ...
            for index in range(elements):
                ...
                while True:
                    ...
                    if (
                        ch == ""
                    ):  # end of file (this assumes a whitespace character after last datum)
                        log.error(
                            "Not enough data in file %s: %1d of %1d numbers read",
                            file_name,
                            index,
                            elements,
                        )
...
def load_fault_mask(file_name, direction, fault_mask, use_binary=False):
    ...
    if use_binary:
        ...
        if (
            len(binary_file_name) > 5
            and binary_file_name[-5:] == "." + direction.lower() + ".bb"
        ):
        ...
        try:
            with open(binary_file_name, "rb") as binary_data_file:
                result = np.fromfile(
                    binary_data_file, dtype="bool", count=face_count
                ).reshape(mask_extent_kji)
    ...
    with open(ascii_file_name, "r") as data_file:
        ...
        while kf.find_keyword_pair(data_file, "MULT", trans_keyword_dict[direction]):
            ...
            plus_minus_offset = [
                ...
                0,
                0,
            ]  # only possibly set to 1 in direction of interest
            if (
                "MINUS" not in line.split()
            ):  # indicates data apply to 'negative' faces of specified cells
            ...
            while kf.number_next(data_file):
                ...
                #                        count += 1
                count += box.volume_of_box(box_kji0) - np.count_nonzero(
                    fault_mask[
                        box_kji0[0, 0] : box_kji0[1, 0] + 1,
                        box_kji0[0, 1] : box_kji0[1, 1] + 1,
    ...
    if use_binary:
        ...
        try:
            ...
            log.warn(
                "Failed to write data to binary fault mask file %s", binary_file_name
            )
            # todo: could delete the binary file in case a corrupt file is left for use next time
