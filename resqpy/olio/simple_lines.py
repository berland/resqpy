        ...
        with open(filename, "r") as fp:
            ...
            while True:
                ...
                if len(line) == 0:
                    assert (
                        point_count == 0
                    ), "unterminated list of line points at end of file"
                    break
                words = line.split()
                if len(words) == 0:
                    continue  # blank line
                ...
                if np.all(np.isclose(point, end_of_line)):
                    ...
                    point_list = np.concatenate(
                        (point_list, point.reshape((1, 3))), axis=0
                    )
                    point_count += 1
                else:
...
def nearest_rods(line_list, projection, grid, axis, ref_slice0=0, plus_face=False):
    ...
    for line in line_list:
        ...
        rod_list_list.append(
            grid.nearest_rod(
                line_a, projection, axis, ref_slice0=ref_slice0, plus_face=plus_face
            )
        )
...
def drape_lines(
    line_list, pillar_list_list, grid, ref_k=0, ref_kp=0, offset=-1.0, snap=False
):
