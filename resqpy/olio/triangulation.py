...
def _dt_simple(po, plot_fn=None, progress_fn=None):
    def flip(ei):
        ...
        if t1n < 0:
            t1n = 2
        if not vec.in_circumcircle(
            p[t[t0, 0]], p[t[t0, 1]], p[t[t0, 2]], p[t[t1, t1n]]
        ) and not vec.in_circumcircle(
            p[t[t1, 0]], p[t[t1, 1]], p[t[t1, 2]], p[t[t0, t0n]]
        ):
    ...
    dxy = max_xy - min_xy
    assert (
        dxy[0] > 0.0 and dxy[1] > 0.0
    ), "points lie in straight line or are conincident"
    p = np.empty((n_p + 3, 2))
    p[:-3] = po[:, :2]
...
def dt(p, algorithm=None, plot_fn=None, progress_fn=None):
    ...
    assert (
        p.ndim == 2 and p.shape[1] >= 2
    ), "bad points shape for 2D Delauney Triangulation"
