...
def print_LocalDepth3dCrs(model, node, z_is_time=False):
    ...
    print(
        "units xy: "
        + str(rqet.length_units_from_node(rqet.find_tag(node, "ProjectedUom")))
    )
    print(
        "units z:  "
        + str(rqet.length_units_from_node(rqet.find_tag(node, "VerticalUom")))
    )  # applicable to time?
    xy_axes = rqet.node_text(rqet.find_tag(node, "ProjectedAxisOrder"))
    print("x y axes: " + str(xy_axes))
    z_inc_down = rqet.bool_from_text(
        rqet.node_text(rqet.find_tag(node, "ZIncreasingDownward"))
    )  # todo: check applicability to Time3dCrs?
    ...
    print(
        "z offset: " + str(rqet.node_text(rqet.find_tag(node, "ZOffset")))
    )  # todo: check applicability to Time3dCrs?
    ...
    if parent_xy_crs_node_type == "ProjectedCrsEpsgCode":
        print(
            "parent xy crs: EPSG code "
            + str(rqet.node_text(rqet.find_tag(parent_xy_crs_node, "EpsgCode")))
        )
    elif parent_xy_crs_node_type == "ProjectedUnknownCrs":
        print("parent xy crs: unknown")
    parent_z_crs_node = rqet.find_tag(
        node, "VerticalCrs"
    )  # todo: check applicability to Time3dCrs?
    parent_z_crs_node_type = rqet.node_type(parent_z_crs_node)
    if parent_z_crs_node_type == "VerticalCrsEpsgCode":
        print(
            "parent z crs: EPSG code "
            + str(rqet.node_text(rqet.find_tag(parent_z_crs_node, "EpsgCode")))
        )
...
def print_LocalTime3dCrs(model, node):
    ...
    print(
        "time units: " + str(rqet.time_units_from_node(rqet.find_tag(node, "TimeUom")))
    )
...
def print_IjkGridRepresentation(model, node, detail_level=0, grid=None):
    ...
    if pw_node is not None:
        ...
        print(
            "parent window present (grid is a local grid); parent uuid: "
            + str(parent_uuid)
        )
    ...
    if grid.has_split_coordinate_lines:
        ...
        print(
            "number of split pillars: "
            + str(rqet.node_text(rqet.find_tag(split_root, "Count")))
        )
    else:
    ...
    if pillar_is_def_root is None:
        print(
            "grid does not include boolean array indicating which pillar geometries are defined!"
        )
    else:
        ...
        if pillar_is_def_type == "BooleanConstantArray":
            ...
            if detail_level > 1:
                if grid.geometry_defined_for_all_pillars():
                    print(
                        "geometry is defined for all pillars with explicit boolean array"
                    )
                else:
    ...
    if cell_is_def_root is None:
        print(
            "grid does not include boolean array indicating which cell geometries are defined!"
        )
    else:
        ...
        if cell_is_def_type == "BooleanConstantArray":
            ...
            if detail_level > 1:
                ...
                if (
                    grid.geometry_defined_for_all_cells_cached is None
                    or not grid.geometry_defined_for_all_cells_cached
                ):
                    assert cell_is_def_array is not None
                    cells_defined = np.count_nonzero(cell_is_def_array)
                    print(
                        "geometry is defined for {0:1d} cells ({1:4.2f}%)".format(
                            cells_defined,
                            100.0 * float(cells_defined) / float(cell_count),
                        )
                    )
                ...
                if cells_all_defined:
                    print(
                        "geometry is defined for all cells with explicit boolean array"
                    )
    ...
    #   elif cells_all_defined:
        ...
        for local in [True, False]:
            ...
            print(
                local_str
                + " bounding values of geometry min, max ("
                + laziness_str
                + "):"
            )
            for xyz in range(3):
                print(
                    " {0}: {1:12.3f} {2:12.3f}".format(
                        "xyz"[xyz], xyz_box[0, xyz], xyz_box[1, xyz]
                    )
                )
...
def print_hdf5_line(name, group_or_dataset):
    ...
    if isinstance(group_or_dataset, h5py.Group):
        return None
    print(
        name + "  " + str(group_or_dataset.shape) + "  " + str(group_or_dataset.dtype)
    )
    ...
    for index in range(time_series.number_of_timestamps()):
        if index:
            print(
                "{0:>5d}  {1} {2:>5d}".format(
                    index,
                    rts.simplified_timestamp(time_series.timestamp(index)),
                    time_series.step_days(index),
                )
            )
        else:
            print(
                "{0:>5d}  {1}  step (days)".format(
                    0, rts.simplified_timestamp(time_series.timestamp(0))
                )
            )
...
def print_reference_node_and_return_referenced_part(node, heading):
    ...
    if content_type is None or content_type[:4] != "obj_" or uuid_str is None:
        return None
    if uuid_str[0] == "_":
        uuid_str = uuid_str[1:]  # tolerate fespi quirk
...
def print_Property(model, node, detail_level=0):
    ...
    if facet_node_list is not None:
        for facet_node in facet_node_list:
            print(
                "facet: "
                + str(rqet.node_text(rqet.find_tag(facet_node, "Facet")))
                + ": "
                + str(rqet.find_tag_text(facet_node, "Value"))
            )
            if (
                not facet_type
            ):  # arbitrarily pick up first facet in list to use in helping to guess uom, if needed
                facet_type = rqet.node_text(rqet.find_tag(facet_node, "Facet"))
                facet = rqet.node_text(rqet.find_tag(facet_node, "Value"))
    citation_title = rqet.node_text(
        rqet.find_tag(rqet.find_tag(node, "Citation"), "Title")
    )
    if citation_title is not None:
        (
            ...
            derived_facet_type,
            derived_facet,
        ) = rprop.property_kind_and_facet_from_keyword(citation_title)
        if derived_property_kind is not None:
            ...
            if derived_facet_type is None:
                ...
                print(
                    "(derived kind: "
                    + str(derived_property_kind)
                    + "; facet: "
                    + str(derived_facet_type)
                    + ": "
                    + str(derived_facet)
                    + ")"
                )
    ...
    if time_index_node is None:
        ...
        time_series_part = print_reference_node_and_return_referenced_part(
            time_series_ref_node, "time series"
        )
        if detail_level > 0:
            ...
            if time_series_node is None:
                ...
                print(
                    "   number of timestamps in series: "
                    + str(time_series.number_of_timestamps())
                )
            if time_index is not None:
                print(
                    "timestamp for this part: "
                    + str(rts.simplified_timestamp(time_series.timestamp(time_index)))
                )
    ...
    if units is not None:
        print("units: " + units)
        if (
            units in ["unknown", "Euc"]
            and property_kind is not None
            and detail_level > 0
        ):
            guessed_uom = rprop.guess_uom(
                property_kind,
                minimum,
                maximum,
                grid,
    ...
    single_prop_collection.add_part_to_dict(
        part_name, trust_uom=units is not None and units not in ["", "unknown", "Euc"]
    )
    ...
    if grid and grid.inactive is not None:
        masked_array = single_prop_collection.cached_part_array_ref(
            part_name, masked=True
        )
...
def print_CategoricalProperty(model, node, detail_level=0):
    ...
    if lookup_node is None:
        ...
        print_reference_node_and_return_referenced_part(
            lookup_node, "lookup table reference"
        )
...
def print_TriangulatedSetRepresentation(model, node, detail_level=0):
    ...
    if detail_level > 0:
        for patch in rqet.list_of_tag(node, "TrianglePatch"):
            print(
                "   patch index:     "
                + required(rqet.find_tag_int(patch, "PatchIndex"))
            )
            print("   triangle count:  " + required(rqet.find_tag_int(patch, "Count")))
            print(
                "   node count:      " + required(rqet.find_tag_int(patch, "NodeCount"))
            )
            if detail_level > 1:
                print(
                    "   node null value: "
                    + optional(rqet.find_tag_int(patch, "NullValue"))
                )
...
def print_GridConnectionSetRepresentation(model, node, detail_level=0):
    ...
    if detail_level > 0:
        ...
        for feature in rqet.list_of_tag(ci_node, "FeatureInterpretation"):
            print_reference_node_and_return_referenced_part(
                feature, "feature interpretation reference"
            )
            if detail_level > 1:
                face_count_per_feature = len(
                    gcs.raw_list_of_cell_face_pairs_for_feature_index(feature_index)[0]
                )
...
def print_MdDatum(model, node, detail_level=0):
    ...
    if detail_level > 0:
        ...
        crs_part = print_reference_node_and_return_referenced_part(
            crs_ref, "local coordinate reference system reference"
        )
        if detail_level > 1:
...
def print_WellboreFrameRepresentation(model, node, detail_level=0):
    ...
    print_reference_node_and_return_referenced_part(
        trajectory_ref, "wellbore trajectory reference"
    )
...
def print_WellboreTrajectoryRepresentation(model, node, detail_level=0):
    ...
    print(
        "start md:  " + required(rqet.find_tag_float(node, "StartMd")) + " " + unit_str
    )
    print(
        "finish md: " + required(rqet.find_tag_float(node, "FinishMd")) + " " + unit_str
    )
    ...
    if md_ref_node is None:
        ...
        print_reference_node_and_return_referenced_part(
            md_ref_node, "measured depth datum reference"
        )
    geom_node = rqet.find_tag(node, "Geometry")
    if geom_node is None:
    ...
    if ds_ref_node is not None:
        print_reference_node_and_return_referenced_part(
            ds_ref_node, "deviation survey reference"
        )
    if detail_level > 1:
        try:
...
def print_TectonicBoundaryFeature(model, node, detail_level=0):
    ...
    print(
        "tectonic boundary kind: "
        + required(rqet.find_tag_text(node, "TectonicBoundaryKind"))
    )
...
def print_GeneticBoundaryFeature(model, node, detail_level=0):
    ...
    print(
        "genetic boundary kind: "
        + required(rqet.find_tag_text(node, "GeneticBoundaryKind"))
    )
...
def print_node(model, node, detail_level=0):
    ...
    if obj_type.endswith("Property"):
        ...
        if support_node is None:
            ...
            print_reference_node_and_return_referenced_part(
                support_node, "supporting representation reference"
            )
        ...
        if interp_node is None:
            ...
            print_reference_node_and_return_referenced_part(
                interp_node, "represented interpretation reference"
            )
        ...
        if feature_node is None:
            ...
            print_reference_node_and_return_referenced_part(
                feature_node, "interpreted feature reference"
            )
