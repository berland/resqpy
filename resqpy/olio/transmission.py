...
def half_cell_t(
    ...
    if darcy_constant is None:
        ...
        darcy_constant = (
            0.008527 if length_units == "m" else 0.001127
        )  # these values from Nexus keyword ref.

    if perm_k is None or perm_j is None or perm_i is None or ntg is None:
        pc = grid.extract_property_collection()
        basic_5_parts = pc.basic_static_property_parts(
            realization=realization, share_perm_parts=True
        )
    ...
    if isinstance(grid, grr.RegularGrid):
        ...
        raise ValueError(
            f"grid {type(grid)} is neither RegularGrid nor Grid object in call to half_cell_t()"
        )


def half_cell_t_regular(
    grid, perm_k=None, perm_j=None, perm_i=None, ntg=None, darcy_constant=None
):
    ...
    face_areas = np.array(
        (
            axis_lengths[1] * axis_lengths[2],
            axis_lengths[2] * axis_lengths[0],
...
def half_cell_t_irregular(
    ...
    km = (
        kp
    ) = None  # raw points k indices for K- and K+ faces, by layer, when K gaps present
    if grid.k_gaps:
    ...
    if grid.has_split_coordinate_lines:
        ...
        if grid.k_gaps:
            edge_vectors.append(
                p[kp][:, pfc] - p[km][:, pfc]
            )  # k edge vectors, shape (nk, nj, ni, 2, 2, 3)
            edge_vectors.append(
                p[:, pfc[:, :, 1, :]] - p[:, pfc[:, :, 0, :]]
            )  # j edge vectors, shape (nk + k_gaps + 1, nj, ni, 2, 3) with jp removed
            edge_vectors.append(
                p[:, pfc[:, :, :, 1]] - p[:, pfc[:, :, :, 0]]
            )  # i edge vectors, shape (nk + k_gaps + 1, nj, ni, 2, 3) with ip removed
        else:
            edge_vectors.append(
                p[1:, pfc] - p[:-1, pfc]
            )  # k edge vectors, shape (nk, nj, ni, 2, 2, 3)
            edge_vectors.append(
                p[:, pfc[:, :, 1, :]] - p[:, pfc[:, :, 0, :]]
            )  # j edge vectors, shape (nk + 1, nj, ni, 2, 3) with jp removed
            edge_vectors.append(
                p[:, pfc[:, :, :, 1]] - p[:, pfc[:, :, :, 0]]
            )  # i edge vectors, shape (nk + 1, nj, ni, 2, 3) with ip removed
    else:
        if grid.k_gaps:
            edge_vectors.append(
                p[kp] - p[km]
            )  # k edge vectors, shape (nk, nj + 1, ni + 1, 3)
            edge_vectors.append(
                p[:, 1:, :] - p[:, :-1, :]
            )  # j edge vectors, shape (nk + k_gaps + 1, nj, ni + 1, 3)
            edge_vectors.append(
                p[:, :, 1:] - p[:, :, :-1]
            )  # i edge vectors, shape (nk + k_gaps + 1, nj + 1, ni, 3)
        else:
            edge_vectors.append(
                p[1:] - p[:-1]
            )  # k edge vectors, shape (nk, nj + 1, ni + 1, 3)
            edge_vectors.append(
                p[:, 1:] - p[:, :-1]
            )  # j edge vectors, shape (nk + 1, nj, ni + 1, 3)
            edge_vectors.append(
                p[:, :, 1:] - p[:, :, :-1]
            )  # i edge vectors, shape (nk + 1, nj + 1, ni, 3)

    half_t = np.empty(
        (grid.nk, grid.nj, grid.ni, 3, 2)
    )  # 3 is K,J,I; 2 is -/+ polarity (face)
    ...
    for axis in range(3):
        ...
        if axis == 0:  # k
            if grid.has_split_coordinate_lines:
                half_axis_vectors = 0.125 * np.abs(
                    np.sum(edge_vectors[0], axis=(3, 4))
                )  # shape (nk, nj, ni, 3) with 3 being xyz length components of vector
                face_areas = projected_tri_area(
                    p[:, pfc[:, :, 0, 0]], p[:, pfc[:, :, 0, 1]], p[:, pfc[:, :, 1, 1]]
                ) + projected_tri_area(
                    p[:, pfc[:, :, 0, 0]], p[:, pfc[:, :, 1, 1]], p[:, pfc[:, :, 1, 0]]
            ...
            if grid.k_gaps:
                minus_face_areas = face_areas[
                    km
                ]  # shape (nk, nj, ni, 3) where 3 is xyz projection axis (ie. yz plane, xz plane, xy plane)
                plus_face_areas = face_areas[kp]
            else:
                minus_face_areas = face_areas[:-1]
                plus_face_areas = face_areas[1:]
            half_axis_length_sqr = np.sum(
                half_axis_vectors * half_axis_vectors, axis=-1
            )  # shape (nk, nj, ni)
            zero_length_mask = np.logical_or(
                np.any(np.isnan(half_axis_vectors), axis=-1),
            ...
            minus_face_t = np.where(
                ...
                np.NaN,
                perm_k
                * np.sum(half_axis_vectors * minus_face_areas, axis=-1)
                / half_axis_length_sqr,
            )
            plus_face_t = np.where(
                zero_length_mask,
                np.NaN,
                perm_k
                * np.sum(half_axis_vectors * plus_face_areas, axis=-1)
                / half_axis_length_sqr,
            )
        #         log.debug(f'cell 0 K: half axis vector: {half_axis_vectors[0, 0, 0]}; + face areas: {plus_face_areas[0, 0, 0]}; + face t: {plus_face_t[0, 0, 0]}')
            ...
            if grid.has_split_coordinate_lines:
                ...
                if grid.k_gaps:
                    ...
                    face_areas = projected_tri_area(
                        p[km][:, :-1], p[km][:, :, 1:], p[kp][:, :, 1:]
                    ) + projected_tri_area(
                        p[km][:, :-1], p[kp][:, :, 1:], p[kp][:, :, :-1]
                    )
                else:
                    half_axis_vectors = 0.125 * np.abs(
                        edge_vectors[1][1:, :, 1:]
...
def fault_connection_set(grid, skip_inactive=False):
    ...
    def juxtapose(grid, p, pv, pam, pap, pbm, pbp, tol=0.001):
        ...
        def fractional_area(
            ...
            if (fla, flb) == (4, 4):  # diagram 1
                ...
                5,
                4,
            ):  # diagram 5 (diagram 4 is a special case of 5 and 2)
                ...
                4,
                5,
            ):  # diagram 5 mirror (diagram 4 is a special case of 5)
                ...
                2,
                3,
            ):  # diagram 5 (reverse perspective), similar to 1.0 - diagram 10
                ...
                5,
                3,
            ):  # diagram 11 (reverse perspective) similar to 1.0 - diagram 10
                ...
                3,
                1,
            ):  # diagram 7 (only accurate if pillars parallel and layer constant thickness?)
                ...
                1,
                3,
            ):  # diagram 7 mirror (only accurate if pillars parallel and layer constant thickness?)
                ...
                5,
                1,
            ):  # diagram 9 (only accurate if pillars parallel and layer constant thickness?)
                ...
                6,
                1,
            ):  # diagram 8; solution only accurate if pillars are parallel
        ...
        pamln = np.empty(
            paml.shape[0]
        )  # pillar points normalised to scalar distances on mean pillar vector, arbitrary origin
        ...
        for km in range(grid.nk):  # for each layer on -ve side of fault
            km_top = (
                grid.k_raw_index_array[km] if grid.k_gaps else km
            )  # index of top points on -ve side of fault
            km_bot = km_top + 1
            paml_top = pamln[
                km_top
            ]  # point distances in local vector space; note local vector is different for pillars a and b
            ...
            while True:
                ...
                if kp >= grid.nk:
                    break
                kp_top = (
                    grid.k_raw_index_array[kp] if grid.k_gaps else kp
                )  # index of top points on +ve side of fault
                kp_bot = kp_top + 1
                papl_top = papln[
                    kp_top
        ...
        return juxta_list, (
            fa_m_downscaling_count,
            fa_p_downscaling_count,
            fa_m_worst_scaling,
            fa_p_worst_scaling,
        )
    ...
    if not grid.has_split_coordinate_lines:
        return None, None
    skip_inactive = (
        skip_inactive and hasattr(grid, "inactive") and grid.inactive is not None
    )

    p = grid.points_ref(masked=False)  # shape (nk + k_gaps + 1, np, 3)
    pv = vec.unit_vectors(
        p[-1] - p[0]
    )  # pillar vectors; shape (np, 3); could postpone to individual pillar work
    (
        col_j_split,
        col_i_split,
    ) = (
        grid.split_column_faces()
    )  # internal only column faces; shapes (nj - 1, ni), (nj, ni - 1)
    pfc = (
        grid.create_column_pillar_mapping()
    )  # pillars for column; shape (nj, ni, 2, 2)

    juxtaposed_j_list = []
    j_ji = np.stack(
        np.where(col_j_split)
    ).T  # ji0 columns where +J face is split; shape (nc, 2)
    warning_count = 0
    for (j, i) in j_ji:
        log.debug(f"J, I  {j}, {i}")
        pam = pfc[
            j, i, 1, 0
        ]  # pillar index for -I edge of J face, for col on -J side of fault
        pap = pfc[
            j + 1, i, 0, 0
        ]  # pillar index for -I edge of J face, for col on +J side of fault
        pbm = pfc[
        ...
        for (km, kp, fa_m, fa_p) in ji_list:
            if skip_inactive and (
                grid.inactive[km, j, i] or grid.inactive[kp, j + 1, i]
            ):
                continue
            juxtaposed_j_list.append(((km, j, i), (kp, j + 1, i), fa_m, fa_p))
    juxtaposed_i_list = []
    i_ji = np.stack(
        np.where(col_i_split)
    ).T  # ji0 columns where +I face is split; shape (nc, 2)
    warning_count = 0
    for (j, i) in i_ji:
        pam = pfc[
            j, i, 0, 1
        ]  # pillar index for -J edge of I face, for col on -I side of fault
        pap = pfc[
            j, i + 1, 0, 0
        ]  # pillar index for -J edge of I face, for col on +I side of fault
        pbm = pfc[
        ...
        for (km, kp, fa_m, fa_p) in ji_list:
            if skip_inactive and (
                grid.inactive[km, j, i] or grid.inactive[kp, j, i + 1]
            ):
                continue
    ...
    fcs.cell_index_pairs[:] = np.array(
        [
            grid.natural_cell_indices(
                np.array([[k, j, i] for ((k, j, i), _, _, _) in combo_list])
            ),
            grid.natural_cell_indices(
                np.array([[k, j, i] for (_, (k, j, i), _, _) in combo_list])
            ),
        ]
    ).T
    ...
    fcs.feature_indices = np.zeros(
        count, dtype=int
    )  # could create seperate features by named fracture
    tbf = rqo.TectonicBoundaryFeature(
        grid.model, extract_from_xml=False, kind="fault", feature_name=feature_name
    )
...
def projected_tri_area(pa, pb, pc):
    ...
    for xyz in range(3):
        ...
        xyz_1 = (xyz + 2) % 3
        pap = np.stack(
            (pa[..., xyz_0], pa[..., xyz_1]), axis=-1
        )  # points projected onto 2D
