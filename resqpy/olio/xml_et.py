...
from resqpy.olio.xml_namespaces import (
    namespace as ns,
    inverse_namespace as inv_ns,
    curly_namespace as cns,
)
...
def list_obj_references(root, skip_hdf5=True):
    ...
    if root is None:
        return None
    results = []
    if node_type(root) == "DataObjectReference" and not (
        skip_hdf5 and stripped_of_prefix(root.tag) == "HdfProxy"
    ):
        results.append(root)
    for child in root:
...
def print_xml_tree(
    ...
    if max_lines and line_count >= max_lines:
        if line_count == max_lines:
            message = (
                "(...xml tree output truncated after " + str(max_lines) + " lines)"
            )
            if to_log:
                print_fn(message)
            else:
...
def write_xml_node(xml_fp, root, level=0, namespace_keys=[]):
    ...
    for key in root.attrib.keys():
        ...
        if stripped_of_prefix(key) == "type":
            ...
            ct_special
            and colon_attrib_key == "PartName"
            and root.attrib[key].startswith("obj_")
        ):
    ...
    if (
        type_pre_colon and type_pre_colon not in ns_keys
    ):  # must be included in the local xml line?
    ...
    #      print(line, end = '') # debug
        ...
        if len(root):
            ...
            if use_tabs:
                ...
                indentation = 3 * level * " "
            for child in root:
                node_count += write_xml_node(
                    xml_fp, child, level=level + 1, namespace_keys=ns_keys
                )
