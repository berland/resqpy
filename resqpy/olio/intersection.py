...
def line_triangles_intersects(line_p, line_v, triangles, line_segment=False):
    ...
    np.divide(
        np.sum(np.cross(p02s, line_rv) * lp_t0s, axis=1), denoms, out=us, where=nz
    )
    np.divide(
        np.sum(np.cross(line_rv, p01s) * lp_t0s, axis=1), denoms, out=vs, where=nz
    )
    ts[:] = np.where(
        np.logical_or(
            np.logical_or(us < 0.0, us > 1.0), np.logical_or(vs < 0.0, us + vs > 1.0)
        ),
        np.nan,
        ts,
    )
...
def line_set_triangles_intersects(line_ps, line_vs, triangles, line_segment=False):
    ...
    denoms = np.dot(
        line_rvs, norms.T
    )  # shape (c, n)  where denoms == 0.0 line is parallel to plane of triangle
    lps_t0s = line_ps.reshape((c, 1, 3)) - triangles[:, 0, :].reshape(
        (1, n, 3)
    )  # shape (c, n, 3)
    ...
    np.seterr(divide="ignore")
    np.divide(
        np.sum(norms.reshape((1, n, 3)) * lps_t0s, axis=2), denoms, out=ts, where=nz
    )  # shape (c, n)
    np.seterr(invalid="ignore")
    if line_segment:
        ts[:] = np.where(np.logical_or(ts < 0.0, ts > 1.0), np.nan, ts)
    ...
    np.divide(
        np.sum(np.cross(cp02s, cl_rvs) * lps_t0s, axis=2), denoms, out=us, where=nz
    )  # shape (c, n)
    cp01s = p01s.reshape((1, n, 3)).repeat(c, axis=0)  # shape (c, n, 3)
    np.divide(
        np.sum(np.cross(cl_rvs, cp01s) * lps_t0s, axis=2), denoms, out=vs, where=nz
    )
    ts[:] = np.where(
        np.logical_or(
            np.logical_or(us < 0.0, us > 1.0), np.logical_or(vs < 0.0, us + vs > 1.0)
    ...
    intersects[:] = line_vs.reshape((c, 1, 3)) * np.repeat(ts, 3).reshape(
        (c, n, 3)
    ) + line_ps.reshape((c, 1, 3))
    np.seterr(all="warn")
    return intersects
...
def poly_line_triangles_intersects(line_ps, triangles):
    ...
    return line_set_triangles_intersects(
        line_ps[:-1], line_ps[1:] - line_ps[:-1], triangles, line_segment=True
    )
...
def distilled_intersects(line_set_intersections):
    ...
    lines, triangles = np.where(
        np.logical_not(np.isnan(line_set_intersections[:, :, 0]))
    )
...
def triangles_for_line(line_set_intersections, line_index):
    ...
    triangles = np.where(
        np.logical_not(np.isnan(line_set_intersections[line_index, :, 0]))
    )[0]
...
def lines_for_triangle(line_set_intersections, triangle_index):
    ...
    lines = np.where(
        np.logical_not(np.isnan(line_set_intersections[:, triangle_index, 0]))
    )[0]
...
def poly_line_triangles_first_intersect(line_ps, triangles, start=0):
    ...
    for c in range(start, len(line_ps) - 1):
        points = line_triangles_intersects(
            line_ps[c], line_ps[c + 1] - line_ps[c], triangles, line_segment=True
        )
...
def line_line_intersect(
    x1, y1, x2, y2, x3, y3, x4, y4, line_segment=False, half_segment=False
):
