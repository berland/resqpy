...
class DataFrame:
    ...
    def __init__(
        ...
        self.column_lookup = (
            None  # string lookup table mapping column index (0 based) to column name
        )
        self.uom_lookup_uuid = uom_lookup_uuid
        self.uom_lookup = (
            None  # string lookup table mapping column index (0 based) to uom
        )
        if support_root is not None:
            ...
            cl_root = self.model.root(
                obj_type="StringTableLookup",
                related_uuid=self.mesh.uuid,
                title="dataframe columns",
            )
            assert (
                cl_root is not None
            ), "column name lookup table not found for dataframe"
            ...
            da = self.mesh.full_array_ref()[
                ..., 2
            ]  # dataframe data as 2D numpy array, defaulting to z values in mesh
            ...
            if df is None:  # existing dara, either in mesh or property
                if existing_count > 0:  # use property data instead of z values
                    if existing_count == 1:
                        if self.realization is not None:
                            assert (
                                existing_pc.realization_for_part(
                                    existing_pc.singleton()
                                )
                                == self.realization
                            )
                    else:
                        ...
                        ), "no realization specified when accessing ensemble dataframe"
                    da = existing_pc.single_array_ref(realization=self.realization)
                    assert (
                        da is not None
                        and da.ndim == 2
                        and da.shape == (self.n_rows, self.n_cols)
                    )
                else:
                ...
                if existing_count > 0:
                    assert (
                        existing_pc.singleton(realization=self.realization) is None
                    ), "dataframe realization already exists"
                self.df = df.copy()
            ...
            assert (
                df is not None
            ), "no dataframe (or support root) provided when instantiating DataFrame object"
            self.df = df.copy()
            ...
            if column_lookup_uuid is not None:
                self.column_lookup = rqp.StringLookup(
                    self.model, root_node=self.model.root(uuid=column_lookup_uuid)
                )
                assert self.column_lookup is not None
                assert self.column_lookup.length() == self.n_cols
                assert all(
                    self.df.columns == self.column_lookup.get_list()
                )  # exact match of column names required!
            if uom_lookup_uuid is not None:
                self.uom_lookup = rqp.StringLookup(
                    self.model, root_node=self.model.root(uuid=uom_lookup_uuid)
                )
    ...
    def write_hdf5_and_create_xml(self):
        ...
        if sl_node is not None:
            self.model.create_reciprocal_relationship(
                self.mesh.root_node, "destinationObject", sl_node, "sourceObject"
            )
        ...
        if ul_node is not None:
            self.model.create_reciprocal_relationship(
                self.mesh.root_node, "destinationObject", ul_node, "sourceObject"
            )
    ...
    def _set_mesh_from_df(self):
        ...
        if self.mesh is None:
            ...
            if self.realization is not None:
                ...
                dataframe_pk_uuid = self.model.uuid(
                    obj_type="PropertyKind", title="dataframe"
                )
                if dataframe_pk_uuid is None:
                    dataframe_pk = rqp.PropertyKind(
                        self.model, title="dataframe", example_uom="Euc"
                    )
...
class TimeTable(DataFrame):
    ...
    def __init__(
        ...
        if support_root is not None:
            ts_root = self.model.root(
                obj_type="TimeSeries", related_uuid=self.mesh.uuid
            )
            assert (
                ts_root is not None
            ), "no time series related to mesh holding dataframe"
...
def dataframe_parts_in_model(model, timetables=None, title=None, related_uuid=None):
    ...
    df_parts_list = model.parts(
        obj_type="Grid2dRepresentation",
        title=title,
    ...
    if timetables is not None:
        ...
        for df_part in df_parts_list:
            is_tt = (
                model.part(
                    obj_type="TimeSeries", related_uuid=model.uuid_for_part(df_part)
                )
                is not None
            )
            if timetables == is_tt:
...
def timetable_parts_in_model(model, title=None, related_uuid=None):
    ...
    return dataframe_parts_in_model(
        model, timetables=True, title=title, related_uuid=related_uuid
    )
...
def dataframe_for_title(model, title, realization=None):
    ...
    return DataFrame(
        model, support_root=model.root_for_part(df_parts[0]), realization=realization
    )
...
def timetable_for_title(model, title, realization=None):
    ...
    return TimeTable(
        model, support_root=model.root_for_part(tt_parts[0]), realization=realization
    )
