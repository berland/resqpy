...
def infill_block_geometry(
    ...
    for j in range(extent[1]):
        for i in range(extent[2]):
            k_top = (
                0  # NB: 'top' & 'bottom' are misleading if k_increase_direction == 'up'
            )
            ...
            if snap_to_top_and_base:
                snap_depth = (
                    depth[k_top, j, i] - k_dir_sign * thickness[k_top, j, i] / 2.0
                )
                snap_x = x[k_top, j, i]
                snap_y = y[k_top, j, i]
            ...
            while True:
                while (
                    k_top < extent[0] and abs(depth[k_top, j, i]) > depth_zero_tolerance
                ):  # skip active layers
                    k_top += 1
                k_base = k_top + 1
                while (
                    k_base < extent[0]
                    and abs(depth[k_base, j, i]) <= depth_zero_tolerance
                ):
                ...
                if k_base >= extent[0]:  # no deeper active cells found
                    if snap_to_top_and_base:
                        snap_depth = (
                            depth[k_top - 1, j, i]
                            + k_dir_sign * thickness[k_top - 1, j, i] / 2.0
                        )
                ...
                void_top_depth = (
                    depth[k_top - 1, j, i]
                    + (thickness[k_top - 1, j, i] / 2.0) * k_dir_sign
                )
                void_bottom_depth = (
                    depth[k_base, j, i] - (thickness[k_base, j, i] / 2.0) * k_dir_sign
                )
                ...
                if void_interval < 0.0:  # overlapping cells
                    if -void_interval < vertical_cell_overlap_tolerance:
                        if nudge:
                            ...
                            for k_nudge in range(extent[0] - k_base):
                                if depth[k_base + k_nudge, j, i] > depth_zero_tolerance:
                                    depth[k_base + k_nudge, j, i] += (
                                        -void_interval * k_dir_sign
                                    )
                                    nudge_count += 1  # debug
                            log.debug(
                                "%1d cells nudged in [ i j ] column [%1d, %1d]",
                                nudge_count,
                                i + 1,
                                j + 1,
                            )
                        ...
                        log.warn(
                            "Cells [%1d, %1d, %1d] and [%1d, %1d, %1d] overlap ...",
                            i + 1,
                            j + 1,
                            k_top,
                            i + 1,
                        ...
                        log.warn(
                            "Skipping rest of i,j column"
                        )  # todo: could abort here
                        break
                assert infill_cell_thickness >= 0.0
                for void_k in range(void_cell_count):
                    depth[k_top + void_k, j, i] = (
                        void_top_depth
                        + (0.5 + void_k) * infill_cell_thickness * k_dir_sign
                    )
                    thickness[k_top + void_k, j, i] = infill_cell_thickness
                    x[k_top + void_k, j, i] = (
                        void_top_x + (0.5 + void_k) * void_x_interval / void_cell_count
                    )
                    y[k_top + void_k, j, i] = (
                        void_top_y + (0.5 + void_k) * void_y_interval / void_cell_count
                    )
                k_top = k_base
...
def determine_corp_ijk_handedness(corner_points, xyz_is_left_handed=True):
    ...
    log.debug(
        "using cell ijk0 [{}, {}, {}] to determine ijk handedness".format(
            cell_kji[2], cell_kji[1], cell_kji[0]
        )
    )
    cell_cp = corner_points[cell_kji]
    origin = cell_cp[0, 0, 0]
    det = vec.determinant(
        cell_cp[0, 0, 1] - origin, cell_cp[0, 1, 0] - origin, cell_cp[1, 0, 0] - origin
    )  # NB. IJK ordering
    if det == 0.0:
        log.warning(
            "indeterminate handedness in cell ijk0 [{}, {}, {}]".format(
                cell_kji[2], cell_kji[1], cell_kji[0]
            )
        )
        return None
...
def determine_corp_extent(corner_points, tolerance=0.003):
    ...
    def neighbours(
        ...
        if (
            vec.manhatten_distance(
                corner_points[sextuple_cell_a_p1], corner_points[sextuple_cell_b_p1]
            )
            <= tolerance
        ) and (
            vec.manhatten_distance(
                corner_points[sextuple_cell_a_p2], corner_points[sextuple_cell_b_p2]
            )
            <= tolerance
        ):
            return True
        if (
            vec.manhatten_distance(
                corner_points[sextuple_cell_a_p1], corner_points[sextuple_cell_b_p2]
            )
            <= tolerance
        ) and (
            vec.manhatten_distance(
                corner_points[sextuple_cell_a_p2], corner_points[sextuple_cell_b_p1]
            )
            <= tolerance
        ):
            return True
    ...
    while redundancy:
        ...
        for e in possible_extents:
            ...
            if neighbours(
                corner_points,
                (0, 0, kji_cell[2], 0, 1, 0),
                (0, 0, kji_cell[2], 0, 1, 1),
                (0, 0, candidate, 0, 0, 0),
                (0, 0, candidate, 0, 0, 1),
                tolerance,
            ):
                if ni is not None and ni != e:
                    log.error(
                        "inconsistent NI values of {} and {} determined from corner points".format(
                            ni, e
                        )
                    )
                    return None
        ...
        if not found:
            ...
            if remaining_attempts <= 0:
                log.error(
                    "failed to determine NI from corner points (out of tries)"
                )  # could assume NJ = 1 here
                return None
    ...
    if ni > 1:
        ...
        log.debug(
            "remaining prime factors after accounting for NI are: "
            + str(prime_factorization)
        )
        possible_extents = factors.all_factors_from_primes(prime_factorization)
    ...
    while redundancy:
        ...
        for e in possible_extents:
            ...
            if (
                vec.manhatten_distance(
                    corner_points[0, 0, kji_cell[2], 1, 0, 0],
                ...
                if nj is not None and nj != e:
                    log.error(
                        "inconsistent NJ values of {} and {} determined from corner points".format(
                            nj, e
                        )
                    )
                    return None
        ...
        if not found:
            ...
            if remaining_attempts <= 0:
                log.error(
                    "failed to determine NJ from corner points (out of tries)"
                )  # could assume or check if NK = 1 here
                return None
...
def translate_corp(
    corner_points, x_shift=None, y_shift=None, min_xy=None, preserve_digits=None
):
...
def triangles_for_cell_faces(cp):
    ...
    tri[0, :, :, 0] = (
        np.mean(cp, axis=(1, 2)).reshape((2, 1, 3)).repeat(4, axis=1).reshape((2, 4, 3))
    )  # k face centres
    tri[1, :, :, 0] = (
        np.mean(cp, axis=(0, 2)).reshape((2, 1, 3)).repeat(4, axis=1).reshape((2, 4, 3))
    )  # j face centres
    tri[2, :, :, 0] = (
        np.mean(cp, axis=(0, 1)).reshape((2, 1, 3)).repeat(4, axis=1).reshape((2, 4, 3))
    )  # i face centres
...
def actual_pillar_shape(pillar_points, tolerance=0.001):
    ...
    xy_drift = np.abs(from_top[:, :, 0]) + np.abs(
        from_top[:, :, 1]
    )  # use Manhattan distance as cheap proxy for true distance
    ...
    from_top[-1, :, 2] = np.where(
        null_pillar_mask, tolerance, from_top[-1, :, 2]
    )  # avoid divide by zero issues
    z_fraction = from_top[:, :, 2] / from_top[-1, :, 2]
    xy_drift = from_top[:, :, :2] - z_fraction.reshape(
        (pp.shape[0], pp.shape[1], 1)
    ) * from_top[-1, :, :2].reshape((1, pp.shape[1], 2))
...
def left_right_foursome(full_pillar_list, p_index):
    ...
    if entry == (0, 1):
        if exit == (-1, 0):
            ...
            raise Exception(
                "code failure whilst taking exit sides from dubious full pillar list"
            )
    elif entry == (0, -1):
        if exit == (-1, 0):
        ...
        raise Exception(
            "code failure whilst taking entry sides from dubious full pillar list"
        )
