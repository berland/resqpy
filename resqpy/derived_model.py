...
def _prepare_simple_inheritance(
    grid, source_grid, inherit_properties, inherit_realization, inherit_all_realizations
):
    collection = None
...
def zone_layer_ranges_from_array(
    zone_array, min_k0=0, max_k0=None, use_dominant_zone=False
):
    ...
    def dominant_zone(zone_array):
        ...
        for k in range(zone_array.shape[0]):
            ...
            log.info(
                "layer "
                + str(k + 1)
                + " (1 based) zone set to most common value "
                + str(dominant)
            )
    ...
    for zone in zone_list:
        ...
        for k0 in range(zone_min_k0, zone_max_k0 + 1):
            assert np.all(
                single_zone_mask[k0]
            ), "unacceptable zone variation with layer " + str(k0 + 1)
        zone_layer_range_list.append((zone_min_k0, zone_max_k0, zone))
    assert (
        len(zone_layer_range_list) > 0
    ), "no zone layer ranges derived from zone array"
    zone_layer_range_list.sort()
    for zone_i in range(1, len(zone_layer_range_list)):
        assert (
            zone_layer_range_list[zone_i][0] > zone_layer_range_list[zone_i - 1][1]
        ), "overlapping zone layer ranges"  # todo: add more info
        if zone_layer_range_list[zone_i][0] > zone_layer_range_list[zone_i - 1][1] + 1:
            log.warning(
                "gap in zonal layer ranges, missing layer(s) being arbitrarily assigned to zone below"
            )  # todo: add more info to log
            zone_layer_range_list[zone_i][0] = zone_layer_range_list[zone_i - 1][1] + 1
    return zone_layer_range_list


def add_zone_by_layer_property(
    epc_file,
    grid_uuid=None,
    ...
    if zone_by_layer_vector is not None:
        assert (
            len(zone_by_layer_vector) == grid.nk
        ), "length of zone by layer vector does not match number of layers in grid"
        ...
        assert (
            zone_by_cell_array is not None
        ), "zone by cell property array not found for uuid: " + str(
            zone_by_cell_property_uuid
        )
        zone_range_list = zone_layer_ranges_from_array(
            zone_by_cell_array, use_dominant_zone=use_dominant_zone
        )
        assert (
            zone_range_list is not None and len(zone_range_list) > 0
        ), "failed to convert zone by cell to zone by layer"
        zone_by_layer = np.full((grid.nk,), null_value, dtype=int)
        for min_k0, max_k0, zone_index in zone_range_list:
            assert (
                0 <= min_k0 <= max_k0 < grid.nk
            ), "zonal layer limits out of range for grid (probable bug)"
    ...
    property_uuid = add_one_grid_property_array(
        epc_file,
        zone_by_layer,
        property_kind,
        grid_uuid=grid.uuid,
...
def add_one_grid_property_array(
    ...
    if (
        ...
        and epc_file
        and (
            (new_epc_file == epc_file)
            or (
                os.path.exists(new_epc_file)
                and os.path.exists(epc_file)
...
def add_one_blocked_well_property(
    ...
    assert (
        bw_root is not None
    ), f"no blocked well object found with uuid {blocked_well_uuid}"
...
def add_wells_from_ascii_file(
    ...
    if crs_uuid is None:
        if z_inc_down is None:
            z_inc_down = True
        crs_root = rqcrs.Crs(
            model, xy_units=length_uom, z_units=length_uom, z_inc_down=z_inc_down
        )
    ...
    log.info(
        "features, interpretations, trajectories and md data added for "
        + str(count)
        + " well"
        + _pl(count)
    )
...
def zonal_grid(
    ...
    def fetch_zone_array(grid, zone_title=None, zone_uuid=None, masked=True):
        ...
        assert (
            properties is not None and properties.number_of_parts() > 0
        ), "no properties found in relation to grid"
        ...
        if zone_title:
            properties = rqp.selective_version_of_collection(
                properties, citation_title=zone_title
        ...
        return properties.cached_part_array_ref(
            part_name, masked=masked
        )  # .copy() needed?
    ...
    if (
        ...
        new_epc_file = None
    assert (
        epc_file or source_grid is not None
    ), "neither epc file name nor source grid supplied"
    ...
    if not single_layer_mode:  # process zone array
        if zone_layer_range_list is None:
            ...
            zone_layer_range_list = zone_layer_ranges_from_array(
                zone_array, k0_min, k0_max, use_dominant_zone=use_dominant_zone
            )
        zone_count = len(zone_layer_range_list)
        # above is list of (zone_min_k0, zone_max_k0, zone) sorted by zone_min_k0
        log.info(
            "following layer ranges are based on top layer being numbered 1 (simulator protocol)"
        )
        for (zone_min_k0, zone_max_k0, zone) in zone_layer_range_list:
            log.info(
                "zone id {0:1d} covers layers {1:1d} to {2:1d}".format(
                    zone, zone_min_k0 + 1, zone_max_k0 + 1
                )
            )
    else:
    ...
    if is_regular:
        ...
        grid.extent_kji = np.array(
            (zone_count, source_grid.nj, source_grid.ni), dtype="int"
        )
    ...
    if source_grid.inactive is None:
        ...
        if inactive_laissez_faire:
            ...
            grid.inactive = np.all(
                source_grid.inactive[k0_min : k0_max + 1], axis=0
            ).reshape(grid.extent_kji)
        else:
            log.debug("setting inactive mask using any mode (strict)")
            grid.inactive = np.any(
                source_grid.inactive[k0_min : k0_max + 1], axis=0
            ).reshape(grid.extent_kji)
    else:
        ...
        for zone_i in range(zone_count):
            ...
            if inactive_laissez_faire:
                grid.inactive[zone_i] = np.all(
                    source_grid.inactive[zk0_min : zk0_max + 1], axis=0
                )
            else:
                grid.inactive[zone_i] = np.any(
                    source_grid.inactive[zk0_min : zk0_max + 1], axis=0
                )
    ...
    if not is_regular:
        ...
        if hasattr(source_grid, "array_cell_geometry_is_defined"):
            ...
            grid.geometry_defined_for_all_cells_cached = (
                source_grid.geometry_defined_for_all_cells_cached
            )
        # copy info for pillar geometry is defined
        grid.geometry_defined_for_all_pillars_cached = (
            source_grid.geometry_defined_for_all_pillars_cached
        )
        if hasattr(source_grid, "array_pillar_geometry_is_defined"):
            grid.array_pillar_geometry_is_defined = (
                source_grid.array_pillar_geometry_is_defined.copy()
            )
        ...
        if grid.geometry_defined_for_all_cells_cached:
            if single_layer_mode:
                ...
                for zone_i in range(zone_count):
                    if zone_i == 0:
                        grid.points_cached[0] = source_points[
                            zone_layer_range_list[zone_i][0]
                        ]
                    grid.points_cached[zone_i + 1] = source_points[
                        zone_layer_range_list[zone_i][1]
                    ]  # or could use 0th element of tuple for zone_i+1
                        ...
                        if zone_i == 0:
                            for k in range(zk0_min, zk0_max + 1):
                                if source_grid.array_cell_geometry_is_defined[k, j, i]:
                                    grid.points_cached[
                                        0, j : j + 2, i : i + 2
                                    ] = source_points[k, j : j + 2, i : i + 2]
                                    break
                        for k in range(zk0_max, zk0_min - 1, -1):
                            if source_grid.array_cell_geometry_is_defined[k, j, i]:
                                grid.points_cached[
                                    zone_i + 1, j : j + 2, i : i + 2
                                ] = source_points[k + 1, j : j + 2, i : i + 2]
                        ...
                        if zone_i == 0:
                            for k in range(zk0_min, zk0_max + 1):
                                if source_grid.array_cell_geometry_is_defined[k, j, i]:
                                    for jp in range(2):
                                        for ip in range(2):
                                            pillar = grid.pillars_for_column[
                                                j, i, jp, ip
                                            ]
                                            grid.points_cached[
                                                0, pillar
                                            ] = source_points[k, pillar]
                                    break
                            ...
                            if source_grid.array_cell_geometry_is_defined[k, j, i]:
                                for jp in range(2):
                                    for ip in range(2):
                                        ...
                                        grid.points_cached[
                                            zone_i + 1, pillar
                                        ] = source_points[k + 1, pillar]
                                grid.array_cell_geometry_is_defined[zone_i, j, i] = True
                                break
        if grid.has_split_coordinate_lines:
            grid.split_pillar_indices_cached = (
                source_grid.split_pillar_indices_cached.copy()
            )
            grid.cols_for_split_pillars = source_grid.cols_for_split_pillars.copy()
            grid.cols_for_split_pillars_cl = (
                source_grid.cols_for_split_pillars_cl.copy()
            )
...
def interpolated_grid(
    ...
    if (
        ...
        new_epc_file = None
    assert (
        grid_a is not None and grid_b is not None
    ), "at least one source grid is missing"
    assert (
        grid_a.grid_representation == "IjkGrid"
        and grid_b.grid_representation == "IjkGrid"
    )
    assert 0.0 <= a_to_b_0_to_1 <= 1.0, "interpolation factor outside range 0.0 to 1.0"
    assert tuple(grid_a.extent_kji) == tuple(
        grid_b.extent_kji
    ), "source grids have different extents"
    assert (
        grid_a.k_direction_is_down == grid_b.k_direction_is_down
    ), "source grids have different k directions"
    assert (
        grid_a.grid_is_right_handed == grid_b.grid_is_right_handed
    ), "source grids have different ijk handedness"
    assert (
        grid_a.pillar_shape == grid_b.pillar_shape
    ), "source grids have different resqml pillar shapes"
    ...
    assert (
        grid_a.geometry_defined_for_all_cells()
    ...
    if not grid_a.has_split_coordinate_lines and not grid_b.has_split_coordinate_lines:
        ...
        grid_a.has_split_coordinate_lines
        and grid_b.has_split_coordinate_lines
        and grid_a.points_cached.shape == grid_b.points_cached.shape
        and grid_a.split_pillar_indices_cached.shape
        == grid_b.split_pillar_indices_cached.shape
        and grid_a.cols_for_split_pillars.shape == grid_b.cols_for_split_pillars.shape
        and grid_a.cols_for_split_pillars_cl.shape
        == grid_b.cols_for_split_pillars_cl.shape
        and np.all(
            grid_a.split_pillar_indices_cached == grid_b.split_pillar_indices_cached
        )
        and np.all(grid_a.cols_for_split_pillars == grid_b.cols_for_split_pillars)
        and np.all(grid_a.cols_for_split_pillars_cl == grid_b.cols_for_split_pillars_cl)
    ):
        work_from_pillars = True
    else:
    ...
    if work_from_pillars:
        ...
        log.warning(
            "interpolating between corner points due to pillar incompatibilities"
        )
    ...
    grid.has_split_coordinate_lines = (
        grid_a.has_split_coordinate_lines or grid_b.has_split_coordinate_lines
    )
    ...
    if work_from_pillars:
        grid.points_cached = (
            grid_a.points_cached * a_weight + grid_b.points_cached * b_weight
        )
        ...
        if grid.nk > 1:
            ...
            log.info(
                "maximum vertical void distance after corner point interpolation: {0:.3f} {1}".format(
        ...
        log.debug(
            "reducing k extent of interpolated corner point array (sharing points vertically)"
        )
        k_reduced_cp_array = np.zeros(
            (grid.nk + 1, grid.nj, grid.ni, 2, 2, 3)
        )  # (nk+1, nj, ni, jp, ip, xyz)
        ...
        primary_pillar_jip = np.zeros(
            (grid.nj + 1, grid.ni + 1, 2), dtype="int"
        )  # (nj + 1, ni + 1, jp:ip)
        primary_pillar_jip[-1, :, 0] = 1
        primary_pillar_jip[:, -1, 1] = 1
        # build extra pillar references for split pillars
        extras_count = np.zeros(
            (grid.nj + 1, grid.ni + 1), dtype="int"
        )  # count (0 to 3) of extras for pillar
        extras_list_index = np.zeros(
            (grid.nj + 1, grid.ni + 1), dtype="int"
        )  # index in list of 1st extra for pillar
        extras_list = []  # list of (jp, ip)
        extras_use = np.negative(
            np.ones((grid.nj, grid.ni, 2, 2), dtype="int")
        )  # (j, i, jp, ip); -1 means use primary
            ...
            for i in range(grid.ni + 1):
                ...
                for jp in range(2):
                    ...
                    for ip in range(2):
                        ...
                        discrepancy = np.max(
                            np.abs(
                                k_reduced_cp_array[:, col_j, col_i, jp, ip, :]
                                - k_reduced_cp_array[
                                    :, p_col_j, p_col_i, primary_jp, primary_ip, :
                                ]
                            )
                        )
                        ...
                        for e in range(extras_count[j, i]):
                            ...
                            if (
                                discrepancy <= split_tolerance
                            ):  # data for this corner aligns with existing extra
                                extras_use[col_j, col_i, jp, ip] = e
                                break
                        if (
                            extras_use[col_j, col_i, jp, ip] >= 0
                        ):  # reusing an existing extra for this pillar
                            continue
                        # add this corner as an extra
                        if (
                            extras_count[j, i] == 0
                        ):  # create entry point for this pillar in extras
        ...
        if grid.has_split_coordinate_lines:
            points_array = np.zeros(
                (grid.nk + 1, (grid.nj + 1) * (grid.ni + 1) + len(extras_list), 3)
            )
            index = 0
            # primary pillars
            for pillar_j in range(grid.nj + 1):
                for pillar_i in range(grid.ni + 1):
                    ...
                    points_array[:, index, :] = k_reduced_cp_array[
                        :, pillar_j - jp, pillar_i - ip, jp, ip, :
                    ]
                    index += 1
            ...
            for pillar_j in range(grid.nj + 1):
                for pillar_i in range(grid.ni + 1):
                    for e in range(extras_count[pillar_j, pillar_i]):
                        ...
                        points_array[:, index, :] = k_reduced_cp_array[
                            :, pillar_j_extra, pillar_i_extra, jp, ip, :
                        ]
                        index += 1
            ...
            for j in range(grid.nj + 1):
                for i in range(grid.ni + 1):
                    ...
                    points_array[:, j, i, :] = k_reduced_cp_array[
                        :, j - jp, i - ip, jp, ip, :
                    ]
        ...
        if grid.has_split_coordinate_lines:
            ...
            for pillar_j in range(grid.nj + 1):
                for pillar_i in range(grid.ni + 1):
                    for e in range(extras_count[pillar_j, pillar_i]):
                        split_pillar_indices_list.append(
                            pillar_j * (grid.ni + 1) + pillar_i
                        )
                        use_count = 0
                        for jp in range(2):
                            j = pillar_j - jp
            ...
            grid.split_pillar_indices_cached = np.array(
                split_pillar_indices_list, dtype="int"
            )
            log.debug(
                "number of uses of extra pillars: "
                + str(len(cols_for_extra_pillar_list))
            )
            ...
            grid.cols_for_split_pillars = np.array(
                cols_for_extra_pillar_list, dtype="int"
            )
            assert len(cumulative_length_list) == len(extras_list)
            grid.cols_for_split_pillars_cl = np.array(
                cumulative_length_list, dtype="int"
            )
            grid.split_pillars_count = len(extras_list)

    collection = _prepare_simple_inheritance(
        grid, grid_a, inherit_properties, inherit_realization, inherit_all_realizations
    )
    ...
    if new_epc_file:
        ...
        ext_uuid, _ = model.h5_uuid_and_path_for_node(
            rqet.find_nested_tags(grid_a.grid_root, ["Geometry", "Points"]),
            "Coordinates",
        )
        write_grid(
            epc_file,
...
def extract_box(
    ...
    def local_pillar_index(extent, box, p):
        ...
        if (
            p_j < 0
            or p_i < 0
            or p_j > box[1, 1] - box[0, 1] + 1
            or p_i > box[1, 2] - box[0, 2] + 1
        ):
            return None
    ...
    assert (
        np.all(box[1, :] >= box[0, :])
        and np.all(box[0, :] >= 0)
        and np.all(box[1, :] < source_grid.extent_kji)
    )
    ...
    if source_grid.k_gaps and box[1, 0] > box[0, 0]:
        ...
        if k_gaps > 0:
            ...
            grid.k_gap_after_array = source_grid.k_gap_after_array[
                box[0, 0] : box[1, 0]
            ].copy()
    ...
    if source_grid.inactive is None:
        ...
        if box_inactive is None:
            ...
            log.debug(
                "setting inactive mask to merge of source grid extraction and mask passed as argument"
            )
            grid.inactive = np.logical_or(
                array_box(source_grid.inactive, box), box_inactive
            )
    ...
    # determine cell geometry is defined
    if hasattr(source_grid, "array_cell_geometry_is_defined"):
        grid.array_cell_geometry_is_defined = array_box(
    ...
    if hasattr(source_grid, "array_pillar_geometry_is_defined"):
        grid.array_pillar_geometry_is_defined = array_box(
    ...
    pillar_box[
        1, :
    ] += 1  # pillar points have extent one greater than cells, in each axis

    if not source_grid.has_split_coordinate_lines:
        log.debug("no split pillars in source grid")
        grid.points_cached = array_box(
            source_points, pillar_box
        )  # should work, ie. preserve xyz axis
    else:
        source_base_pillar_count = (source_grid.nj + 1) * (source_grid.ni + 1)
        log.debug(
            "number of base pillars in source grid: " + str(source_base_pillar_count)
        )
        log.debug(
            "number of extra pillars in source grid: "
            + str(len(source_grid.split_pillar_indices_cached))
        )
        base_points = array_box(
            source_points[:, :source_base_pillar_count, :].reshape(
                (
                    source_grid.nk_plus_k_gaps + 1,
                    source_grid.nj + 1,
                    source_grid.ni + 1,
                    3,
                )
            ...
            pillar_box,
        ).reshape(grid.nk_plus_k_gaps + 1, (grid.nj + 1) * (grid.ni + 1), 3)
        extra_points = np.zeros(
            (
                pillar_box[1, 0] - pillar_box[0, 0] + 1,
                source_points.shape[1] - source_base_pillar_count,
                3,
            )
        )
        ...
        local_cols_cl = np.zeros(
            len(source_grid.split_pillar_indices_cached), dtype=int
        )
        local_index = 0
        for index in range(len(source_grid.split_pillar_indices_cached)):
            ...
            if unique_count <= 0:
                continue
            base_points[:, local_pi, :] = source_points[
                pillar_box[0, 0] : pillar_box[1, 0] + 1,
            ...
            while unique_count > 0:
                ...
                extra_points[:, local_index, :] = source_points[
                    pillar_box[0, 0] : pillar_box[1, 0] + 1, source_pi, :
                ]
    ...
    if inherit_properties:
        ...
        if source_collection is not None:
            # do not inherit the inactive property array by this mechanism
            active_collection = rqp.selective_version_of_collection(
                source_collection, property_kind="active"
            )
    ...
    if new_epc_file:
        ...
        ext_uuid, _ = model.h5_uuid_and_path_for_node(
            rqet.find_nested_tags(source_grid.grid_root, ["Geometry", "Points"]),
            "Coordinates",
        )
        write_grid(
            epc_file,
...
def extract_box_for_well(
    ...
    assert (
        ...
        or blocked_well_uuid is not None
        or column_ji0 is not None
    )
    if radius is None:
        radius = (
            0.0  # cells directly penetrated through a k face will still be included
        )
    assert radius >= 0.0
    ...
    if outer_radius is not None:
        ...
        outer_inactive_mask = np.zeros(
            source_grid.extent_kji, dtype=bool
        )  # intialised to False
    ...
    if trajectory_uuid is not None:
        ...
        assert (
            trajectory_root is not None
        ), "trajectory object not found for uuid: " + str(trajectory_uuid)
        ...
        trajectory.crs_root = (
            source_grid.crs_root
        )  # note: tangent vectors might be messed up, if present
        ...
        bw_root = traj_model.root(
            obj_type="BlockedWellboreRepresentation", uuid=blocked_well_uuid
        )
        ...
        assert (
            bw_box[0, 0] <= max_k0 and bw_box[1, 0] >= min_k0
        ...
        assert (
            0 <= column_ji0[0] < source_grid.nj and 0 <= column_ji0[1] < source_grid.ni
        )
        cols_ji0 = np.array(column_ji0, dtype=int).reshape((1, 2))
        if not well_name:
            well_name = (
                "well for global column "
                + str(column_ji0[1] + 1)
                + ", "
                + str(column_ji0[0] + 1)
            )
    ...
    for k in range(min_k0, end_k0):
        if trajectory is None:
            if blocked_well is None:
                cols, intersect_points = (
                    cols_ji0,
                    centres[k, column_ji0[0], column_ji0[1]].reshape((1, 3)),
                )
            else:
            ...
            if k < source_grid.nk:
                (
                    cols,
                    intersect_points,
                ) = rgs.find_intersections_of_trajectory_with_layer_interface(
                    trajectory,
                    source_grid,
                    k0=k,
                    ref_k_faces="top",
        ...
        if cols is None or len(cols) == 0:
            if not warned:
                log.warning(
                    f"no intersection found between well and {h_or_l}(s) such as: {k}"
                )
                warned = True
            continue
        count = cols.shape[0]
        ...
        layer_mask = np.zeros(
            (source_grid.nj, source_grid.ni), dtype=bool
        )  # to be set True within radius
        if outer_radius is not None:
            outer_layer_mask = np.ones(
                (source_grid.nj, source_grid.ni), dtype=bool
            )  # to be set False within outer_radius
        for intersect in range(count):
            log.debug(
                f"well intersects {h_or_l} {k} in column j0,i0: {cols[intersect, 0]}, {cols[intersect, 1]}"
            )
            if radius > 0.0 or outer_radius is not None:
                if k < source_grid.nk:
                    vectors = centres[k] - intersect_points[intersect].reshape(
                        (1, 1, 3)
                    )
                    distance_sqr = (
                        vectors[..., 0] * vectors[..., 0]
                    ...
                    if radius > 0.0:
                        layer_mask = np.logical_or(
                            layer_mask, np.less_equal(distance_sqr, radius_sqr)
                        )
                    if outer_radius is not None:
                        outer_layer_mask = np.logical_and(
                            outer_layer_mask,
                            np.greater_equal(distance_sqr, outer_radius_sqr),
                        )
                if k > 0 and (
                    not source_grid.k_gaps
                    or k >= source_grid.nk - 1
                    or not source_grid.k_gap_after_array[k - 1]
                ):
                    vectors = centres[k - 1] - intersect_points[intersect].reshape(
                        (1, 1, 3)
                    )
                    distance_sqr = (
                        vectors[..., 0] * vectors[..., 0]
        ...
        if k > min_k0:
            ...
            if outer_radius is not None:
                outer_inactive_mask[k - 1] = np.logical_and(
                    outer_inactive_mask[k - 1], outer_layer_mask
                )
        log.debug(
    ...
    log.info(
        "box for well is: "
        + bx.string_iijjkk1_for_box_kji0(box)
        + " (simulator protocol)"
    )
    ...
    if active_cells_shape in ["tube", "prism"]:
        ...
        box_inactive = np.logical_not(
            inclusion_mask[
                min_k0 : max_k0 + 1, min_j0 : max_j0 + 1, min_i0 : max_i0 + 1
            ]
        )
    ...
    if not new_grid_title:
        if trajectory is not None:
            new_grid_title = (
                "local grid extracted for well: "
                + rqet.citation_title_for_node(trajectory_root)
            )
        elif blocked_well is not None:
            new_grid_title = (
                "local grid extracted for blocked well: "
                + rqet.citation_title_for_node(bw_root)
            )
        elif column_ji0 is not None:
            new_grid_title = (
                "local grid extracted around column i, j (1 based): "
                + str(column_ji0[1] + 1)
                + ", "
                + str(column_ji0[0] + 1)
            )
    ...
    if inherit_well and new_epc_file:
        ...
        if trajectory is None and blocked_well is None:
            ...
            newer_model.copy_part_from_other_model(
                traj_model, rqet.part_name_for_part_root(trajectory_root)
...
def refined_grid(
    ...
    if epc_file:
        ...
        if source_grid is None:
            if source_grid_uuid is None:
                ...
                source_grid = grr.Grid(
                    model_in, grid_root=model_in.root_for_uuid(source_grid_uuid)
                )
        else:
            ...
            source_grid = grr.Grid(
                model_in, grid_root=model_in.root_for_uuid(grid_uuid)
            )
        if model is None:
            model = model_in
    else:
    ...
    if infill_missing_geometry and (
        ...
        source_grid.set_geometry_is_defined(
            treat_as_nan=None,
    ...
    assert (
        source_grid.geometry_defined_for_all_pillars()
    ), "refinement requires geometry to be defined for all pillars"
    assert (
        source_grid.geometry_defined_for_all_cells()
    ), "refinement requires geometry to be defined for all cells"

    assert tuple(fine_coarse.coarse_extent_kji) == tuple(
        source_grid.extent_kji
    ...
    if source_grid.has_split_coordinate_lines:
        ...
        for ck0 in range(source_grid.nk):
            ...
            for cj0 in range(source_grid.nj):
                ...
                for ci0 in range(source_grid.ni):
                    ...
                    fine_cp[
                        fine_k_base : fine_k_base + k_ratio,
                        fine_j_base : fine_j_base + j_ratio,
                        fine_i_base : fine_i_base + i_ratio,
                        0,
                        0,
                        0,
        ...
        for ck0 in range(fine_coarse.coarse_extent_kji[0] + 1):
            ...
            one_if_gap = (
                1
                if source_grid.k_gaps
                and ck0 < fine_coarse.coarse_extent_kji[0] - 1
                and source_grid.k_gap_after_array[ck0]
                else 0
            )
            for flk0 in range(k_ratio + one_if_gap):
                ...
                if grid.k_gaps:
                    if end_k:
                        k_plane = source_points[
                            source_grid.k_raw_index_array[ck0 - 1] + 1, :, :, :
                        ]
                    else:
                        k_plane = (
                            k_fraction
                            * source_points[
                                source_grid.k_raw_index_array[ck0] + 1, :, :, :
                            ]
                            + (1.0 - k_fraction)
                    ...
                    if end_k:
                        ...
                        k_plane = (
                            k_fraction * source_points[ck0 + 1, :, :, :]
        ...
        grid.geometry_defined_for_all_pillars_cached = True
        grid.geometry_defined_for_all_cells_cached = True
        grid.array_cell_geometry_is_defined = np.full(
            tuple(grid.extent_kji), True, dtype=bool
        )
    ...
    if set_parent_window:
        ...
        if isinstance(set_parent_window, str):
            if set_parent_window == "grandparent":
                assert fine_coarse.within_coarse_box is None or (
                    np.all(fine_coarse.within_coarse_box[0] == 0)
                    and np.all(fine_coarse.within_coarse_box[1])
                    == source_grid.extent_kji - 1
                ...
                if source_fine_coarse is not None and (
                    ...
                    assert (
                        source_fine_coarse.fine_extent_kji
                        == source_fine_coarse.coarse_extent_kji
                    ), "parentage involves refinement or coarsening"
                    if source_fine_coarse.within_coarse_box is not None:
                        fine_coarse.within_coarse_box = (
                            source_fine_coarse.within_coarse_box
                        )
                    else:
                        fine_coarse.within_coarse_box = (
                            source_fine_coarse.within_fine_box
                        )
                    pw_grid_uuid = bu.uuid_from_string(
                        rqet.find_nested_tags_text(
                            source_grid.grid_root,
                            ["ParentWindow", "ParentGrid", "UUID"],
                        )
                    )
            else:
                assert set_parent_window == "parent", (
                    "set_parent_window value not recognized: " + set_parent_window
                )
    ...
    if new_grid_title is None or len(new_grid_title) == 0:
        new_grid_title = "grid refined from " + str(
            rqet.citation_title_for_node(source_grid.grid_root)
        )
...
def coarsened_grid(
    ...
    assert (
        not source_grid.has_split_coordinate_lines
    ), "coarsening only available for unsplit grids: use other functions to heal faults first"
    ...
    assert (
        not source_grid.k_gaps
    ), "coarsening of grids with k gaps not currently supported"

    assert tuple(fine_coarse.fine_extent_kji) == tuple(
        source_grid.extent_kji
    ...
    grid.nk_plus_k_gaps = (
        source_grid.nk_plus_k_gaps
    )  # k_gaps not currently supported anyway
    grid.k_direction_is_down = source_grid.k_direction_is_down
    grid.grid_is_right_handed = source_grid.grid_is_right_handed
    grid.pillar_shape = source_grid.pillar_shape
    grid.has_split_coordinate_lines = False
    ...
    )  # note: gets reshaped after being populated
        ...
        for cji in range(grid.ni + 1):
            ...
            if k_ratio_constant:
                coarsened_points[:, natural_coarse_pillar, :] = source_points[
                    0 : source_grid.nk + 1 : k_ratio_constant, natural_fine_pillar, :
                ]
            else:
                coarsened_points[:, natural_coarse_pillar, :] = source_points[
    ...
    if set_parent_window:
        ...
        if isinstance(set_parent_window, str):
            if set_parent_window == "grandparent":
                assert fine_coarse.within_fine_box is None or (
                    np.all(fine_coarse.within_fine_box[0] == 0)
                    and np.all(fine_coarse.within_fine_box[1])
                    == source_grid.extent_kji - 1
                ), "attempt to set grandparent window for grid when parent window is present"
                source_fine_coarse = source_grid.parent_window
                if source_fine_coarse is not None and (
...
def local_depth_adjustment(
    ...
    z_inc_down = rqet.bool_from_text(
        rqet.node_text(rqet.find_tag(crs_root, "ZIncreasingDownward"))
    )
    ...
    log.debug(
        "min z before depth adjustment: " + str(np.nanmin(reshaped_points[:, :, 2]))
    )
    ...
    for pillar in range(reshaped_points.shape[1]):
        ...
        log.debug(
            "adjusting pillar number {0} at x: {1:3.1f}, y: {2:3.1f}, distance: {3:3.1f} by {4:5.3f}".format(
    ...
    log.debug(
        "min z after depth adjustment: " + str(np.nanmin(reshaped_points[:, :, 2]))
    )
    if grid.has_split_coordinate_lines:
    ...
    if new_grid_title is None or len(new_grid_title) == 0:
        new_grid_title = (
            "grid derived from {0} with local depth shift of {1:3.1f} applied".format(
...
def tilted_grid(
    ...
    if new_grid_title is None or len(new_grid_title) == 0:
        new_grid_title = "tilted version ({0:4.2f} degree dip) of ".format(
            abs(dip)
        ) + str(rqet.citation_title_for_node(source_grid.grid_root))
...
def unsplit_grid(
    ...
    assert grid.points_cached.shape == (
        ...
        grid.ni + 1,
        3,
    ), "unsplit points have incorrect shape"
...
def add_faults(
    ...
    def fault_from_pillar_list(
        grid, full_pillar_list, delta_throw_left, delta_throw_right
    ):
        ...
        def pillar_vector(grid, p_index):
            ...
            if k_bot == k_top:  # following coded to treat None directions as downwards
                ...
                return vec.unit_vector(
                    grid.points_cached[k_bot, p_index]
        ...
        if not grid.has_split_coordinate_lines:
            grid.points_cached = grid.points_cached.reshape(
                (grid.nk_plus_k_gaps + 1, (grid.nj + 1) * (grid.ni + 1), 3)
            )
                ...
                for jp in range(2):
                    ...
                    for ip in range(2):
                        ...
                        if exist_lr is None:
                            ...
                            grid.points_cached[:, exist_p, :] += (
                                throw_right_vector if exist_lr else throw_left_vector
                            )
                            continue
                        ...
                        if (
                            exist_p != primary
                        ):  # remove one of the columns currently assigned to exist_p
                            ...
                            found = False
                            for cols_index in range(
                                start, start + grid.cols_for_split_pillars_cl[extra_p]
                            ):
                                if (
                        ...
                        if not new_p_made:  # create a new split of pillar
                            ...
                            grid.points_cached[:, -1, :] = original_p + (
                                throw_right_vector
                                if lr_foursome[jp, ip]
                                else throw_left_vector
                            )
                            grid.split_pillar_indices_cached = np_int_extended(
                                grid.split_pillar_indices_cached, primary
                            )
                            if grid.split_pillars_count is None:
                                grid.split_pillars_count = 0
                            grid.split_pillars_count += 1
                            grid.cols_for_split_pillars = np_int_extended(
                                grid.cols_for_split_pillars, natural_col
                            )
                            cl += 1
                            grid.cols_for_split_pillars_cl = np_int_extended(
                                grid.cols_for_split_pillars_cl, cl
                            )
                            new_p_made = True
    ...
    if full_pillar_list_dict is None:
        ...
        for filename in lines_file_list:
            ...
            face_set_dict, full_pll_dict = grid.make_face_sets_from_pillar_lists(
                pillar_list_list, face_set_id
            )
    ...
    for fault_key in full_pillar_list_dict:
        ...
        fault_from_pillar_list(
            grid, full_pillar_list, left_right_throw[0], left_right_throw[1]
        )
    ...
    if create_gcs and lines_file_list is not None:
        if new_epc_file is not None:
            ...
            grid = grr.Grid(
                model, grid_root=model.root(uuid=grid_uuid), find_properties=False
            )
        grid.set_face_set_gcs_list_from_dict(
            composite_face_set_dict, create_organizing_objects_where_needed=True
        )
...
def fault_throw_scaling(
    ...
    ), "cannot scale fault throws in unfaulted grid"
    assert scaling_factor is not None or (
        connection_set is not None and scaling_dict is not None
    )
    ...
    assert (
        grid.geometry_defined_for_all_pillars()
    ...
    if scaling_factor is not None:  # apply global scaling to throws
        ...
        semi_throws = np.zeros(
            grid.points_cached.shape[1:]
        )  # same throw applied to all layers
        unique_spi = np.unique(grid.split_pillar_indices_cached)
        semi_throws[unique_spi, :] = (
            grid.points_cached[ref_k0, unique_spi, :]
            - unsplit_points[ref_k0, unique_spi, :]
        )
        semi_throws[primaries:, :] = (
            grid.points_cached[ref_k0, primaries:, :]
    ...
    if (
        connection_set is not None and scaling_dict is not None
    ):  # overwrite any global offsets with named fault throw adjustments
        ...
        for fault_index in range(len(connection_set.feature_list)):
            ...
            if fault_scaling == 1.0:
                continue
            log.info(
                "scaling throw on fault "
                + fault_name
                + " by factor of: {0:.4f}".format(fault_scaling)
            )
            kelp_j, kelp_i = connection_set.simplified_sets_of_kelp_for_feature_index(
                fault_index
            )
            ...
            for kelp in kelp_j:
                for ip in [0, 1]:
                    ...
                    p_b = grid.pillars_for_column[
                        kelp[0] + 1, kelp[1], 0, ip
                    ]  # other side of fault
                    mid_point = 0.5 * (
                        grid.points_cached[ref_k0, p_a]
                        + grid.points_cached[ref_k0, p_b]
                    )
                    if np.any(np.isnan(mid_point)):
                        continue
                    if p_a not in p_list:
                        offsets[p_a] = (grid.points_cached[ref_k0, p_a] - mid_point) * (
                            fault_scaling - 1.0
                        )
                        p_list.append(p_a)
                    if p_b not in p_list:
                        offsets[p_b] = (grid.points_cached[ref_k0, p_b] - mid_point) * (
                            fault_scaling - 1.0
                        )
                        p_list.append(p_b)
            for kelp in kelp_i:
    ...
    while cell_range > 0:
        offset_decay = (maths.pow(2.0, cell_range) - 1.0) / (
            maths.pow(2.0, cell_range + 1) - 1.0
        )
    ...
    if new_grid_title is None or len(new_grid_title) == 0:
        new_grid_title = (
            "grid with fault throws scaled by "
            + str(scaling_factor)
            + " from "
            + str(rqet.citation_title_for_node(source_grid.grid_root))
        )
    ...
    gcs_list = []
    if inherit_gcs:
        gcs_roots = model.roots(
            obj_type="GridConnectionSetRepresentation", related_uuid=source_grid.uuid
        )
...
def global_fault_throw_scaling(
    epc_file,
    source_grid=None,
    scaling_factor=None,
    ref_k0=0,
...
def drape_to_surface(
    ...
    if grid.has_split_coordinate_lines:
        ...
        grid.points_cached[:, :jip1, :] += translate.reshape(
            (1, jip1, 3)
        )  # will be broadcast over k axis
        # adjust split pillars
        for p in range(grid.split_pillars_count):
            primary = grid.split_pillar_indices_cached[p]
            grid.points_cached[:, jip1 + p, :] += translate.reshape((1, jip1, 3))[
                0, primary, :
            ]  # will be broadcast over k axis
    else:
        grid.points_cached[:, :, :, :] += translate.reshape(
            (1, grid.points_cached.shape[1], grid.points_cached.shape[2], 3)
        )  # will be broadcast over k axis
...
def add_single_cell_grid(points, new_grid_title=None, new_epc_file=None):
    ...
    one_cell_model = rqi.import_nexus(
        new_epc_file[:-4],
        extent_ijk=(1, 1, 1),
        corp_file=temp_file,
        ijk_handedness="left",
...
def copy_grid(source_grid, target_model=None, copy_crs=True):
    ...
    if hasattr(source_grid, "array_cell_geometry_is_defined"):
        grid.array_cell_geometry_is_defined = (
            source_grid.array_cell_geometry_is_defined.copy()
        )
    grid.geometry_defined_for_all_cells_cached = (
        source_grid.geometry_defined_for_all_cells_cached
    )
...
def write_grid(
    ...
    assert (
        geometry or mode == "a"
    ), "for now, a copy of the grid must be included with property collection"
    assert (
        geometry or property_collection is not None
    ), "write_grid called without anything to write"
    ...
    if grid.inactive is not None and geometry:
        ...
        if inactive_count == grid.inactive.size:
            ...
            log.info(
                f"grid has {grid.inactive.size - inactive_count} active cells out of {grid.inactive.size}"
            )
    collection = property_collection
    if collection is not None:
        log.debug(
            "write_grid(): number of properties in collection: "
            + str(collection.number_of_parts())
        )
    ...
    if geometry or collection is not None:
        ...
        grid.write_hdf5_from_caches(
            hdf5_file, mode=mode, geometry=geometry, imported_properties=collection
        )
        model.h5_release()
    if ext_uuid is None:
        ext_uuid = model.h5_uuid()
    ...
    if geometry:
        ...
        ijk_node = grid.create_xml(
            ext_uuid, add_as_part=True, add_relationships=True, title=grid_title
        )
    ...
    log.debug(
        "write_grid(): number of finishing parts: " + str(model.number_of_parts())
    )
...
def add_edges_per_column_property_array(
    epc_file,
    a,
    property_kind,
    grid_uuid=None,
    ...
    assert (
        a.ndim == 4 and a.shape[2] == 2 and a.shape[3] == 2
    ), "Wrong shape! Expected shape (nj, ni, 2, 2)"
...
def gather_ensemble(
    ...
    composite_model = rq.Model(
        new_epc_file, new_epc=True, create_basics=True, create_hdf5_ext=True
    )
    ...
    for r, case_epc in enumerate(case_epc_list):
        ...
        if r == 0:  # first case
            ...
            composite_model.consolidation = (
                None  # discard any previous mappings to limit dictionary growth
            )
            if shared_time_series:
            ...
            if shared_grids:
                ...
                for grid_uuid in case_model.uuids(obj_type="IjkGridRepresentation"):
                    ...
                    if grid_extent in host_grid_shapes:
                        if title_match_required:
                            ...
                            for host_grid_index in len(host_grid_uuids):
                                if (
                                    grid_extent == host_grid_shapes[host_grid_index]
                                    and case_grid_title
                                    == host_grid_titles[host_grid_index]
                                ):
                    ...
                    ), "failed to match grids when gathering ensemble"
                    composite_model.force_consolidation_uuid_equivalence(
    ...
    if create_epc_lookup and len(epc_lookup_dict):
        epc_lookup = rqp.StringLookup(
            composite_model, int_to_str_dict=epc_lookup_dict, title="ensemble epc table"
        )
