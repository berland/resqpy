...
class Grid:
    ...
    def __init__(
        ...
        self.parent_window = (
            None  #: FineCoarse cell index mapping info between self and parent grid
        )
        ...
        self.grid_representation = (
            None  #: flavour of grid, currently 'IjkGrid' or 'IjkBlockGrid'
        )
        ...
        self.ni = (
            self.nj
        ) = self.nk = None  #: duplicated extent information as individual integers
        self.nk_plus_k_gaps = None  #: int: nk + k_gaps
        self.crs_uuid = (
            None  #: uuid of the coordinate reference system used by the grid's geometry
        )
        ...
        self.k_direction_is_down = (
            None  #: boolean indicating dominant direction of k increase
        )
        self.pillar_shape = (
            None  #: string: often 'curved' is used, even for straight pillars
        )
        self.has_split_coordinate_lines = (
            None  #: boolean; affects dimensionality of points array
        )
        ...
        self.xyz_box_cached = (
            None  #: numpy array of shape (2, 3) being (min max, x y z)
        )
        ...
        self.all_inactive = (
            None  #: numpy bool indicating whether all cells are inactive
        )
        ...
        if extract_basics_from_xml:
            ...
            self.nk = self.extent_kji[
                0
            ]  # for convenience available as individual attribs as well as np triplet
            self.nj = self.extent_kji[1]
            self.ni = self.extent_kji[2]
            self.nk_plus_k_gaps = (
                self.nk
            )  # temporarily, set properly by self.extract_k_gaps()
            self.geometry_root = rqet.find_tag(self.grid_root, "Geometry")
            if geometry_required:
                assert (
                    self.geometry_root is not None
                ), "grid geometry not present in xml"
            if self.geometry_root is None:
                ...
                self.k_direction_is_down = (
                    True  # arbitrary, as 'down' is rather meaningless without a crs
                )
            else:
    ...
    def set_modified(self, update_xml=False, update_hdf5=False):
        ...
        if update_xml:
            ...
            self.model.remove_part(
                rqet.part_name_for_object("obj_IjkGridRepresentation", old_uuid)
            )
        if update_hdf5:
        ...
        if update_xml and update_hdf5:
            self.model.change_uuid_in_hdf5_references(
                self.grid_root, old_uuid, self.uuid
            )
    ...
    def extract_extent_kji(self):
        ...
        self.extent_kji = np.ones(
            3, dtype="int"
        )  # todo: handle other varieties of grid
    ...
    def cell_count(
        self, active_only=False, non_pinched_out_only=False, geometry_defined_only=False
    ):
    ...
    def natural_cell_indices(self, cell_kji0s):
        ...
        return (
            cell_kji0s[..., 0] * self.nj + cell_kji0s[..., 1]
        ) * self.ni + cell_kji0s[..., 2]
    ...
    def extract_has_split_coordinate_lines(self):
        ...
        if split_node is not None:
            self.split_pillars_count = int(
                rqet.find_tag(split_node, "Count").text.strip()
            )
        return self.has_split_coordinate_lines
    ...
    def extract_k_gaps(self):
        ...
        if self.k_gaps:
            self.nk_plus_k_gaps = self.nk + self.k_gaps
            k_gap_after_root = rqet.find_nested_tags(
                self.grid_root, ["KGaps", "GapAfterLayer"]
            )
            assert k_gap_after_root is not None
            bool_array_type = rqet.node_type(k_gap_after_root)
            assert (
                bool_array_type == "BooleanHdf5Array"
            )  # could be a constant array but not handled by this code
            h5_key_pair = self.model.h5_uuid_and_path_for_node(k_gap_after_root)
            assert h5_key_pair is not None
            self.model.h5_array_element(
                h5_key_pair,
                index=None,
                cache_array=True,
                object=self,
            ...
            assert (
                self.k_gap_after_array.ndim == 1
                and self.k_gap_after_array.size == self.nk - 1
            )
    ...
    def extract_parent(self):
        ...
        self.parent_window = (
            None  # FineCoarse cell index mapping info with respect to parent
        )
        ...
        self.parent_grid_uuid = bu.uuid_from_string(
            rqet.find_nested_tags_text(pw_node, ["ParentGrid", "UUID"])
        )
        ...
        refining_flag = (
            None  # gets set True if local grid is a refinement, False if a coarsening
        )
        ...
        for axis in range(3):
            ...
            if intervals_node is None:  # implicit one-to-one mapping
                ...
                parent_count_list_list.append(
                    np.array(self.extent_kji[axis], dtype=int)
                )
                ...
                assert (
                    intervals_info.parent_count_per_interval.ndim == 1
                    and intervals_info.parent_count_per_interval.size == intervals_count
                )
                parent_box[1, axis] = (
                    parent_box[0, axis]
                    + np.sum(intervals_info.parent_count_per_interval)
                    - 1
                )
                assert parent_box[1, axis] < parent_grid_extent_kji[axis]
                ...
                assert (
                    intervals_info.child_count_per_interval.ndim == 1
                    and intervals_info.child_count_per_interval.size == intervals_count
                )
                assert (
                    np.sum(intervals_info.child_count_per_interval)
                    == self.extent_kji[axis]
                )  # assumes both local and parent grids are IjkGrids
                for interval in range(intervals_count):
                    if (
                        intervals_info.child_count_per_interval[interval]
                ...
                if ccw_node is None:
                    ...
                    assert (
                        intervals_info.child_cell_weights.ndim == 1
                        and intervals_info.child_cell_weights.size
                        == self.extent_kji[axis]
                    )
        ...
        if omit_node is not None:
            log.warning(
                "unable to handle parent cell omissions in local grid definition – ignoring"
            )
        # todo: handle omissions

        if refining_flag is None:
            log.warning(
                "local grid has no refinement nor coarsening – treating as a refined grid"
            )
            refining_flag = True
        self.is_refinement = refining_flag

        if refining_flag:  # local grid is a refinement
            self.parent_window = fc.FineCoarse(
                self.extent_kji,
            ...
            for axis in range(3):
                if intervals_count_list[axis] == 1:
                    ...
                    constant_ratio = self.extent_kji[axis] // (
                        parent_box[1, axis] - parent_box[0, axis] + 1
                    )
                    ratio_vector = None
                else:
                    constant_ratio = None
                    ratio_vector = (
                        child_count_list_list[axis] // parent_count_list_list[axis]
                    )
                ...
                if child_weight_list_list[axis] is None:
                    ...
                    proportions_list = []
                    place = 0
                    for coarse_slice in range(
                        parent_box[1, axis] - parent_box[0, axis] + 1
                    ):
                        if ratio_vector is None:
                            proportions_list.append(
                                np.array(
                                    child_weight_list_list[axis][
                                        place : place + constant_ratio
                                    ]
                                )
                            )
                            ...
                            proportions_list.append(
                                np.array(
                                    child_weight_list_list[axis][
                                        place : place + ratio_vector[coarse_slice]
                                    ]
                                )
                            )
                            place += ratio_vector[coarse_slice]
                    self.parent_window.set_proportions_list_of_vectors(
                        axis, proportions_list
                    )
            ...
            for axis in range(3):
                if intervals_count_list[axis] == 1:
                    ...
                    constant_ratio = None
                    ratio_vector = (
                        parent_count_list_list[axis] // child_count_list_list[axis]
                    )
                    self.parent_window.set_ratio_vector(axis, ratio_vector)
                if child_weight_list_list[axis] is None:
                    self.parent_window.set_equal_proportions(axis)
    ...
    def extract_children(self):
        ...
        related_grid_roots = self.model.roots(
            obj_type="IjkGridRepresentation", related_uuid=self.uuid
        )
        if related_grid_roots is not None:
            for related_root in related_grid_roots:
                parent_uuid = rqet.find_nested_tags_text(
                    related_root, ["ParentWindow", "ParentGrid", "UUID"]
                )
    ...
    def extract_inactive_mask(self, check_pinchout=False):
        ...
        active_gpc.inherit_parts_selectively_from_other_collection(
            other=gpc, property_kind="active", continuous=False, categorical=False
        )
        if active_gpc.number_of_parts() > 0:
            if active_gpc.number_of_parts() > 1:
                log.warning(
                    'more than one property found with bespoke kind "active", using last encountered'
                )
            ...
            inactive_gpc.inherit_parts_selectively_from_other_collection(
                other=gpc, property_kind="code", facet_type="what", facet="inactive"
            )
            if inactive_gpc.number_of_parts() == 1:
                inactive_part = inactive_gpc.parts()[0]
                inactive_array = inactive_gpc.cached_part_array_ref(
                    inactive_part, dtype="bool"
                )
    ...
    def cell_geometry_is_defined(
        self, cell_kji0=None, cell_geometry_is_defined_root=None, cache_array=True
    ):
        ...
        if self.geometry_defined_for_all_cells_cached:
            return True
        if (
            hasattr(self, "array_cell_geometry_is_defined")
            and self.array_cell_geometry_is_defined is None
        ):
            delattr(self, "array_cell_geometry_is_defined")
        if hasattr(self, "array_cell_geometry_is_defined"):
            self.geometry_defined_for_all_cells_cached = np.all(
        ...
        is_def_root = self.resolve_geometry_child(
            "CellGeometryIsDefined", child_node=cell_geometry_is_defined_root
        )
        ...
        if is_def_type == "BooleanConstantArray":
            self.geometry_defined_for_all_cells_cached = (
                rqet.find_tag_text(is_def_root, "Value").lower() == "true"
            )
            return self.geometry_defined_for_all_cells_cached
        else:
            ...
            if h5_key_pair is None:
                return None
            result = self.model.h5_array_element(
                h5_key_pair,
                index=cell_kji0,
            ...
            if (
                self.geometry_defined_for_all_cells_cached is None
                and cache_array
                and hasattr(self, "array_cell_geometry_is_defined")
            ):
                self.geometry_defined_for_all_cells_cached = (
                    np.count_nonzero(self.array_cell_geometry_is_defined)
    ...
    def pillar_geometry_is_defined(
        self, pillar_ji0=None, pillar_geometry_is_defined_root=None, cache_array=True
    ):
        ...
        is_def_root = self.resolve_geometry_child(
            "PillarGeometryIsDefined", child_node=pillar_geometry_is_defined_root
        )
        if is_def_root is None:
        ...
        if is_def_type == "BooleanConstantArray":
            ...
            if (
                self.geometry_defined_for_all_pillars_cached is None
                and cache_array
                and hasattr(self, "array_pillar_geometry_is_defined")
            ):
                self.geometry_defined_for_all_pillars_cached = (
                    np.count_nonzero(self.array_pillar_geometry_is_defined)
            ...
            for k0 in range(self.nk):
                for j0 in range(self.nj):
                    for i0 in range(self.ni):
                        if not self.cell_geometry_is_defined(
                            cell_kji0=(k0, j0, i0),
    ...
    def geometry_defined_for_all_pillars(
        self, cache_array=True, pillar_geometry_is_defined_root=None
    ):
    ...
    def set_geometry_is_defined(
        ...
        def infill_partial_pillar(grid, pillar_index):
            points = grid.points_ref(masked=False).reshape(
                (grid.nk_plus_k_gaps + 1, -1, 3)
            )
            ...
            while True:
                ...
                while nan_mask[scan_k]:
                    scan_k += 1
                points[first_k - 1 : scan_k, pillar_index] = np.linspace(
                    points[first_k - 1, pillar_index],
                    points[scan_k, pillar_index],
                    num=scan_k - first_k + 1,
                    endpoint=False,
                )
                first_k = scan_k
        ...
        def fill_holes(grid, holes_mask):
            ...
            while np.any(holes_mask):
                ...
                neighbours[:, 1:] += mask_01[:, :-1]
                foursomes = np.where(
                    np.logical_and(flat_holes_mask, neighbours.flatten() == 4)
                )[0]
                if len(foursomes) > 0:
                    interpolated = 0.25 * (
                        points[:, foursomes - 1, :]
                        + points[:, foursomes + 1, :]
                        + points[:, foursomes - ni_plus_1, :]
                        + points[:, foursomes + ni_plus_1, :]
                    )
                ...
                neighbours[1:, :] += mask_01[:-1, :]
                twosomes = np.where(
                    np.logical_and(flat_holes_mask, neighbours.flatten() == 2)
                )[0]
                if len(twosomes) > 0:
                    interpolated = 0.5 * (
                        points[:, twosomes - ni_plus_1, :]
                        + points[:, twosomes + ni_plus_1, :]
                    )
                ...
                if len(twosomes) > 0:
                    interpolated = 0.5 * (
                        points[:, twosomes - 1, :] + points[:, twosomes + 1, :]
                    )
                    points[:, twosomes, :] = interpolated
                    flat_holes_mask[twosomes] = False
                    modified = True
                ...
                pushable = np.where(
                    np.logical_and(flat_holes_mask, neighbours.flatten() == 3)
                )[0]
                if len(corners) > 0:
                    interpolated = 0.5 * (
                        points[:, corners - ni_plus_1, :] + points[:, corners - 1, :]
                    )
                    points[:, corners, :] = interpolated
                    pushed = (
                        2.0 * points[:, pushable, :]
                        - points[:, pushable - ni_plus_1 - 1, :]
                    )
        ...
        def fill_surround(grid, surround_mask):
            ...
            points_view = (
                points[-1, :, :2]
                .reshape((-1, 2))[: (grid.nj + 1) * (grid.ni + 1), :]
                .reshape((grid.nj + 1, grid.ni + 1, 2))
            )
            modified = False
            if grid.nj > 1:
            ...
            if grid.ni > 1:
                ...
                while i < grid.ni - 1:
                    ...
                    i += 1
            if modified:
                points.reshape((grid.nk_plus_k_gaps + 1, -1, 3))[
                    :-1, : (grid.nj + 1) * (grid.ni + 1), :2
                ][:, surround_mask.flatten(), :] = points_view[
                    surround_mask, :
                ].reshape(
                    (1, -1, 2)
                )
        ...
        if treat_as_nan is not None:
            nan_mask = np.any(
                np.logical_or(np.isnan(points), points == treat_as_nan), axis=-1
            )
        else:
        ...
        if some_areal_dots:
            ...
            if self.k_gaps:
                dot_mask = np.zeros(
                    (self.nk_plus_k_gaps + 1, self.nj + 1, self.ni + 1), dtype=bool
                )
                ...
                dot_mask[1:, :-1, :-1] = np.logical_or(
                    dot_mask[:-1, :-1, :-1], areal_dots
                )
            dot_mask[:, 1:, :-1] = np.logical_or(
                dot_mask[:, :-1, :-1], dot_mask[:, 1:, :-1]
            )
            ...
            if self.has_split_coordinate_lines:
                ...
                nan_mask[:, :primaries] = np.logical_or(
                    nan_mask[:, :primaries], dot_mask.reshape((-1, primaries))
                )
            else:
        ...
        points[:] = np.where(
            np.repeat(np.expand_dims(nan_mask, axis=nan_mask.ndim), 3, axis=-1),
            np.NaN,
            points,
        )
        ...
        if np.all(pillar_defined_mask):
            ...
            self.geometry_defined_for_all_pillars_cached = False
            self.array_pillar_geometry_is_defined = pillar_defined_mask[
                :primary_count
            ].reshape((self.nj + 1, self.ni + 1))
        if pillar_defined_mask.size > primary_count and not np.all(
            pillar_defined_mask[primary_count:]
        ):
            log.warning("at least one split pillar has geometry undefined")

        self.geometry_defined_for_all_cells_cached = False

        primary_nan_mask = nan_mask.reshape((self.nk_plus_k_gaps + 1, -1))[
            :, :primary_count
        ].reshape((self.nk_plus_k_gaps + 1, self.nj + 1, self.ni + 1))
        column_nan_mask = np.logical_or(
        ...
        if self.k_gaps:
            self.array_cell_geometry_is_defined = np.logical_not(
                np.logical_or(
                    column_nan_mask[self.k_raw_index_array],
        ...
        if nullify_partial_pillars:
            partial_pillar_mask = np.logical_and(
                pillar_defined_mask, np.any(nan_mask, axis=0).flatten()
            )
            if np.any(partial_pillar_mask):
                points.reshape((self.nk_plus_k_gaps + 1, -1, 3))[
                    :, partial_pillar_mask, :
                ] = np.NaN
        ...
        if complete_all:
            ...
            top_nan_mask = np.isnan(
                points[0, ..., 0]
                .flatten()[: (self.nj + 1) * (self.ni + 1)]
                .reshape((self.nj + 1, self.ni + 1))
            )
        ...
        if cells_update_needed:
            ...
            if self.geometry_defined_for_all_pillars_cached:
                ...
                self.array_cell_geometry_is_defined = np.repeat(
                    np.expand_dims(column_nan_mask, 0), self.nk, axis=0
                )
                self.geometry_defined_for_all_cells_cached = np.all(
                    self.array_cell_geometry_is_defined
                )
    ...
    def actual_pillar_shape(self, patch_metadata=False, tolerance=0.001):
        ...
        pillar_shape = gf.actual_pillar_shape(
            self.points_ref(masked=False), tolerance=tolerance
        )
        if patch_metadata:
    ...
    def cache_all_geometry_arrays(self):
        ...
        if self.has_split_coordinate_lines:
            ...
            if not hasattr(self, "cols_for_split_pillars"):
                ...
                h5_key_pair = self.model.h5_uuid_and_path_for_node(
                    cpscl_cum_length_root
                )
                self.model.h5_array_element(
                    h5_key_pair,
                    index=None,
    ...
    def create_column_pillar_mapping(self):
        ...
        for j in range(self.nj):
            self.pillars_for_column[j, :, 0, 0] = np.arange(
                j * ni_plus_1, (j + 1) * ni_plus_1 - 1, dtype=int
            )
            self.pillars_for_column[j, :, 0, 1] = np.arange(
                j * ni_plus_1 + 1, (j + 1) * ni_plus_1, dtype=int
            )
            self.pillars_for_column[j, :, 1, 0] = np.arange(
                (j + 1) * ni_plus_1, (j + 2) * ni_plus_1 - 1, dtype=int
            )
            self.pillars_for_column[j, :, 1, 1] = np.arange(
                (j + 1) * ni_plus_1 + 1, (j + 2) * ni_plus_1, dtype=int
            )
        ...
        if self.has_split_coordinate_lines:
            ...
            for extra_index in range(extras_count):
                ...
                for cpscl_index in range(
                    start, self.cols_for_split_pillars_cl[extra_index]
                ):
    ...
    def pillar_foursome(self, ji0, none_if_unsplit=False):
        ...
        for cpscl_index in extras:
            ...
            for csp_index in range(
                start_index, self.cols_for_split_pillars_cl[cpscl_index]
            ):
    ...
    def split_column_faces(self):
        ...
        if not self.has_split_coordinate_lines:
            return None, None
        if (
            hasattr(self, "array_j_column_face_split")
            and self.array_j_column_face_split is not None
            and hasattr(self, "array_i_column_face_split")
            and self.array_i_column_face_split is not None
        ):
            return self.array_j_column_face_split, self.array_i_column_face_split
        if self.nj == 1:
            self.array_j_column_face_split = None
        else:
            self.array_j_column_face_split = np.zeros(
                (self.nj - 1, self.ni), dtype=bool
            )  # NB. internal faces only, index for +ve face
        if self.ni == 1:
            self.array_i_column_face_split = None
        else:
            self.array_i_column_face_split = np.zeros(
                (self.nj, self.ni - 1), dtype=bool
            )  # NB. internal faces only, index for +ve face
    ...
    def find_faults(
        self, set_face_sets=False, create_organizing_objects_where_needed=False
    ):
        ...
        if (
            hasattr(self, "fault_dict")
            and self.fault_dict is not None
            and len(self.fault_dict.keys()) > 0
        ):
            if set_face_sets:
                for f, (j_list, i_list) in self.fault_dict.items():
                    self.face_set_dict[f] = (j_list, i_list, "K")
                self.set_face_set_gcs_list_from_dict(
                    self.fault_dict, create_organizing_objects_where_needed
                )
            return None
        ...
        kelp_j = np.zeros(
            (self.extent_kji[1] - 1, self.extent_kji[2]), dtype="int"
        )  # fault id between cols j, j+1
        kelp_i = np.zeros(
            (self.extent_kji[1], self.extent_kji[2] - 1), dtype="int"
        )  # fault id between cols i, i+1
        ...
        for j in range(self.extent_kji[1] - 1):
            for i in range(self.extent_kji[2]):
                if i == 0 and (
                    ...
                    != self.pillars_for_column[j + 1, i, 0, 0]
                    or self.pillars_for_column[j, i, 1, 1]
                    != self.pillars_for_column[j + 1, i, 0, 1]
                ):
                    last_fault_id += 1
        ...
        for i in range(self.extent_kji[2] - 1):
            for j in range(self.extent_kji[1]):
                if j == 0 and (
                    ...
                    != self.pillars_for_column[j, i + 1, 0, 0]
                    or self.pillars_for_column[j, i, 1, 1]
                    != self.pillars_for_column[j, i + 1, 1, 0]
                ):
                    last_fault_id += 1
        ...
        if kelp_j.size and kelp_i.size:
            for j in range(self.extent_kji[1] - 1):
                for i in range(self.extent_kji[2] - 1):
                    if (bool(kelp_j[j, i]) != bool(kelp_j[j, i + 1])) and (
                        bool(kelp_i[j, i]) != bool(kelp_i[j + 1, i])
                    ):
        ...
        fault_id_list = np.unique(
            np.concatenate((np.unique(kelp_i.flatten()), np.unique(kelp_j.flatten())))
        )[
            1:
        ]  # discard zero from list
        ...
        for fault_id in fault_id_list:
            self.fault_dict[fault_id] = (
                np.stack(np.where(kelp_j == fault_id), axis=1),
                np.stack(np.where(kelp_i == fault_id), axis=1),
            )
        self.fault_id_j = (
            kelp_j.copy()
        )  # fault_id for each internal j kelp, zero is none; extent nj-1, ni
        self.fault_id_i = (
            kelp_i.copy()
        )  # fault_id for each internal i kelp, zero is none; extent nj, ni-1
        if set_face_sets:
            for f, (j_list, i_list) in self.fault_dict.items():
                self.face_set_dict[f] = (j_list, i_list, "K")
            self.set_face_set_gcs_list_from_dict(
    ...
    def fault_throws(self):
        ...
        if (
            hasattr(self, "fault_throw_j")
            and self.fault_throw_j is not None
            and hasattr(self, "fault_throw_i")
            and self.fault_throw_i is not None
        ):
            return (self.fault_throw_j, self.fault_throw_i)
        if not self.has_split_coordinate_lines:
            return None
        if (
            not hasattr(self, "fault_id_j")
            or self.fault_id_j is None
            or not hasattr(self, "fault_id_i")
            or self.fault_id_i is None
        ):
            self.find_faults()
        ...
        self.fault_throw_j = np.where(
            ...
            0.0,
            0.25
            * np.sum(cp[:, 1:, :, :, 0, :, 2] - cp[:, :-1, :, :, 1, :, 2], axis=(3, 4)),
        )
    ...
    def fault_throws_per_edge_per_column(
        self, mode="maximum", simple_z=False, axis_polarity_mode=True
    ):
        ...
        log.debug(
            "computing fault throws per edge per column based on corner point geometry"
        )
        if (
            not self.has_split_coordinate_lines
        ):  # note: no NaNs returned in this situation
        ...
        i_pillar_throws = (
            ...
            - self.points_cached[  # (nk+1, nj, ni-1, jp) +ve dz I- cell > I+ cell
                :, self.pillars_for_column[:, 1:, :, 0], 2
            ]
        )
        j_pillar_throws = (
            self.points_cached[:, self.pillars_for_column[:-1, :, 1, :], 2]
        ...
        if not simple_z:
            i_pillar_throws = np.sign(
                i_pillar_throws
            )  # note: will return zero if displacement is purely horizontal wrt. z axis
            j_pillar_throws = np.sign(j_pillar_throws)
            i_pillar_throws *= vec.naive_lengths(
                self.points_cached[:, self.pillars_for_column[:, :-1, :, 1], :]
                - self.points_cached[:, self.pillars_for_column[:, 1:, :, 0], :]
            )
            j_pillar_throws *= vec.naive_lengths(
                self.points_cached[:, self.pillars_for_column[:-1, :, 1, :], :]
                - self.points_cached[:, self.pillars_for_column[1:, :, 0, :], :]
            )
        ...
        if mode == "mean":
            ...
            if mode == "maximum":
                i_edge_throws = np.where(
                    i_flip_mask, min_i_edge_throws, max_i_edge_throws
                )
                j_edge_throws = np.where(
                    j_flip_mask, min_j_edge_throws, max_j_edge_throws
                )
            elif mode == "minimum":
                i_edge_throws = np.where(
                    i_flip_mask, max_i_edge_throws, min_i_edge_throws
                )
                j_edge_throws = np.where(
                    j_flip_mask, max_j_edge_throws, min_j_edge_throws
                )
            else:
        ...
        if axis_polarity_mode:
            throws = np.zeros(
                (self.nj, self.ni, 2, 2)
            )  # first 2 is I (0) or J (1); final 2 is -ve or +ve face
    ...
    def set_face_set_gcs_list_from_dict(
        self, face_set_dict=None, create_organizing_objects_where_needed=False
    ):
    ...
    def make_face_set_from_dataframe(self, df):
        ...
        for fs_name in names:
            ...
            for row in range(len(fs_ds)):
                ...
                if not k_warning_given and (
                    box_kji0[0, 0] != 0 or box_kji0[1, 0] != self.extent_kji[0] - 1
                ):
                    log.warning(
                ...
                if (
                    len(face) > 1 and face[1] == "-"
                ):  # treat negative faces as positive faces of neighbouring cell
                    box_kji0[0, axis] = max(box_kji0[0, axis] - 1, 0)
                    box_kji0[1, axis] -= 1
                else:
                    box_kji0[1, axis] = min(
                        box_kji0[1, axis], self.extent_kji[axis] - 2
                    )
                ...
                for j in range(box_kji0[0, 1], box_kji0[1, 1] + 1):
                    for i in range(box_kji0[0, 2], box_kji0[1, 2] + 1):
                        if axis == 1:
                            _add_to_kelp_list(
                                self.extent_kji, j_kelp_list, True, (j, i)
                            )
                        elif axis == 2:
                            _add_to_kelp_list(
                                self.extent_kji, i_kelp_list, False, (j, i)
                            )
            self.face_set_dict[fs_name] = (j_kelp_list, i_kelp_list, "K")
            count += 1
        log.info(str(count) + " face sets extracted from dataframe")

    def make_face_sets_from_pillar_lists(
        self,
        pillar_list_list,
        face_set_id,
        axis="K",
        ...
        if axis.upper() == "K":
            ...
            pillar_xy = self.horizon_points(
                ref_k0=ref_slice0, kp=1 if plus_face else 0
            )[:, :, 0:2]
            kelp_axes = "JI"
        else:
            if projection == "xz":
                pillar_xy = self.unsplit_x_section_points(
                    axis, ref_slice0=ref_slice0, plus_face=plus_face
                )[
                    :, :, 0:3:2
                ]  # x,z
            else:  # projection == 'yz'
        ...
        for pillar_list in pillar_list_list:
            ...
            for p in range(len(pillar_list) - 1):
                ...
                while np.any(here != ji_1):
                    ...
                    if abs_dj >= abs_di:
                        ...
                        if di != 0:
                            divergence = vec.point_distance_to_line_2d(
                                pillar_xy[tuple(here)], xy_0, xy_1
                            )
                            side_step[:] = here
                            side_step[1] += i_sign
                            if (
                                side_step[1] >= 0
                                and side_step[1] <= self.extent_kji[kelp_axes_int[1]]
                            ):
                                stepped_divergence = vec.point_distance_to_line_2d(
                                    pillar_xy[tuple(side_step)], xy_0, xy_1
                                )
                        ...
                        if dj != 0:
                            ...
                            if (
                                side_step[0] >= 0
                                and side_step[0] <= self.extent_kji[kelp_axes_int[0]]
                            ):
                                stepped_divergence = vec.point_distance_to_line_2d(
                                    pillar_xy[tuple(side_step)], xy_0, xy_1
                                )
            ...
            self.face_set_dict[face_set_id + id_suffix] = (
                j_kelp_list,
                i_kelp_list,
                axis,
            )
            local_face_set_dict[face_set_id + id_suffix] = (
                j_kelp_list,
                i_kelp_list,
                axis,
            )
    ...
    def check_top_and_base_cell_edge_directions(self):
        ...
        if self.has_split_coordinate_lines:
            ...
            top_j_edge_vectors_p = np.zeros(
                (self.nj, self.ni, 2, 2)
            )  # third axis is ip
            top_i_edge_vectors_p = np.zeros(
                (self.nj, 2, self.ni, 2)
            )  # second axis is jp
            base_j_edge_vectors_p = np.zeros(
                (self.nj, self.ni, 2, 2)
            )  # third axis is ip
            base_i_edge_vectors_p = np.zeros(
                (self.nj, 2, self.ni, 2)
            )  # second axis is jp
                ...
                for i in range(self.ni):
                    for jip in range(2):  # serves as either jp or ip
                        top_j_edge_vectors_p[j, i, jip, :] = (
                            self.points_cached[
                                0, self.pillars_for_column[j, i, 1, jip], :2
                            ]
                            - self.points_cached[
                                0, self.pillars_for_column[j, i, 0, jip], :2
                            ]
                        )
                        base_j_edge_vectors_p[j, i, jip, :] = (
                            self.points_cached[
                                self.nk_plus_k_gaps,
                                self.pillars_for_column[j, i, 1, jip],
                                :2,
                            ]
                            - self.points_cached[
                                self.nk_plus_k_gaps,
                                self.pillars_for_column[j, i, 0, jip],
                                :2,
                            ]
                        )
                        top_i_edge_vectors_p[j, jip, i, :] = (
                            self.points_cached[
                                0, self.pillars_for_column[j, i, jip, 1], :2
                            ]
                            - self.points_cached[
                                0, self.pillars_for_column[j, i, jip, 0], :2
                            ]
                        )
                        base_i_edge_vectors_p[j, jip, i, :] = (
                            self.points_cached[
                                self.nk_plus_k_gaps,
                                self.pillars_for_column[j, i, jip, 1],
                                :2,
                            ]
                            - self.points_cached[
                                self.nk_plus_k_gaps,
                                self.pillars_for_column[j, i, jip, 0],
                                :2,
                            ]
                        )
            ...
            base_j_edge_vectors = base_j_edge_vectors_p.reshape(
                (self.nj, 2 * self.ni, 2)
            )
            base_i_edge_vectors = base_i_edge_vectors_p.reshape(
                (2 * self.nj, self.ni, 2)
            )
        else:
            top_j_edge_vectors = (
                self.points_cached[0, 1:, :, :2] - self.points_cached[0, :-1, :, :2]
            ).reshape((self.nj, self.ni + 1, 2))
            top_i_edge_vectors = (
                self.points_cached[0, :, 1:, :2] - self.points_cached[0, :, :-1, :2]
            ).reshape((self.nj + 1, self.ni, 2))
            base_j_edge_vectors = (
                self.points_cached[-1, 1:, :, :2] - self.points_cached[-1, :-1, :, :2]
            ).reshape((self.nj, self.ni + 1, 2))
            base_i_edge_vectors = (
                self.points_cached[-1, :, 1:, :2] - self.points_cached[-1, :, :-1, :2]
            ).reshape((self.nj + 1, self.ni, 2))
        ...
        if not np.all(dot_j >= 0.0) and np.all(dot_i >= 0.0):
            log.warning(
                "one or more columns of cell edges flip direction: this grid is probably unusable"
            )
            good = False
        log.debug(
            "checking relative direction of edges in neighbouring cells at top of grid (and base)"
        )
        # check direction of similar edges on neighbouring cells, tolerate upto 90 degree difference
        dot_jp = np.sum(
            top_j_edge_vectors[1:, :, :] * top_j_edge_vectors[:-1, :, :], axis=2
        )
        dot_ip = np.sum(
            top_i_edge_vectors[1:, :, :] * top_i_edge_vectors[:-1, :, :], axis=2
        )
        if not np.all(dot_jp >= 0.0) and np.all(dot_ip >= 0.0):
            ...
            good = False
        dot_jp = np.sum(
            base_j_edge_vectors[1:, :, :] * base_j_edge_vectors[:-1, :, :], axis=2
        )
        dot_ip = np.sum(
            base_i_edge_vectors[1:, :, :] * base_i_edge_vectors[:-1, :, :], axis=2
        )
        if not np.all(dot_jp >= 0.0) and np.all(dot_ip >= 0.0):
            log.warning(
    ...
    def point_raw(self, index=None, points_root=None, cache_array=True):
        ...
        if index is not None and not self.geometry_defined_for_all_pillars(
            cache_array=cache_array
        ):
            if len(index) == 3:
                ji = tuple(index[1:])
            ...
            if ji[0] < self.nj and not self.pillar_geometry_is_defined(
                ji, cache_array=cache_array
            ):
                return None
        ...
        try:
            value = self.model.h5_array_element(
                h5_key_pair,
                index=index,
                cache_array=cache_array,
                object=self,
    ...
    def point(
        ...
        if self.has_split_coordinate_lines:
            self.create_column_pillar_mapping()
            pillar_index = self.pillars_for_column[
                index[1], index[2], corner_index[1], corner_index[2]
            ]
            return self.point_raw(
                index=(index[0] + corner_index[0], pillar_index),
                points_root=p_root,
                cache_array=cache_array,
            )
        else:
            index[:] += corner_index
            return self.point_raw(
                index=index, points_root=p_root, cache_array=cache_array
            )
    ...
    def unsplit_points_ref(self, cache_array=False, masked=False):
        ...
        for j in range(1, self.nj):
            result[:, j, 0, :] = 0.5 * (
                points[:, self.pillars_for_column[j - 1, 0, 1, 0], :]
                + points[:, self.pillars_for_column[j, 0, 0, 0], :]
            )
            result[:, j, self.ni, :] = 0.5 * (
                points[:, self.pillars_for_column[j - 1, self.ni - 1, 1, 1], :]
                + points[:, self.pillars_for_column[j, self.ni - 1, 0, 1], :]
            )
        for i in range(1, self.ni):
            result[:, 0, i, :] = 0.5 * (
                points[:, self.pillars_for_column[0, i - 1, 0, 1], :]
                + points[:, self.pillars_for_column[0, i, 0, 0], :]
            )
            result[:, self.nj, i, :] = 0.5 * (
                points[:, self.pillars_for_column[self.nj - 1, i - 1, 1, 1], :]
        ...
        result[:, 0, self.ni, :] = points[
            :, self.pillars_for_column[0, self.ni - 1, 0, 1], :
        ]
        result[:, self.nj, 0, :] = points[
            :, self.pillars_for_column[self.nj - 1, 0, 1, 0], :
        ]
        result[:, self.nj, self.ni, :] = points[
            :, self.pillars_for_column[self.nj - 1, self.ni - 1, 1, 1], :
        ]
        if cache_array:
    ...
    def xyz_box(self, points_root=None, lazy=True, local=False):
        ...
        if self.xyz_box_cached is None or (
            not lazy and not self.xyz_box_cached_thoroughly
        ):
            ...
            if lazy:
                ...
                for kp in [0, 1]:
                    for jp in [0, 1]:
                        for ip in [0, 1]:
                            eight_corners[kp, jp, ip] = self.point(
                                cell_kji0=[
                                    kp * (self.extent_kji[0] - 1),
    ...
    def xyz_box_centre(self, points_root=None, lazy=False, local=False):
        ...
        return np.nanmean(
            self.xyz_box(points_root=points_root, lazy=lazy, local=local), axis=0
        )
    ...
    def horizon_points(self, ref_k0=0, heal_faults=False, kp=0):
        ...
        if self.has_split_coordinate_lines:
            if heal_faults:
                points = (
                    self.unsplit_points_ref()
                )  # expensive operation: would be better to cache the unsplit points
    ...
    def split_horizon_points(self, ref_k0=0, masked=False, kp=0):
        ...
        if self.has_split_coordinate_lines:
            ...
            self.create_column_pillar_mapping()
            assert (
                self.pillars_for_column.ndim == 4
                and self.pillars_for_column.shape == (self.nj, self.ni, 2, 2)
            )
            for j in range(self.nj):
                for i in range(self.ni):
                    hp[j, i, 0, 0, :] = points[
                        ref_k0, self.pillars_for_column[j, i, 0, 0], :
                    ]
                    hp[j, i, 1, 0, :] = points[
                        ref_k0, self.pillars_for_column[j, i, 1, 0], :
                    ]
                    hp[j, i, 0, 1, :] = points[
                        ref_k0, self.pillars_for_column[j, i, 0, 1], :
                    ]
                    hp[j, i, 1, 1, :] = points[
                        ref_k0, self.pillars_for_column[j, i, 1, 1], :
                    ]
        else:
    ...
    def split_x_section_points(self, axis, ref_slice0=0, plus_face=False, masked=False):
        ...
        log.debug(
            f"x-sect: axis {axis}; ref_slice0 {ref_slice0}; plus_face {plus_face}; masked {masked}"
        )
        assert axis.upper() in ["I", "J"]
        assert (
            not self.k_gaps
        ), "split_x_section_points() method is for grids without k gaps; use split_gap_x_section_points()"
    ...
    def split_gap_x_section_points(
        self, axis, ref_slice0=0, plus_face=False, masked=False
    ):
        ...
        log.debug(
            f"k gap x-sect: axis {axis}; ref_slice0 {ref_slice0}; plus_face {plus_face}; masked {masked}"
        )
        assert axis.upper() in ["I", "J"]
        ...
        if self.has_split_coordinate_lines:
            ...
            points = self.points_ref(masked=masked).reshape(
                (self.nk_plus_k_gaps, (self.nj + 1) * (self.ni + 1), 3)
            )
        cpm = self.create_column_pillar_mapping()
        ...
        if self.k_gaps:
            ...
            p = self.split_x_section_points(
                axis, ref_slice0=ref_slice0, plus_face=plus_face, masked=masked
            )
            top = p[:-1]
            base = p[1:]
        return np.stack((top, base), axis=2)

    def unsplit_x_section_points(
        self, axis, ref_slice0=0, plus_face=False, masked=False
    ):
        ...
        assert (
            not self.has_split_coordinate_lines
        ), "cross sectional points for unsplit grids require split_x_section_points()"
        assert (
            not self.k_gaps
        ), "cross sectional points with k gaps require split_gap_x_section_points()"
    ...
    def x_section_points(self, axis, ref_slice0=0, plus_face=False, masked=False):
        ...
        return self.unsplit_x_section_points(
            axis, ref_slice0=ref_slice0, plus_face=plus_face, masked=masked
        )

    def x_section_corner_points(
        self,
        axis,
        ref_slice0=0,
        plus_face=False,
        ...
        if self.k_gaps:
            x_sect = self.split_gap_x_section_points(
                axis, ref_slice0=ref_slice0, plus_face=plus_face, masked=masked
            )
        else:
            if self.has_split_coordinate_lines:
                no_k_gap_xs = self.split_x_section_points(
                    axis, ref_slice0=ref_slice0, plus_face=plus_face, masked=masked
                )
                ...
                simple_xs = self.unsplit_x_section_points(
                    axis, ref_slice0=ref_slice0, plus_face=plus_face, masked=masked
                )
                x_sect = np.empty((self.nk, nj_or_ni, 2, 2, 3))
    ...
    def pixel_map_for_split_horizon_points(
        self, horizon_points, origin, width, height, dx, dy=None
    ):
        ...
        poly_points = poly_points.reshape(
            horizon_points.shape[0], horizon_points.shape[1], 4, 2
        )
        ...
        for j in range(poly_points.shape[0]):
            for i in range(poly_points.shape[1]):
                ...
                if (
                    patch_p_origin[0] + patch_extent[0] > width
                    or patch_p_origin[1] + patch_extent[1] > height
                ):
                    continue
                ...
                old_patch = p_map[
                    patch_p_origin[1] : patch_p_origin[1] + patch_extent[1],
    ...
    def pixel_maps(
        self, origin, width, height, dx, dy=None, k0=None, vertical_ref="top"
    ):
        """Makes a mapping from pixels to cell j, i indices, based on split horizon points for a single horizon.
        ...
        if k0 is not None:
            ...
            p_map = self.pixel_map_for_split_horizon_points(
                hp, origin, width, height, dx, dy=dy
            )
        else:
            ...
            for k0 in range(self.nk):
                rk0 = raw_k[k0] + kp
                p_map[k0] = self.pixel_map_for_split_horizon_points(
                    hp[rk0], origin, width, height, dx, dy=dy
                )
        return p_map
    ...
    def split_horizons_points(self, min_k0=None, max_k0=None, masked=False):
        ...
        if self.has_split_coordinate_lines:
            ...
            for j in range(self.nj):
                for i in range(self.ni):
                    hp[:, j, i, 0, 0, :] = points[
                        min_k0:end_k0, self.pillars_for_column[j, i, 0, 0], :
                    ]
                    hp[:, j, i, 1, 0, :] = points[
                        min_k0:end_k0, self.pillars_for_column[j, i, 1, 0], :
                    ]
                    hp[:, j, i, 0, 1, :] = points[
                        min_k0:end_k0, self.pillars_for_column[j, i, 0, 1], :
                    ]
                    hp[:, j, i, 1, 1, :] = points[
                        min_k0:end_k0, self.pillars_for_column[j, i, 1, 1], :
                    ]
        else:
    ...
    def nearest_rod(self, xyz, projection, axis, ref_slice0=0, plus_face=False):
        ...
        x_sect = self.unsplit_x_section_points(
            axis, ref_slice0=ref_slice0, plus_face=plus_face
        )
    ...
    def coordinate_line_end_points(self):
        ...
        result[:, :, 0, :] = points[0, :primary_pillar_count, :].reshape(
            (self.nj + 1, self.ni + 1, 3)
        )
        result[:, :, 1, :] = points[-1, :primary_pillar_count, :].reshape(
            (self.nj + 1, self.ni + 1, 3)
        )
        return result
    ...
    def z_corner_point_depths(self, order="cellular"):
        ...
        if self.has_split_coordinate_lines:
            ...
            if self.k_gaps:
                for j in range(self.nj):
                    for i in range(self.ni):
                        z_cp[:, j, i, 0, 0, 0] = points[
                            self.k_raw_index_array,
                            self.pillars_for_column[j, i, 0, 0],
                            2,
                        ]
                        z_cp[:, j, i, 1, 0, 0] = points[
                            self.k_raw_index_array + 1,
                            self.pillars_for_column[j, i, 0, 0],
                            2,
                        ]
                        z_cp[:, j, i, 0, 1, 0] = points[
                            self.k_raw_index_array,
                            self.pillars_for_column[j, i, 1, 0],
                            2,
                        ]
                        z_cp[:, j, i, 1, 1, 0] = points[
                            self.k_raw_index_array + 1,
                            self.pillars_for_column[j, i, 1, 0],
                            2,
                        ]
                        z_cp[:, j, i, 0, 0, 1] = points[
                            self.k_raw_index_array,
                            self.pillars_for_column[j, i, 0, 1],
                            2,
                        ]
                        z_cp[:, j, i, 1, 0, 1] = points[
                            self.k_raw_index_array + 1,
                            self.pillars_for_column[j, i, 0, 1],
                            2,
                        ]
                        z_cp[:, j, i, 0, 1, 1] = points[
                            self.k_raw_index_array,
                            self.pillars_for_column[j, i, 1, 1],
                            2,
                        ]
                        z_cp[:, j, i, 1, 1, 1] = points[
                            self.k_raw_index_array + 1,
                            self.pillars_for_column[j, i, 1, 1],
                            2,
                        ]
                ...
                for j in range(self.nj):
                    for i in range(self.ni):
                        z_cp[:, j, i, 0, 0, 0] = points[
                            :-1, self.pillars_for_column[j, i, 0, 0], 2
                        ]
                        z_cp[:, j, i, 1, 0, 0] = points[
                            1:, self.pillars_for_column[j, i, 0, 0], 2
                        ]
                        z_cp[:, j, i, 0, 1, 0] = points[
                            :-1, self.pillars_for_column[j, i, 1, 0], 2
                        ]
                        z_cp[:, j, i, 1, 1, 0] = points[
                            1:, self.pillars_for_column[j, i, 1, 0], 2
                        ]
                        z_cp[:, j, i, 0, 0, 1] = points[
                            :-1, self.pillars_for_column[j, i, 0, 1], 2
                        ]
                        z_cp[:, j, i, 1, 0, 1] = points[
                            1:, self.pillars_for_column[j, i, 0, 1], 2
                        ]
                        z_cp[:, j, i, 0, 1, 1] = points[
                            :-1, self.pillars_for_column[j, i, 1, 1], 2
                        ]
                        z_cp[:, j, i, 1, 1, 1] = points[
                            1:, self.pillars_for_column[j, i, 1, 1], 2
                        ]
    ...
    def corner_points(
        ...
        def one_cell_cp(grid, cell_kji0, points_root, cache_array):
            ...
            if not grid.geometry_defined_for_all_cells():
                if not grid.cell_geometry_is_defined(
                    cell_kji0, cache_array=cache_array
                ):
                    return cp
            corner_index = np.zeros(3, dtype="int")
            for kp in range(2):
        ...
        if cache_cp_array:
            ...
            if self.has_split_coordinate_lines:
                ...
                if self.k_gaps:
                    for j in range(self.nj):
                        for i in range(self.ni):
                            self.array_corner_points[:, j, i, 0, 0, 0, :] = points[
                                self.k_raw_index_array,
                                self.pillars_for_column[j, i, 0, 0],
                                :,
                            ]
                            self.array_corner_points[:, j, i, 1, 0, 0, :] = points[
                                self.k_raw_index_array + 1,
                                self.pillars_for_column[j, i, 0, 0],
                                :,
                            ]
                            self.array_corner_points[:, j, i, 0, 1, 0, :] = points[
                                self.k_raw_index_array,
                                self.pillars_for_column[j, i, 1, 0],
                                :,
                            ]
                            self.array_corner_points[:, j, i, 1, 1, 0, :] = points[
                                self.k_raw_index_array + 1,
                                self.pillars_for_column[j, i, 1, 0],
                                :,
                            ]
                            self.array_corner_points[:, j, i, 0, 0, 1, :] = points[
                                self.k_raw_index_array,
                                self.pillars_for_column[j, i, 0, 1],
                                :,
                            ]
                            self.array_corner_points[:, j, i, 1, 0, 1, :] = points[
                                self.k_raw_index_array + 1,
                                self.pillars_for_column[j, i, 0, 1],
                                :,
                            ]
                            self.array_corner_points[:, j, i, 0, 1, 1, :] = points[
                                self.k_raw_index_array,
                                self.pillars_for_column[j, i, 1, 1],
                                :,
                            ]
                            self.array_corner_points[:, j, i, 1, 1, 1, :] = points[
                                self.k_raw_index_array + 1,
                                self.pillars_for_column[j, i, 1, 1],
                                :,
                            ]
                    ...
                    for j in range(self.nj):
                        for i in range(self.ni):
                            self.array_corner_points[:, j, i, 0, 0, 0, :] = points[
                                :-1, self.pillars_for_column[j, i, 0, 0], :
                            ]
                            self.array_corner_points[:, j, i, 1, 0, 0, :] = points[
                                1:, self.pillars_for_column[j, i, 0, 0], :
                            ]
                            self.array_corner_points[:, j, i, 0, 1, 0, :] = points[
                                :-1, self.pillars_for_column[j, i, 1, 0], :
                            ]
                            self.array_corner_points[:, j, i, 1, 1, 0, :] = points[
                                1:, self.pillars_for_column[j, i, 1, 0], :
                            ]
                            self.array_corner_points[:, j, i, 0, 0, 1, :] = points[
                                :-1, self.pillars_for_column[j, i, 0, 1], :
                            ]
                            self.array_corner_points[:, j, i, 1, 0, 1, :] = points[
                                1:, self.pillars_for_column[j, i, 0, 1], :
                            ]
                            self.array_corner_points[:, j, i, 0, 1, 1, :] = points[
                                :-1, self.pillars_for_column[j, i, 1, 1], :
                            ]
                            self.array_corner_points[:, j, i, 1, 1, 1, :] = points[
                                1:, self.pillars_for_column[j, i, 1, 1], :
                            ]
                ...
                if self.k_gaps:
                    self.array_corner_points[:, :, :, 0, 0, 0, :] = points[
                        self.k_raw_index_array, :-1, :-1, :
                    ]
                    self.array_corner_points[:, :, :, 1, 0, 0, :] = points[
                        self.k_raw_index_array + 1, :-1, :-1, :
                    ]
                    self.array_corner_points[:, :, :, 0, 1, 0, :] = points[
                        self.k_raw_index_array, 1:, :-1, :
                    ]
                    self.array_corner_points[:, :, :, 1, 1, 0, :] = points[
                        self.k_raw_index_array + 1, 1:, :-1, :
                    ]
                    self.array_corner_points[:, :, :, 0, 0, 1, :] = points[
                        self.k_raw_index_array, :-1, 1:, :
                    ]
                    self.array_corner_points[:, :, :, 1, 0, 1, :] = points[
                        self.k_raw_index_array + 1, :-1, 1:, :
                    ]
                    self.array_corner_points[:, :, :, 0, 1, 1, :] = points[
                        self.k_raw_index_array, 1:, 1:, :
                    ]
                    self.array_corner_points[:, :, :, 1, 1, 1, :] = points[
                        self.k_raw_index_array + 1, 1:, 1:, :
                    ]
                else:
                    self.array_corner_points[:, :, :, 0, 0, 0, :] = points[
        ...
        if not self.geometry_defined_for_all_cells():
            if not self.cell_geometry_is_defined(
                cell_kji0, cache_array=cache_resqml_array
            ):
                return None
        if hasattr(self, "array_corner_points"):
            return self.array_corner_points[tuple(cell_kji0)]
        cp = one_cell_cp(
            self, cell_kji0, points_root=points_root, cache_array=cache_resqml_array
        )
        return cp
    ...
    def centre_point(self, cell_kji0=None, cache_centre_array=False):
        ...
        if hasattr(self, "array_centre_point"):
            ...
            return self.array_centre_point[
                tuple(cell_kji0)
            ]  # could check for nan here and return None
        if cache_centre_array:
            ...
            if hasattr(self, "array_corner_points"):
                self.array_centre_point = 0.125 * np.sum(
                    self.array_corner_points, axis=(3, 4, 5)
                )  # mean of eight corner points for each cell
            ...
            if cell_kji0 is None:
                return self.array_centre_point
            return self.array_centre_point[
                cell_kji0[0], cell_kji0[1], cell_kji0[2]
            ]  # could check for nan here and return None
    ...
    def centre_point_list(self, cell_kji0s):
        ...
        for cell in range(len(cell_kji0s)):
            centres_list[cell] = self.centre_point(
                cell_kji0=cell_kji0s[cell], cache_centre_array=True
            )
        return centres_list
    ...
    def thickness(
        ...
        def load_from_property(collection):
            if collection is None:
                return None
            parts = collection.selective_parts_list(
                property_kind="thickness", facet_type="netgross", facet="gross"
            )
            if len(parts) == 1:
            ...
            if len(parts) == 1 and collection.facet_for_part(parts[0]) is None:
                return collection.cached_part_array_ref(parts[0])
            parts = collection.selective_parts_list(
                property_kind="cell length", facet_type="direction", facet="K"
            )
            if len(parts) == 1:
                return collection.cached_part_array_ref(parts[0])
            return None
        ...
        if cache_thickness_array:
            ...
            if self.k_gaps:
                pillar_thickness = (
                    points[self.k_raw_index_array + 1, ..., 2]
                    - points[self.k_raw_index_array, ..., 2]
                )
            else:
            ...
            if self.has_split_coordinate_lines:
                ...
                self.array_thickness = np.abs(
                    0.25
                    * (
                        pillar_thickness[:, pillar_for_col[:, :, 0, 0]]
    ...
    def point_areally(self, tolerance=0.001):
        ...
        if self.has_split_coordinate_lines:
            pillar_for_col = self.create_column_pillar_mapping()
            j_pair_vectors = (
                points[:, pillar_for_col[:, :, 1, :], :]
                - points[:, pillar_for_col[:, :, 0, :], :]
            )
            i_pair_vectors = (
                points[:, pillar_for_col[:, :, :, 1], :]
                - points[:, pillar_for_col[:, :, :, 0], :]
        ...
        if self.k_gaps:
            return np.logical_and(
                layered[self.k_raw_index_array], layered[self.k_raw_index_array + 1]
            )
    ...
    def volume(
        ...
        def load_from_property(collection):
            if collection is None:
                return None
            parts = collection.selective_parts_list(
                property_kind="rock volume", facet_type="netgross", facet="gross"
            )
            if len(parts) == 1:
                return collection.cached_part_array_ref(parts[0])
        ...
        if cache_volume_array or cell_kji0 is None:
            ...
            vol_array = vol.tetra_volumes(
                self.array_corner_points, centres=centre_array, off_hand=off_hand
            )
            if cache_volume_array:
    ...
    def pinched_out(
        ...
        if cache_pinchout_array:
            self.pinchout = np.where(
                np.isnan(self.array_thickness),
                True,
    ...
    def half_cell_transmissibility(
        self, use_property=True, realization=None, tolerance=1.0e-6
    ):
        ...
        if use_property:
            ...
            half_t_resqml = pc.single_array_ref(
    ...
    def transmissibility(
        ...
        if realization is None:
            if (
                hasattr(self, "array_k_transmissibility")
                and self.array_k_transmissibility is not None
            ):
                k_tr = self.array_k_transmissibility
            if (
                hasattr(self, "array_j_transmissibility")
                and self.array_j_transmissibility is not None
            ):
                j_tr = self.array_j_transmissibility
            if (
                hasattr(self, "array_i_transmissibility")
                and self.array_i_transmissibility is not None
            ):
        ...
        if k_tr is None or j_tr is None or i_tr is None:
            ...
            half_t = self.half_cell_transmissibility(
                use_property=use_tr_properties, realization=realization
            )

            if modifier_mode == "faces per cell multiplier":
                pc = self.extract_property_collection()
                half_t_mult = pc.single_array_ref(
                ...
                if half_t_mult is None:
                    ...
                    half_t = np.where(
                        np.isnan(half_t_mult), half_t, half_t * half_t_mult
                    )
            ...
            if k_tr is None:
                ...
                internal_zero_mask = np.logical_or(
                    np.logical_or(np.isnan(slice_a), slice_a < tolerance),
                    np.logical_or(np.isnan(slice_b), slice_b < tolerance),
                )
                if (
                    self.k_gaps
                ):  # todo: scan K gaps for zero thickness gaps and allow transmission there
                    internal_zero_mask[self.k_gap_after_array, :, :] = True
                tr_mult = None
                if modifier_mode == "faces multiplier":
                    tr_mult = pc.single_array_ref(
                        property_kind="transmissibility multiplier",
                        realization=realization,
                        facet_type="direction",
                        facet="K",
                    ...
                    if tr_mult is not None:
                        ...
                        internal_zero_mask = np.logical_or(
                            internal_zero_mask, np.isnan(tr_mult[1:-1, :, :])
                        )
                if tr_mult is None:
                    tr_mult = 1.0
                tr_abs_r = 0.0
                if modifier_mode == "absolute":
                    tr_abs = pc.single_array_ref(
                        property_kind="mat transmissibility",
                        realization=realization,
                        facet_type="direction",
                        facet="K",
                        continuous=True,
                    ...
                    if tr_abs is None:
                        tr_abs = pc.single_array_ref(
                            property_kind="fault transmissibility",
                            realization=realization,
                            facet_type="direction",
                            facet="K",
                            continuous=True,
                    ...
                    if tr_abs is not None:
                        log.debug(
                            "applying absolute K face transmissibility modification"
                        )
                        assert tr_abs.shape == (self.nk + 1, self.nj, self.ni)
                        internal_zero_mask = np.logical_or(
                            internal_zero_mask, tr_abs[1:-1, :, :] <= 0.0
                        )
                        tr_abs_r = np.where(
                            np.logical_or(np.isinf(tr_abs), np.isnan(tr_abs)),
                            0.0,
                            1.0 / tr_abs,
                        )
                k_tr[1:-1, :, :] = np.where(
            ...
            if j_tr is None:
                ...
                if modifier_mode == "absolute":
                    ...
                    if tr_abs is not None:
                        log.debug(
                            "applying absolute J face transmissibility modification"
                        )
            ...
            if i_tr is None:
                ...
                if modifier_mode == "absolute":
                    ...
                    if tr_abs is not None:
                        log.debug(
                            "applying absolute I face transmissibility modification"
                        )
    ...
    def fault_connection_set(
        ...
        if not hasattr(self, "fgcs") or self.fgcs_skip_inactive != skip_inactive:
            self.fgcs, self.fgcs_fractional_area = rqtr.fault_connection_set(
        ...
        new_tr = False
        if compute_transmissibility and not hasattr(
            self, "array_fgcs_transmissibility"
        ):
            self.array_fgcs_transmissibility = self.fgcs.tr_property_array(
                self.fgcs_fractional_area
            )
            new_tr = True

        tr = (
            self.array_fgcs_transmissibility
            if hasattr(self, "array_fgcs_transmissibility")
            else None
        )
    ...
    def pinchout_connection_set(
        ...
        new_tr = False
        if compute_transmissibility and not hasattr(
            self, "array_pgcs_transmissibility"
        ):
        ...
        tr = (
            self.array_pgcs_transmissibility
            if hasattr(self, "array_pgcs_transmissibility")
            else None
        )
    ...
    def k_gap_connection_set(
        ...
        if not hasattr(self, "kgcs") or self.kgcs_skip_inactive != skip_inactive:
            self.kgcs = rqf.k_gap_connection_set(
                self, skip_inactive=skip_inactive, tolerance=tolerance
            )
        ...
        new_tr = False
        if compute_transmissibility and not hasattr(
            self, "array_kgcs_transmissibility"
        ):
        ...
        tr = (
            self.array_kgcs_transmissibility
            if hasattr(self, "array_kgcs_transmissibility")
            else None
        )
    ...
    def cell_inactive(self, cell_kji0, pv_array=None, pv_tol=0.01):
        ...
        if pv_array is not None:  # fabricate an inactive mask from pore volume data
            self.inactive = not (
                pv_array > pv_tol
            )  # NaN in pv array will end up inactive
            return self.inactive[tuple(cell_kji0)]
        return (not self.cell_geometry_is_defined(cell_kji0)) or self.pinched_out(
    ...
    def bounding_box(self, cell_kji0, points_root=None, cache_cp_array=False):
        ...
        result = np.zeros((2, 3))
        cp = self.corner_points(
            cell_kji0, points_root=points_root, cache_cp_array=cache_cp_array
        )
    ...
    def face_centres_kji_01(
        self, cell_kji0, points_root=None, cache_resqml_array=True, cache_cp_array=False
    ):
        ...
        for axis in range(3):
            for zero_or_one in range(2):
                result[axis, zero_or_one] = self.face_centre(
                    cell_kji0,
                    axis,
                    zero_or_one,
                    points_root=points_root,
    ...
    def interface_vector(
        ...
        face_0_centre = self.face_centre(
            cell_kji0,
            axis,
            0,
            points_root=points_root,
        ...
        face_1_centre = self.face_centre(
            cell_kji0,
            axis,
            1,
            points_root=points_root,
    ...
    def interface_vectors_kji(
        ...
        result = np.zeros((3, 3))
        for axis in range(3):
            result[axis] = self.interface_vector(
                cell_kji0,
                axis,
                points_root=points_root,
                cache_resqml_array=cache_resqml_array,
    ...
    def interface_lengths_kji(
        ...
        result = np.zeros(3)
        for axis in range(3):
            result[axis] = self.interface_length(
                cell_kji0,
                axis,
                points_root=points_root,
                cache_resqml_array=cache_resqml_array,
    ...
    def local_to_global_crs(
        ...
        flat_a = a.reshape(
            (-1, 3)
        )  # flattened view of array a as vector of (x, y, z) points, in situ
        ...
        # todo: apply rotation
        if (
            global_z_increasing_downward is not None
        ):  # note: here negation is made in local crs; if z_offset is not zero, this might not be what is intended
            crs_z_increasing_downward_text = rqet.find_tag(
                crs_root, "ZIncreasingDownward"
            ).text
            if crs_z_increasing_downward_text in [
                "true",
                "false",
            ]:  # todo: otherwise could raise exception
        ...
        if global_xy_units is not None:
            ...
            if crs_xy_units_text in ["ft", "m"]:  # todo: else raise exception
                bwam.convert_lengths(
                    flat_a[:, 0], crs_xy_units_text, global_xy_units
                )  # x
                bwam.convert_lengths(
                    flat_a[:, 1], crs_xy_units_text, global_xy_units
                )  # y
        if global_z_units is not None:
            ...
            if crs_z_units_text in ["ft", "m"]:  # todo: else raise exception
                bwam.convert_lengths(
                    flat_a[:, 2], crs_z_units_text, global_z_units
                )  # z
    ...
    def global_to_local_crs(
        ...
        if global_xy_units is not None:
            ...
            if crs_xy_units_text in ["ft", "m"]:  # todo: else raise exception
                bwam.convert_lengths(
                    flat_a[:, 0], global_xy_units, crs_xy_units_text
                )  # x
                bwam.convert_lengths(
                    flat_a[:, 1], global_xy_units, crs_xy_units_text
                )  # y
        if global_z_units is not None:
            crs_z_units_text = rqet.find_tag(crs_root, "VerticalUom").text
            if crs_z_units_text in ["ft", "m"]:  # todo: else raise exception
                bwam.convert_lengths(
                    flat_a[:, 2], global_z_units, crs_z_units_text
                )  # z
        flat_a[:, 0] -= x_offset
        flat_a[:, 1] -= y_offset
        if z_offset != 0.0:
    ...
    def write_hdf5_from_caches(
        ...
        if geometry:
            if (
                always_write_pillar_geometry_is_defined_array
                or not self.geometry_defined_for_all_pillars(cache_array=True)
            ):
                if (
                    not hasattr(self, "array_pillar_geometry_is_defined")
                    or self.array_pillar_geometry_is_defined is None
                ):
                    self.array_pillar_geometry_is_defined = np.full(
                        (self.nj + 1, self.ni + 1), True, dtype=bool
                    )
            ...
            if (
                always_write_cell_geometry_is_defined_array
                or not self.geometry_defined_for_all_cells(cache_array=True)
            ):
                if (
                    not hasattr(self, "array_cell_geometry_is_defined")
                    or self.array_cell_geometry_is_defined is None
                ):
                    self.array_cell_geometry_is_defined = np.full(
                        (self.nk, self.nj, self.ni), True, dtype=bool
                    )
                h5_reg.register_dataset(
                    self.uuid,
            ...
            if self.has_split_coordinate_lines:
                h5_reg.register_dataset(
                    self.uuid,
                    "PillarIndices",
                    self.split_pillar_indices_cached,
                    dtype="uint32",
                )
                h5_reg.register_dataset(
                    self.uuid,
            ...
            if self.k_gaps:
                ...
                h5_reg.register_dataset(
                    self.uuid, "GapAfterLayer", self.k_gap_after_array, dtype="uint8"
                )
            if self.parent_window is not None:
                for axis in range(3):
                    if (
                        self.parent_window.fine_extent_kji[axis]
                        == self.parent_window.coarse_extent_kji[axis]
                    ):
                    ...
                    if self.parent_window.constant_ratios[axis] is not None:
                        if self.is_refinement:
                            pcpi = np.array(
                                [self.parent_window.coarse_extent_kji[axis]], dtype=int
                            )  # ParentCountPerInterval
                            ccpi = np.array(
                                [self.parent_window.fine_extent_kji[axis]], dtype=int
                        ...
                        if self.is_refinement:
                            ...
                            ccpi = np.array(
                                self.parent_window.vector_ratios[axis], dtype=int
                            )
                        else:
                    ...
                    h5_reg.register_dataset(
                        self.uuid, "KJI"[axis] + "Regrid/ParentCountPerInterval", pcpi
                    )
                    h5_reg.register_dataset(
                        self.uuid, "KJI"[axis] + "Regrid/ChildCountPerInterval", ccpi
                    )
                    if (
                        self.is_refinement
                        and not self.parent_window.equal_proportions[axis]
                    ):
                        child_cell_weights = np.concatenate(
                            self.parent_window.vector_proportions[axis]
                        )
                        h5_reg.register_dataset(
                            self.uuid,
        ...
        if write_active and self.inactive is not None:
            if imported_properties is None:
                ...
                imported_properties.imported_list = (
                    filtered_list  # might have unintended side effects elsewhere
                )
            active_mask = np.logical_not(self.inactive)
            imported_properties.add_cached_array_to_imported_list(
                active_mask,
                "active cell mask",
                "ACTIVE",
                discrete=True,
        ...
        if (
            imported_properties is not None
            and imported_properties.imported_list is not None
        ):
            for entry in imported_properties.imported_list:
                if hasattr(imported_properties, entry[3]):  # otherwise constant array
                    h5_reg.register_dataset(
                        entry[0],
                        "values_patch0",
                        imported_properties.__dict__[entry[3]],
                    )
                if entry[10] == "active":
    ...
    def write_hdf5(self):
        ...
        self.write_hdf5_from_caches(
            mode="a", geometry=True, imported_properties=None, write_active=True
        )
    ...
    def write_nexus_corp(
        ...
        if ijk_right_handed is None:
            ...
            log.warning(
                "ijk axes are left handed; inverted (fake) xyz handedness required"
            )
        crs_root = self.extract_crs_root()
        if not local_coords:
            if not global_z_increasing_downward:
                log.warning(
                    "global z is not increasing with depth as expected by Nexus"
                )
                tm.log_nexus_tm("warning")
            if crs_root is not None:  # todo: otherwise raise exception?
                log.info(
                    "converting corner points from local to global reference system"
                )
                self.local_to_global_crs(
                    cp,
                    crs_root,
        ...
        with open(file_name, "w") as header:
                ...
                if local_coords:
                    if crs_root is not None:
                        ...
                        if crs_xy_units_text == "m" and crs_z_units_text == "m":
                            ...
                            header.write(
                                "! local coordinates mixed (or not recognized)\n\n"
                            )
                    else:
                        header.write("! local coordinates unknown\n\n")
                elif (
                    global_xy_units is not None
                    and global_z_units is not None
                    and global_xy_units == global_z_units
                ):
            ...
            if write_nx_ny_nz:
                ...
                header.write(
                    "{0:<7d} {1:<7d} {2:<7d}\n\n".format(
                        self.extent_kji[2], self.extent_kji[1], self.extent_kji[0]
                    )
                )
            if write_rh_keyword_if_needed:
                if ijk_right_handed is None or crs_root is None:
                    log.warning(
                        "unable to determine whether RIGHTHANDED keyword is needed"
                    )
                else:
                    ...
                    if local_coords:
                        ...
                        if not z_inc_down:
                            log.warning(
                                "local z is not increasing with depth as expected by Nexus"
                            )
                            tm.log_nexus_tm("warning")
                    else:
                    ...
                    if xyz_handedness == "unknown":
                        ...
                        if ijk_right_handed == (
                            xyz_handedness == "right"
                        ):  # if either both True or both False
                            header.write("RIGHTHANDED\n\n")
        if write_corp_keyword:
            keyword = "CORP VALUE"
        else:
            keyword = None
        wd.write_array_to_ascii_file(
            file_name,
            corp_extent,
    ...
    def find_cell_for_point_xy(self, x, y, k0=0, vertical_ref="top", local_coords=True):
        ...
        a = np.array(
            [[x, y, 0.0]]
        )  # extra axis needed to keep global_to_local_crs happy
        if not local_coords:
    ...
    def find_cell_for_x_sect_xz(self, x_sect, x, z):
        ...
        assert (
            x_sect.ndim == 5
            and x_sect.shape[2] == 2
            and x_sect.shape[3] == 2
            and 2 <= x_sect.shape[4] <= 3
        )
        n_k = x_sect.shape[0]
        ...
        diffs = np.sum(
            diffs * diffs, axis=-1
        )  # square of distance of each point from given x,z
    ...
    def skin(self, use_single_layer_tactics=False):
        ...
        if (
            self.grid_skin is None
            or self.grid_skin.use_single_layer_tactics != use_single_layer_tactics
        ):
            self.grid_skin = rqgs.GridSkin(
    ...
    def create_xml(
        ...
        if self.grid_representation and not write_geometry:
            rqet.create_metadata_xml(
                node=ijk, extra_metadata={"grid_flavour": self.grid_representation}
            )
        ...
        if self.k_gaps:
            ...
            self.model.create_hdf5_dataset_ref(
                ext_uuid, self.uuid, "GapAfterLayer", root=kgal_values
            )
        ...
        if self.parent_window is not None:
            ...
            for axis in range(3):
                ...
                iiopg_node = rqet.SubElement(
                    regrid_node, ns["resqml2"] + "InitialIndexOnParentGrid"
                )
                ...
                intervals_node = rqet.SubElement(
                    regrid_node, ns["resqml2"] + "Intervals"
                )
                ...
                if self.parent_window.constant_ratios[axis] is not None:
                    ...
                    if self.is_refinement:
                        ...
                        interval_count = self.parent_window.fine_extent_kji[axis]
                ic_node = rqet.SubElement(
                    intervals_node, ns["resqml2"] + "IntervalCount"
                )
                ic_node.set(ns["xsi"] + "type", ns["xsd"] + "positiveInteger")
                ic_node.text = str(interval_count)

                pcpi_node = rqet.SubElement(
                    intervals_node, ns["resqml2"] + "ParentCountPerInterval"
                )
                ...
                ccpi_node = rqet.SubElement(
                    intervals_node, ns["resqml2"] + "ChildCountPerInterval"
                )
                ...
                if (
                    ...
                    ccw_node = rqet.SubElement(
                        intervals_node, ns["resqml2"] + "ChildCellWeights"
                    )
                    ccw_node.set(ns["xsi"] + "type", ns["resqml2"] + "DoubleHdf5Array")
                    ccw_node.text = rqet.null_xml_text

                    ccw_values_node = rqet.SubElement(
                        ccw_node, ns["resqml2"] + "Values"
                    )
                    ccw_values_node.set(
                        ns["xsi"] + "type", ns["resqml2"] + "Hdf5Dataset"
                    )
        ...
        if write_geometry:
            ...
            self.model.create_hdf5_dataset_ref(
                ext_uuid, self.uuid, "Points", root=coords
            )
            ...
            if (
                ...
                pillar_def = rqet.SubElement(
                    geom, ns["resqml2"] + "PillarGeometryIsDefined"
                )
                ...
                self.model.create_hdf5_dataset_ref(
                    ext_uuid, self.uuid, "PillarGeometryIsDefined", root=pd_values
                )
                ...
                pillar_def.set(
                    ns["xsi"] + "type", ns["resqml2"] + "BooleanConstantArray"
                )
                pillar_def.text = "\n"
            ...
            if (
                ...
                cell_def = rqet.SubElement(
                    geom, ns["resqml2"] + "CellGeometryIsDefined"
                )
                ...
                self.model.create_hdf5_dataset_ref(
                    ext_uuid, self.uuid, "CellGeometryIsDefined", root=cd_values
                )
            ...
            if self.has_split_coordinate_lines:
                ...
                scl.set(
                    ns["xsi"] + "type",
                    ns["resqml2"] + "ColumnLayerSplitCoordinateLines",
                )
                scl.text = "\n"
                ...
                self.model.create_hdf5_dataset_ref(
                    ext_uuid, self.uuid, "PillarIndices", root=pi_values
                )

                cpscl = rqet.SubElement(
                    scl, ns["resqml2"] + "ColumnsPerSplitCoordinateLine"
                )
            ...
            if add_relationships:
                if write_geometry:
                    ...
                    self.model.create_reciprocal_relationship(
                        ijk, "destinationObject", self.crs_root, "sourceObject"
                    )
                    ...
                    self.model.create_reciprocal_relationship(
                        ijk, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                    )
        ...
        if (
            write_active
            and self.active_property_uuid is not None
            and self.model.part(uuid=self.active_property_uuid) is None
        ):
            active_collection = rprop.PropertyCollection()
            active_collection.set_support(support=self)
            active_collection.create_xml(
                None,
                None,
                "ACTIVE",
                "active",
...
class RegularGrid(Grid):
    ...
    def __init__(
        ...
        if grid_root is None:
            ...
            self.geometry_defined_for_all_pillars_cached = True
            self.array_cell_geometry_is_defined = np.full(
                tuple(self.extent_kji), True, dtype=bool
            )
        else:
            ...
            if dxyz is None and dxyz_dkji is None:
                ...
                dxi_part = self.property_collection.singleton(
                    property_kind="cell length", facet_type="direction", facet="I"
                )
                dyj_part = self.property_collection.singleton(
                    property_kind="cell length", facet_type="direction", facet="J"
                )
                dzk_part = self.property_collection.singleton(
                    property_kind="cell length", facet_type="direction", facet="K"
                )
                assert (
                    dxi_part is not None
                    and dyj_part is not None
                    and dzk_part is not None
                )
        ...
        if dxyz_dkji is None:
            dxyz_dkji = np.array(
                [[0.0, 0.0, dxyz[2]], [0.0, dxyz[1], 0.0], [dxyz[0], 0.0, 0.0]]
            )
        self.block_origin = np.array(origin).copy()
        self.block_dxyz_dkji = np.array(dxyz_dkji).copy()
        if (
            use_vertical and dxyz_dkji[0][0] == 0.0 and dxyz_dkji[0][1] == 0.0
        ):  # ie. no x,y change with k
    ...
    def make_regular_points_cached(self):
        ...
        for k in range(self.nk):
            self.points_cached[k + 1, 0, 0] = (
                self.points_cached[k, 0, 0] + self.block_dxyz_dkji[0]
            )
        for j in range(self.nj):
            self.points_cached[:, j + 1, 0] = (
                self.points_cached[:, j, 0] + self.block_dxyz_dkji[1]
            )
        for i in range(self.ni):
            self.points_cached[:, :, i + 1] = (
                self.points_cached[:, :, i] + self.block_dxyz_dkji[2]
            )
    ...
    def point_raw(self, index=None, points_root=None, cache_array=True):
        ...
        return self.block_origin + np.sum(
            np.repeat(np.array(index).reshape((3, 1)), 3, axis=-1)
            * self.block_dxyz_dkji,
            axis=0,
        )

    def half_cell_transmissibility(
        self, use_property=None, realization=None, tolerance=None
    ):
    ...
    def centre_point(self, cell_kji0=None):
        ...
        if cell_kji0 is not None:
            ...
            centre = self.block_origin + np.sum(
                self.block_dxyz_dkji
                * np.expand_dims(float_kji0, axis=-1).repeat(3, axis=-1),
                axis=0,
            )
            return centre
    ...
    def create_xml(
        ...
        if add_cell_length_properties:
            ...
            dpc.set_grid(self)
            for axis in range(3):
                dpc.add_cached_array_to_imported_list(
                    None,
                    "regular grid",
                    "D" + "ZYX"[axis],
                    discrete=False,
...
def establish_zone_property_kind(model):
    ...
    if zone_pk_root is None:
        zone_pk = rprop.PropertyKind(
            model, title="zone", parent_property_kind="discrete"
        )
        zone_pk.create_xml()
    else:
...
def extent_kji_from_root(root_node):
    ...
    return (
        rqet.find_tag_int(root_node, "Nk"),
        rqet.find_tag_int(root_node, "Nj"),
        rqet.find_tag_int(root_node, "Ni"),
    )
