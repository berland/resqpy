...
expected_facet_type_dict = {
    ...
    "fault transmissibility": (
        "direction",
        ["I", "J"],
    ),  # local property kind; K faces also permitted
    "mat transmissibility": (
        "direction",
        ["K"],
    ),  # local property kind; I & J faces also permitted
    "saturation": (
        "what",
        [
            "water",
...
class PropertyCollection:
    ...
    def __init__(self, support=None, property_set_root=None, realization=None):
        ...
        assert (
            property_set_root is None or support is not None
        ), "support (grid or wellbore frame) must be specified when populating property collection from property set"

        self.dict = (
            {}
        )  # main dictionary of model property parts which are members of the collection
        ...
        if support is not None:
            ...
            if property_set_root is None:
                ...
                props_list = self.model.parts_list_of_type(
                    type_of_interest="obj_DiscreteProperty"
                )
                discrete_props_list = self.model.parts_list_filtered_by_supporting_uuid(
                    props_list, self.support_uuid
                )
                self.add_parts_list_to_dict(discrete_props_list)
                props_list = self.model.parts_list_of_type(
                    type_of_interest="obj_CategoricalProperty"
                )
                categorical_props_list = (
                    self.model.parts_list_filtered_by_supporting_uuid(
                        props_list, self.support_uuid
                    )
                )
                self.add_parts_list_to_dict(categorical_props_list)
                props_list = self.model.parts_list_of_type(
                    type_of_interest="obj_ContinuousProperty"
                )
                continuous_props_list = (
                    self.model.parts_list_filtered_by_supporting_uuid(
                        props_list, self.support_uuid
                    )
                )
    ...
    def set_support(
        self, support_uuid=None, support=None, model=None, modify_parts=True
    ):
        ...
        if support_uuid is None:
            if self.support_uuid is not None:
                log.warning(
                    "clearing supporting representation for property collection"
                )
            self.model = None
            self.support = None
            self.support_root = None
            ...
            assert (
                model is not None
            ), "model not established when setting support for property collection"
            if self.support_uuid is not None and not bu.matching_uuids(
                support_uuid, self.support_uuid
            ):
                log.warning(
                    "changing supporting representation for property collection"
                )
            ...
            if self.support is None:
                ...
                assert (
                    support_part is not None
                ), "supporting representation part missing in model"
                ...
                if support_type == "obj_IjkGridRepresentation":
                    self.support = grr.any_grid(
                        model, grid_root=self.support_root, find_properties=False
                    )
                elif support_type == "obj_WellboreFrameRepresentation":
                    self.support = rqw.WellboreFrame(
                        model, frame_root=self.support_root
                    )
                elif support_type == "obj_BlockedWellboreRepresentation":
                    self.support = rqw.BlockedWell(
                        model, blocked_well_root=self.support_root
                    )
                    ...
                    self.support = rqf.GridConnectionSet(
                        model, connection_set_root=self.support_root
                    )
                else:
                    raise TypeError(
                ...
                if isinstance(self.support, grr.Grid):
                    self.support_root = self.support.grid_root
    ...
    def supporting_shape(self, indexable_element=None, direction=None):
        ...
        if isinstance(support, grr.Grid):
            if indexable_element is None or indexable_element == "cells":
                ...
                shape_list[
                    axis
                ] += 1  # note: properties for grid faces include outer faces
            elif indexable_element == "column edges":
                shape_list = [
                    (support.nj * (support.ni + 1)) + ((support.nj + 1) * support.ni)
            ...
            if indexable_element is None or indexable_element == "intervals":
                shape_list = [
                    support.node_count - 1
                ]  # all intervals, including unblocked
            ...
            if (
                indexable_element is None
                or indexable_element == "cells"
                or indexable_element == "columns"
            ):
    ...
    def populate_from_property_set(self, property_set_root):
        ...
        assert (
            self.model is not None and self.support is not None
        ), "set support for collection before populating from property set"

        self.property_set_root = property_set_root
        self.time_set_kind_attr = rqet.find_tag_text(property_set_root, "TimeSetKind")
        self.has_single_property_kind_flag = rqet.find_tag_bool(
            property_set_root, "HasSinglePropertyKind"
        )
        self.has_multiple_realizations_flag = rqet.find_tag_bool(
            property_set_root, "HasMultipleRealizations"
        )
        parent_set_ref_root = rqet.find_tag(
            property_set_root, "ParentSet"
        )  # at most one parent set handled here
    ...
    def add_part_to_dict(self, part, continuous=None, realization=None, trust_uom=True):
        ...
        realization_node = rqet.find_tag(
            xml_node, "RealizationIndex"
        )  # optional; if present use to populate realization
        ...
        citation_title = rqet.find_tag(
            rqet.find_tag(xml_node, "Citation"), "Title"
        ).text
        (property_kind, facet_type, facet) = property_kind_and_facet_from_keyword(
            citation_title
        )
        ...
        if kind_node is not None:
            property_kind = (
                kind_node.text
            )  # could check for consistency with that derived from citation title
        else:
        ...
        facet_node = rqet.find_tag(
            xml_node, "Facet"
        )  # might have to handle more than one facet for a property?
        ...
        if support_uuid is None:
            support_uuid = self.support_uuid
        elif self.support_uuid is not None and not bu.matching_uuids(
            support_uuid, self.support.uuid
        ):
    ...
    def inherit_imported_list_from_other_collection(
        self, other, copy_cached_arrays=True, exclude_inactive=False
    ):
    ...
    def inherit_parts_from_other_collection(self, other, ignore_clashes=False):
        ...
        assert (
            self.support_uuid is None
            or other.support_uuid is None
            or bu.matching_uuids(self.support_uuid, other.support_uuid)
        )
        if (
            self.support_uuid is None
            and self.number_of_parts() == 0
            and other.support_uuid is not None
        ):
        ...
        for (part, info) in other.dict.items():
            if part in self.dict.keys():
                if ignore_clashes:
                    continue
                assert False, (
                    "attempt to inherit a part which already exists in property collection: "
                    + part
                )
            self.dict[part] = info
    ...
    def inherit_parts_selectively_from_other_collection(
        ...
        for (part, info) in other.dict.items():
            if (
                ...
                and other.realization_for_part(part) != realization
            ):
                continue
            if support_uuid is not None and not bu.matching_uuids(
                support_uuid, other.support_uuid_for_part(part)
            ):
                continue
            if uuid is not None and not bu.matching_uuids(
                uuid, other.uuid_for_part(part)
            ):
                continue
            ...
            if categorical is not None:
                if categorical:
                    if other.continuous_for_part(part) or (
                        other.string_lookup_uuid_for_part(part) is None
                    ):
                        continue
                else:
                    if not (
                        other.continuous_for_part(part)
                        or (other.string_lookup_uuid_for_part(part) is None)
                    ):
                        continue
            ...
            if indexable is not None and other.indexable_for_part(part) != indexable:
                continue
            if property_kind is not None and not same_property_kind(
                other.property_kind_for_part(part), property_kind
            ):
                continue
            ...
            if citation_title is not None:
                if citation_title_match_starts_with:
                    ...
                    if other.citation_title_for_part(part) != citation_title:
                        continue
            if time_series_uuid is not None and not bu.matching_uuids(
                time_series_uuid, other.time_series_uuid_for_part(part)
            ):
                continue
            if time_index is not None and other.time_index_for_part(part) != time_index:
                continue
            if string_lookup_uuid is not None and not bu.matching_uuids(
                string_lookup_uuid, other.string_lookup_uuid_for_part(part)
            ):
                continue
            if part in self.dict.keys():
                if ignore_clashes:
    ...
    def inherit_similar_parts_for_time_series_from_other_collection(
        self,
        other,
        example_part,
        citation_title_match_starts_with=False,
    ...
    def inherit_similar_parts_for_facets_from_other_collection(
        self,
        other,
        example_part,
        citation_title_match_starts_with=False,
    ...
    def inherit_similar_parts_for_realizations_from_other_collection(
        self,
        other,
        example_part,
        citation_title_match_starts_with=False,
    ...
    def selective_parts_list(
        ...
        temp_collection = selective_version_of_collection(
            self,
    ...
    def singleton(
        ...
        assert (
            len(parts_list) == 1
        ), "More than one property part matches selection criteria"
        return parts_list[0]
    ...
    def single_array_ref(
        ...
        if part is None:
            return None
        return self.cached_part_array_ref(
            part, dtype=dtype, masked=masked, exclude_null=exclude_null
        )
    ...
    def grid_for_part(self, part):
        ...
        assert (
            part is not None
            and self.model.type_of_part(part) == "obj_IjkGridRepresentation"
        )
        return grr.any_grid(
            self.model, grid_root=self.model.root_for_part(part), find_properties=False
        )
    ...
    def string_lookup_for_part(self, part):
        ...
        assert (
            sl_root is not None
        ), "string table lookup referenced by property is not present in model"
    ...
    def part_is_categorical(self, part):
        ...
        return (
            not self.continuous_for_part(part)
            and self.string_lookup_uuid_for_part(part) is not None
        )
    ...
    def establish_has_single_uom(self):
        ...
        for part in self.parts():
            ...
            if common_uom is None:
                ...
                break
        if common_uom is None:
            self.has_single_uom_flag = (
                True  # all uoms are None (probably discrete properties)
            )
    ...
    def assign_realization_numbers(self):
        ...
        assert (
            self.has_single_property_kind()
        ), "attempt to assign realization numbers to properties of differing kinds"
        assert (
            self.has_single_indexable_element()
    ...
    def masked_array(self, simple_array, exclude_inactive=True, exclude_value=None):
        ...
        if (
            ...
            and hasattr(self.support, "inactive")
            and self.support.inactive is not None
            and self.support.inactive.shape == simple_array.shape
        ):
    ...
    def h5_key_pair_for_part(self, part):
        ...
        return model.h5_uuid_and_path_for_node(
            first_values_node
        )  # note: Values node within Values node for properties
    ...
    def cached_part_array_ref(self, part, dtype=None, masked=False, exclude_null=False):
        ...
        if not hasattr(self, cached_array_name):
            ...
            if const_value is None:
                ...
                assert (
                    len(patch_list) == 1
                )  # todo: handle more than one patch of values
                first_values_node = rqet.find_tag(patch_list[0], "Values")
                if first_values_node is None:
                    return None  # could treat as fatal error
                if dtype is None:
                    ...
                    if array_type == "DoubleHdf5Array":
                        ...
                        raise ValueError(
                            "array type not catered for: " + str(array_type)
                        )
                ...
                a = np.full(
                    shape,
                    const_value,
                    dtype=float if self.continuous_for_part(part) else int,
                )
        ...
        if masked:
            ...
            return self.masked_array(
                self.__dict__[cached_array_name], exclude_value=exclude_value
            )
        else:
    ...
    def shape_and_type_of_part(self, part):
        ...
        if hasattr(self, cached_array_name):
            return (
                tuple(self.__dict__[cached_array_name].shape),
                self.__dict__[cached_array_name].dtype,
            )
        ...
        assert len(patch_list) == 1  # todo: handle more than one patch of values
        h5_key_pair = model.h5_uuid_and_path_for_node(
            rqet.find_tag(patch_list[0], "Values")
        )
        if h5_key_pair is None:
            return None, None
        return model.h5_array_shape_and_type(h5_key_pair)

    def facets_array_ref(
        self, use_32_bit=False, indexable_element=None
    ):  # todo: add masked argument
        ...
        assert (
            self.number_of_parts() == facet_count
        ), "collection covers more than facet variability"

        continuous = self.all_continuous()
        if not continuous:
            assert (
                self.all_discrete()
            ), "mixture of continuous and discrete properties in collection"
    ...
    def realizations_array_ref(
        ...
        assert self.number_of_parts() == len(
            r_list
        ...
        if fill_missing:
            for part in self.parts():
                ...
                assert (
                    realization is not None and 0 <= realization < r_extent
                ), "realization missing (or out of range?)"
                pa = self.cached_part_array_ref(part, dtype=dtype)
                a[realization] = pa
                self.uncache_part_array(part)
        else:
    ...
    def time_series_array_ref(
        ...
        if fill_missing:
            for part in self.parts():
                time_index = self.time_index_for_part(part)
                assert (
                    time_index is not None and 0 <= time_index < ti_extent
                ), "time index missing (or out of range?)"
                pa = self.cached_part_array_ref(part, dtype=dtype)
                a[time_index] = pa
                self.uncache_part_array(part)
        else:
    ...
    def combobulated_face_array(self, resqml_a):
        ...
        for axis in range(3):
            for polarity in range(2):
                resqpy_a[..., axis, polarity] = resqml_a[
                    ..., self.face_index_map[axis, polarity]
                ]
    ...
    def discombobulated_face_array(self, resqpy_a):
        ...
        assert (
            resqpy_a.ndim >= 2 and resqpy_a.shape[-2] == 3 and resqpy_a.shape[-1] == 2
        )
    ...
    def cached_normalized_part_array_ref(
        self,
        part,
        masked=False,
        use_logarithm=False,
        ...
        return self.normalized_part_array(
            part,
            masked=masked,
    ...
    def normalized_part_array(
        ...
        if min_value is None or max_value is None:
            ...
            if masked and max_value is ma.masked:
                max_value = None
            self.override_min_max(
                part, min_value, max_value
            )  # NB: this does not modify xml
        if min_value is None or max_value is None:
            return None, min_value, max_value
        if (
            "int" in str(p_array.dtype) and discrete_cycle is not None
        ):  # could use continuous flag in metadata instead of dtype
        ...
        min_value = float(
            min_value
        )  # will return np.ma.masked if all values are masked out
        if masked and min_value is ma.masked:
            min_value = np.nan
    ...
    def add_cached_array_to_imported_list(
        self,
        cached_array,
        source_info,
        keyword,
        ...
        assert (cached_array is not None and const_value is None) or (
            cached_array is None and const_value is not None
        )
        assert count > 0
    ...
    def create_xml_for_imported_list_and_add_parts_to_model(
        self,
        ext_uuid=None,
        support_uuid=None,
        time_series_uuid=None,
        ...
        for (
            ...
            if property_kind == "categorical":
                ...
                local_property_kind_uuid is not None and string_lookup_uuid is not None
            ):
                add_min_max = False
            else:
                add_min_max = True
    ...
    def create_xml(
        ...
        if indexable_element is None:
            if support_type == "obj_IjkGridRepresentation":
                ...
                raise Exception(
                    "indexable element unknown for unsupported supporting representation object"
                )
        ...
        if self.support is not None:
            ...
            if shape_list is not None:
                ...
                if property_array is not None:
                    assert (
                        tuple(shape_list) == property_array.shape
                    ), "property array does not have the correct shape"
        # todo: assertions:
        ...
        if time_series_uuid is None or time_index is None:
            ...
            related_time_series_node = self.model.root_for_part(
                rqet.part_name_for_object("obj_TimeSeries", time_series_uuid)
            )
            time_series = rts.TimeSeries(
                self.model, time_series_root=related_time_series_node
            )
            time_series.create_time_index(time_index, root=p_node)

        self.model.create_supporting_representation(
            support_uuid=support_uuid,
            root=p_node,
        ...
        if (
            find_local_property_kinds
            and property_kind not in supported_property_kind_list
        ):
        ...
        if property_kind_uuid is None:
            p_kind_node.set(
                ns["xsi"] + "type", ns["resqml2"] + "StandardPropertyKind"
            )  # todo: local prop kind ref
            ...
            p_kind_node.set(
                ns["xsi"] + "type", ns["resqml2"] + "LocalPropertyKind"
            )  # todo: local prop kind ref
        ...
        if add_min_max:
            ...
            if const_value is not None:
                if (discrete and const_value != null_value) or (
                    not discrete and not np.isnan(const_value)
                ):
                ...
                if discrete:
                    if min_value is None:
                        try:
                            ...
                            min_value = None
                            log.warning(
                                "no xml minimum value set for discrete property"
                            )
                    if max_value is None:
                        try:
                            ...
                            max_value = None
                            log.warning(
                                "no xml maximum value set for discrete property"
                            )
                else:
                    if min_value is None or max_value is None:
        ...
        if discrete:
            if string_lookup_uuid is not None:
                ...
                assert (
                    sl_root is not None
                ), "string table lookup is missing whilst importing categorical property"
                assert (
                    rqet.node_type(sl_root) == "obj_StringTableLookup"
                ), "referenced uuid is not for string table lookup"
                self.model.create_ref_node(
                    "Lookup",
            ...
            if not uom:
                ...
                if not uom:
                    ...
                    log.warning(
                        f"uom set to Euc for property {title} of kind {property_kind}"
                    )
    ...
    def create_property_set_xml(
        ...
        ), "cannot create xml for property set as model is not set"
        assert (
            self.number_of_parts() > 0
        ), "cannot create xml for property set as no parts in collection"
        ...
        if self.parent_set_root is not None:
            parent_set_ref_node = self.model.create_ref_node(
                "ParentSet",
        ...
        if add_as_part:
            ...
            if add_relationships:
                ...
                for prop_node in prop_node_list:
                    self.model.create_reciprocal_relationship(
                        ps_node, "destinationObject", prop_node, "sourceObject"
                    )

        return ps_node

    def basic_static_property_parts(
        self,
        realization=None,
        ...
        try:
            ntg_part = self.singleton(
                realization=realization, property_kind="net to gross ratio"
            )
        except:
            log.error(
                "problem with net to gross ratio data (more than one array present?)"
            )
            ntg_part = None
        try:
            poro_part = self.singleton(
                realization=realization, property_kind="porosity"
            )
        except:
            log.error("problem with porosity data (more than one array present?)")
            poro_part = None
        perms = selective_version_of_collection(
            self, realization=realization, property_kind="permeability rock"
        )
        if perms is None or perms.number_of_parts() == 0:
            ...
            if perms.number_of_parts() == 1:
                ...
                try:
                    ...
                    if not perm_i_part:
                        perm_i_part = perms.singleton(
                            facet_type="direction", facet="IJ"
                        )
                    if not perm_i_part:
                        perm_i_part = perms.singleton(
                            facet_type="direction", facet="IJK"
                        )
                    if not perm_i_part:
                    ...
                    if not perm_i_part:
                        log.error(
                            "unable to discern which rock permeability to use for I direction"
                        )
                except:
                    log.error(
                        "problem with permeability data (more than one I direction array present?)"
                    )
                    perm_i_part = None
                try:
                    perm_j_part = perms.singleton(facet_type="direction", facet="J")
                    if not perm_j_part:
                        perm_j_part = perms.singleton(
                            facet_type="direction", facet="IJ"
                        )
                    if not perm_j_part:
                        perm_j_part = perms.singleton(
                    ...
                    log.error(
                        "problem with permeability data (more than one J direction array present?)"
                    )
                    perm_j_part = None
                ...
                try:
                    ...
                    log.error(
                        "problem with permeability data (more than one K direction array present?)"
                    )
                    perm_k_part = None
                if perm_k_part is None:
                    assert perm_k_mode in [
                        None,
                        "none",
                        "shared",
                        "ratio",
                    ...
                    if perm_k_mode is None or perm_k_mode == "none":
                        ...
                        log.info(
                            "generating K permeability data using mode "
                            + str(perm_k_mode)
                        )
                        if perm_j_part is not None and perm_j_part != perm_i_part:
                            ...
                            kh = np.sqrt(
                                perms.cached_part_array_ref(perm_i_part)
                                * perms.cached_part_array_ref(perm_j_part)
                            )
                        ...
                        kv_collection.set_support(
                            support_uuid=self.support_uuid, model=self.model
                        )
                        kv_collection.add_cached_array_to_imported_list(
                            kv,
                            "derived from horizontal perm with mode "
                            + str(perm_k_mode),
                            "KK",
                            discrete=False,
                            uom="mD",
    ...
    def basic_static_property_parts_dict(
        self,
        realization=None,
        share_perm_parts=False,
        perm_k_mode=None,
        ...
        five_parts = self.basic_static_property_parts(
            realization=realization,
    ...
    def basic_static_property_uuids(
        self,
        realization=None,
        share_perm_parts=False,
        perm_k_mode=None,
    ...
    def basic_static_property_uuids_dict(
        self,
        realization=None,
        share_perm_parts=False,
        perm_k_mode=None,
        ...
        five_uuids = self.basic_static_property_uuids(
            realization=realization,
            share_perm_parts=share_perm_parts,
            perm_k_mode=perm_k_mode,
            perm_k_ratio=perm_k_ratio,
...
class GridPropertyCollection(PropertyCollection):
    ...
    def __init__(self, grid=None, property_set_root=None, realization=None):
        ...
        if grid is not None:
            log.debug(
                "initialising grid property collection for grid: "
                + str(rqet.citation_title_for_node(grid.grid_root))
            )
            log.debug("grid uuid: " + str(grid.uuid))
        super().__init__(
            support=grid, property_set_root=property_set_root, realization=realization
        )
    ...
    def extend_imported_list_copying_properties_from_other_grid_collection(
        self,
        other,
        box=None,
        refinement=None,
        ...
        def coarsening_weighted_mean(
            coarsening, a, fine_weight, coarse_weight=None, zero_weight_result=np.NaN
        ):
            a_coarsened = np.empty(tuple(coarsening.coarse_extent_kji))
            assert a.shape == tuple(coarsening.fine_extent_kji)
            ...
            if coarse_weight is not None:
                ...
                a_coarsened = np.where(
                    mask, zero_weight_result, a_coarsened / coarse_weight
                )
            return a_coarsened

        def add_to_imported(
            collection, a, title, info, null_value=None, const_value=None
        ):
        ...
        assert other.support is not None and isinstance(
            other.support, grr.Grid
        ), "other property collection has no grid support"
        assert (
            refinement is None or coarsening is None
        ), "refinement and coarsening both specified simultaneously"

        if box is not None:
            assert bxu.valid_box(box, other.grid.extent_kji)
            if refinement is not None:
                assert tuple(bxu.extent_of_box(box)) == tuple(
                    refinement.coarse_extent_kji
                )
            elif coarsening is not None:
                assert tuple(bxu.extent_of_box(box)) == tuple(
                    coarsening.fine_extent_kji
                )
        # todo: any contraints on realization numbers ?

        if (
            coarsening is not None
        ):  # static upscaling of key property kinds, simple sampling of others
            ...
            source_rv = selective_version_of_collection(
                other, realization=realization, property_kind="rock volume"
            )
            source_ntg = selective_version_of_collection(
                other, realization=realization, property_kind="net to gross ratio"
            )
            source_poro = selective_version_of_collection(
                other, realization=realization, property_kind="porosity"
            )
            source_sat = selective_version_of_collection(
                other, realization=realization, property_kind="saturation"
            )
            source_perm = selective_version_of_collection(
                other, realization=realization, property_kind="permeability rock"
            )
            ...
            if source_rv.number_of_parts() == 0:
                log.debug(
                    "computing bulk rock volume from fine and coarse grid geometries"
                )
                ...
                for (part, info) in source_rv.dict.items():
                    if not copy_all_realizations and info[0] != realization:
                        continue
                    fine_rv_array = array_box(
                        other, part, box=box, uncache_other_arrays=uncache_other_arrays
                    )
                    coarse_rv_array = coarsening_sum(coarsening, fine_rv_array)
                    add_to_imported(
                        self,
                        coarse_rv_array,
                        "coarsened from grid " + str(other.support.uuid),
                        info,
                    )
            ...
            for (part, info) in source_ntg.dict.items():
                if not copy_all_realizations and info[0] != realization:
                    continue
                fine_ntg_array = array_box(
                    other, part, box=box, uncache_other_arrays=uncache_other_arrays
                )
                coarse_ntg_array = coarsening_weighted_mean(
                    coarsening,
                    fine_ntg_array,
                    fine_rv_array,
            ...
            for (part, info) in source_poro.dict.items():
                if not copy_all_realizations and info[0] != realization:
                    continue
                fine_poro_array = array_box(
                    other, part, box=box, uncache_other_arrays=uncache_other_arrays
                )
                coarse_poro_array = coarsening_weighted_mean(
                    coarsening,
                    fine_poro_array,
                    fine_nrv_array,
            ...
            for (part, info) in source_sat.dict.items():
                if not copy_all_realizations and info[0] != realization:
                    continue
                fine_sat_array = array_box(
                    other, part, box=box, uncache_other_arrays=uncache_other_arrays
                )
                coarse_sat_array = coarsening_weighted_mean(
                    coarsening,
                    fine_sat_array,
                    fine_sat_weight,
            ...
            for (part, info) in source_perm.dict.items():
                if not copy_all_realizations and info[0] != realization:
                    continue
                fine_perm_array = array_box(
                    other, part, box=box, uncache_other_arrays=uncache_other_arrays
                )
                coarse_perm_array = coarsening_weighted_mean(
                    coarsening,
                    fine_perm_array,
                    fine_nrv_array,
                    coarse_weight=coarse_nrv_array,
            ...
            if realization is None:
                ...
                source_collection = selective_version_of_collection(
            ...
            for (part, info) in source_collection.dict.items():
                ...
                if const_value is None:
                    a = array_box(
                        ...
                        part,
                        box=box,
                        uncache_other_arrays=uncache_other_arrays,
                    )
                else:
                    a = None
                ...
                if refinement is not None and a is not None:  # simple resampling
                    ...
                    assert self.support is not None and tuple(
                        self.support.extent_kji
                    ) == tuple(refinement.fine_extent_kji)
                    k_ratio_vector = refinement.coarse_for_fine_axial_vector(0)
                    a_refined_k = np.empty(
                        (
                            refinement.fine_extent_kji[0],
                            refinement.coarse_extent_kji[1],
    ...
    def import_nexus_property_to_cache(
        self,
        file_name,
        keyword,
        extent_kji=None,
        ...
        if discrete:
            ...
            data_type = "real"
        try:
            import_array = ld.load_array_from_file(
                file_name,
                extent_kji,
                data_type=data_type,
                comment_char="!",
            ...
            log.exception(
                "failed to import {} arrau from file {}".format(keyword, file_name)
            )
            return None
    ...
    def import_vdb_static_property_to_cache(
        self, vdbase, keyword, grid_name="ROOT", uom=None, realization=None
    ):
        ...
        try:
            ...
            if keyword[0].upper() == "I" or keyword in ["KID", "UID", "UNPACK", "DAD"]:
                ...
                dtype = (
                    "float"  # convert to 64 bit; could omit but RESQML states 64 bit
                )
                discrete = False
    ...
    def import_vdb_recurrent_property_to_cache(
        self,
        vdbase,
        timestep,
        keyword,
        ...
        log.info(
            "importing vdb recurrent property {0} array for timestep {1}".format(
                keyword, str(timestep)
            )
        )
        ...
        keyword = keyword.upper()
        try:
            import_array = vdbase.grid_recurrent_property_for_timestep(
                grid_name, keyword, timestep, dtype="float"
            )
            assert import_array is not None
        except:
            # could raise an exception (as for static properties)
            log.error(
                f"failed to import recurrent property {keyword} from vdb for timestep {timestep}"
            )
            return None
    ...
    def import_ab_property_to_cache(
        self,
        file_name,
        keyword,
        extent_kji=None,
        ...
        if extent_kji is None:
            extent_kji = self.grid.extent_kji
        assert file_name[-3:] in [".db", ".fb", ".ib", ".lb", ".bb"], (
            "file extension not in pure binary array expected set for: " + file_name
        )
        if discrete is None:
            discrete = file_name[-3:] in [".ib", ".lb", ".bb"]
        else:
            assert discrete == (file_name[-3:] in [".ib", ".lb", ".bb"]), (
                "discrete argument is not consistent with file extension for: "
                + file_name
            )
        try:
            import_array = abt.load_array_from_ab_file(
                file_name, extent_kji, return_64_bit=False
            )  # todo: RESQML indicates 64 bit for everything
        except:
            log.exception(
    ...
    def decoarsen_imported_list(self, decoarsen_array=None, reactivate=True):
        ...
        if decoarsen_array is None:
            for import_item in self.imported_list:
                if (
                    (import_item[14] is None or import_item[14] == "cells")
                    and import_item[4]
                    and hasattr(self, import_item[3])
                ):
                    if import_item[2] == "ICOARS":
                        decoarsen_array = (
                            self.__dict__[import_item[3]] - 1
                        )  # ICOARS values are one based
                        break
        ...
        if decoarsen_array is None and kid_attr_name is not None:
            ...
            if np.any(kid_mask):
                log.debug(
                    f"{np.count_nonzero(kid_mask)} cells marked as requiring decoarsening in KID data"
                )
                    ...
                    for j0 in range(self.grid.nj):
                        for i0 in range(self.grid.ni):
                            ...
                            if kid[k0, j0, i0] == 0:
                                ...
                                k0, j0, i0
                            ]:  # inactive for reasons other than coarsening
        ...
        log.debug(
            f"{host_count} of {cell_count} are hosts; difference is {cell_count - host_count}"
        )
        ...
        if k_share is None:
            ...
            for import_item in self.imported_list:
                kind = import_item[10]
                if (
                    kind in decoarsen_volume_kinds
                    or kind in decoarsen_area_kinds
                    or kind in decoarsen_length_kinds
                ):
                    sharing_needed = True
                    break
            if sharing_needed:
                    ...
                    for j0 in range(self.grid.nj):
                        for i0 in range(self.grid.ni):
                            if k_share[k0, j0, i0] == 0:
                                ...
                                while (
                                    ke < self.grid.nk
                                    and decoarsen_array[ke, j0, i0] == natural
                                ):
                                    ke += 1
                                je = j0 + 1
                                while (
                                    je < self.grid.nj
                                    and decoarsen_array[k0, je, i0] == natural
                                ):
                                    je += 1
                                ie = i0 + 1
                                while (
                                    ie < self.grid.ni
                                    and decoarsen_array[k0, j0, ie] == natural
                                ):
                                    ie += 1
                                k_share[k0:ke, j0:je, i0:ie] = ke - k0
                                j_share[k0:ke, j0:je, i0:ie] = je - j0
        ...
        for import_item in self.imported_list:
            ...
            if kind in decoarsen_volume_kinds:
                ...
                if kind in ["thickness", "permeability thickness"] or (
                    facet_dir == "K"
                ):
                    ...
                    log.warning(
                        f"decoarsening of length property {keyword} skipped as direction not established"
                    )
            else:
        ...
        if reactivate and hasattr(self.grid, "inactive"):
            ...
            self.grid.inactive = self.grid.inactive.flatten()[decoarsen_array].reshape(
                self.grid.extent_kji
            )
    ...
    def write_nexus_property_generating_filename(
        ...
        if facet_type is not None:
            fname += (
                "_"
                + facet_type.replace(" ", "_")
                + "_"
                + self.facet_for_part(part).replace(" ", "_")
            )
        time_index = self.time_index_for_part(part)
        if time_index is not None:
            fname += "_t_" + str(time_index)
        # could add .dat extension
        self.write_nexus_property(
            part,
    ...
    def write_nexus_collection(
        ...
        for part in self.dict.keys():
            self.write_nexus_property_generating_filename(
                part,
                directory,
...
class WellLogCollection(PropertyCollection):
    ...
    def __init__(self, frame=None, property_set_root=None, realization=None):
        ...
        super().__init__(
            support=frame, property_set_root=property_set_root, realization=realization
        )
        ...
        for log in self.logs():
            ...
            if values.ndim > 1:
                raise NotImplementedError(
                    "Multidimensional logs not yet supported in pandas"
                )
...
class WellLog:
    ...
    def __init__(self, collection, part):
        ...
        if indexable != "nodes":
            raise NotImplementedError(
                "well frame related property does not have nodes as indexable element"
            )
...
class StringLookup:
    ...
    def __init__(self, parent_model, root_node=None, int_to_str_dict=None, title=None):
        ...
        if root_node is not None:
            ...
            for v_node in rqet.list_of_tag(root_node, "Value"):
                ...
                assert key not in self.str_dict, (
                    "key value "
                    + str(key)
                    + " occurs more than once in string lookup table xml"
                )
            ...
            assert len(
                int_to_str_dict
            ), "empty dictionary passed to string lookup initialisation"
    ...
    def is_equivalent(self, other):
        ...
        if (
            ...
            or self.min_index != other.min_index
            or self.max_index != other.max_index
        ):
            return False
    ...
    def set_list_from_dict_conditionally(self):
        ...
        self.str_list = []
        self.stored_as_list = False
        if self.min_index >= 0 and (
            self.max_index < 50 or 10 * len(self.str_dict) // self.max_index > 8
        ):
    ...
    def create_xml(self, title=None, originator=None, add_as_part=True):
        ...
        self.model.create_citation(
            root=sl_node, title=self.title, originator=originator
        )
...
class PropertyKind:
    ...
    def __init__(
        ...
        if root_node is not None:
            ...
            assert (
                ppk_kind_node is not None
            ), "only standard property kinds supported as parent kind"
    ...
    def is_equivalent(self, other_pk):
        ...
        if bu.matching_uuids(self.uuid, other_pk.uuid):
            return True
        return (
            self.title == other_pk.title
            and self.is_abstract == other_pk.is_abstract
            and self.naming_system == other_pk.naming_system
            and self.example_uom == other_pk.example_uom
            and self.parent_kind == other_pk.parent_kind
        )
...
class WellIntervalProperty:
    ...
    def __init__(self, collection, part):
        ...
        assert indexable in [
            "cells",
            "intervals",
        ], "expected cells or intervals as indexable element"
...
class WellIntervalPropertyCollection(PropertyCollection):
    ...
    def logs(self):
        ...
        return (
            WellIntervalProperty(collection=self, part=part) for part in self.parts()
        )

    def to_pandas(self, include_units=False):
        cell_indices = [
            return_cell_indices(i, self.support.cell_indices)
            for i in self.support.cell_grid_link
        ]
        data = {}
        for log in self.logs():
            col_name = log.name
...
def same_property_kind(pk_a, pk_b):
    ...
    if pk_a == pk_b:
        return True
    if pk_a in ["permeability rock", "rock permeability"] and pk_b in [
        "permeability rock",
        "rock permeability",
    ]:
        return True
...
def property_kind_and_facet_from_keyword(keyword):
    ...
    if lk in ["bv", "brv"]:  # bulk rock volume
        ...
        property_kind = (
            "thickness"  # or should these keywords use cell length in K direction?
        )
        facet_type = "netgross"
        ...
        len(lk) == 2 and lk[0] == "t"
    ):  # transmissibility (for unit viscosity)
    ...
    #   elif lk == 'sal':    # todo: salinity; local property kind needed; various units possible in Nexus
        ...
        "mobg",
        "ocip",
    ]:  # todo: check these, especially ocip
...
def guess_uom(property_kind, minimum, maximum, support, facet_type=None, facet=None):
    ...
    def crs_m_or_ft(
        crs_node,
    ):  # NB. models not-so-rarely use metres for xy and feet for z
        if crs_node is None:
            return None
    ...
    if (
        property_kind == "cell length"
    ):  # todo: pass in direction facet to pick up xy_units or z_units
        return from_crs
...
def selective_version_of_collection(
    ...
    view.inherit_parts_selectively_from_other_collection(
        collection,
        realization=realization,
        support_uuid=support_uuid,
        uuid=uuid,
...
def property_over_time_series_from_collection(collection, example_part):
    ...
    view.inherit_similar_parts_for_time_series_from_other_collection(
        collection, example_part
    )
    return view
...
def property_collection_for_keyword(collection, keyword):
    ...
    if property_kind is None:
        ...
        return None
    return selective_version_of_collection(
        collection, property_kind=property_kind, facet_type=facet_type, facet=facet
    )
