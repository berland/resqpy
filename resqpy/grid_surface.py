...
class GridSkin:
    ...
    def __init__(self, grid, quad_triangles=True, use_single_layer_tactics=True):
        ...
        if self.has_split_coordinate_lines:
            ...
            top_surf = generate_torn_surface_for_layer_interface(
                grid, k0=0, ref_k_faces="top", quad_triangles=quad_triangles
            )
            base_surf = generate_torn_surface_for_layer_interface(
                grid, k0=grid.nk - 1, ref_k_faces="base", quad_triangles=quad_triangles
            )
            j_minus_surf = generate_torn_surface_for_x_section(
                grid,
                "J",
                ref_slice0=0,
                plus_face=False,
            ...
            j_plus_surf = generate_torn_surface_for_x_section(
                grid,
                "J",
            ...
            i_minus_surf = generate_torn_surface_for_x_section(
                grid,
                "I",
                ref_slice0=0,
                plus_face=False,
            ...
            i_plus_surf = generate_torn_surface_for_x_section(
                grid,
                "I",
            ...
            self.fault_j_face_cols_ji0 = np.stack(
                (col_j0, col_i0), axis=-1
            )  # fault is on plus j face of these columns
            col_j0, col_i0 = np.where(i_column_faces)
            self.fault_i_face_cols_ji0 = np.stack(
                (col_j0, col_i0), axis=-1
            )  # fault is on plus i face of these columns
            ...
            for col_ji0 in self.fault_j_face_cols_ji0:
                ...
                _, surf = create_column_face_mesh_and_surface(
                    grid,
                    col_ji0,
                    1,
                    1,
            ...
            if len(j_minus_fault_surf_list) > 0:
                j_minus_fault_surf = rqs.CombinedSurface(
                    j_minus_fault_surf_list, grid.crs_uuid
                )
            if len(j_plus_fault_surf_list) > 0:
                j_plus_fault_surf = rqs.CombinedSurface(
                    j_plus_fault_surf_list, grid.crs_uuid
                )
            ...
            if len(i_minus_fault_surf_list) > 0:
                i_minus_fault_surf = rqs.CombinedSurface(
                    i_minus_fault_surf_list, grid.crs_uuid
                )
            if len(i_plus_fault_surf_list) > 0:
                i_plus_fault_surf = rqs.CombinedSurface(
                    i_plus_fault_surf_list, grid.crs_uuid
                )
            ...
            top_surf = generate_untorn_surface_for_layer_interface(
                grid, k0=0, ref_k_faces="top", quad_triangles=quad_triangles
            )
            base_surf = generate_untorn_surface_for_layer_interface(
                grid, k0=grid.nk - 1, ref_k_faces="base", quad_triangles=quad_triangles
            )
            j_minus_surf = generate_untorn_surface_for_x_section(
                grid,
                "J",
                ref_slice0=0,
                plus_face=False,
            ...
            j_plus_surf = generate_untorn_surface_for_x_section(
                grid,
                "J",
                ref_slice0=self.grid.nj - 1,
                plus_face=True,
            ...
            i_minus_surf = generate_untorn_surface_for_x_section(
                grid,
                "I",
                ref_slice0=0,
                plus_face=False,
            ...
            i_plus_surf = generate_untorn_surface_for_x_section(
                grid,
                "I",
                ref_slice0=self.grid.ni - 1,
                plus_face=True,
    ...
    def find_first_intersection_of_trajectory(
        self, trajectory, start=0, start_xyz=None, nudge=None, exclude_kji0=None
    ):
        ...
        if xyz_2 is None:
            ...
            triplet_list = [
                (xyz_1, segment_1, tri_index_1),
                (xyz_2, segment_2, tri_index_2),
            ]

        for (xyz, segment, tri_index) in triplet_list:

            surf_index, surf_tri_index = self.skin.surface_index_for_triangle_index(
                tri_index
            )
            ...
            if surf_index < 6:  # grid skin
                ...
                col = self.skin.surface_list[surf_index].column_from_triangle_index(
                    surf_tri_index
                )
                if surf_index == 0:
                ...
                if (
                    self.use_single_layer_tactics and surf_index > 1
                ):  # now compare against layered representation of column face
                    xyz, k0 = find_intersection_of_trajectory_interval_with_column_face(
                        trajectory,
                        self.grid,
                        segment,
                        kji0[1:],
                    ...
                    if xyz is None:
                        log.error(
                            "unexpected failure to identify skin column face penetration point"
                        )
                        return None, None, None, None, None
                ...
                axis += 1
                log.debug(
                    "penetration through fault face " + "KJI"[axis] + "-+"[polarity]
                )
                assert self.skin.is_combined_list[surf_index]
                fault_surf_list = self.skin.surface_list[surf_index]
                col_surf_index, _ = fault_surf_list.surface_index_for_triangle_index(
                    surf_tri_index
                )
                ...
                if polarity == 0:
                    col_ji0[axis - 1] += 1
                xyz_f, k0 = find_intersection_of_trajectory_interval_with_column_face(
                    trajectory,
                    self.grid,
                    segment,
                    col_ji0,
                ...
                if xyz_f is None:
                    if self.k_gaps:
                        log.debug(
                            "failed to identify fault penetration point; assumed to be in k gap; nudging forward"
                        )
                        if nudge is None or nudge < 0.0:
                            nudge = 0.0
                        nudge += 0.1
                        return self.find_first_intersection_of_trajectory(
                            trajectory,
                            start=segment,
                            start_xyz=xyz,
                            nudge=nudge,
        ...
        if self.k_gaps > 0:
            for k0 in range(self.grid.nk - 1):
                if self.grid.k_gap_after_array[k0]:
                    if self.has_split_coordinate_lines:
                        gap_top_surf = generate_torn_surface_for_layer_interface(
                            self.grid,
                            k0=k0,
                            ref_k_faces="base",
                            quad_triangles=quad_triangles,
                        )
                        gap_base_surf = generate_torn_surface_for_layer_interface(
                            self.grid,
                            k0=k0 + 1,
                            ref_k_faces="top",
                            quad_triangles=quad_triangles,
                        ...
                        gap_top_surf = generate_untorn_surface_for_layer_interface(
                            self.grid,
                            k0=k0,
                            ref_k_faces="base",
                            quad_triangles=quad_triangles,
                        )
                        gap_base_surf = generate_untorn_surface_for_layer_interface(
                            self.grid,
                            k0=k0 + 1,
                            ref_k_faces="top",
                            quad_triangles=quad_triangles,
...
def generate_untorn_surface_for_layer_interface(
    grid, k0=0, ref_k_faces="top", quad_triangles=True, border=None
):
    ...
    if border is None or border <= 0.0:
        ...
        for j in range(1, mesh.shape[0] + 1):
            skirted_mesh[j, 0, :] = skirted_mesh[j, 1] + border * vec.unit_vector(
                skirted_mesh[j, 1] - skirted_mesh[j, 2]
            )
            skirted_mesh[j, -1, :] = skirted_mesh[j, -2] + border * vec.unit_vector(
                skirted_mesh[j, -2] - skirted_mesh[j, -3]
            )
        for i in range(1, mesh.shape[1] + 1):
            skirted_mesh[0, i, :] = skirted_mesh[1, i] + border * vec.unit_vector(
                skirted_mesh[1, i] - skirted_mesh[2, i]
            )
            skirted_mesh[-1, i, :] = skirted_mesh[-2, i] + border * vec.unit_vector(
                skirted_mesh[-2, i] - skirted_mesh[-3, i]
            )
        # fill in corner values
        skirted_mesh[0, 0, :] = (
            skirted_mesh[0, 1] + skirted_mesh[1, 0] - skirted_mesh[1, 1]
        )
        skirted_mesh[0, -1, :] = (
            skirted_mesh[0, -2] + skirted_mesh[1, -1] - skirted_mesh[1, -2]
        )
        skirted_mesh[-1, 0, :] = (
            skirted_mesh[-1, 1] + skirted_mesh[-2, 0] - skirted_mesh[-2, 1]
        )
        skirted_mesh[-1, -1, :] = (
            skirted_mesh[-1, -2] + skirted_mesh[-2, -1] - skirted_mesh[-2, -2]
        )
...
def generate_torn_surface_for_layer_interface(
    grid, k0=0, ref_k_faces="top", quad_triangles=True
):
...
def generate_torn_surface_for_x_section(
    grid,
    axis,
    ref_slice0=0,
    plus_face=False,
    ...
    if grid.k_gaps is None or grid.k_gaps == 0:
        x_sect_points = grid.split_x_section_points(
        ...
        x_sect_mesh = grid.split_gap_x_section_points(
            axis, ref_slice0=ref_slice0, plus_face=plus_face
        )
...
def generate_untorn_surface_for_x_section(
    grid,
    axis,
    ref_slice0=0,
    plus_face=False,
    ...
    x_sect_points = grid.x_section_points(
        axis, ref_slice0=ref_slice0, plus_face=plus_face
    )
    if as_single_layer:
        shape = np.array(x_sect_points.shape)
    ...
    log.debug(
        f"x_sect_mesh.shape: {x_sect_mesh.shape}; grid.extent_kji: {grid.extent_kji}"
    )
...
def find_intersections_of_trajectory_with_surface(trajectory, surface):
    ...
    for traj in trajectory_list:
        all_intersects = meet.poly_line_triangles_intersects(
            traj.control_points, triangles
        )
        _, triangle_indices, intersect_points = meet.distilled_intersects(
            all_intersects
        )  # discard trajectory segment info
...
def find_intersections_of_trajectory_with_layer_interface(
    trajectory, grid, k0=0, ref_k_faces="top", heal_faults=True, quad_triangles=True
):
    ...
    if grid.has_split_coordinate_lines and not heal_faults:
        surface = generate_torn_surface_for_layer_interface(
            grid, k0=k0, ref_k_faces=ref_k_faces, quad_triangles=quad_triangles
        )
    else:
        surface = generate_untorn_surface_for_layer_interface(
            grid, k0=k0, ref_k_faces=ref_k_faces, quad_triangles=quad_triangles
        )

    tri_intersect_list = find_intersections_of_trajectory_with_surface(
        trajectory_list, surface
    )
    ...
    for triangle_indices, intersect_points in tri_intersect_list:
        if triangle_indices is None or intersect_points is None:
            ...
            j_list, i_list = surface.column_from_triangle_index(
                triangle_indices
            )  # todo: check this is valid for torn surfaces
            assert (
                j_list is not None
            ), "failed to derive column indices from triangle indices"
...
def find_first_intersection_of_trajectory_with_surface(
    trajectory, surface, start=0, start_xyz=None, nudge=None, return_second=False
):
    ...
    if knot < trajectory.knot_count - 2 and vec.isclose(
        start_xyz, trajectory.control_points[knot + 1]
    ):
        knot += 1
    ...
    while knot < trajectory.knot_count - 1:
        ...
        intersects = meet.line_triangles_intersects(
            line_p, line_v, triangles, line_segment=True
        )
        if not np.all(np.isnan(intersects)):
            ...
            if len(intersects_indices) > 1:
                ...
                for other_intersect_index_index in range(1, len(intersects_indices)):
                    ...
                    elif return_second and (
                        runner_up is None or other_manhattan < runner_up
                    ):
                        tri_2 = other_tri
                        xyz_2 = other_xyz
...
def find_first_intersection_of_trajectory_with_layer_interface(
    trajectory,
    grid,
    k0=0,
    ref_k_faces="top",
    ...
    col_list = []
    for traj in trajectory_list:
        xyz, knot, tri = find_first_intersection_of_trajectory_with_surface(
            traj, surface, start=start
        )
...
def find_first_intersection_of_trajectory_with_cell_surface(
    trajectory, grid, kji0, start_knot, start_xyz=None, nudge=0.001, quad_triangles=True
):
    ...
    cell_surface.set_to_single_cell_faces_from_corner_points(
    ...
    while knot < trajectory.knot_count - 1:
        ...
        if not np.all(np.isnan(intersects)):
            ...
            xyz = intersects[tri]
            _, axis, polarity = cell_surface.cell_axis_and_polarity_from_triangle_index(
                tri
            )
            log.debug(
                "intersection xyz: "
...
def point_is_within_cell(xyz, grid, kji0, cell_surface=None, false_on_pinchout=True):
    ...
    line_v = centre - xyz
    intersects = meet.line_triangles_intersects(
        xyz, line_v, triangles, line_segment=True
    )
    return np.all(np.isnan(intersects))


def create_column_face_mesh_and_surface(
    grid, col_ji0, axis, polarity, quad_triangles=True, as_single_layer=False
):
    ...
    if not as_single_layer and grid.k_gaps:
        ...
        mesh[0, :, 0, 1, :] = points[
            grid.k_raw_index_array + 1, pillar_index_pair[0], :
        ]
        mesh[0, :, 1, 1, :] = points[
            grid.k_raw_index_array + 1, pillar_index_pair[1], :
        ]
        ...
        col_face_mesh = rqs.Mesh(
            grid.model, xyz_values=col_face_xyz, crs_uuid=grid.crs_uuid
        )
        title = (
            "column face for j0,i0: "
            + str(col_ji0[0])
            + ","
            + str(col_ji0[1])
            + " face "
...
def find_intersection_of_trajectory_interval_with_column_face(
    trajectory,
    grid,
    start_knot,
    col_ji0,
    ...
    _, col_face_surface = create_column_face_mesh_and_surface(
        grid, col_ji0, axis, polarity, quad_triangles=quad_triangles
    )
    t, p = col_face_surface.triangles_and_points()
    triangles = p[t]
    log.debug(
        f"intersecting trajectory segment with column face ji0 {col_ji0} face {'KJI'[axis]}{'-+'[polarity]}"
    )
    if start_xyz is not None:
        if (
            nudge is not None and nudge != 0.0
        ):  # here we typically nudge back up the wellbore
            start_xyz += nudge * vec.unit_vector(
                trajectory.control_points[start_knot + 1] - start_xyz
            )
        line_p = start_xyz
    else:
    ...
    if np.all(np.isnan(intersects)):
        return None, None
    tri = meet.intersects_indices(intersects)[
        0
    ]  # todo: if more than one, find one closest to line_p
    xyz = intersects[tri]
    _, k0 = col_face_surface.column_from_triangle_index(
        tri
    )  # 'column' in method name here maps to layers!
    return xyz, k0


def find_faces_to_represent_surface_elephantine(grid, surface, name):
    ...
    faces_df = pd.DataFrame(
        columns=["name", "i1", "i2", "j1", "j2", "k1", "k2", "face"]
    )
    for axis in range(3):  # k, j, i
        face_letter = "KJI"[axis]
        ap = np.zeros(3, dtype=int)
        ap[axis] = 1
        #      log.debug('considering axis: ' + str(axis))
        if grid.extent_kji[axis] == 1:
            continue
        #      log.debug('computing facial vectors')
        facial_vectors = (
            grid.array_centre_point[
                : grid.nk - ap[0], : grid.nj - ap[1], : grid.ni - ap[2]
            ]
        ...
        full_intersects = meet.line_set_triangles_intersects(
            line_ps, line_vs, triangles, line_segment=True
        )
        #      log.debug('distilling intersections')
        distilled_intersects, _, _ = meet.distilled_intersects(full_intersects)
        k0s, ji0s = np.divmod(
            distilled_intersects, (grid.nj - ap[1]) * (grid.ni - ap[2])
        )
        j0s, i0s = np.divmod(ji0s, grid.ni - ap[2])
        denaturalised_distillation = np.stack(
            (k0s, j0s, i0s, k0s, j0s, i0s), axis=-1
        ).reshape((-1, 6))
        denaturalised_distillation[:, 3 + axis] += 1
        #      log.debug('appending to faces dataframe')
        for i in range(denaturalised_distillation.shape[0]):
            r = {}
            r["name"] = surface.title
            r["i1"] = denaturalised_distillation[i, 2] + 1
            r["j1"] = denaturalised_distillation[i, 1] + 1
            r["k1"] = denaturalised_distillation[i, 0] + 1
            r["i2"] = denaturalised_distillation[i, 5] + 1
...
def find_faces_to_represent_surface_loopy(grid, surface, name, progress_fn=None):
    """Returns a grid connection set containing those cell faces which are deemed to represent the surface."""
    ...
    if grid.ni > 1:
        i_vectors = (
            grid.array_centre_point[:, :, 1:] - grid.array_centre_point[:, :, :-1]
        )
        l = grid.array_centre_point[:, :, :-1]
        u = grid.array_centre_point[:, :, 1:]
        i_vectors = l - u
    ...
    progress_period = triangle_count // 100
    if progress_fn is not None:
        progress_fn(0.0)
    for ti in range(triangle_count):
        if log_count >= progress_period:
            #         log.debug('triangles processed: ' + str(ti))
            #         log.debug('interim face counts: K: ' + str(np.count_nonzero(k_faces)) +
            #                                      '; J: ' + str(np.count_nonzero(j_faces)) +
            #                                      '; I: ' + str(np.count_nonzero(i_faces)))
            log_count = 0
            if progress_fn is not None:
                progress_fn(float(ti) / float(triangle_count))
        log_count += 1
        if not bx.boxes_overlap(grid_box, triangle_boxes[ti]):
            skip_count += 1
            continue
        for k in range(grid.nk):
            for j in range(grid.nj):
                for i in range(grid.ni):
                    if (
                        k < grid.nk - 1
                        and not k_faces[k, j, i]
                        and bx.boxes_overlap(
                            triangle_boxes[ti], k_vector_boxes[k, j, i]
                        )
                    ):
                        meeting = meet.line_triangle_intersect(
                            grid.array_centre_point[k, j, i],
                            k_vectors[k, j, i],
                            triangles[ti],
                            line_segment=True,
                        )
                    ...
                    if (
                        j < grid.nj - 1
                        and not j_faces[k, j, i]
                        and bx.boxes_overlap(
                            triangle_boxes[ti], j_vector_boxes[k, j, i]
                        )
                    ):
                        meeting = meet.line_triangle_intersect(
                            grid.array_centre_point[k, j, i],
                    ...
                    if (
                        i < grid.ni - 1
                        and not i_faces[k, j, i]
                        and bx.boxes_overlap(
                            triangle_boxes[ti], i_vector_boxes[k, j, i]
                        )
                    ):
                        meeting = meet.line_triangle_intersect(
                            grid.array_centre_point[k, j, i],
...
def find_faces_to_represent_surface_staffa(grid, surface, name, progress_fn=None):
    ...
    if grid.nk > 1:
        ...
        column_k_vector_boxes[:, :, 0, :] = np.amin(
            k_vector_boxes[:, :, :, 0, :], axis=0
        )
        column_k_vector_boxes[:, :, 1, :] = np.amax(
            k_vector_boxes[:, :, :, 1, :], axis=0
        )
        k_faces = np.zeros((grid.nk - 1, grid.nj, grid.ni), dtype=bool)
    else:
        k_vectors = None
    ...
    if grid.nj > 1:
        ...
        column_j_vector_boxes[:, :, 0, :] = np.amin(
            j_vector_boxes[:, :, :, 0, :], axis=0
        )
        column_j_vector_boxes[:, :, 1, :] = np.amax(
            j_vector_boxes[:, :, :, 1, :], axis=0
        )
        j_faces = np.zeros((grid.nk, grid.nj - 1, grid.ni), dtype=bool)
    else:
        j_vectors = None
    ...
    if grid.ni > 1:
        ...
        column_i_vector_boxes[:, :, 0, :] = np.amin(
            i_vector_boxes[:, :, :, 0, :], axis=0
        )
        column_i_vector_boxes[:, :, 1, :] = np.amax(
            i_vector_boxes[:, :, :, 1, :], axis=0
        )
        i_faces = np.zeros((grid.nk, grid.nj, grid.ni - 1), dtype=bool)
    else:
        i_vectors = None
    ...
    while ti_base < triangle_count:
        ...
        if bx.boxes_overlap(grid_box, batch_box):
            for j in range(grid.nj):
                if progress_fn is not None:
                    progress_fn(
                        progress_base + progress_batch * (float(j) / float(grid.nj))
                    )
                for i in range(grid.ni):
                    if column_k_vector_boxes is not None and bx.boxes_overlap(
                        batch_box, column_k_vector_boxes[j, i]
                    ):
                        full_intersects = meet.line_set_triangles_intersects(
                            grid.array_centre_point[:-1, j, i],
                            k_vectors[:, j, i],
                            triangles[ti_base:ti_end],
                            line_segment=True,
                        )
                        distilled_intersects, _, _ = meet.distilled_intersects(
                            full_intersects
                        )
                        k_faces[distilled_intersects, j, i] = True
                    if (
                        j < grid.nj - 1
                        and column_j_vector_boxes is not None
                        and bx.boxes_overlap(batch_box, column_j_vector_boxes[j, i])
                    ):
                        full_intersects = meet.line_set_triangles_intersects(
                            grid.array_centre_point[:, j, i],
                            j_vectors[:, j, i],
                            triangles[ti_base:ti_end],
                            line_segment=True,
                        )
                    ...
                    if (
                        i < grid.ni - 1
                        and column_i_vector_boxes is not None
                        and bx.boxes_overlap(batch_box, column_i_vector_boxes[j, i])
                    ):
                        full_intersects = meet.line_set_triangles_intersects(
                            grid.array_centre_point[:, j, i],
                            i_vectors[:, j, i],
...
def find_faces_to_represent_surface(
    grid, surface, name, mode="staffa", progress_fn=None
):
    """Returns a grid connection set containing those cell faces which are deemed to represent the surface."""

    log.debug("finding cell faces for surface")
    if mode == "elephantine":
        return find_faces_to_represent_surface_elephantine(grid, surface, name)
    if mode == "loopy":
        return find_faces_to_represent_surface_loopy(
            grid, surface, name, progress_fn=progress_fn
        )
    if mode == "staffa":
        return find_faces_to_represent_surface_staffa(
            grid, surface, name, progress_fn=progress_fn
        )
...
def generate_surface_for_blocked_well_cells(
    blocked_well,
    combined=True,
    active_only=False,
    min_k0=0,
    ...
    for cell in range(cell_count):
        ...
        if combined:
            ...
            cs.set_to_single_cell_faces_from_corner_points(
                cp, quad_triangles=quad_triangles
            )
...
def populate_blocked_well_from_trajectory(
    blocked_well,
    grid,
    active_only=False,
    quad_triangles=True,
    ...
    def find_next_cell(
        ...
        if (
            np.any(kji0 < 0)
            or np.any(kji0 >= grid.extent_kji)
            or (
                grid.k_gaps
                and axis == 0
                and (
                    (
                        polarity == 1
                        and previous_kji0[0] >= 0
                        and grid.k_gap_after_array[previous_kji0[0]]
                    )
                    or (
                        polarity == 0
                        and kji0[0] < grid.nk - 1
                        and grid.k_gap_after_array[kji0[0]]
                    )
                )
            )
        ):
            if check_for_reentry and not lazy:
                ...
                (
                    ...
                    polarity,
                    segment,
                ) = skin.find_first_intersection_of_trajectory(
                    trajectory, start=segment, start_xyz=xyz, nudge=+0.05
                )
                if xyz_r is None:
                    log.debug("no skin re-entry found after exit through skin")
                    return None
                log.debug(
                    f"skin re-entry after skin exit: kji0: {cell_kji0}; face: {'KJI'[axis]}{'-+'[polarity]}"
                )
                seg_fraction = segment_fraction(
                    trajectory.control_points, segment, xyz_r
                )
        ...
        if not faulted and treat_skin_as_fault and axis != 0:
            faulted = (kji0[axis] == 0 and polarity == 1) or (
                kji0[axis] == grid.extent_kji[axis] - 1 and polarity == 0
            )
        # handle the simplest case of a well behaved k neighbour or unsplit j or i neighbour
        if (
            not grid.pinched_out(cell_kji0=kji0, cache_pinchout_array=True)
            and not faulted
        ):
        ...
        if faulted:
            ...
            if xyz_f is not None and k0 is not None:
                kji0[0] = k0
                seg_fraction = segment_fraction(
                    trajectory.control_points, segment, xyz_f
                )
                return (
                    vec.isclose(xyz, xyz_f, tolerance=0.001),
                    kji0,
                    axis,
                    1 - polarity,
                    segment,
            ...
            log.debug(
                "failed to find entry point in column face after crossing fault; checking entire cross section"
            )
            x_sect_surf = generate_torn_surface_for_x_section(
                grid,
                "KJI"[axis],
            ...
            (
                ...
                trajectory, x_sect_surf, start=segment, start_xyz=xyz, nudge=-0.1
            )
            if xyz_f is not None:
                ...
                kji0[3 - axis] = j_or_i0
                seg_fraction = segment_fraction(
                    trajectory.control_points, segment_f, xyz_f
                )
                return (
                    vec.isclose(xyz, xyz_f, tolerance=0.001),
                    kji0,
            ...
            if xyz_r is None:
                ...
                return None
            log.debug(
                f"skin re-entry after fault face exit: kji0: {cell_kji0}; face: {'KJI'[axis]}{'-+'[polarity]}"
            )
            ...
            while True:
                ...
                if not (np.all(kji0 >= 0) and np.all(kji0 < grid.extent_kji)) or (
                    grid.k_gaps
                    and axis == 0
                    and (
                        (polarity == 0 and grid.k_gap_after_array[kji0[0]])
                        or (polarity == 1 and grid.k_gap_after_array[kji0[0] - 1])
                    )
                ):
                    log.debug(
    ...
    def segment_fraction(trajectory_xyz, segment, xyz):
        ...
        segment_length = vec.naive_length(
            segment_vector
        )  # note this is length in straight line, rather than diff in mds
    ...
    if blocked_well.trajectory.crs_root is grid.crs_root:
        ...
        traj_crs = rqc.Crs(
            blocked_well.model, crs_root=blocked_well.trajectory.crs_root
        )
        traj_crs.convert_array_to(
            grid_crs, trajectory.control_points
        )  # trajectory xyz converted in situ to grid's crs
    ...
    if not trajectory_grid_overlap(trajectory, grid):
        log.error(
            "no overlap of trajectory with grid for trajectory uuid: "
            + str(trajectory.uuid)
        )
        return None
    grid_box = grid.xyz_box(lazy=False)
    ...
    assert (
        z_sign * traj_xyz[0, 2] < grid_top_z
    ), "trajectory does not start above top of grid"  # min z
    ...
    knot = 0
    while (
        knot < trajectory.knot_count - 1 and z_sign * traj_xyz[knot + 1, 2] < grid_top_z
    ):
        knot += 1
    ...
    if lazy:
        ...
        if xyz is None:
            log.error(
                "failed to find intersection of trajectory with top surface of grid"
            )
            return None
        ...
        (
            ...
            polarity,
            entry_knot,
        ) = skin.find_first_intersection_of_trajectory(trajectory)
        if xyz is None:
            log.error(
                "failed to find intersection of trajectory with outer skin of grid"
            )
            return None
        else:
            log.debug(
                "skin intersection x,y,z: "
                + str(xyz)
                + "; knot: "
                + str(entry_knot)
                + "; cell kji0: "
    ...
    while next_cell_info is not None:
        ...
        if not entry_shared:
            ...
            node_mds_list.append(
                back_calculated_md(trajectory, entry_knot, entry_fraction)
            )
            node_count += 1
        ...
        if exit_xyz is None:
            ...
            if point_is_within_cell(traj_xyz[-1], grid, kji0):
                ...
                log.debug(
                    f"kiss detected at cell kji0 {kji0} "
                    + "KJI"[entry_axis]
                    + "-+"[entry_polarity]
                )
                ...
                if np.all(np.array(previous_kji0, dtype=int) >= 0) and np.all(
                    ...
                    kissed[
                        tuple(previous_kji0)
                    ] = True  # stops immediate revisit for kiss and tell tactics
                ...
                pinchout_skip_sign = (
                    -1 if entry_axis == 0 and entry_polarity == 1 else 1
                )
                ...
                for try_index in range(len(offsets_kji)):
                    ...
                    while (
                        np.all(try_kji0 >= 0)
                        and np.all(try_kji0 < grid.extent_kji)
                        and grid.pinched_out(cell_kji0=try_kji0)
                    ):
                        try_kji0[0] += pinchout_skip_sign
                    if (
                        np.any(try_kji0 < 0)
                        or np.any(try_kji0 >= grid.extent_kji)
                        or kissed[tuple(try_kji0)]
                    ):
                        continue
                    ...
                    if try_entry_xyz is not None and vec.isclose(
                        try_entry_xyz, entry_xyz, tolerance=0.02
                    ):
                ...
                if next_cell_info is None:
                    log.debug(
                        "kiss and tell failed to find next cell entry; switching to point in cell tactics"
                    )
                    ...
                    for try_index in range(len(offsets_kji)):
                        ...
                        if point_is_within_cell(sample_point, grid, try_kji0):
                            ...
                            if try_entry_xyz is None or try_entry_knot is None:
                                log.warning(
                                    f"failed to find entry to favoured cell {try_kji0}"
                                )
                            else:
                                ...
                                next_cell_info = (
                                    vec.isclose(
                                        try_entry_xyz, entry_xyz, tolerance=0.01
                                    ),
                                    try_kji0,
                                    try_entry_axis,
                                    try_entry_polarity,
                    ...
                    if next_cell_info is None:
                        log.debug(
                            "sample point not found in immediate neighbours, switching to full column search"
                        )
                        k_offset = 0
                        found = False
                        while not found and k_offset <= max(
                            kji0[0], grid.nk - kji0[0] - 1
                        ):
                            for k_plus_minus in [1, -1]:
                                for j_offset in [0, -1, 1]:
                                    for i_offset in [0, -1, 1]:
                                        try_kji0 = kji0 + np.array(
                                            (
                                                k_offset * k_plus_minus,
                                                j_offset,
                                                i_offset,
                                            )
                                        )
                                        if (
                                            np.any(try_kji0 < 0)
                                            or np.any(try_kji0 >= grid.extent_kji)
                                            or sample_test[tuple(try_kji0)]
                                        ):
                                            continue
                                        sample_test[tuple(try_kji0)] = True
                                        if point_is_within_cell(
                                            sample_point, grid, try_kji0
                                        ):
                                            found = True
                                            log.debug(
                                                f"sample point is in cell {try_kji0}"
                                            )
                                            (
                                                try_entry_xyz,
                                                try_entry_knot,
                                            ...
                                            if (
                                                try_entry_xyz is None
                                                or try_entry_knot is None
                                            ):
                                                log.warning(
                                                    f"failed to find entry to column sampled cell {try_kji0}"
                                                )
                                            else:
                                                try_entry_fraction = segment_fraction(
                                                    traj_xyz,
                    ...
                    if next_cell_info is None:
                        ...
                        if np.any(previous_kji0 == 0) or np.any(
                            previous_kji0 == grid.extent_kji - 1
                        ):
                            log.debug(
                                "looking for skin re-entry after possible kissing exit"
                            )
                            skin = grid.skin(
                                use_single_layer_tactics=use_single_layer_tactics
                            )
            ...
            if next_cell_info is None:
                log.warning(
                    "well blocking got stuck – cells probably omitted at tail of well"
                )
        ...
        #      if exit_xyz is not None:  # usual well-behaved case or non-standard due to kiss
            ...
            # todo: check for inactive cell and make an unblocked interval instead, if required
            node_mds_list.append(
                back_calculated_md(trajectory, exit_knot, exit_fraction)
            )
            node_count += 1
            grid_indices_list.append(0)
            cell_indices_list.append(grid.natural_cell_index(kji0))
            cell_count += 1
            face_pairs_list.append(
    ...
    assert cell_count == (
        node_count - np.count_nonzero(blocked_well.grid_indices == -1) - 1
    )
    ...
    log.info(
        ...
        + " cell"
        + _pl(cell_count)
        + " blocked for well trajectory uuid: "
        + str(trajectory.uuid)
    )
