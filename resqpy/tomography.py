...
def make_pixel_map(
    ...
    p_map = grid.pixel_maps(
        origin, width, height, dx, dy=dy, k0=k0, vertical_ref=vertical_ref
    )
    ...
    model.create_reciprocal_relationship(
        p_node, "destinationObject", grid.grid_root, "sourceObject"
    )
...
def find_pixel_maps(
    grid, min_width=None, max_width=None, min_height=None, max_height=None
):
    ...
    property_parts_list = model.parts_list_of_type(
        type_of_interest="obj_DiscreteProperty"
    )
    log.debug("number of discrete properties: " + str(len(property_parts_list)))
    property_parts_list = model.parts_list_filtered_by_related_uuid(
        property_parts_list, grid.uuid
    )
    log.debug(
        "number of discrete properties related to grid: "
        + str(len(property_parts_list))
    )
    ...
    log.debug(
        "initial property collection size: " + str(pm_collection.number_of_parts())
    )

    pm_collection = rqp.selective_version_of_collection(
        pm_collection,
        count=2,
        property_kind="index",
        facet_type="what",
    ...
    log.debug(
        "selective property collection size: " + str(pm_collection.number_of_parts())
    )
    ...
    if filtering:
        ...
        for part in pm_collection.parts():
            ...
            if support_uuid is None:
                continue
            if pixel_map_mesh_uuid is None or not bu.matching_uuids(
                support_uuid, pixel_map_mesh_uuid
            ):
                pixel_map_mesh_root = model.root_for_uuid(support_uuid)
                pixel_map_extent_ji = np.ones(2, dtype="int")
                pixel_map_extent_ji[0] = (
                    rqet.find_nested_tags_int(
                        pixel_map_mesh_root, ["Grid2dPatch", "SlowestAxisCount"]
                    )
                    - 1
                )
                pixel_map_extent_ji[1] = (
                    rqet.find_nested_tags_int(
                        pixel_map_mesh_root, ["Grid2dPatch", "FastestAxisCount"]
                    )
                    - 1
                )
...
def find_or_make_pixel_map(
    ...
    log.debug(
        f"width range: {min_width} to {max_width}; height range: {min_height} to {max_height}"
    )
    ...
    _, pm_collection = make_pixel_map(
        grid,
        width,
        height,
        origin=origin,
...
def get_pixel_map(
    ...
    pm_collection = find_or_make_pixel_map(
        grid,
        width,
        height,
...
def map_image(
    ...
    for row in range(height):
        pix_row = (
            height - row - 1
        )  # image starts at top left, whilst our pixel has origin bottom left
