...
def import_nexus(
    ...
    if vdb_file:
        ...
        if vdb_case is None:
            ...
            assert vdb_case in case_list, (
                "case " + vdb_case + " not found in vdb: " + vdb_file
            )
            vdbase.set_use_case(vdb_case)
        assert grid_title in vdbase.list_of_grids(), (
            "grid " + str(grid_title) + " not found in vdb"
        )
        ...
        if cp_extent_kji[:2] == (1, 1):  # auto determination of extent failed
            assert (
                extent_ijk is not None
            ), "failed to determine extent of grid from corp data"
            (ni, nj, nk) = extent_ijk
            assert (
                cp_extent_kji[2] == ni * nj * nk
            ), "number of cells in grid corp does not match extent"
            cp_extent = (nk, nj, ni, 2, 2, 2, 3)  # (nk, nj, ni, kp, jp, ip, xyz)
            cp_array = cp_array.reshape(cp_extent)
        elif extent_ijk is not None:
            for axis in range(3):
                assert (
                    cp_extent_kji[axis] == extent_ijk[2 - axis]
                ), "extent of grid corp data from vdb does not match that supplied"
        ...
        log.info(
            "starting import of Nexus corp file "
            + str(corp_file if corp_file else corp_bin_file)
        )
        tm.log_nexus_tm("info")
        ...
        if (
            corp_bin_file
        ):  # bespoke nexus corp bin format, not to be confused with pure binary files used below
            ...
            cp_binary_file = abt.cp_binary_filename(
                corp_file, nexus_ordering=False
            )  # pure binary, not bespoke corp bin used above
            recent_binary_exists = ld.file_exists(
                cp_binary_file, must_be_more_recent_than_file=corp_file
            )
            cp_array = None
            if use_binary and (extent_ijk is not None) and recent_binary_exists:
                try:
                    cp_array = ld.load_array_from_file(
                        cp_binary_file, cp_extent, use_binary=True
                    )
                except:
                    cp_array = None
            if cp_array is None:
                ...
                if use_binary:
                    wd.write_pure_binary_data(
                        cp_binary_file, cp_array
                    )  # NB: this binary file is resequenced, not in nexus ordering!
    ...
    if corp_xy_units == corp_z_units and resqml_xy_units == resqml_z_units:
        ...
        bwam.convert_lengths(
            cp_array[:, :, :, :, :, :, 0:1], corp_xy_units, resqml_xy_units
        )
        bwam.convert_lengths(
            cp_array[:, :, :, :, :, :, 2], corp_z_units, resqml_z_units
        )
    ...
    if vdb_file:
        ...
        inactive_mask = vdbase.grid_kid_inactive_mask(
            grid_title
        )  # TODO: check conversion of KID to boolean for LGRs
        if inactive_mask is not None:
            log.debug("using kid array as inactive cell mask")
            active_mask = np.logical_not(inactive_mask)
        else:
            log.warning(
                "kid array not found, using unpack array as active cell indicator"
            )
            unp = vdbase.grid_unpack(grid_title)
            assert (
                unp is not None
            ), "failed to load active cell indicator mask from vdb kid or unpack arrays"
            active_mask = np.empty((nk, nj, ni), dtype="bool")
            active_mask[:] = unp > 0
            inactive_mask = np.logical_not(active_mask)
    elif active_mask_file:
        active_mask = ld.load_array_from_file(
            active_mask_file, extent_kji, data_type="bool", use_binary=use_binary
        )
        if active_mask is None:
            log.error(
                "failed to load active cell indicator array from file: "
                + active_mask_file
            )
        else:
            inactive_mask = np.logical_not(
                active_mask
            )  # will crash if active mask load failed
    ...
    if shift_to_local:
        ...
        if local_origin_place == "centre":
            ...
            local_origin = (
                np.nanmin(cp_array, axis=(0, 1, 2, 3, 4, 5)) - 1.0
            )  # The -1 ensures all coords are >0
        else:
            assert False
    ...
    if mode == "w":
        model = rq.Model(
            resqml_file_root, new_epc=True, create_basics=True, create_hdf5_ext=True
        )
    else:
    ...
    ijk_node = grid.create_xml(
        ext_uuid=None, title=grid_title, add_as_part=True, add_relationships=True
    )
        ...
        if vdb_static_properties:
            ...
            if len(props) > 0:
                ...
                for keyword in props:
                    prop_import_collection.import_vdb_static_property_to_cache(
                        vdbase, keyword, grid_name=grid_title
                    )
    ...
    #                                                                  discrete = True, uom = None, time_index = None, null_value = None)
        ...
        for (
            ...
            p_null_value,
            p_discrete,
        ) in property_array_files:
            prop_import_collection.import_nexus_property_to_cache(
                p_filename,
                p_keyword,
                grid.extent_kji,
                discrete=p_discrete,
    ...
    #    (filename, property_kind, facet_type, facet, uom, time_index, null_value, discrete)
        ...
        assert (
            len(case_path_list) > 0
        ), "no case directories found with path starting: " + str(
            ensemble_case_dirs_root
        )
        ...
        for case_path in case_path_list:
            ...
            try:
                ...
                if case_number == 0:
                    assert (
                        not case_zero_used
                    ), "more than one case number evaluated to zero"
                    case_zero_used = True
            except:
            ...
            if len(prop_import_collection.imported_list) > 0:
                # create hdf5 file using arrays cached in grid above
                log.info(
                    "writing properties to hdf5 file "
                    + str(resqml_file_root)
                    + ".h5 for case: "
                    + str(case_number)
                )
                ...
                prop_import_collection.create_xml_for_imported_list_and_add_parts_to_model(
                    ext_uuid, time_series_uuid=ts_uuid
                )
                if create_property_set:
                    prop_import_collection.create_property_set_xml(
                        "realisation " + str(case_number)
                    )
                case_count += 1
    ...
    # create hdf5 file using arrays cached in grid above
    if (
        prop_import_collection is not None
        and len(prop_import_collection.imported_list) > 0
    ):
        if decoarsen:
            ...
            if decoarsen_array is not None:
                ...
                prop_import_collection.add_cached_array_to_imported_list(
                    decoarsen_array,
                    "decoarsen",
                    "DECOARSEN",
                    discrete=True,
        ...
        log.info(
            "writing "
            + str(len(prop_import_collection.imported_list))
            + " properties to hdf5 file "
            + resqml_file_root
            + ".h5"
        )
    ...
    if (
        using_vdb
        and vdb_recurrent_properties
        and timestep_selection is not None
        and str(timestep_selection) != "none"
    ):
        ...
        if full_time_series is None:
            log.error(
                "failed to fetch time series from Nexus summary file; recurrent data excluded"
            )
            ...
            timestep_list = vdbase.grid_list_of_timesteps(
                grid_title
            )  # get list of timesteps for which recurrent files exist
            ...
            for timestep_number in timestep_list:
                if isinstance(timestep_selection, list):
                    ...
                    if timestep_selection == "first":
                        ...
                        if (
                            ...
                            and timestep_number != timestep_list[-1]
                        ):
                            continue
                ...
                if stamp is None:
                    log.error(
                        "timestamp number for which recurrent data exists was not found in summary file: "
                        + str(timestep_number)
                    )
                    continue
                recur_prop_list = vdbase.grid_list_of_recurrent_properties(
                    grid_title, timestep_number
                )
                common_recur_prop_set = set()
                if recur_time_series is None:
                    recur_time_series = rts.TimeSeries(
                        model, extract_from_xml=False, first_timestamp=stamp
                    )
                    ...
                    if recur_prop_list is not None:
                        common_recur_prop_set = common_recur_prop_set.intersection(
                            set(recur_prop_list)
                        )
                ...
                if recur_prop_list:
                    for keyword in recur_prop_list:
                        ...
                        step_import_collection.import_vdb_recurrent_property_to_cache(
                            vdbase, timestep_number, keyword, grid_name=grid_title
                        )
                # extend hdf5 with cached arrays for this timestep
                log.info(
                    "number of recurrent grid property arrays for timestep: "
                    + str(timestep_number)
                    + " is: "
                ...
                if decoarsen_array is not None:
                    log.info(
                        "decoarsening recurrent properties for timestep: "
                        + str(timestep_number)
                    )
                    step_import_collection.decoarsen_imported_list(
                        decoarsen_array=decoarsen_array
                    )
                log.info(
                    "extending hdf5 file with recurrent properties for timestep: "
                    + str(timestep_number)
                )
                grid.write_hdf5_from_caches(
                    resqml_file_root + ".h5",
                ...
                prop_import_collection.inherit_imported_list_from_other_collection(
                    step_import_collection
                )
                log.debug(
                    "total number of property arrays after timestep: "
                    + str(timestep_number)
                    + " is: "
            ...
            ts_node = full_time_series.create_xml(
                title="simulator full timestep series"
            )
            ...
            if recur_time_series is not None:
                rts_node = recur_time_series.create_xml(
                    title="simulator recurrent array timestep series"
                )
    ...
    if (
        prop_import_collection is not None
        and prop_import_collection.imported_list is not None
    ):
        prop_import_collection.set_grid(
            grid
        )  # update to pick up on recently created xml root node for grid
        prop_import_collection.create_xml_for_imported_list_and_add_parts_to_model(
            ext_uuid, time_series_uuid=ts_uuid, selected_time_indices_list=ts_selection
        )
        if create_property_set:
            prop_import_collection.create_property_set_xml(
                "property set for import for grid " + str(grid_title)
            )
...
def import_vdb_all_grids(
    ...
    for grid_name in grid_list:
        ...
        import_nexus(
            ...
            if grid_name == "ROOT"
            else None,  # 3 element numpy vector applicable to ROOT
            vdb_file=vdb_file,
...
def import_vdb_ensemble(
    ...
    assert (
        vdb_static_properties or vdb_recurrent_properties
    ), "no properties selected for ensemble import"
    ...
    model = rq.Model(
        epc_file=epc_file
    )  # shouldn't be necessary if just created but it feels safer to re-open the model
    ...
    if vdb_recurrent_properties:
        summary_file = (
            ensemble_list[0][:-4] + ".sum"
        )  # TODO: check timestep summary file extension, .tssum?
        full_time_series = rts.time_series_from_nexus_summary(summary_file)
        if full_time_series is None:
            log.error(
                "failed to extract info from timestep summary file; disabling recurrent property import"
            )
            vdb_recurrent_properties = False
    if vdb_recurrent_properties:
    ...
    if vdb_recurrent_properties:
        ...
        if first_stamp is None:
            log.error(
                "first timestamp number selected for import was not found in summary file: "
    ...
    if vdb_recurrent_properties:
        recur_time_series = rts.TimeSeries(
            model, extract_from_xml=False, first_timestamp=first_stamp
        )
        ...
        for timestep_number in remaining_list:
            ...
            recur_time_series.add_timestamp(stamp)
    if recur_time_series is not None:
        recur_ts_node = recur_time_series.create_xml(
            title="simulator recurrent array timestep series"
        )
        recur_ts_uuid = rqet.uuid_for_part_root(recur_ts_node)
        model.time_series = (
            recur_ts_node  # save as the primary time series for the model
        )
    ...
    for realisation in range(len(ensemble_list)):
        ...
        log.info(
            "processing realisation " + str(realisation) + " from: " + str(vdb_file)
        )
        vdbase = vdb.VDB(vdb_file)
        #      case_list = vdbase.cases()
            ...
            if len(props) > 0:
                for keyword in props:
                    ...
                    if property_kind_list is not None:
                        prop_kind, _, _ = rp.property_kind_and_facet_from_keyword(
                            keyword
                        )
                        if prop_kind not in property_kind_list and prop_kind not in [
                            "active",
                            "region initialization",
                        ]:
                            continue
                    prop_import_collection.import_vdb_static_property_to_cache(
                        vdbase, keyword, realization=realisation
                    )
                if decoarsen:
                    decoarsen_array = prop_import_collection.decoarsen_imported_list()
                    if decoarsen_array is not None:
                        log.debug(
                            "static properties decoarsened for realisation "
                            + str(realisation)
                        )
                grid.write_hdf5_from_caches(
                    hdf5_file,
        ...
        if vdb_recurrent_properties:
            ...
            r_timestep_list = (
                vdbase.list_of_timesteps()
            )  # get list of timesteps for which recurrent files exist
            if len(r_timestep_list) < recur_time_series.number_of_timestamps():
                log.error(
                    "insufficient number of reporting timesteps; skipping recurrent data for realisation "
                    + str(realisation)
                )
            else:
                ...
                for tni in range(recur_time_series.number_of_timestamps()):
                    ...
                    stamp = full_time_series.timestamp(timestep_number)
                    recur_prop_list = vdbase.list_of_recurrent_properties(
                        r_timestep_number
                    )
                        ...
                        for keyword in recur_prop_list:
                            ...
                            if property_kind_list is not None:
                                (
                                    ...
                                    _,
                                    _,
                                ) = rp.property_kind_and_facet_from_keyword(keyword)
        ...
        prop_import_collection.create_xml_for_imported_list_and_add_parts_to_model(
            ext_uuid, time_series_uuid=recur_ts_uuid
        )

        if create_property_set_per_realization:
            prop_import_collection.create_property_set_xml(
                "property set for realization " + str(realisation)
            )

        if complete_collection is not None:
            complete_collection.inherit_parts_from_other_collection(
    ...
    if complete_collection is not None:
        if create_property_set_per_timestep and recur_time_series is not None:
            for tni in range(recur_time_series.number_of_timestamps()):
                ts_collection = rp.selective_version_of_collection(
                    complete_collection, time_index=tni
                )
                if ts_collection.number_of_parts() > 0:
                    ts_collection.create_property_set_xml(
                        "property set for time index " + str(tni)
                    )
        if create_complete_property_set:
            complete_collection.create_property_set_xml(
                "property set for ensemble vdb import"
            )
...
def add_ab_properties(
    ...
    grid_node = model.root_for_ijk_grid(
        uuid=grid_uuid
    )  # will raise an exception if uuid is None and Model has more than 1 grid
    ...
    if prop_import_collection is None:
        ...
        log.info(
            "number of pure binary grid property arrays: "
            + str(prop_import_collection.number_of_imports())
        )
...
def add_surfaces(
...
):  # if True, feature and interpretation objects are created
    ...
    assert surface_file_format in [
        "zmap",
        "rms",
        "roxar",
        "GOCAD-Tsurf",
    ], "unsupported surface file format: " + str(surface_file_format)
    ...
    for surf_file in surface_file_list:
        ...
        if make_horizon_interpretations_and_features:
            feature = rqo.GeneticBoundaryFeature(
                model, kind="horizon", extract_from_xml=False, feature_name=short_name
            )
            feature.create_xml()
...
def grid_from_cp(
    ...
    if all_active and geometry_defined_everywhere:
        ...
        cp_nan_mask = np.any(
            np.isnan(cp_array), axis=(3, 4, 5, 6)
        )  # ie. if any nan per cell
        if not geometry_defined_everywhere and not all_active:
            if treat_as_nan == "inactive":
                ...
                if treat_as_nan == "dots":
                    ...
                    log.debug(
                        "geometry for cells with no length to primary cell diagonal being set to NaN"
                    )
                    dot_mask = np.all(
                        np.abs(cp_array[:, :, :, 1, 1, 1] - cp_array[:, :, :, 0, 0, 0])
                        < dot_tolerance,
                        axis=-1,
                    )
                    ...
                    dot_mask[:] = np.where(
                        np.all(
                            np.abs(j_face_vecs[:, :, :, 0]) < dot_tolerance, axis=-1
                        ),
                        True,
                        dot_mask,
                    )
                    dot_mask[:] = np.where(
                        np.all(
                            np.abs(j_face_vecs[:, :, :, 1]) < dot_tolerance, axis=-1
                        ),
                        True,
                        dot_mask,
                    )
                    dot_mask[:] = np.where(
                        np.all(
                            np.abs(i_face_vecs[:, :, :, 0]) < dot_tolerance, axis=-1
                        ),
                        True,
                        dot_mask,
                    )
                    dot_mask[:] = np.where(
                        np.all(
                            np.abs(i_face_vecs[:, :, :, 1]) < dot_tolerance, axis=-1
                        ),
                        True,
                        dot_mask,
                    )
                    ...
                    if treat_as_nan == "morse":
                        ...
                        len_j_face_vecs_sqr = np.sum(
                            j_face_vecs[..., :2] * j_face_vecs[..., :2], axis=-1
                        )
                        len_i_face_vecs_sqr = np.sum(
                            j_face_vecs[..., :2] * i_face_vecs[..., :2], axis=-1
                        )
                        ...
                        max_len_active_j_face_vecs_sqr = np.max(
                            ma.masked_array(len_j_face_vecs_sqr, mask=dead_mask)
                        )
                        max_len_active_i_face_vecs_sqr = np.max(
                            ma.masked_array(len_i_face_vecs_sqr, mask=dead_mask)
                        )
                        dot_mask = np.where(
                            np.any(
                                len_j_face_vecs_sqr
                                > morse_tol_sqr * max_len_active_j_face_vecs_sqr,
                                axis=-1,
                            ),
                            True,
                            dot_mask,
                        ...
                        dot_mask = np.where(
                            np.any(
                                len_i_face_vecs_sqr
                                > morse_tol_sqr * max_len_active_i_face_vecs_sqr,
                                axis=-1,
                            ),
                            True,
                            dot_mask,
                        ...
                        log.debug(
                            f"morse mask set for {np.count_nonzero(dot_mask)} cells"
                        )
                else:
                    raise Exception("code broken")
                cp_nan_mask = np.logical_or(
                    cp_nan_mask, np.logical_and(inactive_mask, dot_mask)
                )
    ...
    if geometry_defined_everywhere:
        ...
        full_mask = (
            cp_nan_mask.reshape((nk, nj, ni, 1))
            .repeat(24, axis=3)
            .reshape((nk, nj, ni, 2, 2, 2, 3))
        )
        masked_cp_array = ma.masked_array(cp_array, mask=full_mask)
        log.info(
            "number of cells without geometry: " + str(np.count_nonzero(cp_nan_mask))
        )
    ...
    k_reduced_cp_array = ma.masked_array(
        np.zeros((nk_plus_1, nj, ni, 2, 2, 3))
    )  # (nk+1+k_gaps, nj, ni, jp, ip, xyz)
    ...
    if k_gaps:
        ...
        for k in range(nk - 1):
            ...
            if k_gap_after_layer[k]:
                k_reduced_cp_array[raw_k, :, :, :, :, :] = masked_cp_array[
                    k, :, :, 1, :, :, :
                ]
                raw_k += 1
                k_reduced_cp_array[raw_k, :, :, :, :, :] = masked_cp_array[
                ...
                k_reduced_cp_array[raw_k, :, :, :, :, :] = np.where(
                    slice.mask, masked_cp_array[k, :, :, 1, :, :, :], slice
                )
                raw_k += 1
        ...
        k_reduced_cp_array[1:-1, :, :, :, :, :] = np.where(
            slice.mask, masked_cp_array[:-1, :, :, 1, :, :, :], slice
        )
    ...
    primary_pillar_jip = np.zeros(
        (nj_plus_1, ni_plus_1, 2), dtype="int"
    )  # (nj + 1, ni + 1, jp:ip)
    ...
    for j in range(nj_plus_1):
        for i in range(ni_plus_1):
            if active_mask_2D[
                j - primary_pillar_jip[j, i, 0], i - primary_pillar_jip[j, i, 1]
            ]:
                continue
            if (
                i > 0
                and primary_pillar_jip[j, i, 1] == 0
                and active_mask_2D[j - primary_pillar_jip[j, i, 0], i - 1]
            ):
                primary_pillar_jip[j, i, 1] = 1
                continue
            if (
                j > 0
                and primary_pillar_jip[j, i, 0] == 0
                and active_mask_2D[j - 1, i - primary_pillar_jip[j, i, 1]]
            ):
    ...
    if split_pillars:
        points_array = np.zeros(
            (nk_plus_1, (nj_plus_1 * ni_plus_1) + len(extras_list), 3)
        )  # note: nk_plus_1 might include k_gaps
        index = 0
        # primary pillars
        for pillar_j in range(nj_plus_1):
            for pillar_i in range(ni_plus_1):
                ...
                points_array[:, index, :] = np.where(
                    slice.mask, np.nan, slice
                )  # NaN indicates undefined/invalid geometry
                index += 1
        ...
        for pillar_j in range(nj_plus_1):
            for pillar_i in range(ni_plus_1):
                for e in range(extras_count[pillar_j, pillar_i]):
                    ...
                    slice = k_reduced_cp_array[
                        :, pillar_j_extra, pillar_i_extra, jp, ip, :
                    ]
                    points_array[:, index, :] = np.where(
                        slice.mask, np.nan, slice
                    )  # NaN indicates unedefined/invalid geometry
                    index += 1
    ...
    grid.points_cached = (
        points_array  # NB: reference to points_array, array not copied here
    )
    ...
    if split_pillars:
        ...
        log.debug(
            "number of uses of extra pillars: " + str(len(cols_for_extra_pillar_list))
        )
    ...
    for j in range(nj):
        for i in range(ni):
            for jp in range(2):
                for ip in range(2):
                    if (
                        not split_pillars or extras_use[j, i, jp, ip] < 0
                    ):  # use primary pillar
                        pillar_index = (j + jp) * ni_plus_1 + i + ip
                    else:
                        eli = (
                            extras_list_index[j + jp, i + ip] + extras_use[j, i, jp, ip]
                        )
    ...
    grid.geometry_defined_for_all_pillars_cached = (
        True  # following fesapi convention of defining all pillars regardless
    )
    ...
    if ijk_handedness is None or ijk_handedness == "auto":
        ...
        if (
            not geometry_defined_everywhere
            and not grid.array_cell_geometry_is_defined[sample_kji0]
        ):
            where_defined = np.where(
                np.logical_and(
                    grid.array_cell_geometry_is_defined,
                    np.logical_not(grid.pinched_out()),
                )
            )
            assert (
                len(where_defined) == 3 and len(where_defined[0]) > 0
            ), "no extant cell geometries"
            sample_kji0 = (
                where_defined[0][0],
                where_defined[1][0],
                where_defined[2][0],
        ...
        cell_ijk_lefthanded = (
            vec.clockwise(sample_cp[0, 0, 0], sample_cp[0, 1, 0], sample_cp[0, 0, 1])
            >= 0.0
        )
