...
def load_hdf5_array(object, node, array_attribute, tag="Values"):
    ...
    if h5_key_pair is None:
        return None
    return object.model.h5_array_element(
        h5_key_pair,
        index=None,
        cache_array=True,
        object=object,
...
class Polyline(_BasePolyline):
    ...
    def __init__(
        ...
        if poly_root is not None:  # polyline xml node is specified
            ...
            self.nodepatch = (
                rqet.find_tag_int(patch_node, "PatchIndex"),
                rqet.find_tag_int(patch_node, "Count"),
            )
            assert not any(
                map(lambda x: x is None, self.nodepatch)
            )  # Required fields - assert neither are None
            ...
            i is not None
            for i in [set_bool, set_coord, set_crs, set_crsroot, set_title]
        ):
    ...
    def is_convex(self, trust_metadata=True):
        ...
        if (
            trust_metadata
            and self.extra_metadata is not None
            and "is_convex" in self.extra_metadata.keys()
        ):
    ...
    def point_is_inside_xy(self, p, mode="crossing"):
        ...
        assert mode in [
            "crossing",
            "winding",
        ], "unrecognised point inclusion mode: " + str(mode)
    ...
    def segment_length(self, segment_index, in_xy=False):
        ...
        d = 2 if in_xy else 3
        return vu.naive_length(
            self.coordinates[successor, :d] - self.coordinates[segment_index, :d]
        )
    ...
    def interpolated_point(self, fraction, in_xy=False):
        ...
        while True:
            ...
            return (
                seg_fraction * self.coordinates[successor]
                + (1.0 - seg_fraction) * self.coordinates[seg_index]
            )
    ...
    def balanced_centre(self, mode="weighted", n=20, cache=True, in_xy=False):
        ...
        if (
            mode == "sampled"
        ):  # this mode is deprecated as it simply approximates the weighted mode
            ...
            for seg_index in range(seg_count):
                successor = (seg_index + 1) % len(self.coordinates)
                p1, p2 = (
                    self.coordinates[seg_index, :d],
                    self.coordinates[successor, :d],
                )
    ...
    def normalised_xy(self, x, y, mode="square"):
        ...
        if tuple(centre_xy) == (x, y):
            if mode == "square":
                return 0.5, 0.5
            return 0.0, 0.0
        segment, px, py = self.first_line_intersection(
            centre_xy[0], centre_xy[1], x, y, half_segment=True
        )
    ...
    def denormalised_xy(self, norm_x, norm_y, mode="perimeter"):
        ...
        if mode == "square":
            ...
            _, px, py = self.first_line_intersection(
                centre_xy[0], centre_xy[1], n_x, n_y, half_segment=True
            )
            assert px is not None
            fx = abs(norm_x - 0.5)
            fy = abs(norm_y - 0.5)
            ...
            p_xy = np.empty(2)
            p_xy[:] = self.first_line_intersection(
                centre_xy[0], centre_xy[1], n_x, n_y, half_segment=True
            )[1:]
            f = maths.sqrt(norm_x)
            x, y = centre_xy + f * (p_xy - centre_xy)
    ...
    def splined(
        ...
        if rep_int_root is None:
            rep_int_root = (
                self.rep_int_root
            )  # todo: check whether it is legal to have 2 representations for 1 interpretation
    ...
    def create_xml(
        ...
        self.model.create_citation(
            root=polyline, title=self.title, originator=originator
        )
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "points_patch0", root=coords
        )
        ...
        if add_as_part:
            ...
            if add_relationships:
                self.model.create_reciprocal_relationship(
                    polyline, "destinationObject", self.crs_root, "sourceObject"
                )
                if self.rep_int_root is not None:  # Optional
                    self.model.create_reciprocal_relationship(
                        polyline, "destinationObject", self.rep_int_root, "sourceObject"
                    )
                ...
                self.model.create_reciprocal_relationship(
                    polyline, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )
...
class PolylineSet(_BasePolyline):
    ...
    def __init__(
        ...
        if set_root is not None:  # polyline set xml node specified
            ...
            self.rep_int_root = self.model.referenced_node(
                rqet.find_tag(set_root, "RepresentedInterpretation")
            )

            self.extra_metadata = rqet.load_metadata_from_xml(self.root_node)
            for patch_node in rqet.list_of_tag(
                set_root, "LinePatch"
            ):  # Loop over all LinePatches - likely just the one
                assert patch_node is not None  # Required field
                ...
                subpolys = self.convert_to_polylines(
                    closed_array,
                    self.count_perpol,
                    self.coordinates,
                    crs_uuid,
            ...
            assert (
                len(crs_set) == 1
            ), "More than one CRS found in input polylines - only handling the first"
            ...
            if (
                self.crs_root is None
            ):  # If no crs_uuid is provided, assume the main model crs is valid
                self.crs_root = self.model.crs_root
                self.crs_uuid = self.crs_root.attrib["uuid"]
            self.polys = self.convert_to_polylines(
                closed_array,
                self.count_perpol,
                self.coordinates,
                self.crs_uuid,
                ...
                if i == 0:
                    ...
                    if stick != line[7] or i == len(inpoints) - 1:
                        if count <= 2:  # Line has fewer than 2 points
                            ...
                            self.coordinates = np.delete(
                                self.coordinates, -2, 0
                            )  # Remove the second to last entry
                        else:
    ...
    def poly_index_containing_point_in_xy(self, p, mode="crossing"):
        ...
        assert mode in [
            "crossing",
            "winding",
        ], "unrecognised mode when looking for polygon containing point"
    ...
    def create_xml(
        ...
        if self.save_polys:
            for poly in self.polys:
                poly.create_xml(
                    ext_uuid, add_relationships=add_relationships, title=poly.title
                )
        ...
        self.model.create_citation(
            root=polyset, title=self.title, originator=originator
        )
        ...
        if self.boolnotconstant:
            ...
            self.model.create_hdf5_dataset_ref(
                ext_uuid, self.uuid, "indices_patch0", root=ind_val
            )
        else:
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "NodeCountPerPolyline_patch0", root=count_val
        )
        ...
        if add_as_part:
            ...
            if add_relationships:
                self.model.create_reciprocal_relationship(
                    polyset, "destinationObject", self.crs_root, "sourceObject"
                )
                if self.rep_int_root is not None:  # Optional
                    self.model.create_reciprocal_relationship(
                        polyset, "destinationObject", self.rep_int_root, "sourceObject"
                    )
                if self.save_polys:
                    for poly in self.polys:
                        self.model.create_reciprocal_relationship(
                            polyset, "destinationObject", poly.root_node, "sourceObject"
                        )
                ...
                ext_node = self.model.root_for_part(ext_part)
                self.model.create_reciprocal_relationship(
                    polyset, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )
    ...
    def write_hdf5(self, file_name=None, mode="a", save_polylines=False):
        ...
        h5_reg.register_dataset(
            self.uuid, "NodeCountPerPolyline_patch0", self.count_perpol.astype(np.int32)
        )
        if self.boolnotconstant:
    ...
    def get_bool_array(self, closed_node):
        ...
        if rqet.node_type(closed_node) == "BooleanConstantArray":
            ...
            value = rqet.bool_from_text(
                rqet.node_text(rqet.find_tag(closed_node, "Value"))
            )
            return np.full((count), value)
        elif rqet.node_type(closed_node) == "BooleanArrayFromIndexArray":
            count = rqet.find_tag_int(closed_node, "Count")
            indices_arr = load_hdf5_array(
                self, closed_node, "indices_arr", tag="Indices"
            )
            istrue = rqet.bool_from_text(
                rqet.node_text(rqet.find_tag(closed_node, "IndexIsTrue"))
            )
    ...
    def convert_to_polylines(
        self, closed_array, count_perpol, coordinates, crs_uuid, crs_root, rep_int_root
    ):
        ...
        for poly in polylines:
            if poly == polylines[0]:
                ...
                if not curr_crs.is_equivalent(master_crs):
                    ...
                    self.coordinates = np.concatenate(
                        (self.coordinates, poly.coordinates)
                    )
    ...
    def convert_to_charisma(self, file_name):
        ...
        for i, poly in enumerate(self.polys):
            for point in poly.coordinates:
                lines.append(
                    f"INLINE-\t0\t0\t{point[0]}\t{point[1]}\t{point[2]}\t{faultname}\t{i+1}\n"
                )
...
def flatten_polyline(parent_model, poly_root, axis="z", value="0", title=""):
    ...
    if title != "":
        ...
        poly.title = poly.title + f" flattened on {axis} to value {value}"
    poly.uuid = bu.new_uuid()
    if axis.lower() == "x":
        index = 0
    elif axis.lower() == "y":
...
def tangents(points, weight="linear", closed=False):
    ...
    if closed:
        assert (
            knot_count > 2
        ), "closed poly line must contain at least 3 knots for tangent generation"
...
def spline(
    ...
    if max_degrees_per_knot is not None:
        for seg in range(seg_count):
            ...
            turn = vu.degrees_difference(
                tangent_vectors[seg], segment
