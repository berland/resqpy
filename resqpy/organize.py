...
def extract_has_occurred_during(
    parent_node, tag="HasOccuredDuring"
):  # RESQML Occured (stet)
    ...
    if hod_node is None:
        ...
        return (
            rqet.find_nested_tags_text(hod_node, ["ChronoBottom", "UUID"]),
...
def create_xml_has_occurred_during(
    model, parent_node, hod_pair, tag="HasOccuredDuring"
):
    if hod_pair is None:
        return
    ...
    model.create_ref_node(
        "ChronoBottom",
        model.title_for_root(chrono_base_root),
        base_chrono_uuid,
        root=hod_node,
    )
    model.create_ref_node(
        "ChronoTop",
        model.title_for_root(chrono_top_root),
        top_chrono_uuid,
        root=hod_node,
    )
...
class OrganizationFeature:
    ...
    def __init__(
        ...
        if extract_from_xml and self.root_node is not None:
            ...
            self.feature_name = rqet.find_nested_tags_text(
                self.root_node, ["Citation", "Title"]
            )
            self.organization_kind = rqet.find_tag_text(
                self.root_node, "OrganizationKind"
            )
        else:
            self.uuid = bu.new_uuid()
            self.feature_name = feature_name
            assert organization_kind in [
                "earth model",
                "fluid",
                "stratigraphic",
                "structural",
    ...
    def is_equivalent(self, other, check_extra_metadata=True):
        ...
        if other is None or not isinstance(other, OrganizationFeature):
            return False
        if self is other or bu.matching_uuids(self.uuid, other.uuid):
            return True
        if (
            self.feature_name != other.feature_name
            or self.organization_kind != other.organization_kind
        ):
            return False
    ...
    def create_xml(self, add_as_part=True, originator=None):
        ...
        self.model.create_citation(
            root=ofn, title=self.feature_name, originator=originator
        )
...
class GeobodyFeature:
    ...
    def __init__(
        self, parent_model, root_node=None, extract_from_xml=True, feature_name=None
    ):
    ...
    def is_equivalent(self, other, check_extra_metadata=True):
        ...
        if other is None or not isinstance(other, GeobodyFeature):
            return False
        if self is other or bu.matching_uuids(self.uuid, other.uuid):
            return True
        if check_extra_metadata and not equivalent_extra_metadata(self, other):
    ...
    def create_xml(self, add_as_part=True, originator=None):
        ...
        self.model.create_citation(
            root=gfn, title=self.feature_name, originator=originator
        )
...
class BoundaryFeature:
    ...
    def create_xml(self, add_as_part=True, originator=None):
        ...
        self.model.create_citation(
            root=bf, title=self.feature_name, originator=originator
        )
...
class FrontierFeature:
    ...
    def create_xml(self, add_as_part=True, originator=None):
        ...
        self.model.create_citation(
            root=ff, title=self.feature_name, originator=originator
        )
...
class GeologicUnitFeature:
    ...
    def create_xml(self, add_as_part=True, originator=None):
        ...
        self.model.create_citation(
            root=guf, title=self.feature_name, originator=originator
        )
...
class FluidBoundaryFeature:
    ...
    def __init__(self, parent_model, root_node=None, kind=None, feature_name=None):
        ...
        if self.root_node is not None:
            ...
            if xml_kind and self.kind:
                assert (
                    xml_kind.lower() == self.kind.lower()
                ), "Tectonic boundary kind mismatch"
            else:
                self.kind = xml_kind
            self.feature_name = rqet.find_nested_tags_text(
                self.root_node, ["Citation", "Title"]
    ...
    def create_xml(self, add_as_part=True, originator=None):
        ...
        self.model.create_citation(
            root=fbf, title=self.feature_name, originator=originator
        )
        ...
        assert self.kind in [
            ...
            "seal",
            "water oil contact",
        ], "fluid boundary feature kind not recognized"
...
class RockFluidUnitFeature:
    ...
    def __init__(
        ...
        if self.root_node is not None:
            ...
            assert (
                feature_root is not None
            ), "rock fluid top boundary feature missing from model"
            self.top_boundary_feature = BoundaryFeature(
                self.model, root_node=feature_root
            )
            ...
            assert (
                feature_root is not None
            ), "rock fluid bottom boundary feature missing from model"
            self.base_boundary_feature = BoundaryFeature(
                self.model, root_node=feature_root
            )
    ...
    def is_equivalent(self, other, check_extra_metadata=True):
        ...
        if self.base_boundary_feature is not None:
            if not self.base_boundary_feature.is_equivalent(
                other.base_boundary_feature
            ):
                return False
    ...
    def create_xml(self, add_as_part=True, add_relationships=True, originator=None):
        ...
        assert (
            self.feature_name
            and self.phase
            and self.top_boundary_feature
            and self.base_boundary_feature
        )
        ...
        self.model.create_citation(
            root=rfuf, title=self.feature_name, originator=originator
        )
        ...
        if add_as_part:
            ...
            if add_relationships:
                self.model.create_reciprocal_relationship(
                    rfuf, "destinationObject", top_boundary_root, "sourceObject"
                )
                self.model.create_reciprocal_relationship(
                    rfuf, "destinationObject", base_boundary_root, "sourceObject"
                )
...
class TectonicBoundaryFeature:
    ...
    def __init__(
        ...
        if kind is None:
            assert (
                extract_from_xml
            ), "kind must be specified unless extracting tectonic boundary feature from xml"
        else:
            assert kind in [
                "fault",
                "fracture",
            ], "tectonic boundary feature kind not recognized"
    ...
    def create_xml(self, add_as_part=True, originator=None):
        ...
        self.model.create_citation(
            root=tbf, title=self.feature_name, originator=originator
        )
...
class GeneticBoundaryFeature:
    ...
    def __init__(
        ...
        if kind is None:
            if not extract_from_xml:
                kind = "horizon"
        else:
            assert kind in [
                "horizon",
                "geobody boundary",
            ], "genetic boundary feature kind not recognized"
        ...
        if extract_from_xml and self.root_node is not None:
            ...
            if xml_kind and self.kind:
                assert (
                    xml_kind.lower() == self.kind.lower()
                ), "Genetic boundary kind mismatch"
            else:
                self.kind = xml_kind
            self.feature_name = rqet.find_nested_tags_text(
                self.root_node, ["Citation", "Title"]
            ...
            if age_node:
                self.absolute_age = (
                    rqet.find_tag_text(age_node, "DateTime"),
                    rqet.find_tag_int(age_node, "YearOffset"),
    ...
    def is_equivalent(self, other, check_extra_metadata=True):
        ...
        return (
            ...
            and self.absolute_age == other.absolute_age
        )
    ...
    def create_xml(self, add_as_part=True, originator=None):
        ...
        self.model.create_citation(
            root=gbf, title=self.feature_name, originator=originator
        )
...
class WellboreFeature:
    ...
    def create_xml(self, add_as_part=True, originator=None):
        ...
        self.model.create_citation(
            root=wbf, title=self.feature_name, originator=originator
        )
...
class FaultInterpretation:
    ...
    def __init__(
        ...
        self.root_node = root_node
        self.uuid = None
        self.tectonic_boundary_feature = (
            None  # InterpretedFeature RESQML field, when not loading from xml
        )
        ...
        self.throw_interpretation_list = (
            None  # list of (list of throw kind, (base chrono uuid, top chrono uuid)))
        )
        self.extra_metadata = {}
        ...
        if extract_from_xml and self.root_node is not None:
            ...
            interp_feature_ref_node = rqet.find_tag(
                self.root_node, "InterpretedFeature"
            )
            ...
            if self.feature_root is not None:
                self.tectonic_boundary_feature = TectonicBoundaryFeature(
                    self.model,
            ...
            self.maximum_throw = rqet.find_tag_float(
                self.root_node, "MaximumThrow"
            ...
            throw_interpretation_nodes = rqet.list_of_tag(
                self.root_node, "ThrowInterpretation"
            )
            if throw_interpretation_nodes is not None and len(
                throw_interpretation_nodes
            ):
                self.throw_interpretation_list = []
            ...
            assert domain in [
                ...
                "time",
                "mixed",
            ], "unrecognised domain value for fault interpretation"
            assert (
                is_normal is not None
                and isinstance(is_normal, bool)
                and is_normal == (is_listric is None)
            )
            self.uuid = bu.new_uuid()
            ...
            self.throw_interpretation_list = (
                None  # not curerntly supported for direct initialisation
            )
    ...
    def is_equivalent(self, other, check_extra_metadata=True):
        ...
        if self.tectonic_boundary_feature is not None:
            if not self.tectonic_boundary_feature.is_equivalent(
                other.tectonic_boundary_feature
            ):
                return False
        ...
        if self.root_node is not None and other.root_node is not None:
            if rqet.citation_title_for_node(
                self.root_node
            ) != rqet.citation_title_for_node(other.root_node):
                return False
        elif self.root_node is not None or other.root_node is not None:
            return False
        if (
            not equivalent_chrono_pairs(
                self.main_has_occurred_during, other.main_has_occurred_during
            )
            or self.is_normal != other.is_normal
            or self.domain != other.domain
            or self.is_listric != other.is_listric
        ):
            return False
        if (
            (self.maximum_throw is None) != (other.maximum_throw is None)
            or (self.mean_azimuth is None) != (other.mean_azimuth is None)
            or (self.mean_dip is None) != (other.mean_dip is None)
        ):
            return False
        if self.maximum_throw is not None and not maths.isclose(
            self.maximum_throw, other.maximum_throw, rel_tol=1e-3
        ):
            return False
        if self.mean_azimuth is not None and not maths.isclose(
            self.mean_azimuth, other.mean_azimuth, abs_tol=0.5
        ):
            return False
        if self.mean_dip is not None and not maths.isclose(
        ...
        if len(self.throw_interpretation_list) != len(other.throw_interpretation_list):
            return False
        for this_ti, other_ti in zip(
            self.throw_interpretation_list, other.throw_interpretation_list
        ):
    ...
    def create_xml(
        ...
        if self.tectonic_boundary_feature is not None:
            ...
            if tbf_root is not None:
                if tectonic_boundary_feature_root is None:
                    ...
                    assert (
                        tbf_root is tectonic_boundary_feature_root
                    ), "tectonic boundary feature mismatch"
        assert tectonic_boundary_feature_root is not None
        title = rqet.find_nested_tags_text(
            tectonic_boundary_feature_root, ["Citation", "Title"]
        )
        ...
        assert self.domain in [
            ...
            "time",
            "mixed",
        ], "illegal domain value for fault interpretation"
        ...
        if self.throw_interpretation_list is not None and len(
            self.throw_interpretation_list
        ):
            ...
            for (
                throw_kind,
                (base_chrono_uuid, top_chrono_uuid),
            ) in self.throw_interpretation_list:
                if (
                    str(base_chrono_uuid),
                    str(top_chrono_uuid),
                ) != previous_has_occurred_during:
                    previous_has_occurred_during = (
                        str(base_chrono_uuid),
                        str(top_chrono_uuid),
                    )
                    ...
                    create_xml_has_occurred_during(
                        self.model, ti_node, (base_chrono_uuid, top_chrono_uuid)
                    )
        ...
        if add_as_part:
            ...
            if add_relationships:
                self.model.create_reciprocal_relationship(
                    ...
                    "destinationObject",
                    tectonic_boundary_feature_root,
                    "sourceObject",
                )
...
class EarthModelInterpretation:
    ...
    def __init__(
        ...
        if extract_from_xml and self.root_node is not None:
            ...
            self.feature_root = self.model.referenced_node(interp_feature_ref_node)
            if self.feature_root is not None:
                self.organization_feature = OrganizationFeature(
                    self.model,
                    root_node=self.feature_root,
                    feature_name=self.model.title_for_root(self.feature_root),
                )
            ...
            assert domain in [
                ...
                "time",
                "mixed",
            ], "unrecognised domain value for earth model interpretation"
            self.uuid = bu.new_uuid()
    ...
    def create_xml(
        ...
        if self.organization_feature is not None:
            ...
            if of_root is not None:
                if organization_feature_root is None:
                    ...
                    assert (
                        of_root is organization_feature_root
                    ), "organization feature mismatch"
        ...
        assert self.domain in [
            ...
            "time",
            "mixed",
        ], "illegal domain value for horizon interpretation"
        ...
        if add_as_part:
            ...
            if add_relationships:
                self.model.create_reciprocal_relationship(
                    emi, "destinationObject", organization_feature_root, "sourceObject"
                )
...
class HorizonInterpretation:
    ...
    def __init__(
        ...
        if extract_from_xml and self.root_node is not None:
            ...
            self.sequence_stratigraphy_surface = rqet.find_tag_text(
                self.root_node, "SequenceStratigraphySurface"
            )
            self.extra_metadata = rqet.load_metadata_from_xml(self.root_node)
        else:
            ...
            assert domain in [
                ...
                "time",
                "mixed",
            ], "unrecognised domain value for horizon interpretation"
            if sequence_stratigraphy_surface is not None:
                assert sequence_stratigraphy_surface in [
                    "flooding",
                    "ravinement",
                    "maximum flooding",
                    "transgressive",
    ...
    def is_equivalent(self, other, check_extra_metadata=True):
        ...
        if (
            ...
            or not equivalent_chrono_pairs(
                self.has_occurred_during, other.has_occurred_during
            )
            or self.sequence_stratigraphy_surface != other.sequence_stratigraphy_surface
        ):
            return False
        if check_extra_metadata and not equivalent_extra_metadata(self, other):
            return False
    ...
    def create_xml(
        ...
        if self.genetic_boundary_feature is not None:
            ...
            if gbf_root is not None:
                if genetic_boundary_feature_root is None:
                    ...
                    assert (
                        gbf_root is genetic_boundary_feature_root
                    ), "genetic boundary feature mismatch"
        ...
        if self.sequence_stratigraphy_surface is not None:
            assert self.sequence_stratigraphy_surface in [
                "flooding",
                "ravinement",
                "maximum flooding",
                "transgressive",
            ]
            sss_node = rqet.SubElement(
                hi, ns["resqml2"] + "SequenceStratigraphySurface"
            )
            sss_node.set(
                ns["xsi"] + "type", ns["resqml2"] + "SequenceStratigraphySurface"
            )
...
class GeobodyBoundaryInterpretation:
    ...
    def create_xml(
        ...
        if self.genetic_boundary_feature is not None:
            ...
            self.genetic_boundary_feature = GeneticBoundaryFeature(
                self.model, root_node=genetic_boundary_feature_root
            )
        ...
        assert self.domain in [
            ...
            "time",
            "mixed",
        ], "illegal domain value for geobody boundary interpretation"
...
class GeobodyInterpretation:
    ...
    def __init__(
        ...
        if root_node is not None:
            ...
            self.has_occurred_during = extract_has_occurred_during(self.root_node)
            self.composition = rqet.find_tag_text(
                self.root_node, "GeologicUnitComposition"
            )
            self.implacement = rqet.find_tag_text(
                self.root_node, "GeologicUnitMaterialImplacement"
            )
    ...
    def create_xml(
        ...
        if self.geobody_feature is not None:
            ...
            self.geobody_feature = GeobodyFeature(
                self.model, root_node=geobody_feature_root
            )
        ...
        assert self.domain in [
            ...
            "time",
            "mixed",
        ], "illegal domain value for geobody interpretation"
        ...
        if self.composition:
            assert self.composition in [
                ...
                "other chemichal rock",  # chemichal (stet)
                "other chemical rock",
        ...
        if self.implacement:
            ...
            gumi_node = rqet.SubElement(
                gi, ns["resqml2"] + "GeologicUnitMaterialImplacement"
            )
            gumi_node.set(
                ns["xsi"] + "type", ns["resqml2"] + "GeologicUnitMaterialImplacement"
            )
            gumi_node.text = self.implacement

        if self.geobody_shape:
            # note: 'silt' & 'sheeth' believed erroneous, so 'sill' and 'sheet' added
            assert self.geobody_shape in [
                "dyke",
                "silt",
                "sill",
                "dome",
        ...
        if add_as_part:
            ...
            if add_relationships:
                self.model.create_reciprocal_relationship(
                    gi, "destinationObject", geobody_feature_root, "sourceObject"
                )
...
class WellboreInterpretation:
    ...
    def __init__(
        ...
        if extract_from_xml and self.root_node is not None:
            ...
            assert domain in [
                ...
                "time",
                "mixed",
            ], "unrecognised domain value for wellbore interpretation"
            self.domain = domain
            if wellbore_feature is not None:
                self.title = wellbore_feature.feature_name
    ...
    def create_xml(
        ...
        if self.wellbore_feature is not None:
            ...
            if wbf_root is not None:
                if wellbore_feature_root is None:
                    ...
                    assert (
                        wbf_root is wellbore_feature_root
                    ), "wellbore feature mismatch"
        ...
        assert self.domain in [
            ...
            "time",
            "mixed",
        ], "illegal domain value for wellbore interpretation"
