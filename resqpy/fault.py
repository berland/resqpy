...
class GridConnectionSet:
    ...
    def __init__(
        ...
        self.cell_index_pairs = (
            None  # shape (count, 2); dtype int; index normalized for flattened array
        )
        ...
        self.face_index_pairs = (
            None  # shape (count, 2); dtype int32; local to cell, ie. range 0 to 5
        )
        ...
        if extract_from_xml:
            if self.root is None:
                cs_parts_list = self.model.parts_list_related_to_uuid_of_type(
                    self.grid_list[0].uuid, "obj_GridConnectionSetRepresentation"
                )
                assert len(
                    cs_parts_list
                ), "no grid connection set parts found for grid in model"
                if len(cs_parts_list) > 1:
                    log.warning(
                        "more than one grid connection set part in model, using last in list"
                    )
            ...
            self.cell_index_pairs_null_value = rqet.find_nested_tags_int(
                self.root, ["CellIndexPairs", "NullValue"]
            )
            self.face_index_pairs_null_value = rqet.find_nested_tags_int(
                self.root, ["LocalFacePerCellIndexPairs", "NullValue"]
            )
            ...
            self.feature_list = (
                []
            )  # ordered list of (content_type, uuid, title) for faults
            for child in interp_root:
                if rqet.stripped_of_prefix(child.tag) != "FeatureInterpretation":
                    continue
                feature_type = rqet.content_type(
                    rqet.find_tag_text(child, "ContentType")
                )
                ...
                log.debug(
                    "connection set references fault interpretation: " + feature_title
                )
            log.debug(
                "number of faults referred to in connection set: "
                + str(len(self.feature_list))
            )
            assert (
                len(self.feature_list) > 0
            ), "list of fault interpretation references is empty for connection set"
            ...
            for child in interp_root:
                ...
                assert (
                    grid_type == "obj_IjkGridRepresentation"
                ), "only IJK grids supported for grid connection sets"
                grid = self.model.grid(
                    uuid=grid_uuid, find_properties=False
                )  # centralised list of grid objects for shared use
                self.grid_list.append(grid)
            if (
                len(self.grid_list) == 0
            ):  # this code only needed to handle defective datasets generated by earlier versions!
                grid = self.model.grid(
                    find_properties=False
                )  # should find only or ROOT grid
            ...
            if ascii_load_format and ascii_file:
                if ascii_load_format == "nexus":
                    log.debug(
                        "loading connection set (fault) faces from Nexus format ascii file: "
                        + ascii_file
                    )
                    ...
                    log.debug(
                        "ascii format for connection set faces not handled by base resqpy code: "
                        + ascii_load_format
                    )
                assert faces is not None, (
                    "failed to load fault face information from file: " + ascii_file
                )
                self.set_pairs_from_faces_df(
                    faces, create_organizing_objects_where_needed
                )
                ...
                self.set_pairs_from_face_masks(
                    k_faces,
                    j_faces,
                    i_faces,
                    feature_name,
    ...
    def set_pairs_from_kelp(
        ...
        if len(self.grid_list) > 1:
            log.warning(
                "setting grid connection set pairs from kelp for first grid in list only"
            )
    ...
    def set_pairs_from_face_masks(
        ...
        create_organizing_objects_where_needed,
        feature_type="fault",
    ):  # other feature_type values: 'horizon', 'geobody boundary'
        ...
        if len(self.grid_list) > 1:
            log.warning(
                "setting grid connection set pairs from face masks for first grid in list only"
            )
        grid = self.grid_list[0]
        ...
        for fi_part in fi_parts_list:
            if (
                self.model.title_for_part(fi_part).split()[0].lower()
                == feature_name.lower()
            ):
        ...
        if fi_uuid is None:
            if create_organizing_objects_where_needed:
                ...
                for tbf_part in tbf_parts_list:
                    if (
                        feature_name.lower()
                        == self.model.title_for_part(tbf_part).split()[0].lower()
                    ):
                        tbf_root = self.model.root_for_part(tbf_part)
                        if feature_type == "fault":
                            tbf = rqo.TectonicBoundaryFeature(
                                self.model, root_node=tbf_root
                            )
                        else:
                            tbf = rqo.GeneticBoundaryFeature(
                                self.model, kind=feature_type, root_node=tbf_root
                            )
                        break
                if tbf is None:
                    if feature_type == "fault":
                ...
                if feature_type == "fault":
                        ...
                        is_normal=True,
                    )  # todo: set is_normal based on fault geometry in grid?
                elif feature_type == "horizon":
                    fi = rqo.HorizonInterpretation(
                        self.model, extract_from_xml=False, genetic_boundary_feature=tbf
                    )
        ...
        if k_faces is not None:
            for cell_kji0 in np.stack(np.where(k_faces)).T:
                ...
                face_pair_list.append(
                    (self.face_index_map[0, 1], self.face_index_map[0, 0])
                )
        if j_faces is not None:
            for cell_kji0 in np.stack(np.where(j_faces)).T:
                ...
                face_pair_list.append(
                    (self.face_index_map[1, 1], self.face_index_map[1, 0])
                )
        if i_faces is not None:
            for cell_kji0 in np.stack(np.where(i_faces)).T:
                ...
                face_pair_list.append(
                    (self.face_index_map[2, 1], self.face_index_map[2, 0])
                )
    ...
    def set_pairs_from_faces_df(self, faces, create_organizing_objects_where_needed):
        ...
        if len(self.grid_list) > 1:
            log.warning(
                "setting grid connection set pairs from dataframe for first grid in list only"
            )
        grid = self.grid_list[0]
        ...
        for fi_part in fi_parts_list:
            fi_dict[
                self.model.title_for_part(fi_part).split()[0].lower()
            ] = self.model.uuid_for_part(fi_part)
        if create_organizing_objects_where_needed:
        ...
        for name in name_list:
            ...
            if name.lower() in fi_dict:
                ...
                if tbf is None:
                    tbf = rqo.TectonicBoundaryFeature(
                        self.model, extract_from_xml=False, feature_name=name
                    )
                    tbf_root = tbf.create_xml()
                fi = rqo.FaultInterpretation(
                    self.model,
                ...
                for k0 in range(entry["k1"], entry["k2"] + 1):
                    for j0 in range(entry["j1"], entry["j2"] + 1):
                        for i0 in range(entry["i1"], entry["i2"] + 1):
                            ...
                            cell_pair_list.append(
                                (
                                    grid.natural_cell_index((k0, j0, i0)),
                                    grid.natural_cell_index(neighbour),
                                )
                            )
                            face_pair_list.append(
                                (
                                    self.face_index_map[axis, fp],
                                    self.face_index_map[axis, 1 - fp],
                                )
                            )
            feature_index += 1
    ...
    def append(self, other):
        ...
        assert (
            len(self.grid_list) == 1 and len(other.grid_list) == 1
        ), "attempt to merge multi-grid connection sets"
        assert bu.matching_uuids(
            self.grid_list[0].uuid, other.grid_list[0].uuid
        ...
        if self.count is None or self.count == 0:
            ...
            combined_feature_indices = np.concatenate(
                (self.feature_indices, other.feature_indices)
            )
            combined_feature_indices[self.count :] += feature_offset
            combined_cell_index_pairs = np.concatenate(
                (self.cell_index_pairs, other.cell_index_pairs)
            )
            combined_face_index_pairs = np.concatenate(
                (self.face_index_pairs, other.face_index_pairs)
            )
            self.count += other.count
    ...
    def single_feature(self, feature_index=None, feature_name=None):
        ...
        if feature_index < 0 or feature_index >= len(self.feature_list):
            return None
        singleton = GridConnectionSet(
            self.model, extract_from_xml=False, grid=self.grid_list[0]
        )
        (
            singleton.cell_index_pairs,
            singleton.face_index_pairs,
    ...
    def filtered_by_layer_range(self, min_k0=None, max_k0=None):
        ...
        ), "attempt to filter multi-grid connection set by layer range"
        grid = self.grid_list[0]
        if min_k0 <= 0:
            min_k0 = None
    ...
    def filtered_by_cell_mask(self, mask, both_cells_required=True):
        ...
        assert (
            len(self.grid_list) == 1
        ), "attempt to filter multi-grid connection set by cell mask"
        grid = self.grid_list[0]
        ...
        indices = np.where(where_both)[
            0
        ]  # indices into primary axis of original arrays
        if len(indices) == 0:
        ...
        for i in range(len(keep_list)):
            ...
            if i != keep_list[i]:
                self.feature_indices[
                    np.where(self.feature_indices == keep_list[i])[0]
                ] = i
    ...
    def cache_arrays(self):
        ...
        if (
            self.cell_index_pairs is None
            or self.face_index_pairs is None
            or (self.feature_list is not None and self.feature_indices is None)
        ):
        ...
        if self.cell_index_pairs is None:
            ...
            h5_key_pair = self.model.h5_uuid_and_path_for_node(
                cell_index_pairs_node, tag="Values"
            )
        ...
        if self.face_index_pairs is None:
            ...
            face_index_pairs_node = rqet.find_tag(
                self.root, "LocalFacePerCellIndexPairs"
            )
            h5_key_pair = self.model.h5_uuid_and_path_for_node(
                face_index_pairs_node, tag="Values"
            )
            assert h5_key_pair is not None
            self.model.h5_array_element(
                h5_key_pair,
        ...
        if len(self.grid_list) > 1 and self.grid_index_pairs is None:
            ...
            assert (
                grid_index_node is not None
            ), "grid index pair data missing in grid connection set"
            h5_key_pair = self.model.h5_uuid_and_path_for_node(
                grid_index_node, tag="Values"
            )
            assert h5_key_pair is not None
            self.model.h5_array_element(
                h5_key_pair,
        ...
        if self.feature_indices is None:
            ...
            elements_node = rqet.find_nested_tags(
                interp_root, ["InterpretationIndices", "Elements"]
            )
            #         elements_node = rqet.find_nested_tags(interp_root, ['FaultIndices', 'Elements'])
            h5_key_pair = self.model.h5_uuid_and_path_for_node(
                elements_node, tag="Values"
            )
            assert h5_key_pair is not None
            self.model.h5_array_element(
                h5_key_pair,
            ...
            cl_node = rqet.find_nested_tags(
                interp_root, ["InterpretationIndices", "CumulativeLength"]
            )
            ...
            assert self.fi_cl.shape == (
                self.count,
            ), "connection set face pair(s) not assigned to exactly one fault"  # rough check
    ...
    def fault_name_for_feature_index(self, feature_index, strip=True):
        ...
        return self.feature_name_for_feature_index(
            feature_index=feature_index, strip=strip
        )
    ...
    def raw_list_of_cell_face_pairs_for_feature_index(self, feature_index):
        ...
        return (
            self.cell_index_pairs[matches],
            self.face_index_pairs[matches],
            self.grid_index_pairs[matches],
        )
    ...
    def simplified_sets_of_kelp_for_feature_index(self, feature_index):
        ...
        cell_pairs, face_pairs = self.list_of_cell_face_pairs_for_feature_index(
            feature_index
        )
        simple_j_set = (
            set()
    ...
    def rework_face_pairs(self):
        ...
        for i in range(self.count):
            ...
            if new_faces is not None and np.any(self.face_index_pairs[i] != new_faces):
                ...
                modifications += 1
        log.debug(
            "number of face pairs modified during neighbour based reworking: "
            + str(modifications)
        )
    ...
    def write_hdf5(self, file_name=None, mode="a"):
        ...
        h5_reg.register_dataset(
            self.uuid, "LocalFacePerCellIndexPairs", self.face_index_pairs
        )
        # uuid/FaultIndices/elements  (N,)  uint32
        h5_reg.register_dataset(
            self.uuid, "InterpretationIndices/elements", self.feature_indices
        )
        #      h5_reg.register_dataset(self.uuid, 'FaultIndices/elements', self.feature_indices)
        # uuid/FaultIndices/cumulativeLength  (N,)  uint32
        h5_reg.register_dataset(
            self.uuid, "InterpretationIndices/cumulativeLength", one_to_one
        )
    ...
    def create_xml(
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "CellIndexPairs", root=cip_values
        )
        ...
        if len(self.grid_list) > 1 and self.grid_index_pairs is not None:
            ...
            self.model.create_hdf5_dataset_ref(
                ext_uuid, self.uuid, "GridIndexPairs", root=gip_values
            )
        ...
        self.model.create_hdf5_dataset_ref(
            ext_uuid, self.uuid, "LocalFacePerCellIndexPairs", root=fip_values
        )
        ...
        if self.feature_indices is not None and self.feature_list is not None:
            ...
            self.model.create_hdf5_dataset_ref(
                ext_uuid, self.uuid, "InterpretationIndices/elements", root=el_values
            )
            ...
            for (f_content_type, f_uuid, f_title) in self.feature_list:
                ...
                if f_content_type == "obj_FaultInterpretation":
                    fi_part = rqet.part_name_for_object(
                        "obj_FaultInterpretation", f_uuid
                    )
                    ...
                    fi_part = rqet.part_name_for_object(
                        "obj_HorizonInterpretation", f_uuid
                    )
                    fi_root = self.model.root_for_part(fi_part)
                    self.model.create_ref_node(
                        "FeatureInterpretation",
        ...
        for grid in self.grid_list:
            self.model.create_ref_node(
                "Grid",
                self.model.title_for_root(grid.grid_root),
                grid.uuid,
            ...
            if add_relationships:
                for (obj_type, f_uuid, _) in self.feature_list:
                    ...
                    self.model.create_reciprocal_relationship(
                        gcs, "destinationObject", fi_root, "sourceObject"
                    )
                ...
                self.model.create_reciprocal_relationship(
                    gcs, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )
                self.model.create_reciprocal_relationship(
                    gcs,
                    "destinationObject",
                    self.grid_list[0].grid_root,
                    "sourceObject",
                )
    ...
    def write_simulator(
        ...
        def write_nexus_header_lines(fp, axis, polarity, fault_name, grid_name="ROOT"):
            ...
            if len(fault_name) > 256:
                log.warning(
                    "exported fault name longer than Nexus limit of 256 characters: "
                    + fault_name
                )
        ...
        def write_row(gcs, fp, name, i, j, k1, k2, axis, polarity):
            ...
            fp.write(
                "\t{0:1d}\t{1:1d}\t{2:1d}\t{3:1d}\t{4:1d}\t{5:1d}\t1.0\n".format(
        ...
        with open(filename, mode, newline="") as fp:
            for feature_index in range(len(self.feature_list)):
                ...
                (
                    cell_index_pairs,
                    face_index_pairs,
                ) = self.list_of_cell_face_pairs_for_feature_index(feature_index)
                for side in sides:
                    ...
                    i = (
                        ...
                        polarity
                    ) = None  # only needed to placate flake8 which whinges incorrectly otherwise
                    for row in range(cell_indices.shape[0]):
                        ...
                        if k is not None:
                            if (
                                axis_p != axis
                                or polarity_p != polarity
                                or ip != i
                                or jp != j
                                or kp != k2 + 1
                            ):
                                write_row(
                                    self, fp, feature_name, i, j, k, k2, axis, polarity
                                )
                                k = None
                            else:
                                k2 = kp
                ...
                if grid_ind_pairs is None or grid_ind_pairs[i, a_or_b] == gridindex:
                    if face_ind_pairs[i, a_or_b, 0] != 0:
                        entry = (
                            ...
                            int(face_ind_pairs[i, a_or_b, 0])
                            - 1,  # in the outputs j=0, i=1
                            int(face_ind_pairs[i, a_or_b, 1]),
                        )  # in the outputs negativeface=0, positiveface=1
    ...
    def get_column_edge_bool_array_for_feature(
        self, feature, gridindex=0, min_k=0, max_k=0
    ):
        ...
        """
        cell_face_list = self.get_column_edge_list_for_feature(
            feature, gridindex, min_k, max_k
        )

        fault_by_column_edge_mask = np.zeros(
            (self.grid_list[gridindex].nj, self.grid_list[gridindex].ni, 2, 2),
            dtype=bool,
        )
    ...
    def get_property_by_feature_index_list(
        self, feature_index_list=None, property_name="Transmissibility multiplier"
    ):
    ...
    def get_column_edge_float_array_for_feature(
        self,
        feature,
        ...
        prop_values = self.get_property_by_feature_index_list(
            feature_index_list=[feature], property_name=property_name
        )
        if prop_values == []:
            return None
        property_value_by_column_edge = np.where(
            fault_by_column_edge_mask, prop_values[0], np.nan
        )
        return property_value_by_column_edge

    def get_combined_fault_mask_index_value_arrays(
        self,
        gridindex=0,
        min_k=0,
        max_k=0,
        ...
        if feature_list is None:
            feature_list = range(len(self.feature_list))
        sum_unmasked = None

        for i, feature in enumerate(feature_list):
            fault_by_column_edge_mask = self.get_column_edge_bool_array_for_feature(
                feature, gridindex, min_k=min_k, max_k=max_k
            )
            property_value_by_column_edge = (
                self.get_column_edge_float_array_for_feature(
                    feature, fault_by_column_edge_mask, property_name=property_name
                )
            )
            if i == 0:
                ...
                combined_index = np.full(
                    (fault_by_column_edge_mask.shape), -1, dtype=int
                )
                combined_index = np.where(
                    fault_by_column_edge_mask, feature, combined_index
                )
    ...
    def tr_property_array(
        self, fa=None, tol_fa=0.0001, tol_half_t=1.0e-5, apply_multipliers=False
    ):
        ...
        feature_mult_list = (
            self.get_property_by_feature_index_list() if apply_multipliers else None
        )
        ...
        for grid in self.grid_list:
            half_t_list.append(
                grid.half_cell_transmissibility()
            )  # todo: pass realization argument
        single_grid = self.number_of_grids() == 1
        kji0 = (
            self.grid_list[0].denaturalized_cell_indices(self.cell_index_pairs)
            if single_grid
            else None
        )
        ...
        for e in range(count):
            axis_m, polarity_m = self.face_index_inverse_map[
                self.face_index_pairs[e, 0]
            ]
            axis_p, polarity_p = self.face_index_inverse_map[
                self.face_index_pairs[e, 0]
            ]
            if single_grid:
                kji0_m = kji0[e, 0]
                ...
                kji0_m = self.grid_list[gi_m].denaturalized_cell_index(
                    self.cell_index_pairs[e, 0]
                )
                kji0_p = self.grid_list[gi_p].denaturalized_cell_index(
                    self.cell_index_pairs[e, 1]
                )
            half_t_m = half_t_list[gi_m][
                kji0_m[0], kji0_m[1], kji0_m[2], axis_m, polarity_m
            ]
            half_t_p = half_t_list[gi_p][
                kji0_p[0], kji0_p[1], kji0_p[2], axis_p, polarity_p
            ]
            ...
            if fa is not None:
                ...
                if fa_m < tol_fa or fa_p < tol_fa:
                    continue
            mult_tr = (
                feature_mult_list[self.feature_indices[e]] if apply_multipliers else 1.0
            )
    ...
    def inherit_features(self, featured):
        ...
        def sorted_paired_cell_face_index_position(cell_face_index, a_or_b):
            ...
            t = [
                tuple(r) for r in sp
            ]  # could use numpy fields based sort instead of tuple list?
            t.sort()
            return np.array(t)
        ...
        featured_index_map = (
            []
        )  # maps from feature index in featured to extended feature list in this set
        feature_uuid_list = [
            bu.uuid_from_string(u) for _, u, _ in self.feature_list
        ]  # bu call probably not needed
        for featured_triplet in featured.feature_list:
            featured_uuid = bu.uuid_from_string(
                featured_triplet[1]
            )  # bu call probably not needed
            if featured_uuid in feature_uuid_list:
                ...
                self.model.copy_part_from_other_model(
                    featured, featured.model.part(uuid=featured_uuid)
                )
        ...
        cfp_a_featured = sorted_paired_cell_face_index_position(
            cell_face_index_featured, 0
        )
        cfp_b_featured = sorted_paired_cell_face_index_position(
            cell_face_index_featured, 1
        )
        ...
        for (a_or_b, cfp_self) in [
            (0, cfp_a_self),
            (1, cfp_b_self),
        ]:  # could risk being lazy and only using one side?
            for (cell_face_index, place) in cfp_self:
                ...
                if cell_face_index == previous_cell_face_index:
                    ...
                    if featured_place is None:
                        ...
                        self.feature_indices[
                            place
                        ] = previous_feature_index = featured_index_map[featured_index]
    ...
    def compact_indices(self):
        ...
        null_mask = np.logical_or(
            self.cell_index_pairs == self.cell_index_pairs_null_value,
    ...
    for j in range(grid.nj):
        for i in range(grid.ni):
            ...
            while True:
                ...
                if (skip_inactive and dead[ka, j, i]) or (
                    grid.k_gaps and grid.k_gap_after[ka]
                ):
                    ka += 1
                    continue
                kb = ka + 1
                ...
                cip_list.append(
                    (
                        grid.natural_cell_index((ka, j, i)),
                        grid.natural_cell_index((kb, j, i)),
                    )
                )
                ka = kb + 1
...
def k_gap_connection_set(
    grid, skip_inactive=True, feature_name="k gap connection", tolerance=0.001
):
    ...
    dead = grid.extract_inactive_mask() if skip_inactive else None
    flip_z = grid.k_direction_is_down != rqet.find_tag_bool(
        grid.crs_root, "ZIncreasingDownward"
    )
    ...
    for k in range(grid.nk - 1):
        if grid.k_gap_after_array[k]:
            k_gap_pillar_z = (
                p[grid.k_raw_index_array[k + 1]][..., 2]
                - p[grid.k_raw_index_array[k] + 1][..., 2]
            )
            if grid.has_split_coordinate_lines:
                pfc = grid.create_column_pillar_mapping()  # pillars for column
                k_gap_z = 0.25 * np.sum(
                    k_gap_pillar_z[pfc], axis=(2, 3)
                )  # resulting shape (nj, ni)
            else:
                k_gap_z = 0.25 * (
                    k_gap_pillar_z[:-1, :-1]
                    + k_gap_pillar_z[:-1, 1:]
        ...
        layer_mask = np.logical_and(
            np.logical_not(np.isnan(k_gap_z)), k_gap_z < tolerance
        )
        if skip_inactive:
            layer_mask = np.logical_and(
                layer_mask, np.logical_not(np.logical_or(dead[k], dead[k + 1]))
            )
        # layer mask now boolean array of shape (nj, ni) set True where connection needed
        ji_list = np.stack(
            np.where(layer_mask)
        ).T  # numpy array being list of [j, i] pairs
        for (j, i) in ji_list:
            cip_list.append(
                (
                    grid.natural_cell_index((k, j, i)),
                    grid.natural_cell_index((k + 1, j, i)),
                )
            )
...
def add_connection_set_and_tmults(model, fault_incl, tmult_dict):
    ...
    Args:
       ...
       tmult_dict - dictionary of fault name: tmult pairs (must align with faults in include file)
    Returns:
    ...
    for fault in tmult_dict.keys():
        ...
        tbf = rqo.TectonicBoundaryFeature(
            parent_model=model, extract_from_xml=False, feature_name=fault, kind="fault"
        )
        tbf.create_xml()
        fint = rqo.FaultInterpretation(
            parent_model=model,
            extract_from_xml=False,
            tectonic_boundary_feature=tbf,
            is_normal=True,
    ...
    if isinstance(fault_incl, list):
        if len(fault_incl) > 1:
            ...
            temp_faults = os.path.join(
                os.path.dirname(model.epc_file), "faults_combined_temp.txt"
            )
    ...
    model.store_epc()
    if os.path.exists(
        os.path.join(os.path.dirname(model.epc_file), "faults_combined_temp.txt")
    ):
        os.remove(
            os.path.join(os.path.dirname(model.epc_file), "faults_combined_temp.txt")
        )
...
def standardize_face_indicator_in_faces_df(faces):
    ...
    temp[faces["face"].str.len() == 1] = (
        faces["face"][faces["face"].str.len() == 1] + "+"
    )
    for u in temp.unique():
        s = str(u)
        assert len(s) == 2, "incorrect length to face string in fault dataframe: " + s
        assert s[0] in "IJK", (
            "unknown direction (axis) character in fault dataframe: " + s
        )
...
def remove_external_faces_from_faces_df(faces, extent_kji, remove_all_k_faces=False):
    ...
    for i in range(len(faces)):
        ...
        if (
            (entry["i1"] <= 0 and f == "I-")
            or (entry["j1"] <= 0 and f == "J-")
            or (entry["k1"] <= 0 and f == "K-")
            or (entry["i2"] >= max_i0 and f == "I+")
            or (entry["j2"] >= max_j0 and f == "J+")
            or (entry["k2"] >= max_k0 and f == "K+")
        ):
            continue
...
def _make_k_gcs_from_cip_list(grid, cip_list, feature_name):
    ...
    pcs.feature_indices = np.zeros(
        count, dtype=int
    )  # could create seperate features by layer above or below?
    gbf = rqo.GeneticBoundaryFeature(
        grid.model, extract_from_xml=False, kind="horizon", feature_name=feature_name
    )
    gbf_root = gbf.create_xml()
    fi = rqo.HorizonInterpretation(
        grid.model, extract_from_xml=False, genetic_boundary_feature=gbf
    )
