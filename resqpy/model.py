...
class Model:
    ...
    def initialize(self):
        ...
        self.h5_dict = (
            {}
        )  # dictionary keyed on hdf5 uuid.bytes; mapping to hdf5 file name (full path)
        ...
        self.time_series = (
            None  # extracted as speed optimization (single time series only for now)
        )
        self.parts_forest = (
            {}
        )  # dictionary keyed on part_name; mapping to (content_type, uuid, xml_tree)
        self.rels_present = False
        self.rels_forest = (
            {}
        )  # dictionary keyed on part_name; mapping to (uuid, xml_tree)
        self.other_forest = (
            {}
        )  # dictionary keyed on part_name; mapping to (content_type, xml_tree); used for docProps
    ...
    def parts(
        ...
        if uuid is not None:
            ...
            for part in parts_list:
                if bu.matching_uuids(uuid, self.uuid_for_part(part)):
        ...
        if related_uuid is not None:
            parts_list = self.parts_list_filtered_by_related_uuid(
                parts_list, related_uuid
            )
        if len(parts_list) == 0:
            return None
        if sort_by:
    ...
    def load_part(self, epc, part_name, is_rels=None):
        ...
        try:
            ...
            if is_rels:
                ...
                (part_type, part_uuid, _) = self.parts_forest[
                    part_name
                ]  # part_type must already have been established
            ...
            with epc.open(part_name) as part_xml:
                ...
                if is_rels:
                    ...
                    if (
                        self.crs_root is None and part_type == "obj_LocalDepth3dCrs"
                    ):  # randomly assign first crs as primary crs for model
    ...
    def tidy_up_forests(
        self, tidy_main_tree=True, tidy_others=False, remove_extended_core=True
    ):
        ...
        if tidy_others:
            ...
            for part in deletion_list:
                log.debug(
                    "removing docProps part due to lack of xml tree etc.: " + str(part)
                )
                if tidy_main_tree:
                    self.remove_part_from_main_tree(part)
                del self.other_forest[part]
        if (
            remove_extended_core and "docProps/extendedCore.xml" in self.other_forest
    ...
    def load_epc(self, epc_file, full_load=True, epc_subdir=None, copy_from=None):
        ...
        if self.modified:
            log.warning(
                "loading model from epc, discarding previous in-memory modifications"
            )
            self.initialize()
            ...
            for name in names:
                ...
                if name != "[Content_Types].xml":
                    if name.startswith("docProps"):
                        self.other_forest[name] = (
                            None,
                            None,
                        )  # used for non-uuid parts, ie. docProps
                    else:
                    ...
                    if rqet.stripped_of_prefix(child.tag) == "Override":
                        ...
                        if part_name.startswith("docProps"):
                            if part_name not in self.other_forest:
                                log.warning(
                                    "docProps entry in Content_Types does not exist as part in epc: "
                                    + part_name
                                )
                                continue
                            ...
                            if part_name not in self.parts_forest:
                                if epc_subdir is None:
                                    log.warning(
                                        "entry in Content_Types does not exist as part in epc: "
                                        + part_name
                                    )
                                continue
                        ...
                        if (
                            "Extension" in child.attrib.keys()
                            and child.attrib["Extension"] == "rels"
                        ):
                ...
                for name in names:
                    ...
                    if name.startswith("_rels/"):
                        ...
                        if not load_success:
                            self.fell_part(part_name)
                if copy_from:
                    self.change_filename_in_hdf5_rels(
                        os.path.split(epc_file)[1][:-4] + ".h5"
                    )
    ...
    def store_epc(
        self, epc_file=None, main_xml_name="[Content_Types].xml", only_if_modified=False
    ):
        ...
        with zf.ZipFile(epc_file, mode="w") as epc:
            ...
            for part_name, (_, part_tree) in self.other_forest.items():
                if part_tree is None:
                    log.warning(
                        "No xml tree present to write for other part: " + part_name
                    )
                    continue
                if part_name[0] == "/":
                    part_name = part_name[1:]
            ...
            if self.rels_present:
                for part_name, (_, part_tree) in self.rels_forest.items():
                    if part_tree is None:
                        log.warning(
                            "No xml tree present to write for rels part: " + part_name
                        )
                        continue
                    with epc.open(part_name, mode="w") as part_xml:
                        rqet.write_xml(part_xml, part_tree, standalone="yes")
    ...
    def parts_list_of_type(self, type_of_interest=None, uuid=None):
        ...
        for part_name in self.parts_forest:
            if (
                type_of_interest is None
                or self.parts_forest[part_name][0] == type_of_interest
            ) and (
                uuid is None or bu.matching_uuids(uuid, self.parts_forest[part_name][1])
            ):
    ...
    def list_of_parts(self, only_objects=True):
        ...
        for part in pl:
            ...
            if dir_free_part.startswith("obj_") and not dir_free_part.startswith(
                "obj_Epc"
            ):
    ...
    def parts_list_filtered_by_related_uuid(
        self, parts_list, uuid, uuid_is_source=None
    ):
        ...
        if this_part is not None:
            rels_part_root = self.root_for_part(
                rqet.rels_part_name_for_part(this_part), is_rels=True
            )
        ...
        for part in parts_list:
            if part in filtered_list:
                continue
            rels_part_root = self.root_for_part(
                rqet.rels_part_name_for_part(part), is_rels=True
            )
    ...
    def supporting_representation_for_part(self, part):
        ...
        return bu.uuid_from_string(
            rqet.find_nested_tags_text(
                self.root_for_part(part), ["SupportingRepresentation", "UUID"]
            )
        )
    ...
    def change_uuid_in_hdf5_references(self, node, old_uuid, new_uuid):
        ...
        for ref_node in node.iter(ns["eml"] + "PathInHdfFile"):
            try:
                ...
                new_path_in_hdf = (
                    ref_node.text[:uuid_place]
                    + new_uuid_str
                    + ref_node.text[uuid_place + len(old_uuid_str) :]
                )
                log.debug(
                    "path in hdf update from: "
                    + ref_node.text
                    + " to: "
                    + new_path_in_hdf
                )
    ...
    def change_filename_in_hdf5_rels(self, new_hdf5_filename=None):
        ...
        for rel_name, entry in self.rels_forest.items():
            ...
            for child in rel_root:
                if (
                    child.attrib["Id"] == "Hdf5File"
                    and child.attrib["TargetMode"] == "External"
                ):
                    child.attrib["Target"] = new_hdf5_filename
                    count += 1
        log.info(
            str(count) + " hdf5 filename" + _pl(count) + " set to: " + new_hdf5_filename
        )
        if count > 0:
            self.set_modified()
    ...
    def copy_part(self, existing_uuid, new_uuid, change_hdf5_refs=False):
        ...
        new_uuid_str = str(new_uuid)
        log.debug(
            "copying xml part from uuid: " + old_uuid_str + " to uuid: " + new_uuid_str
        )
        existing_parts_list = self.parts_list_of_type(uuid=existing_uuid)
        if len(existing_parts_list) == 0:
            log.warning(
                "failed to find existing part for copying with uuid: " + old_uuid_str
            )
            return None
        assert len(existing_parts_list) == 1, (
            "more than one existing part found with uuid: " + old_uuid_str
        )
    ...
    def root_for_ijk_grid(self, uuid=None, title=None):
        ...
        if uuid is None and not title:
            ...
            grid_root = self.root(
                obj_type="IjkGridRepresentation", uuid=uuid, title=title
            )
    ...
    def citation_title_for_part(
        self, part
    ):  # duplicate functionality to title_for_part()
    ...
    def add_grid(self, grid_object, check_for_duplicates=False):
        ...
        if check_for_duplicates:
            for g in self.grid_list:
                if g.grid_root is grid_object.grid_root:
                    return
                    ...
                    if (
                        ...
                        if target_path:
                            if self.epc_directory:
                                ...
                                h5_full_path = os.path.join(
                                    self.epc_directory, target_file
                                )
                            else:
    ...
    def h5_access(self, uuid=None, mode="r", override=True, file_path=None):
        ...
        if (
            self.h5_currently_open_mode is not None
            and self.h5_currently_open_mode != mode
        ):
            self.h5_release()
        if file_path:
            file_name = file_path
        else:
            file_name = self.h5_file_name(
                uuid=uuid, override=override, file_must_exist=(mode == "r")
            )
        ...
        self.h5_currently_open_root = h5py.File(
            file_name, mode
        )  # could use try to trap file in use errors?
        return self.h5_currently_open_root
    ...
    def h5_array_element(
        ...
        def reshaped_index(index, shape_tuple, required_shape):
            ...
            if tail > 0:
                assert (
                    shape_tuple[-tail:] == required_shape[-tail:]
                ), "not enough indices to allow reshaped indexing"
            natural = 0
            extent = 1
        ...
        if cache_array:
            ...
            if required_shape is None or shape_tuple == required_shape:
                ...
                object.__dict__[array_attribute][:] = np.array(
                    h5_root[h5_key_pair[1]], dtype=dtype
            ...
            if result.size == 1:
                if dtype is float or (
                    ...
                    return float(result)
                elif dtype is int or (
                    isinstance(dtype, str) and dtype.startswith("int")
                ):
                    return int(result)
                elif dtype is bool or (
                    isinstance(dtype, str) and dtype.startswith("bool")
                ):
                    return bool(result)
    ...
    def add_part(
        self, content_type, uuid, root, add_relationship_part=True, epc_subdir=None
    ):
        ...
        if use_other:
            ...
            part_name = rqet.part_name_for_object(
                content_type, uuid, prefixed=False, epc_subdir=epc_subdir
            )
    ...
    def remove_part(self, part_name, remove_relationship_part=True):
        ...
        if remove_relationship_part:
            if "docProps" in part_name:
                ...
                related_parts = self.parts_list_filtered_by_related_uuid(
                    self.list_of_parts(), rqet.uuid_in_part_name(part_name)
                )
                for relative in related_parts:
                    (rel_uuid, rel_tree) = self.rels_forest[
                        rqet.rels_part_name_for_part(relative)
                    ]
    ...
    def referenced_node(self, ref_node, consolidate=False):
        ...
        if (
            ...
            and uuid in self.consolidation.map
        ):
    ...
    def create_ref_node(self, flavour, title, uuid, content_type=None, root=None):
        ...
        if use_version_string:
            version_str = rqet.SubElement(
                ref_node, ns["eml"] + "VersionString"
            )  # I'm guessing what this is
    ...
    def title_for_part(
        self, part_name
    ):  # duplicate functionality to citation_title_for_part()
    ...
    def create_doc_props(self, add_as_part=True, root=None, originator=None):
        ...
        created.set(
            ns["xsi"] + "type", ns["dcterms"] + "W3CDTF"
        )  # not sure of namespace here
        ...
        if add_as_part:
            ...
            if self.rels_present:
                ...
                core_rel = rqet.SubElement(
                    rel_tree.getroot(), ns["rels"] + "Relationship"
                )
    ...
    def create_crs(
        ...
        warnings.warn(
            "model.create_crs is Deprecated, will be removed", DeprecationWarning
        )
        crs = rqc.Crs(
            self,
            x_offset=x_offset,
        ...
        crs_node = crs.create_xml(
            add_as_part=add_as_part, root=root, title=title, originator=originator
        )
    ...
    def create_hdf5_ext(
        ...
        if add_as_part:
            ...
            if self.rels_present and file_name:
                (uuid, rel_tree) = self.rels_forest[
                    rqet.rels_part_name_for_part(
                        rqet.part_name_for_object(
                            "obj_EpcExternalPartReference", ext_uuid
                        )
                    )
                ]
                assert bu.matching_uuids(uuid, ext_uuid)
                rel_node = rqet.SubElement(
                    rel_tree.getroot(), ns["rels"] + "Relationship"
                )
    ...
    def create_hdf5_dataset_ref(
        self, hdf5_uuid, object_uuid, group_tail, root, title="Hdf Proxy"
    ):
    ...
    def create_supporting_representation(
        self,
        grid_root=None,
        support_uuid=None,
        root=None,
    ...
    def create_patch(
        ...
        outer_values_node.set(
            ns["xsi"] + "type", ns["resqml2"] + hdf5_type
        )  # may also be constant array type
        ...
        if discrete:
            if null_value is None:
                if str(xsd_type).startswith("u"):
                    null_value = (
                        4294967295  # 2^32 - 1, used as default even for 64 bit data!
                    )
                else:
                    null_value = -1
            null_value_node = rqet.SubElement(
                outer_values_node, ns["resqml2"] + "NullValue"
            )
        ...
        if const_value is None:
            ...
            inner_values_node = rqet.SubElement(
                outer_values_node, ns["resqml2"] + "Values"
            )
            inner_values_node.set(ns["xsi"] + "type", ns["eml"] + "Hdf5Dataset")
            inner_values_node.text = rqet.null_xml_text

            self.create_hdf5_dataset_ref(
                ext_uuid,
                p_uuid,
                "values_patch{}".format(patch_index),
                root=inner_values_node,
            )

        else:

            const_value_node = rqet.SubElement(
                outer_values_node, ns["resqml2"] + "Value"
            )
            const_value_node.set(ns["xsi"] + "type", ns["xsd"] + xsd_type)
            const_value_node.text = str(const_value)

            const_count_node = rqet.SubElement(
                outer_values_node, ns["resqml2"] + "Count"
            )
    ...
    def create_time_series_ref(self, time_series_uuid, root=None):
        ...
        return self.create_ref_node(
            "TimeSeries", "time series", time_series_uuid, root=root
        )
    ...
    def create_reciprocal_relationship(
        self, node_a, rel_type_a, node_b, rel_type_b, avoid_duplicates=True
    ):
        ...
        def id_str(uuid):
            stringy = str(uuid)
            if (
                not (rqet.pretend_to_be_fesapi or rqet.use_fesapi_quirks)
                or not stringy[0].isdigit()
            ):
                return stringy
            return "_" + stringy
        ...
        if avoid_duplicates:
            ...
            for existing in existing_rel_nodes:
                if (
                    rqet.stripped_of_prefix(existing.attrib["Type"]) == rel_type_a
        ...
        if avoid_duplicates:
            ...
            for existing in existing_rel_nodes:
                if (
                    rqet.stripped_of_prefix(existing.attrib["Type"]) == rel_type_b
    ...
    def duplicate_node(self, existing_node, add_as_part=True):
        ...
        if add_as_part:
            ...
            if self.part_for_uuid(uuid) is None:
                ...
                log.warning(
                    "rejected attempt to add a duplicated part with an existing uuid"
                )
        return new_node
    ...
    def copy_part_from_other_model(
        self,
        other_model,
        part,
        realization=None,
        ...
        if not force:
            assert (
                self.part_for_uuid(uuid) is None
            ), "part copying failure: uuid exists for different part!"
        ...
        if consolidate and not force:
            if self.consolidation is None:
                self.consolidation = cons.Consolidation(self)
            resident_uuid = self.consolidation.equivalent_uuid_for_part(
                part, immigrant_model=other_model
            )
        else:
            resident_uuid = None
        ...
        if resident_uuid is None:
            ...
            if realization is not None and rqet.node_type(root_node).endswith(
                "Property"
            ):
                ri_node = rqet.find_tag(root_node, "RealizationIndex")
                if ri_node is None:
                    ri_node = rqet.SubElement(
                        root_node, ns["resqml2"] + "RealizationIndex"
                    )
            ...
            # copy hdf5 data
            hdf5_count = whdf5.copy_h5(
                other_h5_file_name,
                self_h5_file_name,
                uuid_inclusion_list=[uuid],
                mode="a",
            )
            ...
            if hdf5_count:
                ...
                ext_part = rqet.part_name_for_object(
                    "obj_EpcExternalPartReference", h5_uuid, prefixed=False
                )
            ...
            for ref_node in rqet.list_obj_references(root_node):
                ...
                if consolidate:
                    resident_referred_node = self.referenced_node(
                        ref_node, consolidate=True
                    )
                if force:
                    continue
                if resident_referred_node is None:
                    ...
                    if referred_node is None:
                        ...
                        if (
                            other_model.type_of_part(referred_part)
                            == "obj_EpcExternalPartReference"
                        ):
                            continue
                        if referred_part in self.list_of_parts():
                            continue
                        self.copy_part_from_other_model(
                            other_model, referred_part, consolidate=consolidate
                        )
        ...
        for source_flag in [True, False]:
            other_related_parts = other_model.parts_list_filtered_by_related_uuid(
                other_model.list_of_parts(), resident_uuid, uuid_is_source=source_flag
            )
            for related_part in other_related_parts:
                ...
                if not force and (related_part in self.parts_forest):
                    ...
                    if consolidate:
                        resident_related_uuid = (
                            self.consolidation.equivalent_uuid_for_part(
                                related_part, immigrant_model=other_model
                            )
                        )
                        if resident_related_uuid is None:
                            continue
                        resident_related_part = rqet.part_name_for_object(
                            other_model.type_of_part(related_part),
                            resident_related_uuid,
                        )
                ...
                if (
                    not force
                    and resident_related_part
                    in self.parts_list_filtered_by_related_uuid(
    ...
    def copy_all_parts_from_other_model(
        self, other_model, realization=None, consolidate=True
    ):
    ...
    def sort_parts_list_by_timestamp(self, parts_list):
        ...
        sort_list = []
        for index, part in enumerate(parts_list):
            timestamp = rqet.find_nested_tags_text(
                self.root_for_part(part), ["Citation", "Creation"]
            )
...
def new_model(epc_file):
    ...
    return Model(
        epc_file=epc_file, new_epc=True, create_basics=True, create_hdf5_ext=True
    )
