...
class Crs:
    ...
    def __init__(
        ...
        if crs_root is not None:
            ...
            self.rotation = rqet.find_tag_float(
                self.crs_root, "ArealRotation"
            )  # todo: extract uom attribute from this node
            parent_xy_crs = rqet.find_tag(self.crs_root, "ProjectedCrs")
            if (
                parent_xy_crs is not None
                and rqet.node_type(parent_xy_crs) == "ProjectedCrsEpsgCode"
            ):
                self.epsg_code = rqet.find_tag_text(
                    parent_xy_crs, "EpsgCode"
            ...
            if axis_order is not None:
                assert axis_order in [
                    ...
                    "northing westing",
                    "westing northing",
                ], "invalid axis order: " + str(axis_order)
        ...
        self.rotated = not maths.isclose(
            self.rotation, 0.0, abs_tol=1e-8
        ) and not maths.isclose(self.rotation, 2.0 * maths.pi, abs_tol=1e-8)
        if self.rotated:
            self.rotation_matrix = vec.rotation_matrix_3d_axial(
                2, maths.degrees(self.rotation)
            )
            self.reverse_rotation_matrix = vec.rotation_matrix_3d_axial(
                2, maths.degrees(-self.rotation)
            )

        self.null_transform = (
            maths.isclose(self.x_offset, 0.0, abs_tol=1e-8)
            and maths.isclose(self.y_offset, 0.0, abs_tol=1e-8)
            and maths.isclose(self.z_offset, 0.0, abs_tol=1e-8)
            and not self.rotated
        )
    ...
    def is_right_handed_xyz(self):
        ...
        return (
            self.axis_order
            in ["northing easting", "southing westing", "westing northing"]
            == self.z_inc_down
        )
    ...
    def has_same_epsg_code(self, other_crs):
        ...
        if (
            self.epsg_code is None
            or other_crs.epsg_code is None
            or self.epsg_code == other_crs.epsg_code
        ):
            return True
        return False
    ...
    def is_equivalent(self, other_crs):
        ...
        if self.z_inc_down != other_crs.z_inc_down:
            return False
        if (
            self.time_units is not None or other_crs.time_units is not None
        ) and self.time_units != other_crs.time_units:
            return False
        ...
        if self.null_transform and other_crs.null_transform:
            return True
        if (
            maths.isclose(self.x_offset, other_crs.x_offset, abs_tol=1e-4)
            and maths.isclose(self.y_offset, other_crs.y_offset, abs_tol=1e-4)
            and maths.isclose(self.z_offset, other_crs.z_offset, abs_tol=1e-4)
            and maths.isclose(self.rotation, other_crs.rotation, abs_tol=1e-4)
        ):
            return True
    ...
    def convert_to(self, other_crs, xyz):
        ...
        xyz = (
            wam.convert_lengths(xyz[0], self.xy_units, other_crs.xy_units),
    ...
    def convert_from(self, other_crs, xyz):
        ...
        xyz = (
            wam.convert_lengths(xyz[0], other_crs.xy_units, self.xy_units),
    ...
    def create_xml(
        ...
        if add_as_part:
            self.model.add_part(
                "obj_" + flavour, bu.uuid_from_string(crs.attrib["uuid"]), crs
            )
        if self.model.crs_root is None:
            self.model.crs_root = crs
