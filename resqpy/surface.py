...
class _BaseSurface:
    ...
    def create_interpretation_and_feature(
        self, kind="horizon", name=None, interp_title_suffix=None, is_normal=True
    ):
        ...
        if kind in ["horizon", "geobody boundary"]:
            feature = rqo.GeneticBoundaryFeature(
                self.model, kind=kind, extract_from_xml=False, feature_name=name
            )
            feature.create_xml()
            if kind == "horizon":
            ...
            feature = rqo.TectonicBoundaryFeature(
                self.model, kind=kind, extract_from_xml=False, feature_name=name
            )
            feature.create_xml()
...
class TriangulatedPatch:
    ...
    def extract_crs_root_and_uuid(self):
        ...
        if self.crs_root is None:
            ...
            self.crs_uuid = bu.uuid_from_string(
                rqet.find_tag_text(self.crs_root, "UUID")
            )
        return self.crs_root, self.crs_uuid
    ...
    def triangles_and_points(self):
        ...
        if h5_key_pair is None:
            log.warning(
                "No Triangles found in xml for patch index: " + str(self.patch_index)
            )
            return (None, None)
        try:
            self.model.h5_array_element(
            ...
            log.error(
                "hdf5 triangles failure for triangle patch " + str(self.patch_index)
            )
            raise
        return (self.triangles, self.points)
    ...
    def set_to_sail(self, n, centre, radius, azimuth, delta_theta):
        ...
        log.debug(
            "sail xyz box: {0:4.2f}:{1:4.2f}  {2:4.2f}:{3:4.2f}  {4:4.2f}:{5:4.2f}".format(
    ...
    def set_from_irregular_mesh(self, mesh_xyz, quad_triangles=False):
        ...
        if quad_triangles:
            ...
            self.points = np.concatenate(
                (mesh_xyz.copy().reshape((-1, 3)), quad_centres)
            )
            ...
            triangles = np.empty(
                (mesh_shape[0] - 1, mesh_shape[1] - 1, 4, 3), dtype=int
            )  # flatten later
            nic = ni - 1
            ...
            triangles = np.empty(
                (mesh_shape[0] - 1, mesh_shape[1] - 1, 2, 3), dtype=int
            )  # flatten later
            for j in range(mesh_shape[0] - 1):
    ...
    def set_from_sparse_mesh(self, mesh_xyz):
        ...
        self.node_count = n_p
        triangles = np.zeros(
            (2 * (mesh_shape[0] - 1) * (mesh_shape[1] - 1), 3), dtype=int
        )  # truncate later
        nt = 0
        for j in range(mesh_shape[0] - 1):
            for i in range(mesh_shape[1] - 1):
                nan_nodes = np.count_nonzero(
                    np.isnan(mesh_xyz[j : j + 2, i : i + 2, 2])
                )
                if nan_nodes > 1:
                    continue
                if nan_nodes == 0:
    ...
    def set_from_torn_mesh(self, mesh_xyz, quad_triangles=False):
        ...
        if quad_triangles:
            ...
            self.points = np.concatenate(
                (mesh_xyz.copy().reshape((-1, 3)), quad_centres.reshape((-1, 3)))
            )
            mesh_size = mesh_xyz.size // 3
    ...
    def set_to_cell_faces_from_corner_points(self, cp, quad_triangles=True):
        ...
        if quad_triangles:
            ...
            self.node_count = 14
            self.points = np.concatenate(
                (cp.copy().reshape((-1, 3)), quad_centres.reshape((-1, 3)))
            )
            ...
            for axis in range(3):
                ...
                for ip in range(2):
                    ...
                    triangles[axis, ip, 0, 2] = triangles[
                        axis, ip, 1, 1
                    ] = base_p + ips * (ip2)
                    triangles[axis, ip, 1, 2] = triangles[
                        axis, ip, 2, 1
                    ] = base_p + ips * (ip1 + ip2)
                    triangles[axis, ip, 2, 2] = triangles[
                        axis, ip, 3, 1
                    ] = base_p + ips * (ip1)
...
class Surface(_BaseSurface):
    ...
    def __init__(
        ...
        self.boundaries = (
            None  # todo: read up on what this is for and look out for examples
        )
        ...
        if extract_from_xml:
            ...
            self.set_from_mesh_file(
                mesh_file, mesh_format, quad_triangles=quad_triangles
            )
    ...
    def extract_patches(self, surface_root):
        ...
        for child in surface_root:
            ...
            triangulated_patch = TriangulatedPatch(
                self.model, patch_index=patch_index, patch_node=child
            )
        ...
        assert (
            len(paired_list)
            and paired_list[0][0] == 0
            and len(paired_list) == paired_list[-1][0] + 1
        )
        ...
        for triangulated_patch in self.patch_list:
            ...
            if points_offset == 0:
                ...
                self.triangles = np.concatenate(
                    (self.triangles, t.copy() + points_offset)
                )
    ...
    def set_from_triangles_and_points(self, triangles, points):
        ...
        tri_patch = TriangulatedPatch(
            self.model, patch_index=0, extract_from_xml=False, crs_uuid=self.crs_uuid
        )
    ...
    def set_from_mesh_file(self, filename, format, quad_triangles=False):
        ...
        assert (
            x is not None and y is not None or z is not None
        ), "failed to read surface from zmap file"
        assert (
            x.size == y.size and x.size == z.size
        ), "non matching array sizes from zmap reader"
        assert (
            x.shape == y.shape and x.shape == z.shape
        ), "non matching array shapes from zmap reader"
        ...
        if ref_depth is None:
            for patch in self.patch_list:
                ...
                if ref_depth is None or patch_min < ref_depth:
                    ref_depth = patch_min
        assert (
            ref_depth is not None
        ), "no z values found for vertical rescaling of surface"
        self.triangles = (
            None  # invalidate any cached triangles & points in surface object
        )
        self.points = None
        for patch in self.patch_list:
    ...
    def write_hdf5(self, file_name=None, mode="a"):
        ...
        for triangulated_patch in self.patch_list:
            ...
            h5_reg.register_dataset(
                self.uuid, "points_patch{}".format(triangulated_patch.patch_index), p
            )
            h5_reg.register_dataset(
                self.uuid, "triangles_patch{}".format(triangulated_patch.patch_index), t
            )
    ...
    def create_xml(
        ...
        if self.represented_interpretation_root is not None:
            ...
            interp_title = rqet.find_nested_tags_text(
                interp_root, ["Citation", "Title"]
            )
        ...
        for patch in self.patch_list:
            ...
            triangles_null = rqet.SubElement(
                triangles_node, ns["resqml2"] + "NullValue"
            )
        ...
        if add_as_part:
            ...
            if add_relationships:
                ...
                self.model.create_reciprocal_relationship(
                    tri_rep, "destinationObject", crs_root, "sourceObject"
                )
                if self.represented_interpretation_root is not None:
                    self.model.create_reciprocal_relationship(
                        tri_rep,
                        "destinationObject",
                        self.represented_interpretation_root,
                        "sourceObject",
                    )
                ...
                self.model.create_reciprocal_relationship(
                    tri_rep, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )
...
class PointSet(_BaseSurface):
    ...
    def __init__(
        ...
        self.patch_ref_list = (
            []
        )  # ordered list of (patch hdf5 ext uuid, path in hdf5, point count)
        self.patch_array_list = (
            []
        )  # ordered list of numpy float arrays (or None before loading), each of shape (N, 3)
        ...
        if point_set_root is not None:
            ...
            for child in rqet.list_of_tag(point_set_root, "NodePatch"):
                ...
                if self.crs_uuid is None:
                    ...
                    assert bu.matching_uuids(
                        crs_uuid, self.crs_uuid
                    ), "mixed coordinate reference systems in point set"
                ext_uuid = rqet.find_nested_tags_text(
                    geom_node, ["Points", "Coordinates", "HdfProxy", "UUID"]
                )
                assert ext_uuid, "missing hdf5 uuid in goemetry xml for point set patch"
                hdf5_path = rqet.find_nested_tags_text(
                    geom_node, ["Points", "Coordinates", "PathInHdfFile"]
                )
                assert (
                    hdf5_path
                ), "missing internal hdf5 path in goemetry xml for point set patch"
            ...
            if random_point_count:
                ...
                for p_i in range(random_point_count):
                    points[p_i, :2] = polyline.denormalised_xy(
                        points[p_i, 0], points[p_i, 1]
                    )
            ...
            for poly in polyset.polys:
                if poly == polyset.polys[0]:
                    ...
                    if poly.isclosed and vec.isclose(
                        poly.coordinates[0], poly.coordinates[-1]
                    ):
                    ...
                    if not curr_crs.is_equivalent(master_crs):
                        shifted = curr_crs.convert_array_to(
                            master_crs, poly.coordinates
                        )
                        ...
                        if poly.isclosed and vec.isclose(
                            ...
                            poly_coords = np.concatenate(
                                (poly_coords, poly.coordinates[:-1])
                            )
                        else:
                            poly_coords = np.concatenate(
    ...
    def single_patch_array_ref(self, patch_index):
        ...
        h5_key_pair = (
            self.patch_ref_list[patch_index][0],
            self.patch_ref_list[patch_index][1],
        ...
        assert (
            self.temp_points.ndim == 2 and self.temp_points.shape[1] == 3
    ...
    def full_array_ref(self):
        ...
        for patch_index in range(self.patch_count):
            self.full_array[
                full_index : full_index + self.patch_ref_list[patch_index][2]
            ] = self.patch_array_list[patch_index]
            full_index += self.patch_ref_list[patch_index][2]
        assert (
            full_index == point_count
        ), "point count mismatch when constructing full array for point set"
        return self.full_array
    ...
    def create_xml(
        ...
        if crs_root is None:
            if self.crs_uuid is None:
                ...
                crs_root = self.model.root_for_part(
                    self.model.part_for_uuid(self.crs_uuid)
                )
        ...
        for patch_index in range(self.patch_count):
            ...
            self.model.create_hdf5_dataset_ref(
                ext_uuid, self.uuid, "points_{}".format(patch_index), root=coords
            )
        ...
        if add_as_part:
            ...
            if add_relationships:
                # todo: add multiple crs'es (one per patch)?
                self.model.create_reciprocal_relationship(
                    ps_node, "destinationObject", crs_root, "sourceObject"
                )
                if self.represented_interpretation_root is not None:
                    self.model.create_reciprocal_relationship(
                        ps_node,
                ...
                ext_node = self.model.root_for_part(ext_part)
                self.model.create_reciprocal_relationship(
                    ps_node, "mlToExternalPartProxy", ext_node, "externalPartProxyToMl"
                )
    ...
    def convert_to_charisma(self, file_name):
        ...
        for patch in self.patch_array_list:
            for points in patch:
                lines.append(
                    f"INLINE :\t1 XLINE :\t1\t{points[0]}\t{points[1]}\t{points[2]}\n"
                )
...
class Mesh(_BaseSurface):
    ...
    def __init__(
        ...
        if root_node is not None:
            ...
            assert (
                self.ni is not None and self.nj is not None
            ), "mesh extent info missing in xml"
            ...
            assert (
                self.crs_uuid is not None
            ), "crs reference missing in mesh geometry xml"
            ...
            if flavour == "Point3dLatticeArray":
                ...
                self.regular_origin = (
                    rqet.find_tag_float(origin_node, "Coordinate1"),
                    rqet.find_tag_float(origin_node, "Coordinate2"),
                    rqet.find_tag_float(origin_node, "Coordinate3"),
                )
                assert (
                    self.regular_origin is not None
                ), "origin missing in xml for regular mesh (lattice)"
                offset_nodes = rqet.list_of_tag(
                    point_node, "Offset"
                )  # first occurrence for FastestAxis, ie. I; 2nd for J
                assert (
                    len(offset_nodes) == 2
                ), "missing (or too many) offset nodes in xml for regular mesh (lattice)"
                ...
                for j_or_i in range(2):  # 0 = J, 1 = I
                    ...
                    assert (
                        axial_offset_node is not None
                    ), "missing offset offset node in xml"
                    self.regular_dxyz_dij[1 - j_or_i] = (
                        rqet.find_tag_float(axial_offset_node, "Coordinate1"),
                    ...
                    assert (
                        stride is not None and count is not None
                    ), "missing spacing info in xml"
                    assert (
                        count == (self.nj, self.ni)[j_or_i] - 1
                    ), "unexpected value for count in xml spacing info for regular mesh (lattice)"
                    assert (
                        stride > 0.0
                    ), "spacing distance is not positive in xml for regular mesh (lattice)"
                ...
                self.ref_z_h5_key_pair = self.model.h5_uuid_and_path_for_node(
                    z_ref_node, tag="Values"
                )
                support_geom_node = rqet.find_tag(point_node, "SupportingGeometry")
                if (
                    rqet.node_type(support_geom_node)
                    ...
                    assert (
                        self.ref_uuid
                    ), "missing supporting representation info in xml for z-value mesh"
                    self.ref_mesh = Mesh(
                        self.model, root_node=self.model.root_for_uuid(self.ref_uuid)
                    )
                    assert (
                        self.nj == self.ref_mesh.nj and self.ni == self.ref_mesh.ni
                    )  # only this supported for now
                    niosr_node = rqet.find_tag(
                        support_geom_node, "NodeIndicesOnSupportingRepresentation"
                    )
                    start_value = rqet.find_tag_int(niosr_node, "StartValue")
                    assert (
                        start_value == 0
                    ), "only full use of supporting mesh catered for at present"
                    offset_nodes = rqet.list_of_tag(
                        niosr_node, "Offset"
                    )  # first occurrence for FastestAxis, ie. I; 2nd for J
                    assert (
                    ...
                    for j_or_i in range(2):  # 0 = J, 1 = I
                        assert (
                            rqet.node_type(offset_nodes[j_or_i])
                            == "IntegerConstantArray"
                        ), "variable step not catered for"
                        assert (
                            rqet.find_tag_int(offset_nodes[j_or_i], "Value") == 1
                        ), "step other than 1 not catered for"
                    ...
                    self.regular_origin = (
                        rqet.find_tag_float(orig_node, "Coordinate1"),
                        rqet.find_tag_float(orig_node, "Coordinate2"),
                        rqet.find_tag_float(orig_node, "Coordinate3"),
                    )
                    assert (
                        self.regular_origin is not None
                    ), "origin missing in xml for reg&z mesh (lattice)"
                    offset_nodes = rqet.list_of_tag(
                        support_geom_node, "Offset"
                    )  # first occurrence for FastestAxis, ie. I; 2nd for J
                    assert (
                        len(offset_nodes) == 2
                    ), "missing (or too many) offset nodes in xml for reg&z mesh (lattice)"
                    self.regular_dxyz_dij = np.empty((2, 3))
                    for j_or_i in range(2):  # 0 = J, 1 = I
                        axial_offset_node = rqet.find_tag(
                            offset_nodes[j_or_i], "Offset"
                        )
                        assert (
                            axial_offset_node is not None
                        ), "missing offset offset node in xml"
                ...
                self.explicit_h5_key_pair = self.model.h5_uuid_and_path_for_node(
                    point_node, tag="Coordinates"
                )
            ...
            assert mesh_format in [
                ...
                "roxar",
                "zmap",
            ]  # 'roxar' is treated synonymously with 'rms'
            ...
            if mesh_flavour != "explicit":
                ...
                dxyz_dij = np.array(
                    [
                        [(max_x - min_x) / (self.ni - 1), 0.0, 0.0],
                        [0.0, (max_y - min_y) / (self.nj - 1), 0.0],
                    ],
                    dtype=float,
                )
            ...
            assert (
                xyz_values.ndim == 3
                and xyz_values.shape[2] == 3
                and xyz_values.shape[0] > 1
                and xyz_values.shape[1] > 1
            )
            self.flavour = "explicit"
            ...
            and dxyz_dij is not None
            and crs_uuid is not None
            and z_values is None
        ):
            ...
            and z_supporting_mesh_uuid is not None
        ):
            ...
            self.ref_mesh = Mesh(
                self.model, root_node=self.model.root_for_uuid(z_supporting_mesh_uuid)
            )
            assert self.ref_mesh is not None
            assert self.ref_mesh.nj == nj and self.ref_mesh.ni == ni
            self.full_array = self.ref_mesh.full_array_ref().copy()
            self.full_array[..., 2] = z_values.reshape(
                tuple(self.full_array.shape[:-1])
            )
            self.crs_uuid = crs_uuid
            self.crs_root = self.model.root_for_uuid(self.crs_uuid)
            self.uuid = bu.new_uuid()
    ...
    def full_array_ref(self):
        ...
        if self.flavour == "explicit":
            ...
            assert (
                self.explicit_h5_key_pair is not None
            ), "h5 key pair not established for mesh"
            try:
                self.model.h5_array_element(
                    self.explicit_h5_key_pair,
                    cache_array=True,
                    object=self,
            ...
            x_i0 = np.linspace(
                0.0, (self.nj - 1) * self.regular_dxyz_dij[1, 0], num=self.nj
            )
            y_i0 = np.linspace(
                0.0, (self.nj - 1) * self.regular_dxyz_dij[1, 1], num=self.nj
            )
            z_i0 = np.linspace(
                0.0, (self.nj - 1) * self.regular_dxyz_dij[1, 2], num=self.nj
            )
            x_full = np.linspace(
                x_i0,
                x_i0 + (self.ni - 1) * self.regular_dxyz_dij[0, 0],
                num=self.ni,
                axis=-1,
            )
            y_full = np.linspace(
                y_i0,
                y_i0 + (self.ni - 1) * self.regular_dxyz_dij[0, 1],
                num=self.ni,
                axis=-1,
            )
            z_full = np.linspace(
                z_i0,
                z_i0 + (self.ni - 1) * self.regular_dxyz_dij[0, 2],
                num=self.ni,
                axis=-1,
            )
            self.full_array = (
                np.stack((x_full, y_full, z_full), axis=-1) + self.regular_origin
            )
            if (
                self.flavour == "reg&z"
            ):  # overwrite regular z values with explicitly stored z values
                assert (
                    self.ref_z_h5_key_pair is not None
                ), "h5 key pair missing for mesh z values"
                try:
                    self.model.h5_array_element(
                        self.ref_z_h5_key_pair,
                        cache_array=True,
                        object=self,
            ...
            if self.ref_mesh is None:
                ...
                assert (
                    self.ref_mesh is not None
                ), "failed to instantiate object for referenced mesh"
            self.full_array = self.ref_mesh.full_array_ref().copy()
            assert (
                self.ref_z_h5_key_pair is not None
            ), "h5 key pair missing for mesh z values"
    ...
    def surface(self, quad_triangles=False):
        ...
        return Surface(
            self.model, extract_from_xml=False, mesh=self, quad_triangles=quad_triangles
        )
    ...
    def write_hdf5(self, file_name=None, mode="a", use_xy_only=False):
        ...
        if self.flavour == "explicit":
            if use_xy_only:
                h5_reg.register_dataset(
                    self.uuid, "points", a[..., :2]
                )  # todo: check what others use here
            else:
    ...
    def create_xml(
        ...
        ), "failed to resolve xml root of crs for mesh; crs uuid: " + str(self.crs_uuid)
        ...
        if self.flavour == "regular":
            ...
            self.model.create_solitary_point3d(
                "Origin", p_node, self.regular_origin
            ...
            for j_or_i in range(2):  # 0 = J, 1 = I
                ...
                dxyz /= d_value
                o_node = rqet.SubElement(
                    p_node, ns["resqml2"] + "Offset"
                )  # note: 1st Offset is for J axis , 2nd for I
                ...
                space_node.set(
                    ns["xsi"] + "type", ns["resqml2"] + "DoubleConstantArray"
                )  # nothing else catered for just now
                space_node.text = "\n"
            ...
            for j_or_i in range(2):  # 0 = J, 1 = I
                ...
                o_node.set(
                    ns["xsi"] + "type", ns["resqml2"] + "IntegerConstantArray"
            ...
            self.model.create_hdf5_dataset_ref(
                ext_uuid, self.uuid, "zvalues", root=v_node
            )
            ...
            self.model.create_solitary_point3d(
                "Origin", sg_node, self.regular_origin
            )  # todo: check xml namespace
        ...
        if add_as_part:
            ...
            if add_relationships:
                self.model.create_reciprocal_relationship(
                    g2d_node, "destinationObject", crs_root, "sourceObject"
                )
                if self.represented_interpretation_root is not None:
                    self.model.create_reciprocal_relationship(
                        g2d_node,
                ...
                if ref_root is not None:  # used for ref&z flavour
                    self.model.create_reciprocal_relationship(
                        g2d_node, "destinationObject", ref_root, "sourceObject"
                    )
                if (
                    self.flavour == "ref&z"
                    or self.flavour == "explicit"
                    or self.flavour == "reg&z"
                ):
                    ext_part = rqet.part_name_for_object(
                        "obj_EpcExternalPartReference", ext_uuid, prefixed=False
                    )
